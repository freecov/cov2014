...
function selection_attachments_delete() {
	var cf = confirm( gettext("Are you sure you want to delete the selected attachments?") );
	if (cf == true) {
...
/* function selection_email_linkrel */
/* try to link the email selection to relations */
function selection_email_linkrel() {
	set('action', 'link_rel_sel');
	document.getElementById('velden').submit();
}
...
function selection_email_move() {
	...
	if (doit) {
		...
		alert(gettext("No mails selected."));
	}
}
...
function createNewFolder() {
	var str = Prompt.show( gettext("Enter the folder name") );
...
function editCurrentFolder() {
	var str = Prompt.show(gettext("Supply new name for the folder:"));
	if (str && str != null && str != 'null' && str != '') {
...
function updateProject() {
	...
	var address_id = document.getElementById('list_of_address').value;
	document.location.href = '?mod=email&address_id='+address_id+'&project_id='+project_id+'&folder_id='+folder_id;
}
...
function viewStarredMail() {
	set('action', 'viewStarredMail');
	document.getElementById('velden').submit();
}

/**
 * Toggles private state of email. Refreshes the list so that the
 * changes are visible.
 */
function toggle_private_state(id) {
	set('action', 'toggle_private_state');
	set('mail_id', id);
	document.getElementById('velden').submit();
}
...
function delete_all_trash() {
	var cf = confirm( gettext("Are you sure you want to delete the entire trashbin, including all its subfolders?") );
	if (cf == true) {
		$('#infowait').show();
		loadXML('?mod=email&action=deleteAllTrash');
	}
}

if (document.getElementById('mailactions')) {
	document.getElementById('mailactions').onchange = function() {
		eval(document.getElementById('mailactions').value);
		setTimeout("document.getElementById('mailactions').value = 'void(0);';", 200);
	}
}
$(document).ready(function() {
	$('div[id^=contentpreview_]').hide();
	$('div[id^=preview_]').bind('mouseenter', function(e) {
		var $this = $(this);
		var x = $this.attr("id");
		of_x = $this.offsetLeft;
		of_y = $this.offsetTop;
...
function force_mail_retrieve(user_id) {
	var url = 'index.php?mod=email&action=retrieve&user_id='+user_id;
	$('#infowait').show();
	loadXML(url);
}
