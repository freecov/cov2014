 ...
 */
	...
	$venster->addMenuItem("&nbsp;&nbsp;".gettext("force mail retrieval"), "javascript: force_mail_retrieve('".$_SESSION["user_id"]."');");
	...
	$table->addTableRow();
		$table->addTableData();
			if ($list_of_address) {
				...
				$addressinfo[0] = $address_data->getAddressById($list_of_address);
	...
	$view->defineComplexMapping("data_actions", array(
		...
		array(
			...
			"src"   => "%is_star",
			"alt"   => "Starred",
			"link"	=> array("javascript: set_star('", "%id", "');")
		),
		array(
			"type"  => "action",
			"src"   => "%is_public_i",
