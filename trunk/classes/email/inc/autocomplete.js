...
function init_layer(ary) {
	...
	for (i=2;i<ret.length-1;i++) {
		flag++;
		tmp = ret[i].split('|');
		tmp[0] = '<a href="javascript: void(0);" onclick="javascript: set_complete(\''+field+'\', \''+tmp[0]+'\', \''+tmp[3]+'\', \''+tmp[2]+'\');">&lt;'+tmp[0]+'&gt;</a>';
...
function set_complete(field, email, address_id, address_name) {
	var el = document.getElementById(field);
	var str = el.value;
	var repl = '';
	...
	el.value = repl+', ';
	el.focus();
	aid = document.getElementById('mailrelation').value;
	if (address_id && address_name && aid == 0) {
		selectRel(address_id, address_name);
	}
}
