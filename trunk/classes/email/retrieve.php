...
Class Email_retrieve Extends Email_data {
	...
	public function __construct($user_id=0, $use_output=1) {
		...
		/* get autoreply settings for this user if any */
		$this->autoreply = $this->getAutoreplyByUserId($this->user_id);
	}
	...
	private function refresh_opener() {
		...
		if ($this->use_output) {
			echo "if (document.getElementById('mod').value == 'email') { document.getElementById('velden').submit(); }";
			echo "\n$('#infowait').hide();";
		}
	}
	...
	public function mailConnect($settings) {
		switch ($settings["mail_imap"]) {
			...
			case 3:
				...
				break;
			default:
				$protocol = "110/pop3/novalidate-cert";
				break;
		}
