...
Class Email_data {
	...
	/* set_star {{{ */
	/**
	 * Method to mark mail from "starred".
	 *
	 * @param int $id Id of the email.
	 */
	public function set_star($id, $output = null) {
		$_REQUEST["action"] = '';
		$q = sprintf("select star from mail_messages where id = %d", $id);
		$res = sql_query($q);
		$state = sql_result($res,0);

		if ($state == 0) {
			$q = sprintf("update mail_messages set star = 1 where id = %d", $id);
		} else {
			$q = sprintf("update mail_messages set star = 0 where id = %d", $id);
		}
		sql_query($q);
		header(sprintf("Location: /?mod=email&folder_id=%d", $_REQUEST["folder_id"]));
