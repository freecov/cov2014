<?php
	...
	if (array_key_exists("project", $_REQUEST) && array_key_exists("bcard", $_REQUEST["project"])) {
		$issueinfo["bcard_id"] = $_REQUEST["project"]["bcard"];
	}
	/* make array which can be imploded in sql queries */
	$fields = array();
	$values = array();
	$fields[] = "reference_nr";   $values[] = sprintf("'%s'",   $issueinfo["reference_nr"]);
	...
	if ($issueinfo["ex_month"] > 0 && $issueinfo["ex_day"] > 0 && $issueinfo["ex_year"] > 0) {
		$fields[] = "execution_time"; $values[] = sprintf("%d",   mktime(0, 0, 0, $issueinfo["ex_month"], $issueinfo["ex_day"], $issueinfo["ex_year"]));
	} else {
		$fields[] = "execution_time"; $values[] = sprintf("%d",   0);
	}
	$fields[] = "description";    $values[] = sprintf("'%s'", $issueinfo["description"]);
	$fields[] = "solution";       $values[] = sprintf("'%s'", $issueinfo["solution"]);
	$fields[] = "remarks";        $values[] = sprintf("'%s'", $issueinfo["remarks"]);
	...
	$fields[] = "supplies";       $values[] = sprintf("'%s'", $issueinfo["supplies"]);
	$fields[] = "internal_notes"; $values[] = sprintf("'%s'", $issueinfo["internal_notes"]);
	$fields[] = "bcard_id";       $values[] = sprintf("%d",   $issueinfo["bcard_id"]);
	if ($issueinfo["timeofarrival_hours"] > 0 && $issueinfo["timeofarrival_minutes"] > 0) {
		$fields[] = "arrival";    $values[] = sprintf("%d",   mktime($issueinfo["timeofarrival_hours"], $issueinfo["timeofarrival_minutes"], 0, 1, 1, 1970));
	}
	if ($issueinfo["timeofdeparture_hours"] > 0 && $issueinfo["timeofdeparture_minutes"] > 0) {
		$fields[] = "departure";  $values[] = sprintf("%d",   mktime($issueinfo["timeofdeparture_hours"], $issueinfo["timeofdeparture_minutes"], 0, 1, 1, 1970));
	}
