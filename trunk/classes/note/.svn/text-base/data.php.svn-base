...
Class Note_data {
	...
	public function getNoteById($id) {
		...
		$row["extra_names"] = $extra_names;
		$row["human_date"]  = date("d-m-Y H:i:s", $row["timestamp"]);
		if ($row["address_businesscards_id"]) {
			$bcard = $address->getAddressById($row["address_businesscards_id"], "bcards");
		} else {
			$bcard = $address->getRCBCByAddressId($row["address_id"], 1);
		}
		$row["bcard"] = $bcard;
		if ($row["phonecall_start"]) {
			$row["phonecall_start_human"] = date("d-m-Y H:i:s", $row["phonecall_start"]);
			$row["phonecall_contactname"] = $row["bcard"]["fullname"];
		}
		if ($row["phonecall_end"]) {
			$row["phonecall_end_human"] = date("d-m-Y H:i:s", $row["phonecall_end"]);
		}
	 ...
	 *
	 * @param int $id Address id to grab notes for
	 * @param int $active If set to 0 will return notes flagged as 'done'
	 * @param int $custcontact If set to 1 will return notes flagged as 'Customer Contact'
	 * @param int $include_drafts If set to 1, will return all notes including drafts
	 *
	 * @return Array notes as they are in the db
	 */
	public function getNotesByContact($id, $active=1, $custcontact = 0, $include_drafts = 0) {
		$user                 = new User_data();
		...
		if ($include_drafts == 0) {
			$drafts = "AND is_draft != 1";
		} else {
			$drafts = "";
		}
		if ($active) {
			$sql = sprintf("SELECT * FROM notes WHERE address_id = %d AND (is_done is null OR is_done = 0) AND is_support != %d %s ORDER BY timestamp DESC", $id, $is_support_neg, $drafts);
		} else {
			$sql = sprintf("SELECT * FROM notes WHERE address_id = %d AND is_done = 1 AND is_support != %d %s ORDER BY timestamp DESC", $id, $is_support_neg, $drafts);
		}
		$res = sql_query($sql);
		$notes = array();
	...
	public function getNotecountByUserId($user) {
		...
		$q_sender = sprintf("sender = %d", $user);
		$sql_draft  = "SELECT COUNT(*) FROM notes WHERE $q_sender AND is_draft = 1";
		...
		$res = sql_query($sql_draft);
		$return["draft"] = sql_result($res,0);
	...
	public function store2db($note) {
		...
		if (!$note["id"]) {
			...
			/* If the AFP field is submitted, put some standard values in the note */
					...
					$sql  = "INSERT INTO notes (is_support, timestamp, extra_recipients, subject, body, sender, is_read, is_done, address_id, project_id, campaign_id, user_id, is_draft, phonecall, phone_nr, address_businesscards_id, phonecall_start, phonecall_end) VALUES (";
					$sql .= sprintf("%d, %d, '%s', '%s', '%s'", $note["is_support"], time(), implode(",", $extra), $note["subject"], $note["body"]);
					$sql .= sprintf(",%d, %d, %d, %d, %d, %d, %d, %d", $note["from"], $read, $done, $note["address_id"], $note["project_id"], $note["campaign_id"], $users[0], $note["is_draft"]);
					$sql .= sprintf(", %d, '%s', %d, %d, %d", $note["phonecall"], $note["phone_nr"], $note["address_businesscards_id"], $note["phonecall_start"], ($note["phonecall"]?time():0));
					$sql .= ")";
					$res = sql_query($sql);
			...
			if (!$newnote && ($note["is_todo"] || $note["is_sales"])) {
				...
				$sql .= sprintf("%d, %d, '%s', '%s', '%s'", $note["is_support"], time(), implode(",", $users), $note["subject"], $note["body"]);
			...
			$sql .= sprintf("is_support=%d, timestamp=%d, extra_recipients='%s'", $note["is_support"], time(), implode(",", $users));
			...
			$sql .= sprintf(", phonecall=%d, phone_nr='%s', address_businesscards_id=%d, phonecall_start=%d, phonecall_end=%d", $note["phonecall"], $note["phone_nr"], $note["address_businesscards_id"], $note["phonecall_start"], ($note["phonecall"]?time():0));
		...
		if ($note["campaign_id"]) {
			...
			$output = new Layout_output();
				$output->start_javascript();
					$output->addCode("
						parent.document.getElementById('options8').checked = true;
	...
	public function delete_draft($id) {
		$sql = sprintf("DELETE FROM notes WHERE id = %d AND is_draft = 1", $id);
