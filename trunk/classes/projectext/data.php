...
Class ProjectExt_data Extends Project_data {
	...
	*/
	...
	public function extSaveDepartment() {
		...
		$output->addCode("setTimeout('closepopup()', 1000);");
	...
	public function extSaveMetaField() {
		...
		$output = new Layout_output();
		$output->start_javascript();
		$output->addCode("parent.location.href='index.php?mod=projectext&action=".(($_REQUEST["activity_id"]) ? "extOpenActivity":"defineMetaFields")."&department_id=".$_REQUEST["department_id"]."&activity_id=".$_REQUEST["activity_id"]."';");
		$output->addCode("setTimeout('closepopup()', 1000);");
		$output->end_javascript();
		$output->exit_buffer();
	}
	...
	/*}}}*/
	/* prepareMetaSearchQuery {{{*/
	public function prepareMetaSearchQuery($searchkey = "") {
		if (!$searchkey) {
		...
		if (($start["ts"] && $end["ts"]) && $_REQUEST["projectext"]["metatype"]=="date") {
			...
			$join = " RIGHT JOIN projects_ext_metavalues ON project.id = projects_ext_metavalues.project_id ";
			$cond = sprintf(" AND (meta_id = %d AND meta_value %s '%%%s%%') ", $meta_field, $like, $searchkey) ;
		}
		return array(
			"join" => $join,
			"cond" => $cond
	...
	public function extMergeTemplate() {
		...
		if ($_REQUEST["project"]["bcard"]) {
			...
			if (!(trim($rcpt["business_address"]) || trim($rcpt["personal_address"]))) {
				...
				$rcpt["pobox"]     = $addressinfo["pobox"];
				$rcpt["pobox_zipcode"]     = $addressinfo["pobox_zipcode"];
				$rcpt["pobox_city"]     = $addressinfo["pobox_city"];
				$rcpt["bsn"]     = $addressinfo["bsn"];
			} else {
			...
			if ($rcpt["business_email"]) {
				$rcpt["email"] = $rcpt["business_email"];
		...
		global $rcpt_phone, $rcpt_fax, $rcpt_website, $rcpt_tav, $rcpt_contact, $rcpt_project, $rcpt_email;
		global $rcpt_pobox, $rcpt_pobox_zipcode, $rcpt_pobox_city, $rcpt_birthday, $rcpt_bsn;
		...
		$rcpt_pobox       = $rcpt["pobox"];
		$rcpt_pobox_zipcode = $rcpt["pobox_zipcode"];
		$rcpt_pobox_city   = $rcpt["pobox_city"];
		$rcpt_bsn          = $rcpt["bsn"];

		$extra_rcpts = explode(",", $project_info["all_address_ids"]);
		$i = 1;
		$ercpt = array();
		foreach ($extra_rcpts as $extra_id) {
			$ercpt[$i] = $address_data->getAddressByID($extra_id);
			$ercpt[$i]["letterinfo"] = $address_data->generate_letterinfo(array(
				"contact_initials"     => $ercpt[$i]["initials"],
				"contact_letterhead"   => $ercpt[$i]["letterhead"],
				"contact_commencement" => $ercpt[$i]["commencement"],
				"contact_givenname"    => $ercpt[$i]["givenname"],
				"contact_infix"        => $ercpt[$i]["infix"],
				"contact_surname"      => $ercpt[$i]["surname"],
				"title"                => $ercpt[$i]["title"]
			));
			$ercpt["tav"] = $ercpt["letterinfo"]["tav"];
			$ercpt["contact_person"] = $ercpt["letterinfo"]["contact_person"];
			/* rcpt variables */
			global ${"rcpt_companyname_$i"}, ${"rcpt_street_$i"}, ${"rcpt_address_$i"}, ${"rcpt_zipcode_$i"}, ${"rcpt_city_$i"};
			global ${"rcpt_phone_$i"}, ${"rcpt_fax_$i"}, ${"rcpt_website_$i"}, ${"rcpt_tav_$i"}, ${"rcpt_contact_$1"}, ${"rcpt_project_$i"}, ${"rcpt_email_$i"};
			global ${"rcpt_birthday_$i"}, ${"rcpt_pobox_$i"}, ${"rcpt_pobox_zipcode_$i"}, ${"rcpt_pobox_city_i$"}, ${"rcpt_bsn_$i"};

			${"rcpt_companyname_$i"} = $ercpt[$i]["companyname"];
			${"rcpt_street_$i"}      = $ercpt[$i]["address"];
			${"rcpt_address_$i"}     = $ercpt[$i]["address"];
			${"rcpt_zipcode_$i"}     = $ercpt[$i]["zipcode"];
			${"rcpt_city_$i"}        = $ercpt[$i]["city"];
			${"rcpt_phone_$i"}       = $ercpt[$i]["phone_nr"];
			${"rcpt_fax_$i"}         = $ercpt[$i]["fax_nr"];
			${"rcpt_website_$i"}     = preg_replace("/http:\/\//s", "", $ercpt[$i]["website"]);
			${"rcpt_email_$i"}       = $ercpt[$i]["email"];
			${"rcpt_mobile_$i"}      = $ercpt[$i]["mobile_nr"];
			${"rcpt_tav_$i"}         = $ercpt[$i]["tav"];
			${"rcpt_contact_$i"}     = $ercpt[$i]["contact_person"];
			${"rcpt_project_$i"}     = $project_name;
			${"rcpt_birthday_$i"}    = $ercpt[$i]["birthday"];
			${"rcpt_pobox_$i"}       = $ercpt[$i]["pobox"];
			${"rcpt_pobox_zipcode_$i"} = $ercpt[$i]["pobox_zipcode"];
			${"rcpt_pobox_city_$i"}  = $ercpt[$i]["pobox_city"];
			${"rcpt_bsn_$i"}         = $ercpt[$i]["bsn"];
		}
