<?php
	...
	Class User_output {
		...
		*/
		...
		/* showRadiusSettings {{{ */
		/**
		 * Show radiussettings with posibility to alter them
		 *
		 * @return void
		 */
		public function showRadiusSettings() {
			$user_data = new User_data();
			$user_name = $user_data->getUserNameById($_SESSION["user_id"]);
			if ($user_name != "administrator") {
				die("no access");
			}
			$sql = "SELECT * FROM radius_settings";
			$res = sql_query($sql);
			$row = sql_fetch_assoc($res);
			$output = new Layout_output();
			$output->layout_page("radius settings");
			$venster = new Layout_venster(array("title" => gettext("Settings"), "subtitle" => "Radius"));
			$venster->addVensterData();
			$table = new Layout_table(array("width" => "100%"));
			$table->addTableRow();
				$table->insertTableData(gettext("server").":".gettext("port"), "", "header");
				$table->addTableData("", "data");
					$table->addTextField("radius[radius_server]", $row["radius_server"]);
					$table->addCode(":");
					$table->addTextField("radius[radius_port]", $row["radius_port"]);
				$table->endTableData();
			$table->endTableRow();
			$table->addTableRow();
				$table->insertTableData(gettext("NAS ip"), "", "header");
				$table->addTableData("", "data");
					$table->addTextField("radius[nas_ip]", $row["nas_ip"]);
				$table->endTableData();
			$table->endTableRow();
			$table->addTableRow();
				$table->insertTableData(gettext("shared secret"), "", "header");
				$table->addTableData("", "data");
					$table->addTextField("radius[shared_secret]", $row["shared_secret"]);
				$table->endTableData();
			$table->endTableRow();
			$table->addTableRow();
				$table->insertTableData(gettext("authentication type"), "", "header");
				$table->addTableData("", "data");
					$table->addSelectField("radius[auth_type]", array("PAP" => "PAP", "CHAP" => "CHAP"), $row["auth_type"]);
				$table->endTableData();
			$table->endTableRow();
			$table->addTableRow();
				$table->insertTableData(" ", "", "header");
				$table->addTableData("", "data");
					$table->insertAction("save", gettext("save"), "javascript: document.getElementById('radiussettings').submit();");
				$table->endTableData();
			$table->endTableRow();
			$table->endTable();
			$venster->addTag("form", array("method" => "post", "action" => "index.php", "id" => "radiussettings"));
			$venster->addHiddenField("mod", "user");
			$venster->addHiddenField("action", "save_radius");
			$venster->addCode($table->generate_output());
			$venster->endTag("form");
			$venster->endVensterData();
			$output->addCode($venster->generate_output());
