...
if ($_REQUEST["datemask"] && !$calendar_item["is_repeat"]) {
...
if ($_REQUEST["subject"]) {
	$calendar_item["subject"] = $_REQUEST["subject"];
}
if ($_REQUEST["event"]) {
	$calendar_item["alldayevent"] = 1;
}
if ($_REQUEST["relations"]) {
	$rel = explode(",", $_REQUEST["relations"]);
	$calendar_item["address_id"] = $rel[0];
	unset($rel[0]);
	$calendar_item["multirel"] = implode(",", $rel);
}
/* get relation name */
...
if ($calendar_item["multiprivate"]) {
	...
	foreach ($address_ids as $aid) {
		...
		$multiprivate[$aid] = preg_replace("/ {1,}/", " ", sprintf("%s %s %s", $info["givenname"], $info["infix"], $info["surname"]));
	}
	unset($address_ids);
	...
	$multiprivate = array(
		$calendar_item["private_id"] => preg_replace("/ {1,}/", " ", sprintf("%s %s %s", $info["givenname"], $info["infix"], $info["surname"]))
	);
	unset($calendar_item["private_id"]);
	$privname = "";
}
...
/* dimdim meeting */
$dimdim_id = $calendar_item["dimdim_id"];
if ($dimdim_id) {
	$dimdim_data = new Dimdim_data();
	$dimdim = $dimdim_data->getMeetingById($dimdim_id);
}

/* handle repeat_end stuff */
if ($calendar_item["is_repeat"] && (int)$calendar_item["repeat_end"] > 0) {
	$calendar_item["repeat_use_end_date"] = 1;
	$calendar_item["repeat_end_day"] = date("d", $calendar_item["repeat_end"]);
	$calendar_item["repeat_end_month"] = date("m", $calendar_item["repeat_end"]);
	$calendar_item["repeat_end_year"] = date("Y", $calendar_item["repeat_end"]);
}
/* start output */
...
$venster->addVensterData();
	...
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			...
			$table->addCode( $useroutput->user_selection("appointmentuser", $calendar_item["user"], 0, 0, 0, 0, 0, 0, 1) );
		$table->endTableData();
	$table->endTableRow();
		...
		$table->addTableData("", "data");
			...
			$table1->addTableRow();
				...
				$table1->addTableData(array("align" => "right"), "data");
					...
					$table1->insertAction("close", gettext("close"), "javascript:closepopup();");
					if ($id) {
						$table1->insertAction("delete", gettext("delete"), sprintf("javascript:calendaritem_remove(%d, %d, %d, %d);", $id, $_REQUEST["user"], $calendar_item["is_repeat"], ($_REQUEST["user"] == $_SESSION["user_id"])?0:1));
					}
	...
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			...
			if ($ret !== false) {
				$table->addTextArea("appointment[description]", nl2br($calendar_item["description"]), array("style"=>"width: 570px; height: 300px;"), "contents");
				$table->addCode($ret);
			} else {
				$table->addTextArea("appointment[description]", nl2br($calendar_item["description"]), array("style"=>"width: 570px; height: 300px;"), "contents");
			}
	...
	/* dim dim */
	$table->addTableRow();
		$table->insertTableData(gettext("dimdim meeting"), "", "header");
		$table->addTableData("", "data");
		$table->addHiddenField("appointment[dimdim_meeting_id]", $dimdim_id);
		$table->insertTag("span", $dimdim["name"], array(
				"id" => "meeting_name",
				"style" => "font-weight: bold;"
			));
		$visibility_new = (!$dimdim_id) ? "visible" : "hidden";
		$visibility_del = ($dimdim_id) ? "visible" : "hidden";

		$table->addTag("span", array("id" => "dimdim_icon", "style" => "visibility: ".$visibility_new));
		$table->insertAction("new", gettext("new"), "javascript: makeDimdimMeeting();");
		$table->endTag("span");

		$table->addTag("span", array("id" => "dimdim_icon_delete", "style" => "visibility: ".$visibility_del));
		$table->insertAction("delete", gettext("delete"), "javascript: deleteDimdim(".$dimdim_id.");");
		$table->endTag("span");
	...
	$table->addTableRow();
		...
		$table->addTableData(array("align"=>"right"), "data");
			$table->insertAction("close", gettext("close"), "javascript:closepopup();");
			if ($id) {
				$table->insertAction("delete", gettext("delete"), sprintf("javascript:calendaritem_remove(%d, %d, %d, %d);", $id, $_REQUEST["user"], $calendar_item["is_repeat"], ($_REQUEST["user"] == $_SESSION["user_id"])?0:1));
			}
...
if (is_array($multirel)) {
	...
	foreach ($multirel as $i=>$n) {
		if ($i) {
			$output->addCode("\n");
			$output->addCode("selectRel($i, '".addslashes($n)."');");
		}
	}
	$output->addCode("\n}\n");
