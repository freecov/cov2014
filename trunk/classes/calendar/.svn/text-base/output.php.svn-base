...
Class Calendar_output {
	...
	/* batch_reg_input {{{ */
	/**
	 * Allow a user to register a bunch of hours on an activity.
	 * This function wont report back to the calendar, because there's
	 * no start and endtime, but a couple of units instead.
	 *
	 * @param int $id The item to edit. If it's set to 0 create a new item
	 * @param int $project_id The project to link this registration item to
	 */
	public function batch_reg_input($id, $project_id) {
		require(self::include_dir."batch_reg_input.php");
	}
	/* }}} */
	/* misc_reg_input {{{ */
	/**
	 * Allow a user to register 'items' on a project.
	 * All a user has to provide is a user, a price, is it billable, project and a description
	 *
	 * @param int $id The item to edit. If it's set to 0 create a new item
	 * @param int $project_id The project to link this registration item to
	 */
	public function misc_reg_input($id, $project_id) {
		require(self::include_dir."misc_reg_input.php");
	}
	/* }}} */
	/* show_km {{{ */
	/**
	 ...
	 * @param string from_cms Optional field. when set will use an absolute path to classes/calendar/inc/calendar.php
	 */
	function show_calendar($day, $month, $year, $from_cms = 0) {
		...
		$output->addCode("
			...
			function upd_$name(dday, dmonth, dyear) {
				...
				if ($day.id == 'todotimestamp_day') {
					document.getElementById('todotimestamp_end_day').value = parseInt(dday);
					document.getElementById('todotimestamp_end_month').value = parseInt(dmonth);
					document.getElementById('todotimestamp_end_year').value = parseInt(dyear);
					}
				}
		");
	...
	foreach ($data as $key => $item) {
		...
		$dim = $dimdim_data->getMeetingById($data[$key]["dimdim_id"]);
		$data[$key]["dimdim_name"] = $dim["name"];
		$data[$key]["dimdim_room"] = $dim["room"];
		$data[$key]["dimdim_username"] = $userinfo["dimdim_username"];
		$data[$key]["dimdim_password"] = $userinfo["dimdim_password"];
		$data[$key]["dimdim_presenter"] = ($data[$key]["modified_by"] == $_SESSION["user_id"]) ? 1 : 0;
		$data[$key]["dimdim_guest"] = ($data[$key]["modified_by"] == $_SESSION["user_id"]) ? 0 : 1;
	}
	...
	$view = new Layout_view();
		...
		$view->defineComplexMapping("complex_time", array(
			array(
				...
				"src"  => "delete",
				"alt"  => gettext("delete"),
				"link" => array("javascript: calendaritem_remove(", "%id", ", ", "%user_id", ", ", "%is_repeat", ", ", $datestamp, ", 0);"),
				"check" => "%show_actions"
			),
			array(
				"type" => "action",
		...
		$view->defineComplexMapping("complex_dimdim", array(
			array(
				"type"  => "action",
				"src"   => "user_chat",
				"alt"   => gettext("dimdim meeting"),
				"check" => "%dimdim_id"
			),
			array(
				"type"  => "link",
				"link"  => array("javascript: popup('http://webmeeting.dimdim.com:80/portal/JoinForm.action?confKey=", "%dimdim_room", "')"),
				"text"  => "%dimdim_name",
				"check" => "%dimdim_guest"
			),
			array(
				"type"  => "link",
				"link"  => array("javascript: popup('http://webmeeting.dimdim.com:80/portal/start.action?name=", "%dimdim_username", "&password=", "%dimdim_password", "')"),
				"text"  => "%dimdim_name",
				"check" => "%dimdim_presenter"
			)
		));
	...
	public function ask_repeat($data) {
		...
		case "delete" :
			...
			$output->layout_page("", 1);
				...
				$output->addCode("
					function remove_all() {
						parent.document.getElementById('action').value = 'deleteallrep';
						parent.document.getElementById('id').value = ".$_REQUEST["id"].";
						parent.document.getElementById('user_id').value = ".$_REQUEST["user"].";
						if (confirm(gettext('remove calenderitem')+' ?')) {
							if (confirm(gettext('send deletion notification to users ?'))) {
								parent.document.getElementById('notifyuser').value = 'send';
							}
							parent.document.getElementById('calendarform').submit();
						}
						closepopup();
					}
					function remove_one() {
						parent.document.getElementById('action').value = 'deleteonerep';
						parent.document.getElementById('id').value = ".$_REQUEST["id"].";
						parent.document.getElementById('user_id').value = ".$_REQUEST["user"].";
						parent.document.getElementById('datestamp').value = ".$_REQUEST["timestamp"].";
						if (confirm(gettext('remove calenderitem')+' ?')) {
							if (confirm(gettext('send deletion notification to users ?'))) {
								parent.document.getElementById('notifyuser').value = 'send';
							}
	 ...
	 * Print month calendar
	 */
	public function print_month_mu($user_data) {
		require(self::include_dir."print_month_mu.php");
	}
	/* }}} */
}
?>
