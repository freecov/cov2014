	...
	if ($perms["xs_cms_level"] < 3) {
		...
		$view->defineComplexMapping("complex_actions", array(
			array(
				...
				"src"   => "edit",
				"alt"   => gettext("edit"),
				"link"  => array("javascript: oldpopup('?mod=cms&action=editTemplate&id=", "%id", "', 'template_", "%id", "', 960, 630, 1);"),
				"check" => "%edit_access"
			),
			array(
				"type" => "action",
		...
		$venster->start_javascript();
			$venster->addCode("
				function popWindow() {
					...
					oldpopup('?mod=cms&action=editTemplate', windowid, 960, 630, 1);
				}
			");
		$venster->end_javascript();
		$venster->insertAction("new", gettext("new template"), "javascript: popWindow();");
		$venster->insertAction('reload', gettext('refresh page'), '?mod=cms&action=siteTemplates&hidenav=1');
	}
