...
$output->addTag("form", array(
	...
	$frame->addVensterData();
		...
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("CRM form mode"), "", "header");
			$table->addTableData("", "data");
				$table->addSelectField("formsettings[user_insertmode]", array(
					0 => gettext("Normal/Full mode"),
					1 => gettext("Minimal (only name and email)")
				), $formsetting["user_insertmode"]);
			$table->endTableData();
		$table->endTableRow();
