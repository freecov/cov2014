 ...
 */
	...
	if (is_numeric($_REQUEST['siteroot'])) {
		$siteroot_title = $cms_data->getPageById($_REQUEST['siteroot']);
	}
	$venster->addVensterData();
		...
		/* siteroot title */
		if (isset($siteroot_title)) {
			$tbl->addTableRow();
				$tbl->insertTableData(gettext("siteroot title"), "", "header");
				$tbl->addTableData(array("colspan"=>3), "data");
					$tbl->addTextField("siteroot_title", $siteroot_title['pageTitle'], array(
						"style" => "width: 350px;"
					));
				$tbl->endTableData();
			$tbl->endTableRow();
		...
		$tbl->addTableRow($searchopt);
			...
			$tbl->addTableData(array("colspan"=>3), "data");
				...
				$sel = array(
					0 => gettext('disabled'),
					1 => gettext('website title first, then page'),
					2 => gettext('page title first, then website title')
				);
				$tbl->addSelectField("cms[search_use_pagetitle]", $sel, $cms["search_use_pagetitle"]);
			$tbl->endTableData();
		$tbl->endTableRow();
		/* siteroot is public */
		$tbl->addTableRow();
		...
		$tbl->addTableRow($searchopt);
			/* yahoo sitemaps check */
			$tbl->addTabledata("", "header");
				$tbl->addCode(gettext("Bing sitemap key"));
			$tbl->endTableData();
			$tbl->addTableData(array("colspan"=>3), "data");
				$tbl->addTextField("cms[bing_key]", $cms["bing_key"], array("style" => "width: 300px"));
				$tbl->insertAction("help", gettext("info"), "javascript: popup('http://www.bing.com/webmaster/', 'googlev1')");
			$tbl->endTableData();
		$tbl->endTableRow();
		...
		$tbl->addTableRow($searchopt);
			$tbl->addTabledata("", "header");
				$tbl->addCode(gettext("Piwik site id"));
				/* Piwik is open source */
			$tbl->endTableData();
			$tbl->addTableData(array("colspan"=>3), "data");
				$tbl->addTextField("cms[piwik_analytics]", ($cms["piwik_analytics"]) ? $cms['piwik_analytics'] : '', array("style" => "width: 300px"));
				$tbl->insertAction("help", gettext("info"), "javascript: popup('http://piwik.org/', 'piwik')");
			$tbl->endTableData();
		$tbl->endTableRow();
		$tbl->addTableRow($searchopt);
			$tbl->addTableData(array("colspan" => 4), "header");
		...
		$tbl->addTableRow($searchopt);
			$tbl->insertTableData(gettext("recaptcha private key"), "", "header");
			$tbl->addTableData(array("colspan"=>3), "data");
				$tbl->addTextField("cms[recaptcha_private]", $cms["recaptcha_private"], array('style' => 'width: 300px'));
				require('classes/recaptcha/recaptchalib.php');
				$url = recaptcha_get_signup_url($_SERVER['HTTP_HOST'], 'Covide CMS');
				$tbl->insertAction("help", gettext("info"), sprintf("javascript: popup('%s', 'recaptcha')", $url));
			$tbl->endTableData();
		$tbl->endTableRow();
		$tbl->addTableRow($searchopt);
			$tbl->insertTableData(gettext("recaptcha public key"), "", "header");
			$tbl->addTableData(array("colspan"=>3), "data");
				$tbl->addTextField("cms[recaptcha_public]", $cms["recaptcha_public"], array('style' => 'width: 300px'));
			$tbl->endTableData();
		$tbl->endTableRow();
		$tbl->addTableRow($searchopt);
			$tbl->insertTableData(gettext("recaptcha theme"), "", "header");
			$tbl->addTableData(array("colspan"=>3), "data");
				$opts = array(
					'' => 'default (red)',
					'white' => 'white',
				...
				$tbl->addSelectField("cms[recaptcha_theme]", $opts, $cms["recaptcha_theme"]);
			$tbl->endTableData();
		$tbl->endTableRow();
