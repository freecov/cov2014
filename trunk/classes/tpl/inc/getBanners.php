 ...
 */
	...
	if (count($banner) < $count) {
		...
		$q = sprintf("select id from cms_gallery_photos
			where pageid = -1 AND id NOT IN (%s) and internal_stat < rating", implode(",", $_banner));
		$res = sql_query($q);
		while ($row = sql_fetch_assoc($res)) {
			$banner[]= $row["id"];
		}
		...
		if (count($banner) < $count) {
			...
			$q = sprintf("select id from cms_gallery_photos where pageid = -1
				AND id NOT IN (%s)", implode(",", $_banner));
			$res = sql_query($q);
			while ($row = sql_fetch_assoc($res)) {
				$banner[]= $row["id"];
			}
	...
	$buf = "";
	foreach ($banner as $k=>$v) {
		$buf.= $this->displayBanner($v);
		if (!$horizontal)
			$buf.= "<br>";
	}
	$buf = str_replace("'", "\\\\'", $buf);
	$output = new Layout_output();
	$output->insertTag("span", "", array(
		"id" => "sponsorlist"
	));
	$output->start_javascript();
	$output->addCode(sprintf("
		setTimeout(\"document.getElementById('sponsorlist').innerHTML = '%s';\", 100);"
		, $buf));
