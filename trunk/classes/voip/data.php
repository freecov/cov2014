	...
	public function sendSMS($userid, $body) {
		if ($userid) {
			...
			if ($userinfo["address_id"]) {
				/* get mobile number */
				$sql = sprintf("SELECT personal_mobile_nr FROM address_businesscards WHERE id = %d", $userinfo["address_id"]);
				$res = sql_query($sql);
				$row = sql_fetch_assoc($res);
				if ($row["personal_mobile_nr"]) {
					/* normalize the number to send */
					$msisdn = $this->normalizeMsisdn($row["personal_mobile_nr"], $settings);
				} else {
					return false;
				}
			} else {
