...
Class Address_output {
	...
	const todo_include_dir = "classes/todo/inc/";
	...
	/* show_edit_private {{{ */
	/**
	 * Show screen to edit private address
	 *
	 * @param int $id The address id
	 * @param int $view_only if set all fields will be read only
	 */
	 ...
	 */
	public function showAddressBlock($name, $subject = "", $content = "") {
		$output = new Layout_output();
		if (!$content == "") {
			$output->addTag("div", array("id" => $name));
			$output->insertTag("h1", gettext($subject));
	...
	*/
	 ...
	 */
	public function addcla_multi($searchoptions, $inline = 0) {
	...
	public function edit_hrm($user_id) {
		require(self::include_dir."edit_hrm.php");
	}
	/* }}} */
	/* show_metagroups {{{ */
	/**
	 * Show a list of meta groups
	 */
	public function show_metagroups() {
		require(self::include_dir."show_metagroups.php");
	}
	/* }}} */
	/* show_metafields {{{ */
	/**
	...
	public function show_metafields() {
		require(self::include_dir."show_metafields.php");
	}
	/* }}} */
	/* getProjectBcardsByRelationXML {{{ */
	public function getProjectBcardsByRelationXML($address_id, $current) {
		...
		foreach ($address_id as $a) {
			if ($a) {
				...
				foreach ($data as $v) {
					if ($v["has_no_alt"] == 0)
						$v["fullname"] = $v["alternative_name"]." - ".$v["fullname"];
					if (trim($v["fullname"])) {
	...
	public function movePrivate2Public($id, $subaction="") {
		...
		$output->layout_page("", 1);
			...
			$venster->addVensterData();
				...
				$venster->addTag("br");
				$venster->addTag("br");
				$venster->insertAction("close", gettext("cancel"), "javascript: closepopup();");
				$venster->endTag("form");
	...
	public function showheaders() {
		require(self::include_dir."show_headers.php");
	}
	public function editTitles() {
		require(self::include_dir."edit_headers.php");
	}
	public function getDebtornrById($id) {
		$address_data = new Address_data();
		$info = $address_data->getAddressById($id);
		echo $info["debtor_nr"];
		exit();
	}
	/* removeSelection {{{ */
	/**
	 * Remove the contacts that are in a specific selection
	 *
	 * @param int The selection info id
	 */
	public function removeSelection($info, $inactive = 0) {
		require(self::include_dir."removeSelection.php");
	}
	/* }}} */
	/* activateSelection {{{ */
	/**
	 * activate the contacts that are in a specific selection
	 *
	 * @param int The selection info id
	 */
	public function activateSelection($info, $inactive = 0) {
		require(self::include_dir."activateSelection.php");
	}
	/* }}} */
	/* showPrivate{{{ */
	/**
	 * Show all info about private contact
	 *
	 * @param int The address id of the private contact
	 */
	public function showPrivate($id) {
		require(self::include_dir."showPrivate.php");

	}
	/* }}} */
	/* exportVcard{{{ */
	/**
	 * Convert bussinesscard to vcard (.vcf) markup
	 *
	 * @param int The address id of the bussinesscard
	 */
	public function exportVcard() {
		require(self::include_dir."exportVcard.php");

	}
	/* }}} */
	/* importVcard{{{ */
	/**
	 * Imports .vcf vCard to bussinesscard
	 *
	 * @param int The address id of the bussinesscard
	 */
	public function importVcard() {
		require(self::include_dir."importVcard.php");
	 ...
	 * @param int The address id of the bussinesscard
	 */
	public function importVcard_process() {
		require(self::include_dir."importVcard_process.php");

	}
	/* }}} */
	/* selectRows {{{ */
	public function selectRows($type) {
		require(self::include_dir."selectRows.php");
	}
	/* }}} */
	/* dublicate_show {{{ */
	/**
	 * Show duplicte data page
	 *
	 * @param int The id of dublicate record
	 * @return bool true on success, false on error
	 */
	function duplicate_show($id, $duplicate_with) {
		require(self::include_dir."duplicate_show.php");
	}
	/* }}} */
