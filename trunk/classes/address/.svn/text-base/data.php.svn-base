<?php
	...
	Class Address_data {
		 ...
		 * else show text
		 *
		 * @todo move this to the output handler
		 *
		 * @param string $phonenr the phonenumber to parse
		 * @param int $address_id optional address id to link call to relation
		 * @param int $user_id optional userid to link call to user
		 * @return string the link or text to show
		 */
		public function show_phonenr($phonenr, $address_id = 0, $user_id = 0) {
			if ($GLOBALS["covide"]->license["has_voip"]) {
				$output = new Layout_output();
				$output->insertLink($phonenr, array(
					"href" => "javascript: loadXML('index.php?mod=voip&action=call&number=".preg_replace("/[\(|\)]/si","",str_replace("-","",str_replace(" ","",str_replace("+","00",$phonenr))))."&address_id=$address_id&user_id=$user_id');"
				));
		...
		public function generate_letterinfo($data) {
			/* use the same hask/trick as in insertAction, html_output, to reduce memory usage */
			require_once(self::include_dir."dataGenerateLetterinfo.php");
			Address_data_letterinfo::add($this);
			return Address_data_letterinfo::generateLetterInfo($data);
		}
		/* }}} */
		/* store2db {{{ */
		/**
			...
			if (!$addressid) {
				...
				if ($type == 0) {
					$sql = sprintf("
						...
						ON
							address.id = address_businesscards.address_id AND address_businesscards.rcbc = 1
						WHERE address.id=%d", $addressid);
				}
				$res = sql_query($sql);
				$addressinfo = sql_fetch_assoc($res);
				/* fix companyname for persons in relationlist */
				if ($addressinfo["is_person"]) {
					$companyname_temp = $addressinfo["surname"].", ".$addressinfo["givenname"]." ".$addressinfo["infix"];
		...
		public function saveLayoutAddress($request) {
			$user_id = $_SESSION["user_id"];
			$user_data = new User_data();
			$current_settings = $user_data->getUserdetailsById($user_id);
			$cur_dash = $current_settings["addresslayout"];
			$cur_dash = unserialize($cur_dash);
			$cur_dash[$request["col"]] = $request["relcard"];
			$cur_dash = serialize($cur_dash);
			$sql = sprintf("UPDATE users SET addresslayout = '%s' WHERE id = %d", $cur_dash, $user_id);
			$res = sql_query($sql);
		}
		/* }}} */
		/* getRelationsArray {{{ */
		 ...
		 *
		 * @todo Fix the if (!$t) { ... $sql .= "AND address$t madness. Inside this if block the $t should be gone
		 *
		...
		public function getBcardsByRelationID($address_id, $search="", $classification = "") {
			...
			$query = sprintf("SELECT address_businesscards.*,address_businesscards.id as bc_id, google_address_sync.google_id FROM address_businesscards LEFT JOIN google_address_sync ON address_businesscards.id = google_address_sync.address_id AND google_address_sync.user_id = ".$_SESSION["user_id"]." WHERE (address_businesscards.address_id = %1\$d OR multirel %2\$s '(^|\\\\,)%1\$d(\\\\,|$)') %3\$s ORDER BY rcbc DESC,surname,givenname", $address_id, $regex_syntax, $sql_cla);
			$res = sql_query($query);
			while($row = sql_fetch_assoc($res)) {
				if (trim($row["business_phone_nr"])) { $row["business_phone_nr_link"]  = $this->show_phonenr($row["business_phone_nr"], $address_id, $_SESSION["user_id"]); }
				if (trim($row["business_mobile_nr"])) { $row["business_mobile_nr_link"] = $this->show_phonenr($row["business_mobile_nr"], $address_id, $_SESSION["user_id"]); }
				if (trim($row["personal_phone_nr"])) { $row["personal_phone_nr_link"]  = $this->show_phonenr($row["personal_phone_nr"], $address_id, $_SESSION["user_id"]); }
				if (trim($row["personal_mobile_nr"])) { $row["personal_mobile_nr_link"] = $this->show_phonenr($row["personal_mobile_nr"], $address_id, $_SESSION["user_id"]); }
				...
				if ($user_info["xs_funambol"] && $GLOBALS["covide"]->license["has_funambol"] && !$GLOBALS["covide"]->license["disable_basics"]) {
		...
		/* toggleGoogleSync {{{ */
		/**
		 * Toggle synchronization state of a businesscard for a user to the Google Contacts service
		 *
		 * @param int $bc_id The businesscard id to sync/desync
		 * @param string $toggleaction Can be 'activate' or 'deactivate'
		 * @param int $user_id The user to do this action for
		 *
		 * @return void
		 */
		public function toggleGoogleSync($bc_id, $toggleaction, $user_id) {
			// grab user data
			$user_data = new User_data();
			$user_info = $user_data->getUserDetailsById($user_id);
			if ($user_info["google_username"] && $user_info["google_password"]) {
				// grab businesscard data
				$address_info = $this->getAddressById($bc_id, "bcards");
				$output = new Layout_output();
				$google_data = new Google_data();
				if ($toggleaction == "activate") {
					// check if we already have this one in our google_address_sync table
					$sql = sprintf("SELECT COUNT(*) FROM google_address_sync WHERE user_id = %d AND address_id = %d", $user_id, $bc_id);
					$res = sql_query($sql);
					if (sql_result($res, 0) == 0) {
						// create the item in google service
						$google_id = $google_data->createGoogleContact($user_info["google_username"], $user_info["google_password"], $address_info);
						// add the entry to our sync table
						$sql = sprintf("INSERT INTO google_address_sync (user_id, address_id, google_id) VALUES (%d, %d, '%s');", $user_id, $bc_id, $google_id);
						$res = sql_query($sql);
						$image = $output->replaceImage("f_nieuw.gif");
					} else {
						$this->toggleGoogleSync($bc_id, 'deactivate', $user_id);
						exit();
					}
				} elseif ($toggleaction == "deactivate") {
					$sql = sprintf("SELECT google_id FROM google_address_sync WHERE address_id = %d AND user_id = %d", $bc_id, $user_id);
					$res = sql_query($sql);
					if (sql_num_rows($res)) {
						$google_id = sql_result($res, 0);
						if ($google_data->deleteGoogleContact($user_info["google_username"], $user_info["google_password"], $google_id)) {
							$sql = sprintf("DELETE FROM google_address_sync WHERE user_id = %d AND address_id = %d AND google_id = '%s'", $user_id, $bc_id, $google_id);
							$res = sql_query($sql);
							// delete the item in google service
							$image = $output->replaceImage("f_oud.gif");
						} else {
							$image = $output->replaceImage("f_nieuw.gif");
						}
					} else {
						$this->toggleGoogleSync($bc_id, 'activate', $user_id);
						exit();
					}
				}
				// alter the bullet in the addresslist
				echo sprintf("document.getElementById('toggle_sync_%d').src='%s';", $bc_id, $image);
				echo "\n$('#infowait').hide();";
			} else {
				$message = gettext("No google username and/or password for this user. Please specify those in the usersettings.");
				echo sprintf("alert('%s');", $message);
			}
		}
		/* }}} */
		...
		public function getEmployeeBirthdays() {
			$q = "SELECT id, givenname, surname, timestamp_birthday FROM address_businesscards WHERE timestamp_birthday != 0 AND timestamp_birthday IS NOT NULL AND id IN (select address_id from users where is_active = 1 AND address_id IS NOT NULL)";
			$res = sql_query($q);
			while ($row = sql_fetch_assoc($res)) {
		...
		public function import_save($data) {
			...
			$output->start_javascript();
				...
				$output->addCode("setTimeout('closepopup();', 100);");
			$output->end_javascript();
			$output->layout_page_end();
			$output->exit_buffer();
		}
		...
		public function savecla_multi($data) {
			$ids = ($data["inline"]) ? $data["addressid"] : $data["options"]["ids"];
			$classificication_ids = $data["address"]['classification'];

			if ($data["inline"]) {
				if ($data["addresstype"] == "relations") {
					$sql = sprintf("UPDATE address_info SET classification = '%s' WHERE address_id IN (%s) ", $classificication_ids, $ids);

					/* Also update classificiaton in rcbc */
					$sql_bc = sprintf("UPDATE address_businesscards SET classification = '%s' WHERE address_id IN (%s) AND rcbc = 1", $classificication_ids, $ids);
					$res_bc = sql_query($sql_bc);
				} else if ($data["addresstype"] == "bcards") {
					$sql_upd = sprintf("UPDATE address_businesscards SET classification = '%s' WHERE id IN (%s)", $classificication_ids, $ids);
					$res_upd = sql_query($sql_upd);

					/*make update on address_info*/
					$sql = sprintf("SELECT id, address_id, rcbc, classification FROM address_businesscards WHERE id IN (%s) and rcbc=1", $ids);
					$res = sql_query($sql);
					while ($row = sql_fetch_assoc($res)) {
						if ($row['rcbc']) {
							$sql = sprintf("UPDATE address_info SET classification = '%s' WHERE address_id=%s ", $classificication_ids, $row['address_id']);
							$res_upd = sql_query($sql);
						}
					}
				}
			} else {
				$common_cla = explode("|", $data["options"]["common_cla"]);

				/* Get previous classificaiton */
				if ($data["addresstype"] == "relations") {
					$sql = sprintf("SELECT id, address_id, rcbc, classification FROM address_businesscards WHERE address_id IN (%s) and rcbc=1", $ids);
				} else {
					$sql = sprintf("SELECT id, address_id, rcbc, classification FROM address_businesscards WHERE id IN (%s)", $ids);
				}
				$res = sql_query($sql);
				while ($row = sql_fetch_assoc($res)) {
					$cur_cla = explode("|", $row['classification']);
					$diff_cla = array_diff($cur_cla, $common_cla);
					$new_cla = $classificication_ids . "|" . implode("|", $diff_cla);
					/* make unique */
					$new_cla = explode("|", $new_cla);
					$new_cla = array_unique($new_cla);
					$new_cla = implode("|", $new_cla);

					$sql_upd = sprintf("UPDATE address_businesscards SET classification = '%s' WHERE id = %s", $new_cla, $row['id']);
					$res_upd = sql_query($sql_upd);

					/*make update on address_info*/
					if ($row['rcbc']) {
						$sql = sprintf("UPDATE address_info SET classification = '%s' WHERE address_id=%s ", $new_cla, $row['address_id']);
						$res_upd = sql_query($sql);
					}
				}
			}
		...
		public function save_hrminfo($hrmdata) {
			...
			if ($row["count"]) {
				...
				$sql .= sprintf("gender = %d, contract_type = '%s', contract_hours = %d, contract_holidayhours = %d, timestamp_stop = %d, gross_wage = %01.2f, overhead = %d, kilometer_allowance = %d WHERE user_id = %d",
					$hrmdata["gender"], $hrmdata["contract_type"], $hrmdata["contract_hours"], $hrmdata["contract_holidayhours"], $contract_stop, $hrmdata["gross_wage"], $hrmdata["overhead"], $hrmdata["kilometer_allowance"], $hrmdata["user_id"]);
			} else {
				/* no record found, so insert this info */
				$sql  = "INSERT INTO employees_info (user_id, social_security_nr, timestamp_started, timestamp_birthday, gender, contract_type, contract_hours, contract_holidayhours, timestamp_stop, evaluation, gross_wage, overhead, kilometer_allowance) ";
				$sql .= sprintf("VALUES (%d, '%s', %d, %d, %d, '%s', %d, %d, %d, '%s', %01.2f, %d, %d)",
					$hrmdata["user_id"], $hrmdata["social_security_nr"], $contract_start, $birthday, $hrmdata["gender"], $hrmdata["contract_type"],
					$hrmdata["contract_hours"], $hrmdata["contract_holidayhours"], $contract_stop, $hrmdata["evaluation"], $hrmdata["gross_wage"], $hrmdata["overhead"], $hrmdata["kilometer_allowance"]);
			}
			$res = sql_query($sql);
		...
		/* get_metagroups{{{ */
		/**
		 * Get the global metagroup from db
		 * @param ID if looking for a specific global field
		 */
		public function get_metagroups($id = 0) {
			if ($id != 0) {
				$group_id = "AND id = " . $id;
			} else {
				$group_id = "";
			...
			$sql = "SELECT * FROM meta_groups WHERE 1 = 1 " . $group_id . " ORDER BY id";
			$res = sql_query($sql);
			$fields = array();
			$i = 0;
			while ($row = sql_fetch_assoc($res)) {
				...
				switch ($row["fieldtype"]) {
					case 1:
						$fields[$i]["h_fieldtype"] = gettext("short text");
						break;
					case 2:
						$fields[$i]["h_fieldtype"] = gettext("long text");
						break;
					case 3:
						$fields[$i]["h_fieldtype"] = gettext("date");
						break;
					case 4:
						$fields[$i]["h_fieldtype"] = gettext("yes/no");
						break;
				}
			}
			return $fields;
		...
		public function get_global($id=0) {
			if($id != 0) {
				$meta_id = "AND meta_table.id = $id";
			} else {
				$meta_id = "";
			}
			$sql = "SELECT meta_table.*, meta_groups.name as groupname FROM meta_table LEFT JOIN meta_groups ON meta_groups.id = meta_table.group_id WHERE  meta_table.record_id = 0 $meta_id ORDER BY id";
			$res = sql_query($sql);
			$fields = array();
			$i = 0;
			while ($row = sql_fetch_assoc($res)) {
		...
		public function saveExportInfo($exportoptions) {
			...
			$q = sprintf("insert into address_selections (user_id, datetime, info) values (%d, %d, '%s')",
				$_SESSION["user_id"], time(), serialize($exportoptions));
			sql_query($q);
		...
		/* getExportInfo {{{ */
		/**
		 * Get saved settings from database to regenerate the addresslist as we had it on savetime
		 *
		 * @param int $id The specific snapshot
		 * @param int $user_id Optional user_id to grab the view for
		 *
		 * @return array options array to pass to address_data->getRelationsList
		 */
		public function getExportInfo($id, $user_id = 0) {
			if (!$user_id) {
				$user_id = $_SESSION["user_id"];
			}
		...
		public function keepPrivate($id) {
			$q = sprintf("update address_businesscards set sync_added = 0 where id = %d", $id);
			sql_query($q);
		}
		/* }}} */
		/* deactivateSelection{{{*/
		...
		*/
		public function deactivateSelection($options) {
			if (is_array($options)) {
				...
				foreach ($addresses["address"] AS $k) {
					...
					$q = sprintf("update address set is_active = 0 where id = %d", $id);
					sql_query($q);
				}
			}
			/* small output to close window */
		...
		/* }}} */
		/* activateSelectionExec{{{*/
		/**
		* Sets a addressbook selection on active
		*
		* @param array with options for getRelationlist
		*/
		public function activateSelectionExec($options) {
			if (is_array($options)) {
				$options["options"]["nolimit"] = 1;
				$addressdata = new Address_data();
				$addresses = $addressdata->getRelationsList($options["options"]);
				foreach ($addresses["address"] AS $k) {
					...
					$q = sprintf("update address set is_active = 1 where id = %d", $id);
					sql_query($q);
				}
			}
			/* small output to close window */
		...
		*/
		 ...
		 */
		public function checkrcbc($addressdata, $update_modified_field = 0) {
		 ...
		 * @param int $verbose Use the getAddressById function when this param is set
		 *
		 * @return array RCBC info, false if none exists
		 */
		public function getRCBCByAddressId($address_id, $verbose = 0) {
			if ($verbose) {
				$sql = sprintf("SELECT id FROM address_businesscards WHERE rcbc = 1 AND address_id = %d", $address_id);
				$res = sql_query($sql);
				$bcid = sql_result($res, 0);
				return $this->getAddressById($bcid, "bcards");
			} else {
		...
		public function saveSortAndSelect($data, $type) {
			...
			switch ($type) {
			case "private":
				$sql_field = "default_address_fields_private";
				break;
			case "users":
				$sql_field = "default_address_fields_users";
				break;
			case "bcards":
					...
					$output->addCode("
						if (parent) {
							if (parent.location.href.match(/\?mod=address/gi)) {
								...
								if (parent.document.getElementById('classifications_frm')) {
									parent.document.getElementById('classifications_frm').submit();
								} else {
									parent.document.getElementById('deze').submit();
								}
							}
		...
		/* }}} */
		/* updateExportInfoXML {{{ */
		public function updateExportInfoXML($ids, $oldexportinfo) {
			$options = $this->getExportInfo($oldexportinfo);
			$options["ids"] = $ids;
			$newid = $this->saveExportInfo($options);
			echo $newid;
		}
		/* }}} */
		/* getNewsletterData {{{ */
		/**
		 * Get information needed to send a newsletter
		 * I know we should actually use getRelationsList and limit
		 * what is done there. But since that function is huge, and I want
		 * to get this done without too much codechanges I created this function
		 *
		 * Example call to this function:
		 * $list = $address_data->getNewsletterData(array("addresstype"=>$_REQUEST["addresstype"], "emailtype"=>$_REQUEST["emailtype"], "nolimit"=>1, "sub"=>$sub, "newsletterselection" => 1));
		 *
		 * @todo integrate this into getRelationsList();
		 * @param array $settings information needed to come to a nice sql query. See description for an example
		 * @return array emailaddresses and such
		 */
		public function getNewsletterData($settings) {
			if (!is_array($settings)) {
				die("invalid usage of the getNewsletterData function");
			}
			$addresstype = $settings["addresstype"];
			if (is_array($settings["classifications"])) {
				$sq = " ((";

				$regex_syntax = sql_syntax("regex");
				if ($settings["addresstype"]=="bcards") {
					$regex_field = "address_businesscards.classification";
				} else {
					$regex_field = "address_info.classification";
				}

				if ($settings["classifications"]["selectiontype"]=="and") {
					$op = "and";
					$cla_positive_sq = "1=1";
				} else {
					$op = "or";
				...
				$cla_positive = explode("|", $settings["classifications"]["positive"]);
				$cla_negative = explode("|", $settings["classifications"]["negative"]);
				...
				if ($addresstype == "users") {
					if (!$settings["sub"] || $settings["sub"] == "actief") {
						$active_users = 1;
					} else {
						$active_users = 0;
					}
					...
					$query_count = sprintf("
						...
						FROM address
						INNER JOIN address_info ON
							address_info.address_id=address.id
						WHERE
							(address.is_public=1 OR address.user_id= %d)
							AND (%s)",
						$_SESSION["user_id"], $sq);
					$query_zoek = sprintf("
						SELECT address.*, address_info.classification, address_info.provision_perc
						FROM address
						INNER JOIN address_info ON
							address_info.address_id=address.id
						WHERE
					...
					$query_count = "SELECT count(address_businesscards.id) FROM address_businesscards $bjoin WHERE ((address_businesscards.business_email != '' OR address_businesscards.personal_email != '') AND $sq AND ((address_businesscards.rcbc = 0 OR address_businesscards.rcbc IS NULL) OR (address_businesscards.rcbc = 1 AND address.is_active=1))) ";
					$query_zoek = "SELECT
					...
					FROM address_businesscards $bjoin WHERE ((address_businesscards.business_email != '' OR address_businesscards.personal_email != '') AND $sq AND ((address_businesscards.rcbc = 0 OR address_businesscards.rcbc IS NULL) OR (address_businesscards.rcbc = 1 AND address.is_active=1))) ";
				}

				/* Private addresses only */
				if ($addresstype != "bcards" && $addresstype != "users") {
					$query .= " AND (address$t.is_public=1 OR ((address$t.is_public=0) AND (address$t.user_id=".(int)$_SESSION["user_id"]."))) ";
				}

				if (($settings["addresstype"] != "bcards") && ($settings["addresstype"] != "private") && ($settings["addresstype"] != "users") && ($settings["addresstype"] != "overig")) {
					if ($settings["addresstype"] == "nonactive") {
				...
				while ($row = sql_fetch_assoc($result)) {
					$addressinfo[$i]["id"] = $row["id"];
					$addressinfo[$i]["email"] = $row["email"];
					$addressinfo[$i]["personal_email"] = $row["personal_email"];
					$i++;
				}
			}
			return $addressinfo;
		...
		/* remove_view {{{ */
		/**
		 * Remove stored address selection from the session
		 */
		public function remove_view() {
			unset($_SESSION["address_view"]);
			echo "document.location.href='?mod=address';";
		}
		/* }}} */
		/* duplicate_delete {{{ */
		/**
		 * Delete duplicte data
		 *
		 * @param int The id of dublicate record
		 * @return bool true on success, false on error
		 */
		function duplicate_delete($id) {
			//if(!$id) return false;
			$del_sql = sprintf("DELETE FROM address WHERE id = %d", $id);
			$res = sql_query($del_sql);

			$del_sql = sprintf("DELETE FROM address_businesscards WHERE address_id = %d", $id);
			$res = sql_query($del_sql);
		...
		/* duplicate_approve {{{ */
		/**
		 * Approve the duplicte data
		 *
		 * @param int The id of dublicate record
		 * @return bool true on success, false on error
		 */
		function duplicate_approve($id) {
			$upd_sql = sprintf("UPDATE address SET duplicate_with = 0 WHERE id = %d", $id);
			$res = sql_query($upd_sql);

			$upd_sql = sprintf("UPDATE address_businesscards SET duplicate_with = 0 WHERE address_id = %d", $id);
			$res = sql_query($upd_sql);
		...
		/* getById {{{ */
		/**
		 * Get the businesscards by ID
		 *
		 * @param int The address_businesscars id
		 * @return array record array
		 */
		 function getById($id) {
			 $sql = sprintf("SELECT * FROM address WHERE id = %s", $id);
			 $res = sql_query($sql);
			 return $data = sql_fetch_assoc($res);
		 }
		/* }}} */
		/* validate_mail {{{ */
		/**
