	...
	public function __construct() {
		...
		switch ($_REQUEST["action"]) {
			...
			case "save_bcard" :
				...
				break;
			case "saveLayoutAddress" :
				$address_data = new Address_data();
				$address_data->saveLayoutAddress($_REQUEST);
				break;
			case "cardrem" :
				$address_data = new Address_data();
				...
				if ($_REQUEST["closewin"] == 1) {
					...
					$output->start_javascript();
						$output->addCode("parent.document.getElementById('deze').submit();");
			...
			case "togglesync" :
				...
				break;
			case "togglegooglesync" :
				$address_data = new Address_data();
				$address_data->toggleGoogleSync($_REQUEST["bc_id"], $_REQUEST["toggleaction"],
					($_REQUEST["user_id"]) ? $_REQUEST["user_id"]:$_SESSION["user_id"]);
				break;
			case "export" :
				$address_output = new Address_output();
			...
			case "addcla_inline" :
				$address_output = new Address_output();
				$address_output->addcla_multi($_REQUEST["info"], 1);
				break;
			case "addcla_multi" :
				$address_output = new Address_output();
			...
			case "showmetagroups" :
				$address_output = new Address_output();
				$address_output->show_metagroups();
				break;
			case "glob_metalist" :
				$address_output = new Address_output();
			...
			case "activateSelection":
				$address_output = new Address_output();
				$address_output->activateSelection($_REQUEST["info"]);
				break;
			case "activateSelectionExec":
				$address_data = new Address_data();
				$address_data->activateSelectionExec($_REQUEST);
				break;
			...
			case "selectRows" :
				$address_output = new Address_output();
				$address_output->selectRows($_REQUEST["type"]);
				break;
			case "saveSortAndSelect" :
				$address_data = new Address_data();
				$address_data->saveSortAndSelect($_REQUEST["sort"], $_REQUEST["type"]);
				break;
			case "updateExportInfoXML" :
				$address_data = new Address_data();
				$address_data->updateExportInfoXML($_REQUEST["ids"], $_REQUEST["oldexportinfo"]);
				break;
			case "remove_view" :
				$address_data = new Address_data();
				$address_data->remove_view();
				break;
			case "duplicate_show" :
				$address_output = new Address_output();
				$address_output->duplicate_show($_REQUEST["id"], $_REQUEST["duplicate_with"]);
				break;
			case "duplicate_approve" :
				$address_data = new Address_data();
				$address_data->duplicate_approve($_REQUEST["id"]);
				break;
			case "duplicate_delete" :
				$address_data = new Address_data();
				$address_data->duplicate_delete($_REQUEST["id"]);
				break;
			case "validateEmail" :
				$address_data = new Address_data();
				$address_data->validate_mail($_REQUEST["email"]);
				break;
			default :
