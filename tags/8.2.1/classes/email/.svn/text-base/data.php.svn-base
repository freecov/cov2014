...
Class Email_data {
	...
	public function get_tracker_items($mail_id, $id="", $limit=0, $only_queue=0) {
		$emails = array();
		if ($mail_id) {
			if ($only_queue) 
	...
	/* getAutoreplyDB {{{ */
	/**
	 * Get the DB object for the postfix virtual vacation database
	 *
	 * @return MDB2 database object
	 */
	public function getAutoreplyDB() {
	...
	public function getAutoreplyByUserID($user_id) {
		$db_postfix = $this->getAutoreplyDB();
	...
	/* autoreplySave {{{ */
	/**
	 * Save a users autoreply to the database
	 *
	 * @param array $autoreply Form fields
	 * @param int $return if set will return, if not will call javascript window.close()
	 * @return true on success, false on failure
	 */
	public function autoreplySave($autoreply, $return = 1) {
		$db_postfix = $this->getAutoreplyDB();
