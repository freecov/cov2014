 ...
 */
	...
	$venster->addVensterData();
		...
		$tbl->addTableRow();
			$tbl->addTableData();
				if (strtolower($_SESSION["locale"]) == "nl_nl") {
					$tbl->insertAction("ftype_pdf", gettext("cms quickstart"), "javascript: popup('documentation/manual/quickstart_cms_7_nl.pdf', 'manual');");
					$tbl->addSpace();
					$tbl->insertTag("a", gettext("cms quickstart"), array("href" => "javascript: popup('documentation/manual/quickstart_cms_7_nl.pdf', 'manual');"));

					$tbl->addSpace(2);
					$tbl->insertAction("ftype_pdf", gettext("cms manual"), "javascript: popup('documentation/manual/manual_cms_7_nl.pdf', 'manual');");
					$tbl->addSpace();
					$tbl->insertTag("a", gettext("cms manual"), array("href" => "javascript: popup('documentation/manual/manual_cms_7_nl.pdf', 'manual');"));
				}
				$tbl->addSpace(2);
