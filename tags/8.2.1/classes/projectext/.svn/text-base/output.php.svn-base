				...
				if ($xs) {
					$tbl->addTableRow();
						...
						$tbl->addTableData(array("width" => "95%"), "data");
							$this->switchFieldType($tbl, $output_only, $v, 0, $project_id);
						$tbl->endTableData();
					$tbl->endTableRow();
				} elseif (!$output_only) {
			...
			case 2:
				...
				if ($output_only) {
					if ($v["value"] > 0) {
						...
						$tbl->insertAction("go_calendar", gettext("plan in calendar"), "javascript: popup('index.php?mod=calendar&action=edit&id=0&user=".$_SESSION["user_id"]."&project_id=".$project_id."&datemask=".$v["value"]."', 'cal', 0, 0, 1);");
					} else {
						$tbl->addCode("--");
					}
				} else {
	...
	public function genExtraProjectFields($project_id=0, $output_only=0) {/*{{{*/
		...
		foreach ($fields as $k=>$v) {
			$tbl->addTableRow();
				...
				$tbl->endTableData();
				$tbl->addTableData(array("width" => "95%"), "data");
					$this->switchFieldType($tbl, $output_only, $v, 1, $project_id);
				$tbl->endTableData();
			$tbl->endTableRow();
		}
		$tbl->endTable();
					...
					$tbl->addTableRow();
						...
						$tbl->addTableData();
							...
							$tbl->addTag("div", array("id" => "project_bcard_layer"));
								$tbl->addHiddenField("project[bcard]", 0);
							$tbl->endTag("div");
						$tbl->endTableData();
					$tbl->endTableRow();
					$tbl->endTable();
