 ...
 */
	...
	$c_metacount = 0;
	$q_metacount = sprintf("SELECT COUNT(*) FROM cms_metadata WHERE `value` LIKE '%%%s%%'", $keywords_plain);
	...
	if ($this->enable_ft_search) {
		$qc = sprintf("select count(*) from cms_data where %s and (%s ('%s' %s) %s)",
			$cms["base"], $cms["match"], $keywords, $type, $cms["meta"]);
	} else {
		$qc = sprintf("select count(*) from cms_data where %s and (%s %s)",
			$cms["base"], $this->alternativeSearch($fields, $keywords_plain), $cms["meta_plain"]);
	}
	...
	if ($this->enable_ft_search) {
		...
		$q = sprintf("select id, pageTitle, pageHeader, datePublication
			...
			$cms["base"],
			$this->alternativeSearch($fields, $keywords_plain),
			$cms["meta_plain"],
			$start-1,
			$pagesize
		);
