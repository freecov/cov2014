...
Class Project_output {
	 ...
	 * @return void
	 */
	public function payrollExport($start = 0, $end = 0) {
		$calendar_data = new Calendar_data();
		$project_data = new Project_data();
		...
		$hourslist = $project_data->getHoursList(array("start" => $start, "end" => $end));
		$items = array();
		if ($hourslist["items"]) {
			foreach ($hourslist["items"] as $item) {
				$hours = ($item["timestamp_end"] - $item["timestamp_start"]);
				$hours = $hours/3600;

				//add hours to day counter of the user
				if ($items[$item["user_id"]][date("d/m/y", $item["timestamp_start"])]["hours"]) {
					$items[$item["user_id"]][date("d/m/y", $item["timestamp_start"])]["hours"] += $hours;
				} else {
					$items[$item["user_id"]][date("d/m/y", $item["timestamp_start"])]["hours"] = $hours;
				}
			}
		...
		foreach ($items as $user_id => $payrollitems) {
			...
			foreach ($payrollitems as $day => $info) {
				...
				$csv[] = $hrminfo[0]["kilometer_allowance"];
