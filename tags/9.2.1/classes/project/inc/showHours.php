...
$q = sprintf("SELECT COUNT(id) FROM mail_messages WHERE folder_id = %d AND project_id = %d", $archive_mail["id"], $_REQUEST["id"]);
$r = sql_query($q);
...
if ($_REQUEST["showcomm"] == 1) {
	...
	$sql = sprintf("SELECT id, timestamp, user_id, subject, 'note' as type FROM notes WHERE project_id = %1\$d AND timestamp BETWEEN %3\$d AND %4\$d
		...
		UNION
		SELECT id, IF(date_received, date_received, date) as timestamp, user_id, subject, 'mail' as type FROM mail_messages WHERE folder_id = %2\$d AND project_id = %1\$d AND date_received BETWEEN %3\$d AND %4\$d
		UNION
...
if (array_key_exists("showall", $_REQUEST) && $_REQUEST["showall"] == 1 && ($userinfo["xs_projectmanage"] || $_SESSION["user_id"] == $projectinfo[0]["manager"])) {
	$_SESSION["showallprojectdetails"][$projectinfo[0]["id"]] = 1;
}
if (array_key_exists("showall", $_REQUEST) && $_REQUEST["showall"] == 0 && ($userinfo["xs_projectmanage"] || $_SESSION["user_id"] == $projectinfo[0]["manager"])) {
...
$grand_total_service_raw = $bulklist["total_service_raw"] + $hourslist["total_service_raw"];
...
if (!$this->has_declaration) {
	...
	$view_misclist->defineComplexMapping("complex_actions", array(
		...
		if (is_array($commitems)) {
			$table->addTableRow();
				$table->insertTableData(gettext("communication items"), array("colspan" => 2), "header");
			$table->endTableRow();
			$table->addTableRow();
		...
		$table->addTableRow();
			$table->addTableData(array("colspan" => 2));
				...
				$table_km->addTableRow(array("class" => "list_record"));
					$table_km->insertTableHeader(gettext("date"), array("class" => "list_header_center"));
					...
					$table_km->insertTableHeader(gettext("purchase"), array("class" => "project_purchase list_header_center"));
					$table_km->insertTableHeader(gettext("marge"), array("class" => "project_marge list_header_center"));
					$table_km->insertTableHeader(gettext("price"), array("class" => "project_price list_header_center"));
					$table_km->insertTableHeader(gettext("actions"), array("class" => "project_actions list_header_center"));
				$table_km->endTableRow();
				foreach($kmitems as $userid => $kminfo) {
					if (is_array($kminfo["items"])) {
						...
						foreach ($kminfo["items"] as $kmitem) {
							$table_km->addTableRow(array("class" => "list_record"));
								...
								$table_km->insertTableData(number_format($marge, 2), array("class" => "list_data_clean project_marge valign_top", "align" => "right"));
		...
		$table->addTableRow();
			$table->addTableData("", "header");
				...
				$table->addSpace(2);
				$table->addCode(gettext("total service hours").": ".$grand_total_service_raw.":00");
			$table->endTableData();
...
if ($this->has_declaration) {
	...
	if ($userinfo["xs_projectmanage"] || $_SESSION["user_id"] == $projectinfo[0]["manager"]) {
		if (!is_array($commitems)) {
