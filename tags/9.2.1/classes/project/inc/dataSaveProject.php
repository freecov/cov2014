...
$layout_convertsion = new Layout_conversion();

$projectinfo = $_REQUEST["project"];

// parse money string to integer number
$projectinfo['budget'] = $layout_convertsion->parseMoney($projectinfo['budget'],true);
...
if ($projectinfo["id"]) {
	...
	if (!$projectinfo["master"]) {
		...
		$sql .= sprintf(", kilometer_allowance=%d", $projectinfo["kilometer_allowance"]);
	...
	if (!$projectinfo["master"]) {
		...
		$fields[] = "kilometer_allowance";     $values[] = sprintf("%d",   $projectinfo["kilometer_allowance"]);
...
if ($debug) {
	...
	$output->layout_page("", 1);
		$output->start_javascript();
			$output->addCode("
				if (parent.document.getElementById('projectsearch')) {
					parent.document.getElementById('projectsearch').submit();
				} else if (parent.document.getElementById('hiddeninfo')) {
					parent.document.getElementById('hiddeninfo').submit();
			...
			if ($GLOBALS["covide"]->license["has_project_ext"] && !$projectinfo["master"]) {
				...
				$output->addCode("
					...
					document.location.href='index.php?mod=project&action=edit&id=".$projectinfo["id"]."';
				");
			} else {
				$output->addCode("
					//opener.document.location.href = opener.document.location.href;
