...
Class Google_data {
	...
	public function syncGoogleCalendar($user_id, $username, $password) {
		...
		$sql = sprintf("SELECT calendar_id, google_id FROM calendar_user right join calendar on calendar.id = calendar_user.calendar_id WHERE timestamp_start >= %d AND user_id = %d AND google_id != '0'", (mktime()-86400), $user_id);
		$res = sql_query($sql);
		...
		foreach ($cal as $v) {
			...
			if (sql_num_rows($res)) {
				...
				$sql = sprintf("UPDATE calendar SET subject = '%s', body = '%s', location = '%s', timestamp_start = %d, timestamp_end = %d WHERE id = %d",
					$v["subject"], $v["description"], $v["location"], $v["timestamp_start"], $v["timestamp_end"], sql_result($res, 0));
				$res = sql_query($sql);
			} else {
				...
				$sql = sprintf("INSERT INTO calendar (subject, body, location, timestamp_start, timestamp_end) VALUES ('%s', '%s', '%s', %d, %d)",
					$v["subject"], $v["description"], $v["location"], $v["timestamp_start"], $v["timestamp_end"]);
				$res = sql_query($sql);
