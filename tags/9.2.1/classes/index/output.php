...
Class Index_output {
	...
	public function show_index() {
		...
		$tbl->addTableRow();
			$tbl->addTableData();
				$tbl->insertTag("b", gettext("selection"));
			$tbl->endTableData();
		$tbl->endTableRow();
		$tbl->addTableRow();
			$tbl->addTableData();
				$tbl->addSelectField("search[start_day]", $days, $_REQUEST["search"]["start_day"]);
				$tbl->addSelectField("search[start_month]", $months, $_REQUEST["search"]["start_month"]);
				$tbl->addSelectField("search[start_year]", $years, $_REQUEST["search"]["start_year"]);
			$tbl->endTableData();
		$tbl->endTableRow();
		$tbl->addTableRow();
			$tbl->addTableData();
				$tbl->addSelectField("search[end_day]", $days, $_REQUEST["search"]["end_day"]);
				$tbl->addSelectField("search[end_month]", $months, $_REQUEST["search"]["end_month"]);
				$tbl->addSelectField("search[end_year]", $years, $_REQUEST["search"]["end_year"]);
			$tbl->endTableData();
		$tbl->endTableRow();
		$tbl->addTableRow();
			$tbl->addTableData();
		...
		$venster->addVensterData();
			...
			if (count($data["notes"]) > 0) {
				...
				$view->addMapping(gettext("done"), "%%complex_isdone");
				...
				$view->defineComplexMapping("complex_isdone", array(
					array(
						"type" => "action",
						"src"  => "ok",
						"check" => "%is_done"
			...
			/* support */
			if (count($data["support"]["items"]) > 0) {
				$venster->insertTag("b", gettext("support"));
				$view = new Layout_view();
				$view->addData($data["support"]["items"]);
				...
				$this->limit_view(count($data["support"]["items"]), $view, $venster, $data["support"]["items"], "support");
				$venster->addTag("br");
			}
			
			/* projects */
			if (count($data["projects"]) > 0) {
				$venster->insertTag("b", gettext("projects"));
				$view = new Layout_view();
				$view->addData($data["projects"]);
				/* map fields to output columns */
				$view->addMapping(gettext("name"), "%%complex_name");
				$view->addMapping(gettext("description"), "%description");
				...
				$this->limit_view(count($data["projects"]), $view, $venster, $data["projects"], "projects");
				$venster->addTag("br");
			}
			
			/* sales */
			if (count($data["sales"]) > 0) {
				$venster->insertTag("b", gettext("sales"));
				$view = new Layout_view();
				$view->addData($data["sales"]);

				/* map fields to output columns */
				$view->addMapping(gettext("salesitem"), "%%complex_subject");
				$view->addMapping(gettext("contact"), "%%complex_relation");
				$view->addMapping(gettext("project"), "%%complex_project");
				...
				$view->defineComplexMapping("complex_relation", array(
					array(
						"type" => "multilink",
				...
				$this->limit_view(count($data["sales"]), $view, $venster, $data["sales"], "sales");
				$venster->addTag("br");
			}
