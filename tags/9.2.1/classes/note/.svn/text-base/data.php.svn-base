	...
	public function store2db($note) {
		...
		if (!$note["id"]) {
			...
			/* If the AFP field is submitted, put some standard values in the note */
					...
					$sql .= sprintf(", %d, '%s', %d, %d, %d", $note["phonecall"], $note["phone_nr"], $note["address_businesscards_id"], $note["phonecall_start"], ($note["phonecall"]?mktime():0));
					$sql .= ")";
					$res = sql_query($sql);
			...
			$sql .= sprintf(", phonecall=%d, phone_nr='%s', address_businesscards_id=%d, phonecall_start=%d, phonecall_end=%d", $note["phonecall"], $note["phone_nr"], $note["address_businesscards_id"], $note["phonecall_start"], ($note["phonecall"]?mktime():0));
