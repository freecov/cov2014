...
switch ($options["note_type"]) {
	...
	case "show"   : $subtitle = gettext("sent notes, unread");   break;
	case "drafts" : $subtitle = gettext("draft notes");          break;
	case "old"    : $subtitle = gettext("received notes, done"); break;
	default       : $subtitle = gettext("received notes");       break;
}
$settings = array(
	"title"    => gettext("Notes"),
	"subtitle" => $subtitle
...
unset($settings);
$venster->addMenuItem(gettext("new note"), "javascript: note_new();", "", 0);
if ($_SESSION["locale"] == "nl_NL") {
	$venster->addMenuItem(gettext("help (wiki)"), "http://wiki.covide.nl/Notities", array("target" => "_blank"), 0);
}
$venster->addMenuItem(gettext("received"), "./?mod=note&short_view=".$_short_view );
$venster->addMenuItem(gettext("received, done"), "./?mod=note&action=old&short_view=".$_short_view );
...
$venster->addVensterData();
	...
	$table->addTableData();
		...
		$table->start_javascript();
			$table->addCode("document.getElementById('searchsearchkey').focus();");
