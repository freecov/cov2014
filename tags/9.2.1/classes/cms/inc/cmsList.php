 ...
 */
	...
	$fields["def"] = array(
		...
		"pageHeader"  => gettext("page header"),
		"keywords"    => gettext("keywords"),
		"datumPublicatie" => gettext("publication date"),
		"thumb"       => gettext("page thumbnail")
	);
	/* add shop field if this cms has one */
	$q = sprintf("select cms_shop from cms_license");
	$res2 = sql_query($q);
	if (sql_result($res2,0) == 1)
		$fields["def"]["shopPrice"] = gettext("shop price");
	...
	$sel = array(
		...
		gettext("default fields") => array(
			...
			"pageLabel"   => gettext("label"),
			"keywords"    => gettext("keywords"),
			"datePublication" => gettext("publication date"),
			"shopPrice"   => gettext("shop price")
		),
		gettext("metadata velden (optionele metadata module)") => array()
	);
	$sel2 = array(
	...
	$sel = array(
		...
		"content" => gettext("at content marker")." (%%list%%)"
	);
	$tbl->addTableRow();
		$tbl->addTableData(array("colspan"=>3), "header");
			$tbl->insertAction("up", "", "");
