<?php
	...
	if ((int)$_REQUEST["day"] > 0)   { $day   = $_REQUEST["day"];   } else { $day   = (date("d") > 2) ? date("d") : 2; }
	...
	$output->addHiddenField("user_id", "");
	$output->addHiddenField("returnto", "monthview");
	$output->addHiddenField("month", $month);
	$output->addHiddenField("year", $year);
	$output->endTag("form");
	...
	$table->addTableRow();
		$table->addTableData();
			...
			$table->addHiddenField("user_id", "");
			$table->insertAction("print", gettext("print"), "javascript: oldpopup('index.php?mod=calendar&action=print_month&month=".$month."&year=".$year."', 'printmonth', 800, 300, 1);");
	...
	$table->addTableRow();
		$table->addTableData();
			$table_cal = new Layout_table(array("cellspacing" => 1, "width" => "100%", "style" => "border: 0px solid #CCC"));
			$table_cal->addTableRow();
				$table_cal->insertTableData(gettext("week"), array("width" => "1%", "style" => "border: 0px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("sun"), array("width" => "14%", "style" => "border: 0px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("mon"), array("width" => "14%", "style" => "border: 0px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("tue"), array("width" => "14%", "style" => "border: 0px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("wed"), array("width" => "14%", "style" => "border: 0px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("thu"), array("width" => "14%", "style" => "border: 0px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("fri"), array("width" => "14%", "style" => "border: 0px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("sat"), array("width" => "14%", "style" => "border: 0px solid #CCC"), "header");
			$table_cal->endTableRow();
			$table_cal->addTableRow();
				$table_cal->insertTableData(date("W", mktime(0, 0, 0, $month, 1, $year)), "", "header");
				/* skip the weekdays before first of the month */
				$daystoskip = date("w", mktime(0, 0, 0, $month, 1, $year));
				$dow = $daystoskip;
				for ($i = 0; $i < $daystoskip; $i++) {
					$table_cal->insertTableData("&nbsp;", array("style" => "border: 0px solid #CCC"));
				}
				for ($i=0; $i<date("t",mktime(0,0,0,$month,1,$year)); $i++) {
					if (($i+1) == date("d") && $month == date("m")) {
						$table_cal->addTableData(array("align" => "left", "style" => "border: 0px solid #CCC"), "header");
					} else {
						$table_cal->addTableData(array("align" => "left", "style" => "border: 0px solid #CCC"), "data");
					}
							...
							$table_day->addTableData(array("NOWRAP" => "NOWRAP"));
								if (count($calendar_data->calendar_items)) {
									foreach ($calendar_data->calendar_items as $v) {
										$table_day->insertAction("info", gettext("show"), "javascript: toonInfo(".$v["id"].", ".$user_id.");");
										if ($v["is_event"] != 1) {
					...
					if ($dow==7) {
						...
						if (!(($i+1)==date("t",mktime(0,0,0,date("m"),1,date("Y"))))) {
							$table_cal->endTableRow();
							$table_cal->addTableRow();
							$table_cal->insertTableData(date("W", mktime(0, 0, 0, $month, ($i+8), $year)), "", "header");
						}
						$dow=0;
					}
				}
