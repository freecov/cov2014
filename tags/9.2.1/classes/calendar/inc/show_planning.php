...
$hours = array();
for ($i=0; $i<24; $i++) {
	if($user["hour_format"] == 1) { 
		$prefDate = "g:i A"; 
	} else { 
		$prefDate = "H:i"; 
	}
	$hour_time = mktime($i, 0, 0, date("m"), date("d"), date("Y"));
	$hours[$i*60] = date($prefDate, $hour_time);
}
$days = array();
for ($i=1;$i<32;$i++)
	$days[$i] = $i;
...
$daystart = ($_REQUEST["daystart"]) ? ($_REQUEST["daystart"]) : (9*60);
$dayend = ($_REQUEST["dayend"]) ? ($_REQUEST["dayend"]) : (17*60);
...
$output->layout_page();
	...
	$output->addHiddenField("user_id", 0);
	...
	$venster->addVensterData();
			...
			$table_al->addTableData();
				...
				$table->addTableRow();
					...
					$table->insertAction("calendar_today", gettext("daily view"), sprintf("?mod=calendar&extrauser=%s&day=%d&month=%d&year=%d",
						...
						$table->insertAction("forward", gettext("search"), "javascript: document.getElementById('planning').submit();");
					$table->endTableData();
		...
		for ($i=0;$i<7;$i++) {
			...
			foreach ($items_arr[$_users[0]][$i] as $k=>$v) {
				if ($k > $daystart-1 && $k < $dayend && ($k % 30 == 0)) {

				$table->addTableRow();
					if ($k % 60 == 0) {
						$prefDate = ($user["hour_format"]) ? "g:i A" : "H:i";
						$datestring = date($prefDate, ($weekstart+$offset+(24*$i*60*60)+($k*60)));
						$table->insertTableData($datestring, array("rowspan" => 2), "data");
					}
					foreach ($_users as $user_id) {
						$xsperm = $calendar_data->checkPermission($user_id, $_SESSION["user_id"]);
						...
						$table->addTableData("", "data");
							...
							if ($calendar_items[$user_id][$i][$items_arr[$user_id][$i][$k]]["show_actions"] || $user_id == $_SESSION["user_id"]) {
								if ($items_arr[$user_id][$i][$k]) {
									$act = sprintf("toonInfo(%s, %s);", $calendar_items[$user_id][$i][$items_arr[$user_id][$i][$k]]["id"], $user_id);
								} else {
							...
							$table->insertTag("div", substr($legenda_text[$calendar_items[$user_id][$i][$items_arr[$user_id][$i][$k]]["app_type"]], 0, 9), array(
								"style" => "cursor: pointer; font-size: 10px; color: #fff; line-height: 10px; cursor: hand; width: 50px; height: 10px;".$legenda[$calendar_items[$user_id][$i][$items_arr[$user_id][$i][$k]]["app_type"]],
								"onclick" => $act
							));
						$table->endTableData();
					}
