...
Class Campaign_data {
	...
	public function getCampaignsBySearch($options) {
		...
		while ($row = sql_fetch_assoc($res)) {
			...
			if ($row["type"] == 2) {
				$row["is_callscript"] = 1;
				$q = sprintf("SELECT count(*) FROM campaign_records WHERE answer LIKE '%%%d%%' AND campaign_id = %d AND (call_again <= %d AND call_again > 0)", "5", $row["id"], mktime());
				$r = sql_query($q);
				if (sql_result($r, 0)) {
					$row["is_recallable"] = 1;
				} else {
	...
	public function save_edit_record($id, $options, $callscript=0, $calltime=0) {
		...
		if (in_array('2', $options)) {
			...
			$q = sprintf("update address_info set classification = '%s' where address_id = %d",
				$new_cla, $address_id);
			sql_query($q);
			$qu = sprintf("update address_businesscards set classification = '%s' where address_id = %d",
				$new_cla, $address_id);
			sql_query($qu);
		}
	...
	public function recallscript($id) {
		/* get current record of this user */
		$q = sprintf("select id from campaign_records where campaign_id = %d and answer LIKE '%%%d%%' AND (call_again <= %d AND call_again > 0)", $id, "5", mktime());
		$res = sql_query($q);
		if (sql_num_rows($res) > 0) {
			/* get current record for this user */
			$record = sql_result($res);
