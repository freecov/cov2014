...
elseif ($options["active"] == 1)
	$is_solved = "is_solved != 1";
elseif ($options["active"] == -1)
	$is_solved = "(is_solved != 1 OR is_solved = 1)";
else
	$is_solved = "is_solved = 1";
...
if ($options["search"]) {
	$search = sprintf(" AND (issues.solution $like '%%%1\$s%%' OR issues.description $like '%%%1\$s%%' OR issues.reference_nr like '%%%1\$s%%')", $options["search"]);
}
if ($options["date"]) {
	$date_sql = sprintf(" AND issues.timestamp BETWEEN %d AND %d", $options["date"]["start"], $options["date"]["end"]);
}

$sql = sprintf("SELECT issues.*, address.companyname, project.name AS projectname, users.username FROM issues %s WHERE %s", $join, $is_solved);
$sql_count = sprintf("SELECT COUNT(id) FROM issues WHERE %s", $is_solved);

$sql_extra = "";
if ($options["address_id"]) {
	$sql_extra .= sprintf(" AND issues.address_id = %d", $options["address_id"]);
} elseif ($options["user_id"]) {
	$sql_extra .= sprintf(" AND issues.user_id = %d", $options["user_id"]);
} elseif ($options["project_id"]) {
	$sql_extra .= sprintf(" AND issues.project_id = %d", $options["project_id"]);
}

$sql .= $sql_extra.$search.$date_sql;
$sql_count .= $sql_extra;

if (!$options["sort"]) {
	$sql.= " ORDER BY is_solved,timestamp DESC";
} else {
	$sql.= " ORDER BY ".sql_filter_col($options["sort"]);
	if ($options["sort2"]) {
...
while ($row = sql_fetch_assoc($res)) {
	...
	if ($row["is_solved"]) { $row["active"] = 0; $solved++; } else { $row["active"] = 1; }
...
$supportItems["solved"] = $solved;
$supportItems["unsolved"] = $count-$solved;
?>
