...
if ($supportitem[0]["bcard_id"]) {
	$address_info = $address_data->getAddressById($supportitem[0]["bcard_id"], "bcards");
	if (trim($address_info["business_address"])) {
		$address_info["address"] = $address_info["business_address"];
	}
	if (trim($address_info["business_zipcode"])) {
		$address_info["zipcode"] = $address_info["business_zipcode"];
	}
	if (trim($address_info["business_city"])) {
		$address_info["city"] = $address_info["business_city"];
	}
	if (trim($address_info["business_phone_nr"])) {
		$address_info["phone_nr"] = $address_info["business_phone_nr"];
	}
	if (trim($address_info["business_fax_nr"])) {
		$address_info["fax_nr"] = $address_info["business_fax_nr"];
	}
	if (trim($address_info["business_mobile_nr"])) {
		$address_info["mobile_nr"] = $address_info["business_mobile_nr"];
	}
} else {
...
$venster->addVensterData();
	...
	unset($view);
       $table = new Layout_table(array("cellspacing"=>0));
	$table->addTableRow();
...
$venster->addVensterData();
	...
	$table->addTableRow();
		$table->insertTableHeader(gettext("remarks"), array("align"=>"left"));
	$table->endTableRow();
...
$venster->addVensterData();
	...
	$table->addTableRow();
		$table->insertTableHeader(gettext("Date"), array("align"=>"left"));
	$table->endTableRow();
	...
	$table->addTableRow();
		$table->AddTableData();
			$table->addTag("b");
				$table->addCode(gettext("time of arrival").":", array("align"=>"left"));
			$table->endTag("b");
			$table->addSpace(1);
			if (sprintf("%d", $supportitem[0]["arrival"]) > 0 ) {
				$table->addCode(date("H:i", $supportitem[0]["arrival"]));
			}
		$table->endTableData();
	$table->endTableRow();
	...
	$table->addTableRow();
		$table->AddTableData();
			$table->addTag("b");
				$table->addCode(gettext("time of departure").":", array("align"=>"left"));
			$table->endTag("b");
			$table->addSpace(1);
			if (sprintf("%d", $supportitem[0]["departure"]) > 0 ) {
				$table->addCode(date("H:i", $supportitem[0]["departure"]));
			}
		$table->endTableData();
	$table->endTableRow();
...
$venster->addVensterData();
	...
	$table->addTableRow();
		$table->addTableData();
		$table->addCode($supportitem[0]["supplies"]);
		$table->addSpace("1");
		$table->addTableData();
	$table->endTableRow();
...
if (!$is_printing) {
	...
	$venster->addVensterData();
		...
		unset($view);
		   $table = new Layout_table(array("cellspacing"=>1));
		$table->addTableRow();
			$table->insertTableHeader(gettext("internal notes"));
		$table->endTableRow();

		$table->addTableRow();
			$table->addTableData();
			$table->addCode($supportitem[0]["internal_notes"]);
			$table->addSpace("1");
			$table->addTableData();
		$table->endTableRow();
