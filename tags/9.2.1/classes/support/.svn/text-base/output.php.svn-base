...
Class Support_output {
  ...
  /* }}} */
	...
	/* show_info {{{ */
	/**
	 * Show supportitem in the infolayer, to be used with the javascript function loadXML
	 *
	 * @param int $id The support item id
	 *
	 * @return void
	 */
	public function show_info($id) {
		$support_data = new Support_data();
		$supportinfo = $support_data->getSupportItemById($id);

		$fields[gettext("reference number")] = $supportinfo["reference_nr"];
		$fields[gettext("date")] = $supportinfo["human_date"];
		$fields[gettext("user")] = $supportinfo["rcpt_name"];
		$fields[gettext("description")] = $supportinfo["description"];
		$fields[gettext("solution")] = $supportinfo["solution"];
		$fields[gettext("remarks")] = $supportinfo["remarks"];
		$fields[gettext("internal notes")] = $supportinfo["internal_notes"];
		$fields[gettext("relation")] = $supportinfo["relname"];
		if ($GLOBALS["covide"]->license["has_project"] || $GLOBALS["covide"]->license["has_project_declaration"]) {
			$fields[gettext("project")] = $supportinfo["project_name"];
		}
		$output = new Layout_output();
		$output->insertAction("info", gettext("view"), sprintf("index.php?mod=support&action=search&search[keyword]=%s", urlencode(substr($supportinfo["description"], 0, 100))));
