	...
	if (!$this->has_declaration) {
		...
		$venster_main->addCode("
			function showAdditionalInfo() {
				if (document.getElementById('additionalinfodiv').style.display == 'block') {
	...
	if (!$this->has_declaration) {
		$table->addTableRow();
			...
			$table->addTableData("", "data");
				...
				if ($this->has_declaration) {
					$sel["document.location.href='index.php?mod=projectdeclaration&action=register_item&project_id=".$projectinfo[0]["id"]."';"] = gettext("register new item");
					$sel["document.location.href='index.php?mod=projectdeclaration&action=send_batch&project_id=".$projectinfo[0]["id"]."&manager=".$projectinfo[0]["manager"]."';"] = gettext("send declaration");
				} else {
					$sel["popup('?mod=calendar&action=batch_reg_input&id=0&project_id=".$projectinfo[0]["id"]."', 'batchinput', 750, 600, 1);"] = gettext("batch hour input");
					$sel["popup('?mod=calendar&action=misc_reg_input&id=0&project_id=".$projectinfo[0]["id"]."', 'otherinput', 750, 600, 1);"] = gettext("other project costs");
					$sel["showHourInfo();"] = gettext("toggle hours and costs");
					$sel["showAdditionalInfo();"] = gettext("toggle additional info");
					$sel["popup('index.php?mod=project&action=showprojecthours&id=".$projectinfo[0]["id"]."', 'hour_overview', 750, 600, 1);"] = gettext("print")."/".gettext("pdf");
				}

				if ($projectinfo[0]["group_id"]) {
					$sel["document.location.href='index.php?mod=project&action=showinfo&id=".$projectinfo[0]["group_id"]."&master=1';"] = ($this->has_declaration) ? gettext("to overview") : gettext("to masterprojects");
				}
				if ($GLOBALS["covide"]->license["has_project_ext_samba"]) {
					$sel["popup('?mod=projectext&action=extGenerateDocumentTree&id=".$_REQUEST["id"]."', 'generate', 750, 600, 1);"] = gettext("generate document");
					$sel["javascript: popup('index.php?mod=projectext&action=netshare&id=".$projectinfo[0]["id"]."', 'share', '800', '600', 0);"] = gettext("open networkshare");
				}

				$table->addSelectField("project[actions]", $sel);
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
