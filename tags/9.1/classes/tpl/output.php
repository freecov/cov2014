...
Class Tpl_output {
	...
	public function load_js($id) {
		...
		$code = sprintf("<script language='Javascript1.2' type='text/javascript' src='%s'></script>", $f);
		$code.= "<!-- js include [$id] -->\n";
	...
	public function displayPoll($pollid = -1) {
		$cms_data =& $this->cms;
		$polldata = $cms_data->getPollById($pollid);
		if ($_COOKIE["pollvoted".$polldata["id"]] == "yes")
			return $this->showPollResults($polldata["id"]);
		else
			return $this->showPoll($polldata["id"]);
	}
	private function showPollResults($pollid) {
		$cms_data =& $this->cms;
		$polldata = $cms_data->getPollById($pollid);
		$output = new Layout_output();
	...
	private function showPoll($pollid) {
		$cms_data =& $this->cms;
		$polldata = $cms_data->getPollById($pollid);
		$output = new Layout_output();
		$output->addTag("div", array("id" => "pollcontainer"));
	...
	public function loadJQuery() {
		$this->loadJS("jquery.js", self::include_dir_main);
	}
	/* }}} */
	/* loadMooTools {{{ */
	/**
