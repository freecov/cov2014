...
if ($_REQUEST["campaign"]) {
	...
	if ($GLOBALS["covide"]->license["multivers"] || $GLOBALS["covide"]->license["exact"]) {
		if ($GLOBALS["covide"]->license["hrm"]) {
			$venster->addMenuItem(gettext("new person"), "javascript: popup('index.php?mod=address&action=edit&addresstype=private', 'addressedit', 800, 600, 1);");
		...
		if ($user->checkPermission("xs_addressmanage") || $user->checkPermission("xs_relationmanage"))
			$venster->addMenuItem(gettext("new contact"), "javascript: popup('index.php?mod=address&action=edit&addresstype=relations', 'addressedit', 800, 600, 1);");
	...
	$venster->addMenuItem(gettext("classifications"), "javascript: search_cla();");
	$venster->generateMenuItems();
}
/* end menu items }}} */
		...
		$table->addTableData( array("align"=>"right") );
			...
			if ($user->checkPermission("xs_addressmanage") || $user->checkPermission("xs_usermanage")) {
				if ($addresstype == "nonactive") {
					$actionsel[gettext("selection actions")]["javascript: popup('index.php?mod=address&action=deleteSelection&info='+exportinfo, 'delete_selection', 450, 350, 1);"] = gettext("delete non-active selection");
				} else {
					$actionsel[gettext("selection actions")]["javascript: popup('index.php?mod=address&action=removeSelection&info='+exportinfo, 'remove_selection', 450, 350, 1);"] = gettext("put selection on non-active");
				}
				$actionsel[gettext("selection actions")]["javascript: popup('index.php?mod=address&action=addcla_multi&info='+exportinfo, 'addcla_multi', 350, 150, 1);"] = gettext("manage selection classification");
				$actionsel[gettext("selection actions")]["javascript: document.location.href='index.php?dl=1&mod=address&action=export&info='+exportinfo"] = gettext("export selection");
				$actionsel[gettext("selection actions")]["javascript: popup('index.php?mod=address&action=print&info='+exportinfo, 'print', 0, 0, 1);"] = gettext("print selection");
			}

			$actionsel[gettext("global actions")]["document.location.href = 'index.php?mod=index&search[private]=0&search[address]=1';"] = gettext("extensive search");
			if ($user->checkPermission("xs_addressmanage")) {
				$actionsel[gettext("global actions")]["popup('index.php?mod=address&action=export', 'addressexport', 500, 400, 1);"] = gettext("export all addresses");
				$actionsel[gettext("global actions")]["popup('index.php?mod=address&action=import', 'addressimport', 700, 600, 1);"] = gettext("import addresses");
				$actionsel[gettext("global actions")]["popup('index.php?mod=address&action=importVcard', 'vcardimport', 700, 600, 1);"] = gettext("import vCard");
				$actionsel[gettext("global actions")]["document.location.href = '?mod=templates';"] = gettext("letter templates");
				if ($user->checkPermission("xs_classmanage")) {
					$actionsel[gettext("global actions")]["document.location.href = 'index.php?mod=classification&action=show_classifications';"] = gettext("classification mngt");
				}
				$actionsel[gettext("global actions")]["document.location.href = 'index.php?mod=address&action=showheaders';"] = gettext("header mngt");
				$actionsel[gettext("global actions")]["document.location.href = 'index.php?mod=address&addresstype=nonactive';"] = gettext("inactive contacts");
				$actionsel[gettext("global actions")]["document.location.href = 'index.php?mod=address&action=glob_metalist';"] = gettext("global metafields");
			}
			if ($GLOBALS["covide"]->license["has_hypo"] && $user->checkPermission("xs_hypo") && !$GLOBALS["covide"]->license["disable_basics"]) {
				$actionsel[gettext("global actions")]["document.location.href = 'index.php?mod=mortgage';"] = gettext("morgage module");
			}

			if ($GLOBALS["covide"]->license["multivers"] || $GLOBALS["covide"]->license["exact"] && !$GLOBALS["covide"]->license["disable_basics"]) {
				$actionsel[gettext("global actions")]["popup('./fimport.php?e=1', 'forceimport', 0, 0, 1);"] = gettext("force import");
			}

			if ($GLOBALS["covide"]->license["has_voip"]) {
				$actionsel[gettext("global actions")]["newVoipCall();"] = gettext("direct voip call");
			}
			$table->addSelectField("addressactions", $actionsel, "");
	$table->endTableData();
$table->endTableRow();
$table->endTable();
...
if (!preg_match("/^searchRel/", $action)) {
	$table->addTableData();
		...
		if ($_REQUEST["addresstype"] == "bcards") $table->endTag("b");

		$table->addCode(" | ");
		$table->insertLink(gettext("select and sort"), array(
			"href" => "javascript: popup('index.php?mod=address&action=selectRows&type=".$_REQUEST["addresstype"]."', 'sortandselect', 700, 600, 1);",	
		));
...
if ($user->checkPermission("xs_addressmanage") || $user->checkPermission("xs_usermanage")) {
	$table->addTableRow(1);
		$table->addTableData(array("style"=>"text-align: right"));
			$table->addSelectField("addressactions", $actionsel, "");
			unset($actionsel);
		$table->endTableData();
	$table->endTableRow();
