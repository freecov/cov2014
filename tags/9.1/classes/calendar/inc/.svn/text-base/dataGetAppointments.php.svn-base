...
foreach ($normal_events as $row) {
	...
	if ($row["is_repeat"] != 1 && $row["alldayevent"]) {
		...
		if ($b_time == 0) {
			// starttime is midnight, try with off-by-one
			$b_time = (date("H",$row["timestamp_end"]+1)*60)+date("i",$row["timestamp_end"]+1);
		}
		if ($e_time == 0) {
			// endtime is midnight, try with off-by-one
			$e_time = (date("H",$row["timestamp_end"]-1)*60)+date("i",$row["timestamp_end"]-1);
		}
	}
