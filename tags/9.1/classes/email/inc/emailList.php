 ...
 */
	...
	$table->addTableRow();
		$table->addTableData();
			$table->insertAction("mail_new", gettext("new email"), "javascript: emailSelectFrom()");
			$table->addSpace(3);

			if ($list_of_address) {
				$table->insertAction("addressbook", gettext("contact profile"), "?mod=address&action=relcard&id=".$list_of_address);
			...
			if ($project_id) {
				...
				if ($projectinfo[0]["address_id"]) {
					...
					$table->insertAction("addressbook", gettext("Go to the contact card of: ").$addressinfo, "?mod=address&action=relcard&id=".$projectinfo[0]["address_id"]);
					$table->insertAction("view_all", gettext("Show emails of contact: ").$addressinfo, "?mod=email&folder_id=".$folder_id."&address_id=".$projectinfo[0]["address_id"]);
				}
				$table->insertAction("folder_project", gettext("Go to the project card"), "?mod=project&action=showinfo&id=".$project_id);
		...
		$table->addTableData();
			...
			$table->addCode( gettext("search for").": ");
		$table->endTableData();
		$table->addTableData();
		...
		$table->addTableData( array("align"=>"right") );
			...
			if (!$GLOBALS["covide"]->license["disable_basics"]) {
				$sel = array("void(0);" => "- ".gettext("more email actions")." -");
				$sel[gettext("selection actions")] = array(
					"javascript: selection_email_move();" => gettext("move selected email"),
					"javascript: selection_email_togglestate();" => gettext("toggle read/new status of selected emails"),
					"javascript: selection_email_delete();" => gettext("delete selected emails"),
				);
				$sel[gettext("attachment actions")] = array(
					"javascript: selection_attachments_save();" => gettext("save selected attachments in filesystem module"),
					"javascript: selection_attachments_download();" => gettext("download selected attachments"),
					"javascript: selection_attachments_delete();" => gettext("delete selected attachments"),
					"javascript: selection_attachments_zip();" => gettext("download selected attachments as zip-file"),
				);
				$sel[gettext("global actions")] = array(
					"emailSelectFrom();" => "+ ".gettext("new email"),
					"set('action', 'signatures'); submitform();" => "+ ".gettext("predefined content"),
					"set('action', 'filters'); submitform();" => "+ ".gettext("alter mailfilters"),
					"set('action', 'templates'); submitform();" => "+ ".gettext("email templates"),
					"popup('index.php?mod=email&action=show_permissions&user_id=".$_SESSION["user_id"]."', 'mailpermissions', 500, 350, 1);" => "+ ".gettext("share folders"),
					"popup('index.php?mod=email&action=retrieve&user_id=".$_SESSION["user_id"]."', 'mailfetch', 400, 350, 1);" => "+ ".gettext("force mail retrieval"),
					"popup('?mod=newsletter', '', 980, 700, 1);" => "+ ".gettext("newsletter"),
				);
				if ($GLOBALS["covide"]->license["has_postfixadmin"] && $GLOBALS["covide"]->license["postfixdsn"])
					$sel[gettext("global actions")]["popup('index.php?mod=email&action=edit_pa_autoreply&user_id=".$_SESSION["user_id"]."');"] = "+ ".gettext("autoreply");
				$table->addSelectField("mailactions]", $sel, "");
			}
		$table->endTableData();
	$table->endTableRow();
	/* if archive */
