...
if ($calendar_item["note_id"])
	...
	$fields[gettext("note")] = "<a href=\"javascript: popup('index.php?mod=note&action=edit&calendar_id=".$calendar_item["id"]."&address_id=".$calendar_item["relation"]."&project_id=".$calendar_item["project_id"]."');\">".gettext("make note")."</a>";
...
$table->addTableRow();
	...
	$table->addTableData("", "data");
		if (!$calendar_item["is_registered"]) {
			...
			$table->insertAction("delete", gettext("delete"), sprintf("javascript: calendaritem_remove(%d);", $calendar_item["id"]));
			if ($calendar_item["user_id"] == $_SESSION["user_id"] && $calendar_item["project_id"] && !$GLOBALS["covide"]->license["has_project_declaration"])
				$table->insertAction("calendar_reg_hour", gettext("register_hours"), sprintf("javascript: calendaritem_reg(%d, %d, %d);", $calendar_item["id"], $calendar_item["user_id"], $datestamp));
		}
	$table->endTableData();
$table->endTableRow();
$table->endTable();
