			...
			$venster->addVensterData();
				...
				$table1->addTableRow();
					$table1->addTableData(array("align"=>"center", "colspan" => 2));
						...
						if ($_REQUEST["uri"]) {
							$table1->addCode(gettext("referrer").": ");
							$table1->insertTag("u", strip_tags(stripslashes($_REQUEST["uri"])));
						}
					$table1->endTableData();
				$table1->endTableRow();
				$table1->addTableRow();
				...
				$table1->addTableRow();
					$table1->addTableData(array("colspan"=>2, "align"=>"right"));
						...
						if ($GLOBALS["covide"]->sslmode == 2) {
							...
							$table1->addCheckBox("use_ssl", 1, ($GLOBALS["covide"]->license["force_ssl"] == 1) ? 1:0);
							$table1->endTag("div");
						}
					$table1->endTableData();
				$table1->endTableRow();
				...
				$table1->addTableRow(array("style" => "visibility: hidden;", "id" => "progressbar"));
					$table1->addTableData(array("colspan" => 2, "align" => "center"));
						...
						$table1->insertTag("marquee", $div->generate_output(), array(
