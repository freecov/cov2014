...
Class Cms_output {
	...
	public function cmsSitemap() {
		...
		if (($user_info["xs_cms_level"] == 3 && !$GLOBALS["covide"]->license["cms_lock_settings"])
			...
			$menuitems[gettext("site templates")] = "popup('?mod=cms&action=siteTemplates', new String().concat('templates', Math.random()*(Math.pow(2,20))), 970, 680, 1);";
		...
		if ($user_info["xs_cms_level"] >= 2 && $cms_license["cms_permissions"])
			$menuitems[gettext("cms account manager")] = "popup('?mod=cms&action=editAccountsList', 'permissions', 640, 480, 1);";
		...
		if ($user_info["xs_cms_level"] >= 2) {
			$menuitems[gettext("site information")] = "popup('?mod=cms&action=editSiteInfo', 'siteinfo', 640, 480, 1);";
		...
		$venster->addVensterData();
			...
			$tbl = new Layout_table(array(
				"cellspacing" => 1,
				"cellpadding" => 1,
				"style" => "background-color: #999; width: 100%;"
			));
			$tbl->addTableRow(array("style" => "background-color: #ddd;"));
				...
				$tbl->addTableHeader(array("colspan" => 2, "align" => "right"));
					...
					$tbl->endTag("nobr");
				$tbl->endTableHeader();
				$tbl->addTableHeader(array("colspan"=>2));
	...
	public function viewRestorePoint($id) {
		...
		echo $mail_data->stylehtml($data["autosave_data"]);
	...
	public function cmsBannerList() {
		...
		$view->addMapping( gettext("name"), "%name" );
		$view->addMapping( gettext("rating"), "%rating" );
		...
		$venster->insertAction("new", gettext("new"), "?mod=cms&action=bannerEdit");
		$venster->insertAction("close", gettext("close"), "javascript: window.close();");
