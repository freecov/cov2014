		...
		function fp_addcol(img, code) {
			if (img) {
				if (navigator.userAgent.indexOf("MSIE 6.0") != -1) {
					return code.concat('&nbsp;<img border="0" src="img/spacer.gif" style="filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'themes/default/icons/', img ,'.png?version=' + current_date.getTime() + '\', sizingMethod=\'image\');\">');
				} else {
					return code.concat('&nbsp;<img border="0" src="themes/default/icons/', img, '.png?version=' + current_date.getTime() + '">');
				}
			} else {
				return code;
			}
		...
		function fp(id, titel, rDelete, rView, rManage, rEdit, niveau, color, parentpage, uit, matched, locked, sticky, isactive, isredirect, ismenuitem, ispublic, istemplate, multidel, searchinfo, dateinfo, daterange, islijst, metainfo, isform, allowplak, isgallery, pageLabel, versionControl, theme, important, address){
			...
			if (niveau > 0) {
				if (uit==0) {
					...
					code = code.concat('<img src="img/cms/plus.gif" border=0 ');
					code = code.concat('alt="uitklappen"></a>');
				} else if (uit==1) {
					code = code.concat('<a href="javascript: cmsCollapse(\''+id+'\');">');
					code = code.concat('<img src="img/cms/min.gif" border=0 ');
					code = code.concat('alt="inklappen"></a>');
				} else {
					code = code.concat('<img src="img/cms/page.gif" border=0>');
				}
			}
			...
			code = code.concat('&nbsp;'+pageLabel+'</div></td>');
			code = code.concat('</div>');
			...
			if (niveau == 0) {
				...
				code = code.concat(document.getElementById('cms_icon_info').innerHTML + '</a></td><td><nobr style="display: none;" id="cms_options_', current_table_row , '">');
				...
				if (address==1)
					code = fp_addcol('toggle_log', code);

				if (address==2)
					code = fp_addcol('toggle_log_related', code);
