...
if (array_key_exists("search", $_REQUEST)) {
	...
	if (array_key_exists("user_id", $_REQUEST["search"])) {
		...
		if ($user->checkPermission("xs_usermanage") || $user->checkPermission("xs_notemanage"))
			$options["user_id"] = $_REQUEST["seacrh"]["user_id"];
		else
...
$venster->addMenuItem(gettext("done"), "./?mod=note&action=old&short_view=".$_short_view );
...
$venster->addVensterData();
	...
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			...
			$table->addCode( $useroutput->user_selection("searchuser_id", $_REQUEST["search"]["user_id"], 1, 1, 0, 1, 1) );
		} else {
	...
	$table->addTableRow();
		...
		$table->addTableData(array("align" => "right"), "data");
			...
			$table->addSelectField("search[from_year]",  $years,  ($_REQUEST["search"]["from_year"]?$_REQUEST["search"]["from_year"]:date("Y")));
			$table->addTag("br");
	...
	$table->addTableRow();
		$table->addTableData(array("colspan" => 2), "data");
			$table->insertAction("ok", gettext("search"), "javascript: document.getElementById('searchform').submit();");
		$table->endTableData();
	$table->endTableRow();
	$table->endTable();
