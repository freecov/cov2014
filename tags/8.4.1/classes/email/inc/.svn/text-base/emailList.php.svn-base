 ...
 */
	...
	if (!array_key_exists($folder_id, $mailData->_folders)) {
		...
		unset($user_data);
		$folderName = sprintf("%s (%s %s)", $folderName, gettext("shared by"), $folder_username);
	}
	...
	$table->addTableRow();
		...
		$table->addTableData( array("align"=>"right") );
			...
			if (!$GLOBALS["covide"]->license["disable_basics"]) {
				$sel = array(
					"void(0);" => "- ".gettext("more email actions")." -",
					...
					"popup('index.php?mod=email&action=show_permissions&user_id=".$_SESSION["user_id"]."');" => "+ ".gettext("share folders"),
				...
				if ($GLOBALS["covide"]->license["has_postfixadmin"] && $GLOBALS["covide"]->license["postfixdsn"])
					$sel["popup('index.php?mod=email&action=edit_pa_autoreply&user_id=".$_SESSION["user_id"]."');"] = "+ ".gettext("autoreply");
				else
					$sel["popup('index.php?mod=email&action=edit_autoreply&user_id=".$_SESSION["user_id"]."');"] = "+ ".gettext("autoreply");
	...
	$view->defineComplexMapping("header_actions", array(
		array(
			"type" => "action",
			"src"  => "mail_folder_move",
			"alt"  => gettext("move selected email"),
			"link" => "javascript: selection_email_move();"
		),
		array(
			"type" => "action",
			"src"  => "toggle",
			"alt"  => gettext("toggle read/new status of selected emails"),
			"link" => "javascript: selection_email_togglestate();"
		),
		array(
			"type" => "action",
			"src"  => "delete",
			"alt"  => gettext("delete selected emails"),
	...
	$table_folders_shared = new Layout_table();
