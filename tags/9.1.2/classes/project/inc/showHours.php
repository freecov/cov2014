...
if ($this->has_declaration) {
	...
	$venster_main->addMenuItem(gettext("batch hour input"), "javascript: popup('?mod=calendar&action=batch_reg_input&id=0&project_id=".$projectinfo[0]["id"]."', 'batchinput', 750, 600, 1);");
	$venster_main->addMenuItem(gettext("other project costs"), "javascript: popup('?mod=calendar&action=misc_reg_input&id=0&project_id=".$projectinfo[0]["id"]."', 'otherinput', 750, 600, 1);");
	$venster_main->addMenuItem(gettext("toggle hours and costs"), "javascript: showHourInfo();");
	$venster_main->addMenuItem(gettext("toggle additional info"), "javascript: showAdditionalInfo();");
...
$venster_main->addVensterData();
	...
	if (!$this->has_declaration) {
		$table->addTableRow();
			...
			$table->endTableData();
			$table->addTableData(array("width" => "15%"), "data");
				$table->addSpace();
			$table->endTableData();
			$table->addTableData(array("width" => "35%"), "data");
				$table->addSpace();
