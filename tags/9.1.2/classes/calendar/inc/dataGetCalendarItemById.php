...
if ($id==0) {
	...
	$sql = sprintf("SELECT calendar.*, calendar_user.user_id FROM calendar inner join calendar_user on calendar.id = calendar_user.calendar_id WHERE id = %d AND user_id = %d;", $id, $user_id);
	$res = sql_query($sql);
