<?php
	...
	$table->addTableRow();
		$table->addTableData();
			...
			$table->insertAction("print", gettext("print"), "javascript: popup('index.php?mod=calendar&action=print_month&month=".$month."&year=".$year."', 'printmonth', 800, 300, 1);");
	...
	$table->addTableRow();
		$table->addTableData();
			...
			$table_cal->addTableRow();
				...
				if ($dow!=0) {
					for ($i=0; $i!=7-$dow; $i++) {
						$table_cal->insertTableData("&nbsp;", array("style" => "border: 0px solid #CCC"), "header");
					}
				}
			$table_cal->endTableRow();
			$table_cal->endTable();
