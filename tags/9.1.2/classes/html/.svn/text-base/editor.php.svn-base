...
class Layout_editor {
	...
	private $tinymce_folder = "tinymce322";
	 ...
	 */
	private function load_tinymce($editor_type=0, $cleanup="true", $txtareaname="contents", $version=3, $allow_font = 0) {
		if ($this->xhtml)
	 ...
	 */
	public function generate_editor($settings=0, $data="", $cleanup="true", $txtareaname="contents", $allow_fonts = 0) {
		...
		if ($loadeditor) {
			switch ($this->editor) {
			case "tinymce" :
				$return = $this->load_tinymce($settings, $cleanup, $txtareaname, "", $allow_fonts);
				break;
			case "wyzz" :
