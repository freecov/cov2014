		...
		*/
		if ($GLOBALS["covide"]->license["has_factuur"]) {
			...
			if (count($finance_info["verkopen"]["data"]) > -1) {
				...
				$venster->addTag("br");
				$venster->addTag("br");
				$venster->insertAction("new", gettext_nl("nieuwe offerte"), "javascript: popup('classes/finance/non-oop/factuur/projecten.php?pagina=nieuw&statusnieuw=0&address_id_new=".$_REQUEST["id"]."');");
				$venster->addSpace();
				$venster->insertTag("a", gettext_nl("nieuwe offerte"), array(
					"href" => "javascript: popup('classes/finance/non-oop/factuur/projecten.php?pagina=nieuw&statusnieuw=0&address_id_new=".$_REQUEST["id"]."');"
				));
