...
if ($user->checkPermission("xs_addressmanage") || $user->checkPermission("xs_usermanage")) {
	...
	$venster->addMenuItem("&nbsp;&nbsp;".gettext("export selection"), "javascript: handle_selectionaction(); document.location.href='index.php?dl=1&mod=address&action=export&info='+exportinfo");
	$venster->addMenuItem("&nbsp;&nbsp;".gettext("export selection in reeleezee format"), "javascript: document.location.href='index.php?dl=1&reeleezee=1&mod=address&action=export&info='+exportinfo");
...
if (!preg_match("/^searchRel/", $action)) {
	$table->addTableData();
		...
		$table->addCode(" | ");
		if ($_REQUEST["addresstype"] == "nonactive") $table->addTag("b");
		$table->insertLink(gettext("inactive contacts"), array(
			"href" => "index.php?mod=address&addresstype=nonactive"
		));
		if ($_REQUEST["addresstype"] == "nonactive") $table->endTag("b");
		$table->addCode(" | ");
