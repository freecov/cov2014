...
Class Todo_data {
	...
	public function getTodosByUserId($user_id, $done=0, $today=0, $options = array()) {
		...
		if($today != 0) {
			...
			$dateToday = "AND timestamp <= ".mktime (0,0,0,$dateMonth,$dateDay,$dateYear);
		} else {
			$dateToday = "";
		}
		if ($options["search"]) {
		...
		if ($done == 1) {
			if ($access["xs_todomanage"]) {
				...
				$sql = sprintf("SELECT * FROM todo WHERE is_done=%d AND user_id=%d %s %s ORDER BY timestamp DESC, timestamp_end DESC", $done, $user_id, $search, $dateToday);
				$sql_count = sprintf("SELECT COUNT(*) FROM todo WHERE is_done=%d AND user_id=%d %s %s", $done, $user_id, $search, $dateToday);
			}
		} else {
			$sql = sprintf("SELECT * FROM todo WHERE is_done=%d AND user_id=%d %s %s ORDER BY status ASC, priority ASC, timestamp_end ASC, timestamp ASC", $done, $user_id, $search, $dateToday);
			$sql_count = sprintf("SELECT COUNT(*) FROM todo WHERE is_done=%d AND user_id=%d %s %s", $done, $user_id, $search, $dateToday);
		}
		...
		while ($row = sql_fetch_assoc($result)) {
			...
			if ($row["timestamp_end"] < mktime() && $row["is_done"] != 1)
				$row["overdue"] = 1;
			else
				$row["overdue"] = 0;
			if ($row["is_done"] == 1) {
