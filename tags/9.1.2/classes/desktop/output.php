		...
		if ($_SESSION["user_id"]) {
			...
			if ($_SESSION["user_id"]) {
				...
				if (!$GLOBALS["covide"]->license["disable_basics"] && $GLOBALS["covide"]->license["has_issues"]) {
					...
					$extsupportcount = 0;
					if ($alerts["supportcalls"]) {
						$extsupportcount = $alerts["supportcalls"];
					}
					$output->insertAction("important", gettext("to external support calls"), "index.php?mod=support&action=list_external");
					$output->addCode("(".$extsupportcount.")");
					$output->endTag("span");
				}
				$output->endTag("p");
				$do_alert = "";
				...
				if ($extsupportcount > $_SESSION["alertcount"]["support"]) {
