...
Class Project_data {
	...
	public function getExceededProjects($user_id) {
		$sql = sprintf("
		SELECT p.id, p.name, p.hours, p.budget, SUM(h.timestamp_end - h.timestamp_start) AS total_hours_in_sec, SUM(a.tarif * ((h.timestamp_end - h.timestamp_start)*(1/3600))) AS total_budget  
		FROM project p, hours_registration h, hours_activities a 
		WHERE p.id = h.project_id 
			AND p.is_active = 1 
			AND p.manager = %d 
			AND h.timestamp_end > p.lfact 
			AND h.is_billable = 1 
			AND h.activity_id = a.id 
		GROUP BY h.project_id 
		HAVING (p.hours > 0 AND ((p.hours*3600)*0.75) < total_hours_in_sec)
			OR (p.budget > 0 AND total_budget > (p.budget*0.75))
		", $user_id);
		$res = sql_query($sql);
	...
	public function saveActivity($data) {
		if ($data["id"]) {
			/* update */
			$sql = sprintf("UPDATE hours_activities SET activity = '%s', tarif = %01.2F, department_id = %d WHERE id = %d", $data["activity"], $data["tarif"], $data["department_id"], $data["id"]);
		} else {
			$sql = sprintf("INSERT INTO hours_activities (activity, tarif, department_id) VALUES('%s', %01.2F, %d)", $data["activity"], $data["tarif"], $data["department_id"]);
		}
		$res = sql_query($sql);
		return true;
	}
  ...
  /* {{{ function dataCheckPermissions() */
	public function dataCheckPermissions($row, $user_id = 0) {
		if (!$user_id) {
  ...
  /* }}} */
	...
	public function getProjectHoursByUserId($user_id, $begin_timestamp=0, $end_timestamp=0) {
		...
		if (!$end_timestamp)
			$end_timestamp = time();
		$sql = sprintf ("SELECT COUNT(project_id) AS count_project_hours, SUM(timestamp_end - timestamp_start) AS total_hours, project_id FROM hours_registration WHERE is_billable = 1 AND timestamp_start  BETWEEN ".$begin_timestamp." AND ".$end_timestamp." AND user_id=%d GROUP BY project_id", $user_id);
		$res = sql_query($sql);
		while ($row = sql_fetch_assoc($res)) {
			/* get non-billable hours */
			$sql2 = sprintf ("SELECT SUM(timestamp_end - timestamp_start) AS total_hours, project_id FROM hours_registration WHERE is_billable = 0 AND timestamp_start  BETWEEN ".$begin_timestamp." AND ".$end_timestamp." AND project_id=%d GROUP BY project_id", $row["project_id"]);
			$res2 = sql_query($sql2);
