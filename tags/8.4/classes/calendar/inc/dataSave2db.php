	...
	if ($appointment["id"]) {
		...
		foreach ($user_arr as $user) {
			...
			if (!$count_user) {
				...
				$sql = sprintf("INSERT INTO calendar_user (calendar_id, user_id, status) VALUES (%d, %d, %d)", $appointment["id"], $user, $status);
				$res = sql_query($sql);
			}
		}
		...
		foreach ($user_arr as $user) {
			$q = sprintf("INSERT INTO calendar_user (calendar_id, user_id, status) VALUES (%d, %d, %d)", $appointmentid, $user, $status);
			$res = sql_query($q);
		}
