...
if ($GLOBALS["covide"]->license["address_migrated"] != 2) {
	set_time_limit(0);
	// migrate all address records to RCBC
	$sql = sprintf("SELECT id FROM address");
	$res = sql_query($sql);
	while ($row = sql_fetch_assoc($res)) {
		$addressinfo = $this->getAddressById($row["id"], "relations", "kantoor", 1);
		$this->checkrcbc($addressinfo);
		unset($addressinfo);
	}
	$q = "update license set address_migrated = 2";
	sql_query($q);
}
...
if ($GLOBALS["covide"]->license["address_strict_permissions"] && !$user->checkPermission("xs_usermanage")) {
...
if ($addresstype == "users" || $addresstype == "private" || $addresstype == "overig") {
	...
	$query_count = "SELECT count(address_businesscards.id) FROM address_businesscards $bjoin WHERE ($sq AND (address_businesscards.rcbc = 0 OR (address_businesscards.rcbc = 1 AND address.is_active=1))) ";
	$query_zoek = "SELECT 
	...
	FROM address_businesscards $bjoin WHERE ($sq AND (address_businesscards.rcbc = 0 OR (address_businesscards.rcbc = 1 AND address.is_active=1))) ";
	$query_csv = "SELECT address_businesscards.multirel, address_businesscards.* FROM address_businesscards $bjoin WHERE ($sq AND (address_businesscards.rcbc = 0 OR (address_businesscards.rcbc = 1 AND address.is_active=1))) ";
}
...
while ($row = sql_fetch_assoc($result)) {
	...
	if ($options["addresstype"] == "relations" || $options["addresstype"] == "nonactive" || $options["addresstype"] == "overig") {
		...
		if ($options["addresstype"] == "private") {
			...
			if (!$is_active && $options["newsletterselection"]) {
	...
	$addressinfo["address"][$i]["personal_phone_nr_link"] = $this->show_phonenr($row["personal_phone_nr"]);
