<?php
	...
	if ($type == "bcards") {
		...
		if ($migrate == 0 && is_array($bcard_info)) {
			unset($bcard_info["id"]);
		...
		if ($type == "relations" || $type == "nonactive") {
			...
			if ($adresinfo["modified_by"] && $adresinfo["modified"]) {
				$adresinfo["changed_by_name"] = $user_data->getUsernameById($adresinfo["modified_by"]);
	...
	if ($adresinfo["contact_birthday"]) {
		$adresinfo["h_birthday"] = date("d-m-Y", $adresinfo["contact_birthday"]);
	}
