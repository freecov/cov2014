	...
	foreach ($mboxSort as $i=>$imap_id) {
		...
		if (array_key_exists($msg_id, $_statuslist_msg)) {
			...
			foreach ($filters as $f) {
				if (!$filter_done) {
					if (!$data["new_folder_id"]) {
						...
						if ($f["recipient"]) {
							$matches_needed++;
							if (strtolower($rcpt) == strtolower($f["recipient"])) {
								$matches_found++;
								$d["recipient"] = 1;
			...
			if (count($this->autoreply)) {
				if (!$data["is_bounce"] && $from && $rcpt && ($this->autoreply["is_active"] || ($this->autoreply["timestamp_start"] <= mktime() && $this->autoreply["timestamp_end"] >= mktime()))) {
					$this->markup(" - sending autoreply to $from for - $rcpt", "black");
					mail($from, "Autoreply: ".$data["subject"], $this->autoreply["body"], "From: $rcpt", "-f$rcpt");
				}
			}
		}
