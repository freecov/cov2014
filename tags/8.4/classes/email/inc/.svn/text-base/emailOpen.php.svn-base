 ...
 */
	...
	$output->layout_page('', $_REQUEST["hide"]);
	...
	if (!$restricted && !$GLOBALS["covide"]->license["disable_basics"]) {
		...
		$venster->addMenuItem( gettext("resend"), "javascript: mail_action('resend')");
	...
	$table_actions->addTableRow();
		$table_actions->addTableData(array("colspan"=>2));
			...
			if (!$restricted && !$GLOBALS["covide"]->license["disable_basics"]) {
				...
				$table_actions->insertAction("support", gettext("new support"), "javascript: mail_action('support');");
				$table_actions->insertAction("mail_resend", gettext("resend"), "javascript: mail_action('resend');");
