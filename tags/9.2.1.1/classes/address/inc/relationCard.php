...
$commitems = array();
$sql = sprintf("SELECT id, timestamp, user_id, subject, 'note' as type FROM notes WHERE address_id = %1\$d AND timestamp BETWEEN %3\$d AND %4\$d
	UNION
...
$sql = sprintf("SELECT id, timestamp, user_id, subject, 'note' as type FROM notes WHERE address_id = %1\$d AND timestamp >= %3\$d
	UNION
...
//show communication items
$buf2->addTag("div", array("id" => "relcard_commitems"));
$buf2->insertTag("h1", gettext("history"));
...
$buf2->addCode($view_com->generate_output());
unset($view_com);
$buf2->addTag("form", array("id" => "comrange", "method" => "get", "action" => "index.php"));
$buf2->addHiddenField("mod", "address");
$buf2->addHiddenField("action", "relcard");
$buf2->addHiddenField("id", $id);
/* use table */
$table = new Layout_table();
$table->addTableRow();
$table->addTableRow();
...
//show communication items
$buf2->addTag("div", array("id" => "relcard_planning"));
$buf2->insertTag("h1", gettext("planning"));
$view_com = new Layout_view();
