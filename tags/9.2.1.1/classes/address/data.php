<?php
	...
	Class Address_data {
		...
		public function getBcardsByRelationID($address_id, $search="", $classification = "") {
			...
			$query = sprintf("SELECT * FROM address_businesscards WHERE (address_id = %1\$d OR multirel %2\$s '(^|\\\\,)%1\$d(\\\\,|$)') %3\$s ORDER BY rcbc DESC,surname,givenname", $address_id, $regex_syntax, $sql_cla);
			$res = sql_query($query);
			while($row = sql_fetch_assoc($res)) {
			...
			if (!$data["inline"]) {
				...
				if (is_array($addresses["address"])) {
					foreach ($addresses["address"] as $address) {
						...
						if ($data["options"]["addresstype"] == "relations") {
							$sql = sprintf("UPDATE address_businesscards SET classification = '%s' WHERE address_id = %d AND rcbc = 1", $cla, $address_id);
						} else {
							$sql = sprintf("UPDATE address_businesscards SET classification = '%s' WHERE id = %d", $cla, $address_id);
						}
						$res = sql_query($sql);
					}
				}
