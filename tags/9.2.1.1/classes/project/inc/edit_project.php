...
if (is_array($address_ids)) {
	$output->addCode("var relations = '".implode(",", $address_ids)."';");
} else {
	$output->addCode("var relations = '".$projectinfo["address_id"]."';");
}
...
if (is_array($address_ids)) {
	$output->addHiddenField("relations", implode(",", $address_ids));
	unset($address_ids);
} else {
	$output->addHiddenField("relations", $projectinfo["addres_id"]);
	unset($projectinfo["address_id"]);
}
/* window widget */
if ($master) {
	...
	if (!$master) {
		...
		if (!$this->has_declaration) {
			$table->addTableRow();
				...
				$table->addTableData("", "data");
					// budget money notation uses dot as thousand separator
					$table->addTextField("project[budget]",number_format($projectinfo["budget"], 0, ',','.'));
				$table->endTableData();
			$table->endTableRow();
			$table->addTableRow();
			...
			$table->addTableRow();
				...
				$table->addTableData("", "data");
					$table->addTextField("project[kilometer_allowance]", $projectinfo["kilometer_allowance"]);
				$table->endTableData();
			$table->endTableRow();
		}
