...
Class Session {
	...
	public function read($session_id) {
		...
		$sql = sprintf("SELECT session_data FROM sessions WHERE session_id = '%s' AND user_agent = '%s' AND expire > %d", sql_escape_string($session_id), sql_escape_string($_SERVER["HTTP_USER_AGENT"]), mktime());
		$res = sql_query($sql);
	...
	public function write($session_id, $session_data) {
		...
		if ($count > 0) {
			//update
			$sql = sprintf("UPDATE sessions SET session_data = '%s', expire = %d WHERE session_id = '%s'", sql_escape_string($session_data), (mktime() + $this->lifetime), sql_escape_string($session_id));
			$res = sql_query($sql);
			return true;
		} else {
			//insert
			$sql = sprintf("INSERT INTO sessions (session_id, session_data, expire, user_agent) VALUES ('%s', '%s', %d, '%s')", sql_escape_string($session_id), sql_escape_string($session_data), (mktime() + $this->lifetime), sql_escape_string($_SERVER["HTTP_USER_AGENT"]));
			$res = sql_query($sql);
			return true;
		}
	}
	...
	public function gc($maxlifetime) {
		$sql = sprintf("DELETE FROM sessions WHERE expire < %d", (mktime() - $maxlifetime));
		$res = sql_query($sql);
	}
	/* }}} */
}
