...
/* if phone is set, and phonenr as well we are logging a voip call */
if ($_REQUEST["phone"] == 1) {
	$note["subject"] = gettext("Phone contact");
	$note["phonecall"] = 1;
}

if ($_REQUEST["phonenr"]) {
	$note["phone_nr"] = $_REQUEST["phonenr"];
	$note["phonecall_start"] = mktime();
}

if ($_REQUEST["bcard_id"]) {
	$note["address_businesscards_id"] = $_REQUEST["bcard_id"];
}
...
$output->addHiddenField("note[phonecall]", ($note["phonecall"]?1:0));
$output->addHiddenField("note[phone_nr]", $note["phone_nr"]);
$output->addHiddenField("note[address_businesscards_id]", $note["address_businesscards_id"]);
$output->addHiddenField("note[phonecall_start]", $note["phonecall_start"]);
$output->addHiddenField("note[phonecall_end]", 0);
...
if($_REQUEST["rcpt_id"]) {
	...
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			$editor = new Layout_editor("wyzz", 570, 300);
