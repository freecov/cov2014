<?php
	...
	$venster = new Layout_venster(array(
		...
		"subtitle" => gettext("move foldertree")
	));
	$venster->addVensterData();
		...
		$table->addTableRow();
			$table->addTableData("", "data");
				...
				$table->addCode( gettext("You are about to move the next folders").": ");
				$table->addTag("br");
				...
				$table->addTag("br");
				$table->insertAction("back", gettext("back"), "javascript: closepopup();");
				$table->addSpace(3);
				if (!$deny) {
					$table->insertAction("ok", gettext("continue to cut"), "javascript: update_pastebuffer()");
					...
					$table->addCode("
						function update_pastebuffer() {
							if (opener) {
								...
								var f = parent.document.getElementById('velden');
							}
