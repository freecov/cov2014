<?php
	...
	$calendar_data = new Calendar_data();
		$table = new Layout_table(array("width" => "100%", "class" => "fl_right"));
		$table->addTableRow();
			$table->addTableData();
				$table->insertTag("h1", gettext("weekly overview")." ".$user_data->getUsernameById($useridtolookup)." ".gettext("week")." ".strftime("%V", mktime(0,0,0,$month,$day,$year)));
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
			$table->addTableData();
		...
		$table->addTableRow();
			$table->addTableData();
				if (!$print) {
					$table->insertAction("print", gettext("print"), "javascript: oldpopup('index.php?mod=calendar&action=print_week&userid=$useridtolookup&day=$day&month=$month&year=$year', 'printweek', 300, 300, 1);");
				}
				$table->insertAction("back", gettext("last week"), sprintf("?mod=calendar&extrauser=%s&day=%d&month=%d&year=%d", $_REQUEST["extrauser"], $day-7, $month, $year));
				$table->insertAction("forward", gettext("next week"), sprintf("?mod=calendar&extrauser=%s&day=%d&month=%d&year=%d", $_REQUEST["extrauser"], $day+7, $month, $year));
			$table->endTableData();
		$table->endTableRow();
		$table->endTable();
?>
