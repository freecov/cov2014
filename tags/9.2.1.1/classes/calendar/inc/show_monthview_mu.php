<?php
	...
	$venster->addMenuItem(gettext("print"), sprintf("javascript: popup('./?mod=calendar&action=print_month_mu&forceday=1&extrauser=%s&day=%d&month=%d&year=%d')", implode(",",$users), $day, $month, $year));
		...
		$table->addTableData();
			...
			$table_cal->addTableRow();
				for ($i=1; $i<=date("t",mktime(0,0,0,$month,1,$year)); $i++) {
					...
					$table_cal->addCode(" (".gettext("week")." ".date("W", mktime(0, 0, 0, $month, $i, $year)).")");
