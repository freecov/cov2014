...
Class ProjectExt_output {
	...
	public function manageExtended() {/*{{{*/
		...
		$venster->addMenuItem(gettext("back"), "?mod=project");
		...
		switch ($v["field_type"]) {
			...
			case 2:
				...
				if ($output_only) {
					if ($v["value"] > 0) {
						...
						$tbl->insertAction("go_todo", gettext("plan in calendar"), "javascript: popup('index.php?mod=calendar&action=edit&id=0&user=".$cal_user."&project_id=".$project_id."&datemask=".$v["value"]."&subject=".$cal_subject."&event=1&relations='+relations, 'cal', 0, 0, 1);");
					} else {
						$tbl->addCode("--");
					}
				} else {
			...
			case 3:
				...
				foreach ($v["default_value"] as $z) {
					$z = htmlentities(trim($z), NULL, "UTF-8");
					$sel[$z] = $z;
				}
				if ($output_only) {
	...
	public function genOverviewSearch($formname="projectsearch") {
		...
		foreach ($data as $k=>$v) {
			...
			} elseif (in_array($v["field_type"], array(0,1,3,5)) ) {
