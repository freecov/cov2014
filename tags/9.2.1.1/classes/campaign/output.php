...
Class Campaign_output {
	...
	public function show_contents($id) {
		...
		$venster->addTag("div", array("id" => "relcard_commitems"));
		...
		/* put reaction in div */
		$venster->addTag("div", array("id" => "relcard_information"));
		...
		*/
		...
		/* put charts in div */
		$venster->addTag("div", array("id" => "relcard_bcards"));
	...
	public function show_edit_record($id) {
		...
		$tblk->addTableRow();
			$tblk->addTableData(array("style"=>"vertical-align: top;"));
				...
				foreach ($campaign_data->actions as $k=>$v) {
					if(in_array($k,$keys)){
						$tblk->insertCheckbox("options[]", $k, 1);
						$tblk->addCode($v);
						$tblk->addTag("br");
					}else{
						$tblk->insertCheckbox("options[]", $k);
						$tblk->addCode($v);
						$tblk->addTag("br");
					}
				}
