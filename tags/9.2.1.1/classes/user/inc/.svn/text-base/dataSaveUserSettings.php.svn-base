<?php
	 ...
	 * @package Covide
	 */
	$calendar_include_dir = "classes/calendar/inc/";
	...
	// prep the external calendars array.
	for ($i = 0; $i < count($userinfo["externalcalendar"]); ++$i) {
		$val = sprintf("%s", $userinfo["externalcalendar"][$i]);
		if ($val != "") 
			$externalcalendars[] = $val;
	}
	...
	if ($userpermissions["xs_usermanage"] == 1 || $userpermissions["xs_limitusermanage"] == 1) {
		...
		if ($userpermissions["xs_usermanage"] == 1) {
			...
			$velden[] = "xs_limited_projectmanage";       $waarden[] = sprintf("%d", $userinfo["xs_limited_projectmanage"]);
		...
		$velden[] = "google_startingpoint";          $waarden[] = sprintf("'%s'", $userinfo["google_startingpoint"]);
		$velden[] = "dimdim_username";               $waarden[] = sprintf("'%s'", $userinfo["dimdim_username"]);
		$velden[] = "dimdim_password";               $waarden[] = sprintf("'%s'", $userinfo["dimdim_password"]);
		$velden[] = "mail_hide_cmsforms";            $waarden[] = sprintf("%d",   $userinfo["mail_hide_cmsforms"]);
		$velden[] = "showbdays";                     $waarden[] = sprintf("%d",   $userinfo["showbdays"]);
		$velden[] = "classificationrestriction";     $waarden[] = sprintf("%d",   str_replace("|", "", $userinfo["classificationrestriction"]));
		$velden[] = "twitter_username";              $waarden[] = sprintf("'%s'", $userinfo["twitter_username"]);
		$velden[] = "twitter_password";              $waarden[] = sprintf("'%s'", $userinfo["twitter_password"]);	
		$velden[] = "popup_newwindow";               $waarden[] = sprintf("%d",   $userinfo["popup_newwindow"]);	

		if ($userinfo["id"]==0) {
			// new user
			if ($userinfo["pers_nr"]) {
				$result = sql_query(sprintf("SELECT id FROM users WHERE pers_nr=%d", $userinfo["pers_nr"]));
		...
		if ($userinfo["id"]==$_SESSION["user_id"]) {
			...
			$query.= sprintf(", dimdim_username='%s'", $userinfo["dimdim_username"]);
			$query.= sprintf(", dimdim_password='%s'", $userinfo["dimdim_password"]);
			$query.= sprintf(", google_startingpoint='%s'", $userinfo["google_startingpoint"]);
			$query.= sprintf(", twitter_username='%s'", $userinfo["twitter_username"]);
			$query.= sprintf(", twitter_password='%s'", $userinfo["twitter_password"]);
			$query.= sprintf(", popup_newwindow=%d", $userinfo["popup_newwindow"]);
	...
	if (!$error) {
		...
		if ($query) {
			...
			//get previous state
			$q = sprintf("SELECT * FROM users WHERE id = %d", $userinfo["id"]);
			$userres = sql_query($q);
			...
			$userid = $userinfo["id"];
			if ( $userid == 0 ) {
				// we are dealing with a new user here. We first need to know the id if the 
				// user created.
				$userid = sql_insert_id("users");
			}

			$sql = sprintf("SELECT id, url FROM calendar_external WHERE userid = %d", $userid);
			$res = sql_query($sql);
			
			require($calendar_include_dir."externalcalendars.php");
			while ($row = sql_fetch_assoc($res)) {
				// print_r( $externalcalendars );
				$url = $row["url"];
				if ( count($externalcalendars == 0) || !in_array( $url, $externalcalendars) ) {
					// echo "$url is no longer wanted!";
					
					$external = new externalCalendar( $row["id"]  );
					$external->delete();
					  
					$query = sprintf("DELETE FROM calendar_external WHERE id = %d", $row["id"]);
					sql_query($query);
				} else {
					$alreadyExists[] = $url;
				}
			...
			for ($i = 0; $i < count($externalcalendars); ++$i) {
				// echo "Dealing with: ".$externalcalendars[$i];
				if ( count($alreadyExists) > 0 ) {
					if ( in_array( $externalcalendars[$i], $alreadyExists ) ) {
				...
				// echo "...Adding<br>";
				$query = sprintf("INSERT INTO calendar_external VALUES ('', %d, '%s');", $userid, $externalcalendars[$i]);
				sql_query($query);
				
				$new_id = sql_insert_id("calendar_external");
				
				// echo "Syncing: ".$newid."<br>";
				$external = new externalCalendar( $new_id  );
		...
		if ($GLOBALS["covide"]->license["mail_shell"]) {
			...
			$mailurlfile   = $fsbasepath."/".$code."-url.txt";
			$calendarurlfile   = $fsbasepath."/".$code."-calendar-url.txt";
			$codefile  = $fsbasepath."/codes.txt";
			$host = $GLOBALS["covide"]->webroot;
			$mailurl = $host."index.php?mod=email&action=retrieve&user_id=";
			$calendarurl = $host."index.php?mod=calendar&action=externalcalendarsync&user_id=";
			...
			if (!file_exists($mailurlfile))
				file_put_contents($mailurlfile, $mailurl);

			if (!file_exists($calendarurlfile))
				file_put_contents($calendarurlfile, $calendarurl);
