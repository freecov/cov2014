...
Class Cms_data {
	...
	public function __construct() {
		...
		$this->linkchecker["startcmd"] = "/usr/bin/linkchecker -C -a -r2 -t2 --no-status -ocsv --timeout=15 #site#";
	...
	public function savePageData($id, $skip_close=0, $req="") {
		...
		$q = sprintf("update cms_data set date_changed = %d, autosave_header = '', autosave_data = '', autosave_info = '',
			...
			mktime(), $req["cms"]["pageTitle"], $req["cms"]["pageAlias"], $req["cms"]["pageLabel"], $ts, $req["cms"]["isSource"], $id);
		sql_query($q);
		...
		if (!$skip_close) {
			echo "<script>
				...
				if ('1' == '".$_REQUEST["syncweb"]."') {
					parent.parent.location.href='$alias';
				} else {
					//parent.opener.location.href='?mod=cms';
					parent.parent.document.getElementById('velden').submit();
				}
				if (confirm(cf)==true) {
	...
	public function getCmsCache($id) {
	  ...
	  if (!$file["datetime"])
		...
		if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) && (strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) == $file["timestamp"])) {
			...
			ob_clean();
			header("Content-Transfer-Encoding: binary");
			header("Expires: ".gmdate("D, j M Y H:i:s", mktime() + (60*60*24))." GMT");
			header("Pragma: public");
	...
	public function getGalleryItem($id) {
		...
		$res = sql_query($q);
		$data = sql_fetch_assoc($res);
		$filename = sprintf("%s/%s/%s/%d_%s.jpg", $GLOBALS["covide"]->filesyspath, "gallery", $data['pageid'], $id, "full");
		if (file_exists($filename)) {
			$info = getimagesize($filename);
	...
	public function cmsGalleryItemDelete($req) {
		...
		if ($req["item"] == -1) {
			...
			while ($row = sql_fetch_assoc($res)) {
				foreach ($types as $t) {
					$file = sprintf("%s/%s/%d/%d_%s.jpg",
						$GLOBALS["covide"]->filesyspath, "gallery", $req["pageid"], $row["id"], $t);
					if (file_exists($file))
						unlink($file);
				}
			...
			foreach ($types as $t) {
				$file = sprintf("%s/%s/%d/%d_%s.jpg",
					$GLOBALS["covide"]->filesyspath, "gallery", $req["pageid"], $req["item"], $t);
				if (file_exists($file))
					unlink($file);
			}
	...
	public function saveSiteInfo($req) {
		...
		if (is_numeric($req["siteroot"])) {
			...
			if ($req['siteroot_title']) {
				$q = sprintf("update cms_data set pageTitle = '%s' where id = %d", $req['siteroot_title'], $req['siteroot']);
				sql_query($q);
			}
		}
