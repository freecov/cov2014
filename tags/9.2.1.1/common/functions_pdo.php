<?php
/**
  * Covide Includes / PDO Compatibility Layer
  *
  ...
  * @since 1.0 25-04-2009
  * @package Covide
  */

/* *************************** */
/* PDO Conversion Library */
/* *************************** */

/* decorator design pattern used to provide compatibility to MDB2 */
class PDO_CompatResult {
	/* @var PDOStatement the PDO statement to work with */
	private $stmt;
	/* @var boolean the last query success state */
	private $success = false;

	/**
	 * Constructor - construct a new PDO statement compat result
	 *
	 * @param PDOStatement stmt the pdo statement to decorate
	...
	/**
	 * fetchRow - provide compatibility to MDB2
	 * @param int fetchMode the fetchmode to use
	 * @since 1.0 25-04-2009
	 * @author Stephan van de Haar <svdhaar@users.sourceforge.net>
	 * @deprecated
	 */
	public function fetchRow($fetchMode = 'MDB2_FETCHMODE_DEFAULT') {
		switch ($fetchMode) {
			...
			default:
				error_trigger(E_USER_ERROR, sprintf('Unknown fetchmode: ', $fetchMode));
		}
	}
	/**
	 * Decorator function
	 * @param string method the called method
	 * @param array params list of parameters the method was called
	 ...
	 * @return mixed the var contents
	 * @author Stephan van de Haar <svdhaar@users.sourceforge.net>
	 * @final
	 */
	public function __get($var) {
		return $this->stmt->{$var};
	}
	/**
	 * Setter
	 * @param string var the requested var
...
function sql_log_query($q, $affected_rows=0) {
	if (preg_match("/^((insert)|(update)|(delete))/si", $q)) {
		...
		if (!preg_match("/^((delete from)|(update)|(insert into)) ((active_calls)|(cms_cache)|(cms_image_cache)|(cms_temp)) /si", $q)) {
			...
			//$str  = sprintf("[%s]\t%s\t%d \"%d\" sql:\"%s\"\n", date("Y-m-d H:i:s"), $ipaddr, $_SESSION["user_id"], $affected_rows, $q);
...
function sql_query_exec_real($query, &$db, $no_log=0) {
	if (preg_match("/^((select)|(show)|(describe))/si", $query)) {
		...
		return new PDO_CompatResult($stmt);
	} else {
...
//-----------------------------------------------------------
function sql_query($query, $db=0, $top=0, $limit=0, $no_log=0) {
	$query = trim($query);
	...
	if ($limit) {
		...
		$query .= sprintf(' LIMIT %d, %d', $top, $limit);
...
function sql_fetch_array($result, $skip_filter=0) {
	return sql_fetch_assoc($result, $skip_filter, array(), PDO::FETCH_BOTH);
}
...
function sql_fetch_row($result, $skip_filter=0) {
	return sql_fetch_assoc($result, $skip_filter, array(), PDO::FETCH_NUM);
}

//}}}------------------------------------------------------------
//{{{ sql_fetch_assoc: return named array with ishtml as non-caps
//---------------------------------------------------------------
function sql_fetch_assoc($result, $skip_filter=0, $skip_fields=array(), $pdoFetchmode = PDO::FETCH_ASSOC) {
	if (!$result) {
		$ret = array();
		return $ret;
	}
	$row = $result->fetch($pdoFetchmode);
...
function sql_result($result, $pos=0, $field="", $skip_filter=0) {
	if (!$result)
		error_trigger(E_USER_ERROR, 'Not a valid result set');

	if ($field) {
		//$result->fetchInto($ret, DB_FETCHMODE_ASSOC, $pos);
		$ret = $result->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT, $pos);
		$return = $ret[$field];
	} else {
		//$result->fetchInto($ret, DB_FETCHMODE_ORDERED, $pos);
		$ret = $result->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT, $pos);
		$return = $ret[0];
	}
...
function sql_num_rows($result="") {
	...
	# mysql DOES return a rowcount on select
...
function sql_syntax($syntax) {
	switch ($syntax) {
		case "casttype"    : return "SIGNED"; break;
		case "escape_char" : return "`"; break;
		case "regex"       : return "REGEXP"; break;
		case "like"        : return "LIKE"; break;
		case "random"      : return "RAND()"; break;
	}
}
//}}}
/* fix_db {{{ */
...
function sql_escape_string($data) {
	...
	$return = $GLOBALS['covide']->db->quote($data);
