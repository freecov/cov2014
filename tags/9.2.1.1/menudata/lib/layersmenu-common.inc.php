...
$_SESSION["layer_parent"] = $parent;
$_SESSION["layer_absolute_path"] = $absolute_path;
...
function LayersMenuCommon()
{
	...
	$parent = $_SESSION["layer_parent"];
	$absolute_path = $_SESSION["layer_absolute_path"];

	$parent = "menudata/";

	$this->dirroot = $parent.$absolute_path.'./';
	$this->libjsdir = $parent.$absolute_path.'./libjs/';
	$this->imgdir = $parent.$absolute_path.'./menuimages/';
	$this->imgwww = $parent.$absolute_path.'menuimages/';
	$this->icondir = $parent.$absolute_path.'./menuicons/';
	$this->iconwww = $parent.$absolute_path.'menuicons/';
	$this->tpldir = $parent.$absolute_path.'./templates/';
	$this->menuStructure = '';
	$this->separator = '|';
...
/* END BENCHMARK CODE */
	...
	*/
	$sql = 'SELECT ' .
		 ...
		 FROM ' . $this->tableName . '
		WHERE parentPage > 0 AND ' . $condition . '
	...
	while ($row = $dbresult->fetchRow(MDB2_FETCHMODE_ASSOC)) {
		if (!$row["href"]) {
			$row["href"] = $row["id"].".htm";
		} else {
			$row["href"].= ".htm";
		...
		if (!$GLOBALS["conversion"])
			$GLOBALS["conversion"] = new Layout_conversion();

		$conversion =& $GLOBALS["conversion"];

		$this->_tmpArray[$row['id']]['parent_id'] = $row['parent_id'];
		$this->_tmpArray[$row['id']]['text']      = $conversion->str2utf8($row['text']);
		$this->_tmpArray[$row['id']]['href']      = $conversion->str2utf8($row['href']);
		$this->_tmpArray[$row['id']]['title']     = $conversion->str2utf8($row['title']);
	...
	unset($dbresult);
	unset($row);
	$this->_depthFirstSearch($menu_name, $this->_tmpArray, $menuparent, 1);
/* BEGIN BENCHMARK CODE
