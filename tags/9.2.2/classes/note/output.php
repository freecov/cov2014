	...
	Class Note_output {
	  ...
	  public function __construct() {
		...
		public function show_note($id, $hidenav = 0, $actions = 1) {
			...
			$venster->addMenuItem(gettext("answer sender"), "javascript: note_reply_single(".$note["id"].");", "", 0);
			...
			$venster->addMenuItem(gettext("make todo"), "javascript: note_plan(".$note["id"].");");
			...
			$venster->addVensterData();
				...
				$view->addMapping(gettext("phoneinfo"), "%%complex_phone");
				...
				$view->defineComplexMapping("complex_phone", array(
					array(
						"text" => array(
							gettext("Phonecall to"),
							" ",
							"%phone_nr",
							" (",
							"%phonecall_contactname",
							") ",
							gettext("from"),
							" ",
							"%phonecall_start_human",
							" ",
							gettext("till"),
							" ",
							"%phonecall_end_human"
		...
		public function show_sent() {
			...
			$output->start_javascript();
				$output->addCode(
					...
					var wclose = setTimeout('closepopup()', 1000);
					"
				);
		...
		/* show_info {{{ */
		/**
		 * Output code to show a specific note in the info layer, to be used with the js function loadXML
		 *
		 * @param int $id The note id to show
		 *
		 * @return void
		 */
		public function show_info($id) {
			$note_data = new Note_data();
			$note = $note_data->getNoteById($id);
			$fields[gettext("date")] = $note["human_date"];
			$fields[gettext("sender")] = $note["from_name"];
			$fields[gettext("recipient")] = $note["to_name"];
			$fields[gettext("subject")] = $note["subject"];
			$fields[gettext("description")] = nl2br($note["body"]);
			$fields[gettext("relation")] = $note["relation_name"];
			if ($GLOBALS["covide"]->license["has_project"] || $GLOBALS["covide"]->license["has_project_declaration"]) {
				$fields[gettext("project")] = $note["project_name"];
			}
			$output = new Layout_output();
			$output->insertAction("info", gettext("view"), sprintf("javascript: popup('index.php?mod=note&action=message&msg_id=%d&hidenav=1');", $id));
