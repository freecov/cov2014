...
function file_attach_multi() {
	...
	ids = ids.replace(/^0,/g,'');
	startStopAttach(true);
	parent.add_attachment_covide(ids);
	startStopAttach(false);
}
function add_attachment(id) {
	startStopAttach(true);
	parent.add_attachment_covide(id);
	startStopAttach(false);
}
function startStopAttach(state) {
	if (state == true) {
		...
		alert('The file(s) have been added to the email');
	}
}

function save_attachment() {
	var description = Prompt.show(gettext("Supply a description"));
...
/* google download/attach handler */
function googleAction(obj) {
	var a  = obj.value;
	var ua = a.split(/\:/g);
	obj.value = 0; //reset selectbox value

	switch (ua[0]) {
		case 'dl':
			/* download and pass to browser */
			popup('?mod=google&action=gdownload&dl=1&file=' + a, 'google_dl', 900, 600, 1);
			break;
		case 'att':
			/* download and pass to email client */
			startStopAttach(true);
			opener.add_attachment_google(a);
			startStopAttach(false);
			break;
		case 'shw':
			popup('?mod=google&action=gview&dl=1&file=' + a, 'google_view', 900, 600, 1);
			break;
	}
}
