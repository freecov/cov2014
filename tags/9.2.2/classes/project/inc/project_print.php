		...
		while ($row = sql_fetch_array($result)) {
		  ...
		  $result = sql_query($query);
			...
			$line = "\"".gettext("Project").":\",\"".$projnaam."\"\n";
                        ...
                        // is there a reason these lines are not translated via gettext?
			if ($from) {
				$line.= "\"datum van/tot:\",\"".utf8_encode(strftime("%d %B %Y",$from))." - ".utf8_encode(strftime("%d %B %Y",$to))."\"\n";
			}
...
<html>
<head>
<title><?php echo gettext("Project details") ?></title>
</head>
<body>
<style type="text/css">
...
<div name="loading" id="loading" style="position:absolute;top:50px;left:50px;" align="center">
	<font size="2"><b><?php echo gettext("Reading data") ?>.... <?php echo gettext("please wait") ?>...</b></font>
</div>
		...
		while ($row = sql_fetch_array($result)) {
		  ...
		  $result = sql_query($query);
			...
			<table width="100%">
				<tr>
					<td>
						<big><b><?php echo gettext("Project") ?>: <?php echo $projnaam ?></b></big>
						<?php if ($from){ ?>
							(<?php echo utf8_encode(strftime("%d %B %Y",$from)) ?> - <?php echo utf8_encode(strftime("%d %B %Y",$to)) ?>)
						<?php } ?>
						<br>
						<?php echo gettext("print date") ?>: <?php echo strftime("%d-%m-%Y %H:%M") ?><BR><BR>
					</td>
					<td align="right">
						f = <?php echo gettext("billing hours") ?><br>
						s = <?php echo gettext("service hours") ?>
					</td>
				</tr>
			</table>
...
<tr>
			...
			<td colspan="6">
				<?php echo gettext("total hours:"); ?> <b><?php echo number_format($totaal,1) ?></b>
			</td>
			<td align="right" colspan="2"><?php echo gettext("total price:"); ?> <b>&euro;&nbsp;<?php echo number_format($totaalprijs,2,",",".") ?></b></td>
		  </tr></></td>
		</tr>
	</table>
	</div>
