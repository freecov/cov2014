<?php
	...
	//get user permissions for the 'new classification' box
	...
	$output->addHiddenField("single_cla", $_REQUEST["single_cla"]);
	...
	if ($_REQUEST["sub_action"] == "add") {
		...
		if (is_array($cla_available)) {
			if ($_REQUEST["single_cla"] && is_array($classifications_selected)) {
				$classifications_selected = array();
			}
	...
	foreach ($classifications_selected as $k=>$v) {
		...
		$output_alt->addCode(addslashes($v));
		...
		$venster->addVensterData();
			...
			$tbl->addTableRow();
				$tbl->addTableData("", "data");
					...
					$tbl->addTag("br");
					$tbl->addSelectField("classifications_available[]", $list, "", ($_REQUEST["single_cla"]?0:1), array("size"=>20, "style"=>"width:300px"));
				$tbl->endTableData();
				$tbl->addTableData("", "data");
				...
				$tbl->addTableData("", "data");
					...
					$tbl->addTag("br");
					$tbl->addSelectField("classifications_selected[]", $classifications_selected, "", ($_REQUEST["single_cla"]?0:1), array("size"=>20, "style"=>"width:300px"));
				$tbl->endTableData();
			$tbl->endTableRow();
			if ($userinfo["xs_classmanage"] || $userinfo["xs_usermanage"]) {
				$tbl->addTableRow();
					$tbl->addTableData(array("colspan"=>3, "id" => "newclarow", "style" => "display: none"), "data");
					...
					$tbl->addTableData(array("colspan"=>3, "id" => "newclarowtext"), "data");
						$tbl->insertAction("new", gettext("create new classification"), "javascript: show_create_classification();");
					$tbl->endTableData();
					$tbl->endTableData();
				$tbl->endTableRow();
			}
