...
$activities = $calendar_data->getActivityNames($_SESSION["user_id"], 1);
...
$output->layout_page("", 1);
	...
	$output->addHiddenField("regitem[reg_name]", "reg_input");
	...
	$venster->addVensterData();
		$venster->addTag("ul", array("class" => "bcard_tab_ul", "style" => "list-style-type: none;"));
			$venster->addTag("li", array("id" => "bcard_li_relation", "class" => "bcard_tab_li selected"));
				$venster->insertLink(gettext("hour registration"), array("href" => sprintf("?mod=calendar&action=reg_input&id=0&timestamp=%d&project_id=%d&address_id=%d", time(), $calendaritem["project_id"], $calendaritem["user_id"])));
			$venster->endTag("li");
			$venster->addTag("li", array("id" => "bcard_li_relation", "class" => "bcard_tab_li"));
				$venster->insertLink(gettext("batch hour input"), array("href" => sprintf("?mod=calendar&action=batch_reg_input&id=0&project_id=%d", $calendaritem["project_id"])));
			$venster->endTag("li");
			$venster->addTag("li", array("id" => "bcard_li_relation", "class" => "bcard_tab_li"));
				$venster->insertLink(gettext("other project costs"), array("href" => sprintf("?mod=calendar&action=misc_reg_input&id=0&project_id=%d", $calendaritem["project_id"])));
			$venster->endTag("li");
		$venster->endTag("ul");
		...
		/*
		$table->addTableRow();
			$table->insertTableData(gettext("hours overtime"), "", "header");
			$table->addTableData("", "data");
				$table->addTextField("regitem[overtime]", 0);
			$table->endTableData();
		$table->endTableRow();
		*/
		$table->addTableRow();
		...
		if ($calendaritem["id"]) {
			$table->addTableRow();
				$table->insertTableData(gettext("update calendar"), "", "header");
				$table->addTableData("", "data");
					$table->addCheckbox("regitem[update_calendar]", 1, $update_calitem);
				$table->endTableData();
			$table->endTableRow();
		}
		$table->addTableRow();
