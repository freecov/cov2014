...
$venster->addVensterData();
		...
		$table->addTableData("", "data");
			...
			$table1->addTableRow();
				...
				$table1->addTableData(array("align" => "right"), "data");
					...
					if ($id) {
						$table1->insertAction("delete", gettext("delete"), sprintf("javascript:calendaritem_remove(%d, %d, %d);", $id, $_REQUEST["user"], $calendar_item["is_repeat"]));
					}
	...
	$table->addTableRow();
		...
		$table->addTableData(array("align"=>"right"), "data");
			...
			if ($id) {
				$table->insertAction("delete", gettext("delete"), sprintf("javascript:calendaritem_remove(%d, %d, %d);", $id, $_REQUEST["user"], $calendar_item["is_repeat"]));
			}
