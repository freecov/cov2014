...
if ($GLOBALS["covide"]->license["address_strict_permissions"] && !$user->checkPermission("xs_addressmanage") && !$_SESSION["visitor_id"]) {
...
if ($addresstype == "users" || $addresstype == "private" || $addresstype == "overig") {
	...
	$query_zoek = sprintf("
		SELECT
			...
			address.user_id, address.debtor_nr, address.duplicate_with relationduplicate,
		...
		WHERE
			(address.is_public=1 OR address.user_id=%d)
			AND (%s)",
		$options["funambol_user"], $_SESSION["user_id"], $sq);
	$query_csv = $query_zoek;
} else {
	...
	$query_zoek = "SELECT
		...
		google_address_sync.google_id
	FROM address_businesscards $bjoin LEFT JOIN google_address_sync ON address_businesscards.id = google_address_sync.address_id AND google_address_sync.user_id = ".$options["funambol_user"]." WHERE ($sq AND (address_businesscards.rcbc = 0 OR (address_businesscards.rcbc = 1 AND address.is_active=1))) ";
