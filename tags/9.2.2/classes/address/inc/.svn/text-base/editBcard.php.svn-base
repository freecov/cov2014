	...
	$venster->addVensterData();
		...
		$table->endTableRow();
			...
			$table->addTableRow(array("class" => "expand_account"));
				...
				$table->addTableData(array("colspan" => 3), "data");
					...
					$table->addCheckBox("address[is_person]", 1, $bcardinfo["is_person"]);
		...
		if ($bcardinfo["rcbc"] != 1) {
			$table->addTableRow();
				...
				$table->addTableData(array("colspan" => 3), "data");
					$table->addCheckBox("bcard[completercbc]", 1, ($_REQUEST['address_id']?0:1));
				$table->endTableData();
			$table->endTableRow();
		}
		$table->endTable();
		...
		if ($bcardinfo["id"] && !$view_only) {
			...
			$metadata   = new Metafields_data();
			$metaoutput = new Metafields_output();
			$metafields = $metadata->meta_list_fields("bcards", "");
