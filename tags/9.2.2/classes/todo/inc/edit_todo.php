<?php
	...
	if ($_REQUEST["mail_id"]) {
		...
		$noteinfo["subject"] = $mailoutput[0]["subject"];
		if ($mailoutput[0]["is_text"]) {
			$noteinfo["body"] = $mailoutput[0]["body"];
		} else {
			$noteinfo["body"] = str_replace("<br>", "\n", $emaildata->html2text($mailoutput[0]["body_html"]));
		}
		unset($emaildata);
	}
	...
	if ($_REQUEST["project_id"] && !$noteinfo["project_id"]) {
		$noteinfo["project_id"] = $_REQUEST["project_id"];
	}
	...
	$venster->addVensterData();
		...
		$table->addTableRow();
			...
			$table->addTableData("", "data");
				$table->addHiddenField("todo[user_id]", $user_id);
				$useroutput = new User_output();
				$table->addCode( $useroutput->user_selection("todouser_id", $user_id, 0, 0, 0, 1, 1, 0, 1) );
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
			/* start date */
			...
			$table->addTableData("", "data");
				...
				$table->addSelectField("todo[timestamp_hours]", $hours, date("H", $noteinfo["timestamp"]));
				$table->addSelectField("todo[timestamp_minutes]", $minutes, date("i", $noteinfo["timestamp"]));
		...
		$table->addTableRow();
			...
			$table->addTableData("", "data");
				...
				$table->addSelectField("todo[timestamp_end_hours]", $hours, date("H", $noteinfo["timestamp_end"]));
				$table->addSelectField("todo[timestamp_end_minutes]", $minutes, date("i", $noteinfo["timestamp_end"]));
