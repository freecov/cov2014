...
function showPerformanceInfo() {
	if (document.getElementById('performance_info_trigger') && document.getElementById('footer_timers')) {
		document.getElementById('performance_info_trigger').style.display = 'none';
		document.getElementById('footer_timers').style.display = 'inline';
	}
}
...
function headersearch() {
	skey = document.getElementById('headersearchkey').value;
	starget = document.getElementById('search_in').value;
	if (skey) {
		if (starget == 1) {
			url = '?mod=index&fromdashboard=1&search[phrase]='+escape(skey)+'&all=1&submit=1';
			document.location.href=url;
		}
		if (starget == 2) {
			url = 'http://www.google.com/search?q='+escape(skey);
			window.open(url);
		}
	}
}
