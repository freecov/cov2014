...
Class Layout_history Extends Layout_output {
	...
	private function history_clean() {
		...
		foreach ($history as $k=>$v) {
			...
			if ($scope[$v["scope"]]>6) {
				...
				unset($history[$k]);
			} elseif ($history[$k]["time"]+(3600*6) < time()) {
				/* garbage collector */
	...
	public function get_history_scope_list() {
		...
		foreach ($scopes as $k=>$scope) {
			$scope_counter++;
			$this->addCode("var ".gettext("screen").$scope_counter." = '".$k."';\n");
		}
		$this->end_javascript();
		return $this->generate_output();
	}
	public function get_history_data() {
		$data = array(
			"0" => "- ".gettext("saved checkpoints")." -",
			"-1" => "- ".gettext("delete all checkpoints")." -"
		);
		...
		$scope_counter = 0;
		foreach ($scopes as $k=>$scope) {
			$scope = array_reverse($scope);
			$scope_counter++;
			foreach ($scope as $id) {
				$data[gettext("screen").$scope_counter][$id] = sprintf("[%s] %s", date("H:i:s", $history[$id]["time"]), $history[$id]["descr"]);
			}
		}
		return $data;
	}
	public function generate_history_call() {
		...
		if ($GLOBALS["covide"]->mobile) {
			$this->load_javascript(self::include_dir."history_mobile.js");
		} else {
			$this->load_javascript(self::include_dir."history.js");
			$this->start_javascript();
			foreach ($history as $k=>$v) {
