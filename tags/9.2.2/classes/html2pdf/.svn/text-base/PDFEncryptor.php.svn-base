...
class PDFEncryptor {
    ...
    function encrypt()
    {
        ...
        if (!file_exists($this->pdfFile)) {
            die("Error: The PDF file does not exist: $this->pdfFile");
        }

        // make sure iText jar file exists
        if (!file_exists($this->iTextPath)) {
            die("Error: The iText jar file does not exist: $this->iTextPath.  You can download it at http://www.lowagie.com/iText");
        }

        // make sure encryptPdf file exists
        if (!file_exists($this->encryptPdfPath)) {
            die("Error: The encrypt_pdf java class does not exist: $this->encryptPdfPath.");
        }

        // make sure the java binary exists
        if (!@is_executable($this->javaPath)) {
            die("Error: java [$this->javaPath] is not executable.  You can download it at http://java.sun.com");
        }
