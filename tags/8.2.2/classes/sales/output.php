		...
		if ($_REQUEST["export_xml"]) {
			...
			if (!is_array($data["data"])) {
				...
				foreach ($data["data"] as $dat) {
					...
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", "project", str_replace("&", "&amp;", $conversion->str2utf8($dat["h_project"])));
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", "prospect", $dat["h_timestamp_prospect"]);
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", "quote", $dat["h_timestamp_proposal"]);
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", "order", $dat["h_timestamp_order"]);
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", "invoice", $dat["h_timestamp_invoice"]);
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", "user", str_replace("&", "&amp;", $conversion->str2utf8($dat["username"])));
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", "score", $dat["expected_score"]);
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", "price", number_format($dat["orig_sum"], 2, ",", "."));
					$string .= "</item>\n";
				}
			}
			$string .= "</export>\n";
