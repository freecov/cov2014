...
$venster->addVensterData();
	...
	$view->addMapping(gettext("birthday"), "%h_birthday");
...
if (!$GLOBALS["covide"]->license["disable_basics"]) {
	...
	$venster->addVensterData();
		...
		$venster->addCode($view->generate_output());
		unset($view);
		$venster->insertAction("new", gettext("create todo"), "?mod=todo&action=edit_todo&address_id=".$id);
		$venster->insertLink(gettext("create todo"), array(
			"href" => "?mod=todo&action=edit_todo&address_id=".$id
		));
	$venster->endVensterData();
