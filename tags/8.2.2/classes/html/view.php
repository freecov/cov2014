	...
	public function generate_output($no_onmouseover=0) {
		...
		foreach ($mappings as $name=>$map) {
			if ($this->_alternative_header[$name]) {
				...
				$table->insertTableHeader($name.$this->generateSortLink($_name), array("class"=>"list_header_center", "style"=>"text-align: ".$this->_align[$_name]));
			} else {
				$table->insertTableHeader($name.$this->generateSortLink($name), array("class"=>"list_header_center", "style"=>"text-align: ".$this->_align[$name]));
			}
		}
		$table->endTableRow();
		$table->endTag("thead");
