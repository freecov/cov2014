...
Class Templates_output {
	...
	public function templatePrint($pdf=0, $finance="") {
			...
			$output->addTag("div", array(
				...
				$tbl->addTableRow();
					...
					if ($settings["address_position"]) {
						$tbl->addTableData();
						if ($img && !$settings["logo_position"]) {
							$tbl->addTag("img", array(
								"src" => $img
							));
						} elseif ($finance_header) {
							$tbl->addCode($finance_header);
							$tbl->addTag("br");
							$tbl->addSpace();
							$tbl->addTag("br");
							$tbl->addSpace();
					...
					$tbl->endTableData();
					/* if position = left */
					if (!$settings["address_position"] && $img && !$settings["logo_position"]) {
						$tbl->addTableData(array("align"=>"right"));
							$tbl->addTag("img", array(
								"src"   => $img,
								"align" => "right"
		...
		$file = sprintf("%s/pdf_%s.pdf", $dir, md5(rand().session_id().mktime()));
		copy($result, $file);
		unlink($result);
