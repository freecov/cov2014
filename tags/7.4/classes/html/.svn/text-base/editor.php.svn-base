...
class Layout_editor {
	...
	private function load_tinymce($mini=0, $cleanup="true", $txtareaname="contents") {
	...
	public function generate_editor($settings=0, $data="", $cleanup="true", $txtareaname="contents") {
		...
		if (($browser["browser_name"] == "moz" && $browser["math_version"] >= 1.8) ||
			...
			($browser["browser_name"] == "saf" && $browser["math_version"] >= 522.13)
		)
		{
			$xinha = 0;
		...
		if ($tinymce)
			return $this->load_tinymce($settings, $cleanup, $txtareaname);
		if ($xinha)
