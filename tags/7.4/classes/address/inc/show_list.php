...
if (!preg_match("/^searchRel/", $action)) {
	...
	if ($user->checkPermission("xs_classmanage")) {
		...
		$venster->addMenuItem(gettext("title mngt"), "index.php?mod=address&action=listTitles");
...
$table->addTableRow();
	...
	$table->addTableData();
		...
		$table->addSpace(2);
		$table->addSelectField("specified", array("", gettext("name"), gettext("relations"), gettext("zipcode"), gettext("city"), gettext("country")), array($_REQUEST["specified"]));
		$table->addSpace(2);
		$table->addSelectField("landSelect", $countryArray);
	...
	for ($i=0; $i!=26; $i++) {
			...
			if (preg_match("/^searchRel(.*)$/", $action)) {
				$table->insertLink(chr(65+$i), array(
					"href" => "index.php?mod=address&action=$action&and_or=".trim($and_or)."&funambol_user=".$_REQUEST["funambol_user"]."&addresstype=".$addresstype."&l=".chr(65+$i)
				) );
				$table->addSpace(1);
			} else {
				if ($_REQUEST["l"] == chr(65+$i))	$table->addTag("b");
				$table->insertLink(chr(65+$i), array(
					"href" => "index.php?mod=address&action=lijst&and_or=".trim($and_or)."&funambol_user=".$_REQUEST["funambol_user"]."&addresstype=".$addresstype."&l=".chr(65+$i)
				) );
				if ($_REQUEST["l"] == chr(65+$i))	$table->endTag("b");
				$table->addSpace(1);
			}
	}
	$table->endTableData();
	...
	$table->addTableRow();
		$table->addTableData(array("colspan"=>27));
			...
			$table->insertLink(gettext("all"), array(
				"href" => "index.php?mod=address&addresstype=relations&sub=alle&funambol_user=".$_REQUEST["funambol_user"]
			) );
			$table->addSpace(2);
			if ($_REQUEST["sub"] == "klanten") $table->addTag("b");
			$table->insertLink(gettext("customers"), array(
				"href" => "index.php?mod=address&addresstype=relations&sub=klanten&funambol_user=".$_REQUEST["funambol_user"]
			) );
			...
			$table->insertLink(gettext("suppliers"), array(
				"href" => "index.php?mod=address&addresstype=relations&sub=leveranciers&funambol_user=".$_REQUEST["funambol_user"]
			) );
...
switch ($_REQUEST["addresstype"]) {
	...
	case "bcards" :
		...
		$view->addMapping(gettext("telephone nr"), "%%phone_nr");
		$view->addMapping(gettext("mobile phone nr"), "%%mobile_nr");
		$view->addMapping(gettext("companies"), "%%complex_companyname");
	...
	default :
		...
		$view->addMapping(gettext("zip code"), "%zipcode", "", "", "", 1);
		$view->addMapping(gettext("city"), "%city", "", "", "", 1);
		$view->addMapping(gettext("country"), "%country", "", "", "", 1);
...
if ($user->checkPermission("xs_addressmanage") || $user->checkPermission("xs_usermanage")) {
	$table->addTableRow(1);
		$table->addTableData(array("style"=>"text-align: right", "colspan"=>2));
			$table->insertAction("delete", gettext("put selection on non-active"), "javascript: popup('index.php?mod=address&action=removeSelection&info=".urlencode($exportinfo)."', 'remove_selection', 450, 350, 1);");
			$table->insertAction("toggle", gettext("manage selection classification"), "javascript: popup('index.php?mod=address&action=addcla_multi&info=".urlencode($exportinfo)."', 'addcla_multi', 350, 150, 1);");
			$table->insertAction("save", gettext("save"), "index.php?dl=1&mod=address&action=export&info=".urlencode($exportinfo));
