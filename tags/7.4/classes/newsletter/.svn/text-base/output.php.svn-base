...
Class Newsletter_output {
	...
	public function selectFormat() {
		...
		$list = $address_data->getRelationsList( array("addresstype"=>$addresstype, "nolimit"=>1, "sub"=>$sub ));
		$emails = array();
		$errors = array();
		...
		$venster->addVensterData();
			...
			$tbl = new Layout_table();
			$tbl->addTableRow();
				$tbl->insertTableData( gettext("total receipients"), "", "header");
				$tbl->insertTableData( count($emails)." ".gettext("ontvanger(s)"), "", "data");
			$tbl->endTableRow();
			$tbl->addTableRow();
				$tbl->insertTableData( gettext("recepients"), "", "header");
				$tbl->addTableData("", "data");
	...
	public function getAddresses() {
		...
		$address_data = new Address_data();
		$list = $address_data->getRelationsList( array("addresstype"=>$_REQUEST["addresstype"], "emailtype"=>$_REQUEST["emailtype"], "nolimit"=>1 ));
