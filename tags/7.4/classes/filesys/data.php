...
Class Filesys_data {
	...
	public function check_folder($settings) {
		...
		if (array_key_exists("address_id", $settings) && !(array_key_exists("project_id", $settings))) {
	...
	public function FS_calculateFileTree($filename) {

		// Check wether mapSize is bigger or smaller than file length
		$startPos = $this->mapSize - strlen($filename);
		($startPos < 1) ? $startPos = 0 : $startPos = $startPos;

		for($i=0;$i<strlen($filename);$i++) {
			$fOrder[$i+$startPos] = $filename{$i};
		}
		// Create empty array with the size of mapSize
		for($i=0;$i<$this->mapSize;$i++) {
			$fArray[] = '0';
		}

		// Fill fileTree digit in matching index
		for($i=$startPos;$i<$this->mapSize;$i++) {
			$fArray[$i] = $fOrder[$i];
		}
		return $fArray;
	}
	
	public function FS_lookUpFile($file, $folder) {
		if(file_exists($folder.'/'.$file)) { 
			$fileExploded = explode(".", $file);
			$filename = $fileExploded[0];
			$fOrder = $this->FS_calculateFileTree($filename);
			$output = $folder.'/';
			foreach($fOrder as $theFolder) {
				$output .= $theFolder.'/';
			}
			$output .= $file;
			return $folder.'/'.$file;
			...
			for($i=0;$i<strlen($filename);$i++) {
				...
				$folder_tree = $folder_tree.'/'.$filename{$i};
				if(file_exists($folder.''.$folder_tree.'/'.$file)) {
				
				return $folder.''.$folder_tree.'/'.$file;
				// Get out of loop
				$i = strlen($filename);
	...
	/* new routine by gerben
	public function FS_readFile($file, $id, $ext) {

		$file = $this->FS_lookUpFile($id.'.'.$ext, $file);
	*/
	
	/* old routine that works */
