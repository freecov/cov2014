...
Class ProjectDeclaration_output {
		...
		$venster->addVensterData();
			...
			$table->addTableRow(array("id" => "layer_btw"));
				...
				$table->addTableData("", "data");
					...
					$table->addSelectField("declaration[btw]", $data, end($data));
				$table->endTableData();
			$table->endTableRow();
			...
			$table->addTableRow(array("id" => "layer_ncnp_perc", "style" => "display: none"));
				...
				$table->endTableData();
				$table->addTableData("", "data");
					//$table->addTextField("declaration[ncnp]", "");
					$sel = $declaration_data->getFieldContent("NCNP_tarif", 1);
					$data = array();
					foreach ($sel as $k=>$v) {
						$data[$v] = $v;
					}
					$table->addSelectField("declaration[perc_NCNP]", $data, 0);
			...
			*/
			...
			//$table->addHiddenField("declaration[perc_NCNP]", 0);
			...
			$table->addTableRow(array("id" => "layer_description"));
				...
				$table->addTableData("", "data");
					...
					$table->addSelectField("declaration[description]", $data, end($data));
				$table->endTableData();
			$table->endTableRow();
		...
		$venster->addVensterData();
			...
			$table->addTableRow();
				...
				$table->addTableData("", "data");
					...
					if ($ncnp) {
						$sel = array(
							"calc"   => gettext("send NCNP calculation")
						);
					} else {
						$sel = array(
							"nocalc"   => gettext("send no calculation"),
							"calc"   => gettext("send normal calculation")
						);
					}
	...
	public function genExtraFields($project_id) {
		...
		$table = new Layout_table(array(
			...
			/* task date */
			if (!$project_id) {
				$declaration["task_date"]   = mktime();
				$declaration["damage_date"] = mktime();
			}
			$table->addTableRow();
