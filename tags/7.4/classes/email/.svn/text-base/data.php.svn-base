...
Class Email_data {
	 ...
	 */
	public function sendMailComplex($id, $return=0, $skip_sender_rewrite=0) {
	...
	private function getEmailBySearchAddSearchQuery($sq, $search, $use_subquery, $archive=0) {
		...
		if ($use_subquery) {
			$sq.= sprintf(" OR id IN (select mail_id from mail_messages_data where header %1\$s '%%%2\$s%%' or body %1\$s '%%%2\$s%%')) ", $like, $search);
			if ($archive)
				$sq.= sprintf(" OR id IN (select mail_id from mail_messages_data_archive where header %1\$s '%%%2\$s%%' or body %1\$s '%%%2\$s%%')) ", $like, $search);

		} else {
			echo $archive;

			if (!$archive) {
				$sq.= sprintf(" OR mail_messages_data.body %1\$s '%%%2\$s%%' or mail_messages_data.header %1\$s '%%%2\$s%%')", $like, $search);
			} else {
				$sq.= sprintf(" OR mail_messages_data.body %1\$s '%%%2\$s%%' OR mail_messages_data.header %1\$s '%%%2\$s%%'
					 OR mail_messages_data_archive.body %1\$s '%%%2\$s%%' OR mail_messages_data_archive.header %1\$s '%%%2\$s%%')", $like, $search);
			}
	...
	public function getEmailSignatureHtml($id) {
		...
		if ($id>0) {
			...
			$q = sprintf("select mail_signature_html from users where id = %d", $_SESSION["user_id"]);
			$res = sql_query($q);
			$sig = sql_result($res,0);
		}
		return $sig;
	...
	public function stripBodyTags($body) {
		$strip_tags = array("!doctype","html","head","title","meta","body","base","link");
	...
	public function stylehtml($body, $view_mode=0) {
	...
	/* Deletes the entire content of the trashbin */
	public function deleteAllTrash() {
		...
		if (is_array($mails)) {
			foreach ($mails as $id=>$v) {
			$q = "select id from mail_messages WHERE folder_id = $id";
			$res = sql_query($q);
			while ($row = sql_fetch_assoc($res)) {
				$this->mail_delete($row["id"]);
			}
		...
		echo "document.location.href = '?mod=email&folder_id=$id'";
