...
Class Email_output {
	...
	public function viewHtml($no_filter=0, $text=0) {
		...
		if (!$no_filter) {
			...
			$body = preg_replace("/(<body[^>]*?>)/sxi", "$1"."\n<script language='Javascript1.2' type='text/javascript'>window.onload = function() { parent.mail_resize_frame(); }</script>\n", $body);
		}
	...
	public function emailMediaGallery() {
		...
		if ($_REQUEST["fullhtml"]) {
			...
			foreach ($data as $k=>$v) {
				if ($v["is_image"]) {
					$venster->addTag("li", array("type" => "circle"));
					$venster->insertTag("a", $v["name"], array("href" => sprintf("javascript: setImage('%d')", $v["id"])));
				}
				$i++;
			}
			if (!$i)
				$venster->addCode(gettext("No valid image attachment found, please upload an attachment first."));
