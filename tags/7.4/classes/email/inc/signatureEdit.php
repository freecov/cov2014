 ...
 */
	...
	/* signature */
	$tbl->addTableRow();
		$tbl->insertTableData(gettext("text signature"), "", "header");
		$tbl->addTableData("", "data");
			//TODO: Don't allow mail to be longer than 78 characters on one line.
			$tbl->addTextArea("mail[signature]", $list[0]["signature"], array("style"=>"width: 650px; height: 180px;", "wrap"=>"off"));
		$tbl->endTableData();
	$tbl->endTableRow();

	/* signature */
	$tbl->addTableRow();
		$tbl->insertTableData(gettext("html signature"), "", "header");
		$tbl->addTableData("", "data");
			$editor = new Layout_editor();
			$ret = $editor->generate_editor(1);
			if ($ret !== false) {
				$tbl->addTextArea("mail[signature_html]", nl2br($list[0]["signature_html"]), array("style" => "width: 650px; height: 180px;"), "contents");
				$tbl->addCode($ret);
			} else {
