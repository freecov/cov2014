 ...
 */
	...
	/*
	if ($this->rss) {
		$output2->insertAction("rss_page", gettext("page specific rss feed"), $this->rss);
		$output2->addSpace();
		$output2->addCode("\n");
	}
	...
	*/
	...
	if ($_SESSION["user_id"] || $_SESSION["visitor_id"]) {
		...
		if ($_SESSION["user_id"]) {
			...
			$output2->addCode(gettext("Logged in as user").": ");
			$output2->insertTag("b", $user_data->getUserNameById($_SESSION["user_id"]));
		} else {
			$output2->addCode(gettext("Logged in as visitor or customer").": ");
