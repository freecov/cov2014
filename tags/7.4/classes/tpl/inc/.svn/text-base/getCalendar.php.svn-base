...
?>
<a name="calendar_mark"></a>
<form id="calform" action="/calendarpage/<?= $this->pageid ?>" method="get">
<input type="hidden" name="start" id="calstart" value="<?=$start?>">
			...
			<? for ($m=0; $m<3; $m++) { ?>
				...
				<td style="vertical-align: top;">
					<table class="calendar_table" cellspacing="0" cellpadding="2">
							...
							<td class="calendar_month" colspan="2">
								<?
									if ($m == 0) {
										...
										echo "<img src='/img/spacer.gif' height='22px;'>";
									}
								?>
							</td>
							<td class="calendar_month" colspan="5" style="text-align: center;"><b><?=date("F Y", mktime(0,0,0,$month+$m, 1, $year)); ?></b></td>
						...
						<tr>
							<? for ($i=0; $i<7; $i++) { ?>
								<td class="calendar_weekday"><?=$weekdays[$i]?></td>
							<? } ?>
						...
						<tr>
							<?
								for ($i=$offset; $i<=date("t", $ts); $i++) {
									...
									/* get items */
									$q = sprintf("select dateid from cms_date_index where pageid = %d and datetime between %d and %d",
										$id, $curts, $nextts);
									$res = sql_query($q);
									$items = array();
									...
									if ($i > 0)
										if (count($items) > 0) {
											...
											echo sprintf("<td class='calendar_day' style='text-align: right'>
												<b><em class=\"tt_tooltip\" onmouseover=\"return escape('%s');\">%d</em></b></td>",
												$txt, $i);
