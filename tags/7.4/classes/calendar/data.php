	...
	Class Calendar_data {
		...
		public function getAppointmentsByAddress($address_id, $history=0) {
			...
			while ($row = sql_fetch_assoc($res)) {
				...
				if (!trim($row["subject"])) {
					$calendaritems[$i]["subject"]     = substr($row["description"], 0, 40);
				} else {
					$calendaritems[$i]["subject"]     = $row["subject"];
				}
				$calendaritems[$i]["body"]            = $row["description"];
				$i++;
			}
		 ...
		 */
		public function getCalendarItemById($id, $funambol=0) {
		...
		public function xml_check() {
			...
			$sql = "SELECT COUNT(*) as counter FROM calendar WHERE is_repeat = 0 AND (((timestamp_start BETWEEN $tstamp_from AND $tstamp_to1) OR (timestamp_end BETWEEN $tstamp_from1 AND $tstamp_to)) or (timestamp_start<=$tstamp_from and timestamp_end>=$tstamp_to))";
			if ($_REQUEST["id"]) {
			...
			if ($_REQUEST["user_id"]) {
				...
				$sql .= sprintf(" AND user_id IN (%s)", implode(",", $_users));
			} else {
		...
		public function checkRegistrationState($item_id) {
			$q = sprintf("select is_registered from calendar where id = %d", $item_id);
			$res = sql_query($q);
