...
Class Calendar_output {
	...
	public function print_main() {
		require_once(self::include_dir."print_main.php");
	}
	/* }}} */
	/* show_edit {{{ */
	/**
	...
	function show_calendar($day, $month, $year) {
		...
		*/
		...
		$output->addCode("
			function calendarPopUp_$name() { eval(\"var wx = window.open('".self::include_dir."calendar.php?ident=$name&start=".mktime(0,0,0,1,1,date("Y")-1)."&end=".mktime(0,0,0,1,1,date("Y")+5)."&sday='+$day.value+'&smonth='+$month.value+'&syear='+$year.value, 'wx', 'toolbar=no,scrollbars=no,location=no,statusbar=no,menubar=no,resizable=no,width=190,height=220,left = 100,top = 80');\"); }
	 ...
	 */
	public function show_monthview_mu() {
		require_once(self::include_dir."show_monthview_mu.php");
	}
	/* }}} */
	/* show_planning {{{ */
	/**
	...
	public function show_planning() {
		require_once(self::include_dir."show_planning.php");
	}
	/* }}} */
	/* show_day {{{ */
	/**
	 ...
	 */
	 public function show_day($data, $datestamp=0, $print=0) {
		...
		$view->defineComplexMapping("complex_location", array(
			array(
				"type"  => "text",
				"text"  => array(gettext("location"), ": ", "%location", ", ", gettext("kilometers"), ": ", "%km", "\n"),
				"check" => "%location"
			)
		));
	 ...
	 */
	 ...
	 public function print_week() {
		require_once(self::include_dir."print_week.php");
	 }
	 /* }}} */
}
?>
