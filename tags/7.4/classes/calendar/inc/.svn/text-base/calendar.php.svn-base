...
<? 	if ($_REQUEST["action"]=="passthru") {	?>
	<script language="Javascript1.2" type="text/javascript">
		opener.upd_<?=$ident?>('<?=date("j",$date)?>','<?=date("n",$date)?>','<?=date("Y",$date)?>');
		window.close();
	</script>
	</body>
	</html>
		...
		<tr>
			<td>
				<select onchange="document.getElementById('formfields').submit();" name="fmonth" class="inputselect">
					<? for ($i=1;$i<=12;$i++) { ?>
						<? if ($i==$month || (mktime(0,0,0,$i,31,$year) > $start && mktime(0,0,0,$i,1,$year) < $end)) { ?>
							<option value="<?=$i?>" <? if ($month==$i) { echo "selected"; } ?>><?= date("M", mktime(0,0,0,$i,1,date("Y")))?></option>
						<? } ?>
					<? } ?>
				</select>
			</td>
			<td>
				<select onchange="document.getElementById('formfields').submit();" name="fyear" class="inputselect">
					<? for ($i=2003;$i<=2010;$i++) { ?>
						<? if ($i==$year || ($i >= date("Y",$start) && $i <= date("Y",$end))) { ?>
