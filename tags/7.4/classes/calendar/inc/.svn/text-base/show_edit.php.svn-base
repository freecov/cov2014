...
for ($i=0;$i!=60;$i=$i+$intervalAmount) {
	if($userdetails["hour_format"] == 1) { $prefDate = "i"; } else { $prefDate = "i"; }
	$hour_time = mktime(0, $i, 0, date("m"), date("d"), date("Y"));
	$minutes_to[$i] = date($prefDate, $hour_time);;
}
$repeat = array(
	0  => gettext("na"),
	1  => gettext("every work day"),
	7  => gettext("weekly"),
	14 => gettext("every other week")
);
$repeat_type = array(
	"0" => gettext("na"),
	"M" => gettext("monthly"),
...
$venster->addVensterData();
		...
		$table->addTableData("", "data");
			...
			$table1->addTableRow();
				...
				$table1->addTableData(array("align" => "right"), "data");
					...
					$table1->insertAction("back", gettext("back"), "javascript:window.close();");
					if ($id) {
						$table1->insertAction("delete", gettext("delete"), sprintf("javascript:calendaritem_remove(%d);", $id));
					}
					$table1->addTag("span", array("id"=>"action_save_top", "style"=>"visibility: hidden;"));
					$table1->insertAction("ok", gettext("save"), sprintf("javascript:calendaritem_save(%d);", $id));
					$table1->endTag("span");
				$table1->endTableData();
			$table1->endTableRow();
			$table1->endTable();
	...
	if ($calendar_item["is_repeat"]) {
		$table->addTableRow();
			...
			$table->addTableData("", "data");
				$table->addTag("div", array("id" => "rep_warning", "style" => "padding: 3px; font-weight: bold; border: 2px dotted red;"));
				$table->addCode(gettext("This is a repeating item. Changes will be saved for all occurrences."));
			$table->endTableData();
		$table->endTableRow();
	}
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			...
			$table_location->addTableRow();
				...
				$table_location->addTableData(array("align"=>"left"), "data");
					...
					$table_location->addSpace(2);
					$table_location->insertLink(gettext("planner"), array("href"=>"http://www.routenet.nl", "target"=>"_blank"));
				$table_location->endTableData();
	...
	$table->endTableRow();
	$table->addTableRow();
		$table->insertTableData(gettext("important meeting"), "", "header");
		$table->addTableData("", "data");
	...
	$table->addTableRow();
		...
		$table->addTableData(array("align"=>"right"), "data");
			...
			if ($id) {
				$table->insertAction("delete", gettext("delete"), sprintf("javascript:calendaritem_remove(%d);", $id));
			}
			$table->addTag("span", array("id"=>"action_save", "style"=>"visibility: hidden;"));
			$table->insertAction("ok", gettext("save"), sprintf("javascript:calendaritem_save(%d);", $id));
			$table->endTag("span");
		$table->endTableData();
	$table->endTableRow();
	$table->endTable();
