...
while ($row = sql_fetch_array($res)) {
	...
	if (($_SESSION["user_id"] != $row["user_id"] && $row["prive"]) || $calperms == "") {
		$row["subject"]     = gettext("private appointment");
	...
	$rt = mktime(0,0,0,1,$r["day"]+1,$r["year"]);
	while ($rt < $start) {
		$r["diff"]+=$r["val"];
		$rt = mktime(0,0,0,1,$r["day"]+$r["diff"]+1,$r["year"]);
	}
	//new day
	if ($row["repeat"]==1 && $start>$rt) {
		$flag=1;
	} else if ($rt>=$start && $rt<$eind) { //less than, cause day ends on 1 second before midnight
		$flag=1;
	}
...
while ($row = sql_fetch_array($res)) {
	...
	if ((int)$row["is_registered"] == 0 && (int)$row["project_id"] && !$GLOBALS["covide"]->license["has_project_declaration"])
...
if ($calperms == "") {
	...
	$this->calendar_items[1] = array(
		...
		// If we use body as well, we'll print the no permission text twice.
		//"body"          => gettext("no permissions on this calendar"),
		"rowspan"       => 36
	);
}
