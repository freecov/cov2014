...
class Layout_venster Extends Layout_output {
  ...
  public function __construct($settings="") {
		...
		if (!$GLOBALS["covide"]->mobile) {
			...
			foreach ($this->menu_items as $n=>$l) {
				$menu->addTableRow();
					$menu->addTableData();
						/* start menu item */
						$menuitem = new Layout_table();
						$menuitem->addTableRow();
							$menuitem->addTableData();
								$menuitem->insertImage("menu_link_l.gif", "", "", 1);
							$menuitem->endTableData();
							$menuitem->addTableData( array(
								"class"    => "menuLnk",
								"width"    => 114,
								"height"   => 18,
								"style"    => "text-align: center;"
								));
								$menuitem->insertLink($n, array(
									"href"   => $l[0],
									"class"  => "menu",
									"target" => ($l[1]) ? "_blank":""
								));
							$menuitem->endTableData();
							$menuitem->addTableData();
								$menuitem->insertImage("menu_link_r.gif", "", "", 1);
							$menuitem->endTableData();
						$menuitem->endTableRow();
						$menuitem->endTable();
						$menu->addCode($menuitem->generate_output());
						unset($menuitem);
						/* end menu item */
					$menu->endTableData();
				$menu->endTableRow();
	...
	public function generateCalendarMobile($month, $day, $year, $userlist) {
		...
		$table->insertAction("back", gettext("vorige maand"), $urlprev);
		$table->addSpace(1);
		$table->addCode(ucfirst(strftime("%B %Y", mktime(0, 0, 0, $month, 1, $year))));
		$table->addSpace(1);
		$table->insertAction("forward", gettext("volgende maand"), $urlnext);
		$table->endTag("div");
		/* table for calendar */
		$caltable = new Layout_table(array("cellspacing"=>1, "class"=>"calendar_table"));
		$caltable->addTableRow();
			$caltable->addTableData();
				$caltable->insertTag("div", gettext("zo")." ", array("class" => "calendar_mobile"));
				$caltable->insertTag("div", gettext("ma")." ", array("class" => "calendar_mobile"));
				$caltable->insertTag("div", gettext("di")." ", array("class" => "calendar_mobile"));
				$caltable->insertTag("div", gettext("wo")." ", array("class" => "calendar_mobile"));
				$caltable->insertTag("div", gettext("do")." ", array("class" => "calendar_mobile"));
				$caltable->insertTag("div", gettext("vr")." ", array("class" => "calendar_mobile"));
				$caltable->insertTag("div", gettext("za")." ", array("class" => "calendar_mobile"));
		...
		$searchtable->addTableRow();
			$searchtable->addTableData();
				...
				$searchtable->insertAction("forward", gettext("zoeken"), "javascript: date_jump();");
			$searchtable->endTableData();
		$searchtable->endTableRow();
		$searchtable->addTableRow();
			$searchtable->addTableData();
				$searchtable->addTextField("search_term", gettext("zoekwoord"), array("style"=>"width: 150px;"), "", 1);
				$searchtable->insertAction("forward", gettext("zoeken"), "javascript: search_jump();");
			$searchtable->endTableData();
		$searchtable->endTableRow();
	...
	public function generateCalendar($month, $day, $year, $userlist) {
		...
		$caltable = new Layout_table(array("cellspacing"=>1, "class"=>"calendar_table"));
		$caltable->addTableRow();
			$caltable->insertTableData(gettext("zo"), "", "header");
			$caltable->insertTableData(gettext("ma"), "", "header");
			$caltable->insertTableData(gettext("di"), "", "header");
			$caltable->insertTableData(gettext("wo"), "", "header");
			$caltable->insertTableData(gettext("do"), "", "header");
			$caltable->insertTableData(gettext("vr"), "", "header");
			$caltable->insertTableData(gettext("za"), "", "header");
