...
function tbs_Locator_PrmRead(&$Txt,$Pos,$HtmlTag,$DelimChrs,$BegStr,$EndStr,&$Loc,&$PosEnd) {
	...
	// Param�res de v�if de la boucle
	...
	while ($Continue) {
		...
		if ($DelimIdx) { // Lecture dans une cha�e

			if ($Chr===$DelimChr) { // Quote rencontr�
				if ($Chr===$Txt[$Pos+1]) { // Double quote => la cha�e continue en d�oublant le quote
					$Pos++;
				} else { // Simple quote => fin de la cha�e
					$DelimIdx = false;
				}
			}

		} else { // Lecture hors cha�e

			if ($BegCnt===0) {

				// Analyse des param�re
				$CheckChr = false;
				if ($Chr===' ') {
					if ($Status===1) {
						$Status = 2;
