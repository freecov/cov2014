	...
	public function getNotes($options = "") {
		...
		switch ($options["note_type"]) {
			case "old"     :
				...
				break;
			case "sent"    :
				$query = "SELECT $fields FROM notes $join where sender=".(int)$options["user_id"]." AND delstatus!=1 $q ORDER BY $order";
				$query_count = "SELECT COUNT(*) FROM notes where sender=".(int)$options["user_id"]." AND delstatus!=1 $q";
				$showasold = 1;
				break;
			case "show"    :
				$query = "SELECT $fields FROM notes $join where sender=".(int)$options["user_id"]." AND is_read=0 AND delstatus!=1 $q ORDER BY $order";
				$query_count = "SELECT COUNT(*) FROM notes where sender=".(int)$options["user_id"]." AND is_read=0 AND delstatus!=1 $q";
				$showasold = 1;
				break;
			case "all" :
			...
			case "current" :
			default        :
				$query = "SELECT $fields FROM notes $join WHERE $q_userid AND (is_done IS NULL OR is_done = 0) AND delstatus!=2 $q ORDER BY $order";
				$query_count = "SELECT COUNT(*) FROM notes WHERE $q_userid AND (is_done IS NULL OR is_done = 0) AND delstatus!=2 $q";
				break;
		}
	...
	public function store2db($note) {
		...
		if (!$note["id"]) {
			foreach ($users as $rcpt) {
				...
				$sql  = "INSERT INTO notes (is_support, timestamp, extra_receipients, subject, body, sender, is_read, is_done, address_id, project_id, user_id) VALUES (";
				$sql .= sprintf("%d, %d, '%s', '%s', '%s'", $note["is_support"], mktime(), implode(",", $extra), $note["subject"], $note["body"]);
				$sql .= sprintf(",%d, %d, %d, %d, %d, %d", $note["from"], $read, $done, $note["address_id"], $note["project_id"], $rcpt);
				$sql .= ")";
				$res = sql_query($sql);
