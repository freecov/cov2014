		...
		if (!array_key_exists($k,$items_covide)) {
			...
			*/
			if ($row_chk["counter"] > 0) {
				/* send note about it */
				$note_subject = gettext("agenda conflict");
				$note_body    = gettext("Er is op de PDA een conflict in de agenda aangemaakt.")."\n".gettext("Pas dit zo spoedig mogelijk aan voor een correcte werking van Covide.")."\n\n";
				$note_body   .= gettext("Agenda punt dat dit conflict veroorzaakt").":\n";
				$note_body   .= gettext("begin").": ".date("d-m-Y H:i", ($data["start"]+$sync["gmt_offset"]))."\n";
				$note_body   .= gettext("eind"). ": ".date("d-m-Y H:i", ($data["end"]+$sync["gmt_offset"]))."\n";
				$note_body   .= gettext("onderwerp").": ".addslashes($data["subject"]);
