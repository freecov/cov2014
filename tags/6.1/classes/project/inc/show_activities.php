...
$output->layout_page();
	...
	$venster = new Layout_venster(array("title" => gettext("manage activiteiten")));
	$venster->addMenuItem(gettext("terug"), "index.php?mod=project");
	...
	$venster->addVensterData();
		...
		foreach ($activities as $activity) {
			$table->addTableRow();
				...
				$table->endTableData();
				$table->addTableData();
					$table->insertAction("save", gettext("opslaan"), "javascript: save_activity(".$activity["id"].");");
					$table->insertAction("delete", gettext("verwijderen"), "javascript: remove_activity(".$activity["id"].");");
					$table->endTag("form");
				$table->endTableData();
			$table->endTableRow();
		}
	...
	$venster = new Layout_venster(array(
		"title" => gettext("activiteiten toevoegen")
	));
	$venster->addVensterData();
		$table = new Layout_table();
		$table->addTableRow();
			...
			$table->endTableData();
			$table->addTableData();
				$table->insertAction("save", gettext("opslaan"), "javascript: save_activity(0);");
				$table->endTag("form");
			$table->endTableData();
		$table->endTableRow();
		$table->endTable();
