...
Class Templates_data {
	...
	public function templateSave() {
		...
		$fields["sender"]              = array("s",$tpl["sender"]);
	...
	public function getTemplateBySearch($address_id="", $start=0, $nolimit=0) {
		$data = array();
		...
		if ($address_id) {
			$q  = sprintf("select * from templates where address_id = %1\$d OR ids %2\$s '(^|\\\\|)%1\$d(\\\\||$)' order by description", $address_id, $regex);
			$qc = sprintf("select count(*) from templates where address_id = %1\$d OR ids %2\$s '(^|\\\\|)%1\$d(\\\\||$)'", $address_id, $regex);
		} else {
			$q  = sprintf("select * from templates order by address_id, description");
			$qc = sprintf("select count(*) from templates");
		}
		if ($nolimit) {
			$res = sql_query($q);
		} else {
