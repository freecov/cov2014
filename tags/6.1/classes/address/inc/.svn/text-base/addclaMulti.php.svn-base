...
$options = unserialize(stripslashes($searchoptions));
if (!is_array($options)) {
...
$output->layout_page("", 1);
	...
	/* window widget */
	$venster = new Layout_venster(array("title" => gettext("classificaties")));
	$venster->addVensterData();
		...
		$table->addTableRow();
			...
			$table->addTableData("", "data");
				$table->insertAction("cancel", gettext("terug"), "javascript: window.close();");
				$table->addTag("span", array("id"=>"action_save", "style"=>"visibility: hidden;"));
					$table->insertAction("save", gettext("opslaan"), "javascript: document.getElementById('addclamulti').submit();");
				$table->endTag("span");
			$table->endTableData();
		$table->endTableRow();
