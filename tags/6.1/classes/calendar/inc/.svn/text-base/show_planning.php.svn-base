...
$output->layout_page();
	...
	/* window widget */
	$venster = new Layout_venster(array("title" => gettext("planning")));
	$venster->addVensterData();
		...
		$table_al->addTableRow(array("style" => "vertical-align: top;"));
			$table_al->addTableData();
				...
				$table->addTableRow();
					...
					$table->insertTableData(gettext("gebruikers"), "", "header");
					$table->insertTableData(gettext("tijden"), "", "header");
					$table->insertTableData("&nbsp;", "", "header");
				$table->endTableRow();
				$table->addTableRow();
					...
					$table->addTableData("", "data");
						...
						$table->addCode( $useroutput->user_selection("users", implode(",", $users), 1, 1, 0, 1, 1) );
					$table->endTableData();
					...
					$table->addTableData(array("style" => "vertical-align: bottom;"), "data");
						$table->insertAction("ok", gettext("zoeken"), "javascript: document.getElementById('planning').submit();");
					$table->endTableData();
				$table->endTableRow();
				$table->addTableRow();
					$table->addTableData("", "data");
						$table->insertAction("back", gettext("vorige week"), "javascript: change_week(-7);");
					$table->endTableData();
					...
					$table->endTableData();
					$table->addTableData("", "data");
						$table->insertAction("forward", gettext("volgende week"), "javascript: change_week(+7);");
					$table->endTableData();
				$table->endTableRow();
				$table->endTable();
		...
		for ($i=1;$i<6;$i++) {
			...
			foreach ($items_arr[$_users[0]][$i] as $k=>$v) {
				...
				$table->addTableRow();
					if ($k % 60 == 0) {
						$datestring = date("H:i", ($weekstart+(24*$i*60*60)+($k*60)));
						$table->insertTableData($datestring, array("rowspan" => 4), "data");
					}
					foreach ($_users as $user_id) {
						$table->addTableData("", "data");
							if ($calendar_items[$user_id][$i][$items_arr[$user_id][$i][$k]]["show_actions"]) {
								if ($items_arr[$user_id][$i][$k])
									$act = "calendaritem_edit(".$calendar_items[$user_id][$i][$items_arr[$user_id][$i][$k]]["id"].", $user_id);";
								else
									$act = "calendaritem_edit(0, $user_id, ".($weekstart+(24*$i*60*60)+($k*60)).");";
							} else {
								$act = "void(0);";
							}
