...
//normal appointments
$q = "select id,timestamp_start AS begin,timestamp_end AS eind,is_private AS prive from calendar where ((timestamp_start >= $start AND timestamp_end <= $eind) AND (user_id = $_user)) AND (is_repeat=0 OR is_repeat IS NULL) ";
$res = sql_query($q);
...
$q = "select subject,description,id,timestamp_start AS begin,timestamp_end AS eind,is_private AS prive,is_repeat AS ".$escape."repeat".$escape.",repeat_type, user_id from calendar where (user_id = $_user AND repeat_type = 'D' $qs)";
$res = sql_query($q);
while ($row = sql_fetch_array($res)) {
