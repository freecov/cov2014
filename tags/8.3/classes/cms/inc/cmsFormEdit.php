 ...
 */
	...
	$cla = $cla_data->getClassifications("", "", "", 1);
	...
	$cms_data->meta_field_types["crm"]      = gettext("crm");
	...
	unset($cms_data->meta_field_types["shop"]);
		...
		$tbl->addTableRow();
			...
			$tbl->addTableData("", "data");
				...
				$tbl->insertTag("li", gettext("With a field of type address block, the field subject, recipient, etc is referred to is the relations email address."));
				$tbl->endTag("ul");
				$tbl->endTag("i");
		...
		$tbl->addTableRow();
			...
			$tbl->addTableData("", "data");
				...
				$tbl->addSpace();
				$tbl->addCode(gettext("this is the recipient email address"));
				$tbl->addTag("br");
				/* is_from */
