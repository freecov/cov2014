 ...
 */
	...
	foreach ($hx as $v) {
		if (preg_match("/^X-ontvanger: /si",$v)) {
			$data["x_recipient"] = trim( preg_replace("/^X-ontvanger: /si","",$v) );
		}
	}
	foreach ($hx as $v) {
	...
	if ($xontvanger) {
		$data["classifications_sent"] = implode("\n", array($data["x_recipient"], $data["x_classification"], $data["x_classifications"], $data["x_negative_classifications"]) );
	}
	//end classifications
