...
if (!$GLOBALS["covide"]->license["disable_basics"]) {
	$venster_calendar = new Layout_venster(array(
		"title" => gettext("calendar")
	));
	$venster_calendar->addVensterData();
		$table = new Layout_table(array("cellspacing"=>1));
			$table->addTableRow();
		...
		$table->endTable();
	$venster_calendar->addCode($table->generate_output());
	$venster_calendar->endVensterData();
}

if ($adminmode || !$GLOBALS["covide"]->disable_basics) {
	/* address settings */
	$venster_address = new Layout_venster(array(
		"title" => gettext("address")
	));
	$venster_address->addVensterData();
		$table = new Layout_table(array("cellspacing"=>1));
		if ($adminmode) {
			$table->addTableRow();
		...
		$table->endTable();
	$venster_address->addCode($table->generate_output());
	$venster_address->endVensterData();
}
...
$venster_access->addVensterData();
	...
	*/
	...
	//XXX: mvb: wtf ? why is this hidden ?
	$table->addTableRow(array("style" => "display: none;"));
		$table->addTableData("", "data");
	...
	*/
	...
	if ($editperms["xs_usermanage"])
		...
		$venster_access->addCode(gettext("No permission to access permissions"));
	unset($table);
$venster_access->endVensterData();
if ($GLOBALS["covide"]->license["has_cms"]) {
	/* window with cms access */
	$venster_cmsaccess = new Layout_venster(array("title" => "CMS"));
	$venster_cmsaccess->addVensterData();
	...
	$table->endTableRow();
	$table->endTable();
	$venster_cmsaccess->addCode($table->generate_output());
	$venster_cmsaccess->endVensterData();
}
...
/* google settings */
$venster_google = new Layout_venster(array(
			"title" => gettext("google")
			));
$venster_google->addVensterData();
$table = new Layout_table(array("cellspacing"=>1));
$table->addTableRow();
...
$table->endTableRow();
$table->endTable();
$venster_google->addCode($table->generate_output());
$venster_google->endVensterData();

/* dimdim settings */
$venster_dimdim = new Layout_venster(array(
			"title" => gettext("dimdim")
			));
$venster_dimdim->addVensterData();
$table = new Layout_table(array("cellspacing"=>1));
$table->addTableRow();
...
$table->endTableRow();
$table->endTable();
$venster_dimdim->addCode($table->generate_output());
$venster_dimdim->endVensterData();
...
if ($venster_address) {
	$table->addCode($venster_address->generate_output());
	unset($venster_address);
}

if ($venster_calendar) {
	$table->addCode($venster_calendar->generate_output());
	unset($venster_calendar);
}
...
if ($adminmode) {
	...
	if ($GLOBALS["covide"]->license["has_cms"]) {
		$table->addCode($venster_cmsaccess->generate_output());
		unset($venster_cmsaccess);
	}
			$table->endTableData();
			$table->addTableData(array("style" => "vertical-align: top;"));
}
		...
		$table->addCode($venster_google->generate_output());
		unset($venster_google);
		/* window with additional dimdim settings */
		$table->addCode($venster_dimdim->generate_output());
		unset($venster_dimdim);
	$table->endTableData();
$table->endTableRow();
$table->endTable();
