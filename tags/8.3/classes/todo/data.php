...
Class Todo_data {
	 ...
	 * @return array The todo information
	 */
	public function getTodosByUserId($user_id, $done=0, $today=0) {
		...
		$sql = sprintf("SELECT * FROM todo WHERE is_done=%d AND user_id=%d %s ORDER BY status ASC, priority ASC, timestamp_end ASC, timestamp ASC", $done, $user_id, $dateToday);
		$res = sql_query($sql);
		$todolist = array();
	...
	public function delete_todo($todo_id, $no_redir=0) {
		...
		if ($todo_id) {
			$sql = sprintf("DELETE FROM todo WHERE id=%d AND user_id=%d", $todo_id, $_SESSION["user_id"]);
			$res = sql_query($sql);
	...
	public function getTodosByProjectId($project_id) {
		...
		$res = sql_query($sql);
		while ($row = sql_fetch_assoc($res)) {
			$row["human_date_from"] = date("d-m-Y", $row["timestamp"]);
			$row["human_date_to"] = date("d-m-Y", $row["timestamp_end"]);
