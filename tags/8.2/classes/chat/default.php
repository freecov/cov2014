					...
					$output->start_javascript();
						$output->addCode("
							function init_chat(ident) {
								//remove record
								opener.handleVoipAction(ident, 0);

								//open popup
								opener.popup('?mod=chat&ident=' + ident, 'chat_".$window_id."', 710, 670, 1);

								var iv = setTimeout('window.close()', 2000);
							}
					...
					while ($row = sql_fetch_assoc($res)) {
						...
						switch ($row["invite"]) {
							case 0:
								...
								$row["decline"] = sprintf("javascript: opener.handleVoipAction('%s', 0); close_window();", $row["ident"]);
								break;
							case 1:
								// current user has received invitation
								$row["accept"]  = sprintf("javascript: opener.handleVoipAction('%s', 1); close_window();", $row["ident"]);
								$row["decline"] = sprintf("javascript: opener.handleVoipAction('%s', 0); close_window();", $row["ident"]);								break;
							case 2:
