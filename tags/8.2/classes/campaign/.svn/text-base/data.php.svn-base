...
Class Campaign_data {
	...
	public function getCampaignsBySearch($options) {
		...
		while ($row = sql_fetch_assoc($res)) {
			...
			if ($row["type"] == 2) {
				$row["is_callscript"] = 1;
				$q = sprintf("SELECT count(*) FROM campaign_records WHERE answer = 5 AND campaign_id = %d AND (call_again <= %d AND call_again > 0)", $row["id"], mktime());
				$r = sql_query($q);
				if (sql_result($r, 0)) {
					$row["is_recallable"] = 1;
				} else {
	...
	public function saveCampaign($req) {
		if (!$req["id"]) {
			...
			$output->start_javascript();
				$output->addCode("opener.location.href='?mod=campaign'; window.close();");
	...
	public function getCampaignRecordsBySearch($options) {
		...
		while ($row = sql_fetch_assoc($res)) {
			...
			/* every other 10 rows, add the data to maintain a good overview */
			$n++;
			if ($n % 11 == 0) {
	...
	public function save_edit_record($id, $options, $callscript=0, $calltime=0) {
		if (count($options)) {
			...
			/* No option is selected. Default action will be "not interested" */
			$keys = 1;
		}
		/* Default settings for every option */
		$q = sprintf("update campaign_records set is_called = %d, answer = %d, user_id = %d, call_again = 0 where id = %d",
		...
		if ($keys == 2) {
			...
			$address_id = $record_data["address_id"];
			
			$q_get = sprintf("SELECT classification FROM address_info WHERE address_id = %d", $address_id);
			$res_get = sql_query($q_get);
			$current_cla = sql_result($res_get, 0);
			$new_cla = preg_replace("/\|{1,}/si", "|", $current_cla."|".$cla_id."|");
			$q = sprintf("update address_info set classification = '%s' where address_id = %d",
				$new_cla, $address_id);
			sql_query($q);
		}
		...
		$output->start_javascript();
			if ($callscript)
				if ($callscript == 1)
					$output->addCode(sprintf("opener.document.getElementById('velden').submit(); document.location.href='?mod=campaign&action=callscript&id=%d'", $campaign));
				elseif ($callscript == 2)
					$output->addCode(sprintf("opener.document.getElementById('velden').submit(); document.location.href='?mod=campaign&action=recallscript&id=%d'", $campaign));
				else
					$output->addCode("opener.document.getElementById('velden').submit(); window.close();");
			else
				$output->addCode("opener.document.getElementById('velden').submit(); window.close();");
		$output->end_javascript();
		$output->exit_buffer();
	...
	public function callscript($id) {
		...
		if ($record) {
			...
			$output->addCode("alert(gettext('Nothing to do, all called!')); window.close();");
		}
		$output->end_javascript();
	...
	public function recallscript($id) {
		/* get current record of this user */
		$q = sprintf("select id from campaign_records where campaign_id = %d and answer = 5 AND (call_again <= %d AND call_again > 0)",
			$id, mktime());
		$res = sql_query($q);
		if (sql_num_rows($res) > 0) {
			/* get current record for this user */
