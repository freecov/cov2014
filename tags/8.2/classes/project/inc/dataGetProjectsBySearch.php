<?php
	...
	if ($options["address_id"]) {
		...
		$sql = sprintf("SELECT id,name,description,is_active,manager,executor FROM project WHERE address_id=%1\$d OR multirel %2\$s ORDER BY UPPER(name)", $options["address_id"], $regexmultirel);
		$res = sql_query($sql);
		$sorted_arr = array();
		...
		if ($options["searchkey"]) {
			...
			$query_main = "select * from projects_master WHERE is_active = 1 AND (1=1 ".$sq.") OR id IN (select group_id from project WHERE is_active > 0 AND group_id > 0 ".$sq.") order by lower(name)";
		} else {
			$query_sub  = "select * from project WHERE is_active = 1 AND group_id = 0 ".$sq." ORDER BY lower(name)";
			$query_sub_count  = "select COUNT(*) from project WHERE is_active = 1 AND group_id = 0 ".$sq;
			$query_main = "select * from projects_master WHERE is_active = 1 AND (1=1 ".$sq.") OR id IN (select group_id from project WHERE group_id > 0 ".$sq.") order by lower(name)";
			$query_main_count = "select COUNT(*) from projects_master WHERE is_active = 1 AND (1=1 ".$sq.") OR id IN (select group_id from project WHERE group_id > 0 ".$sq.") order by lower(name)";
		}
		$pr = array();
		$_use_hour_reg = 1;
		$projects      = array();
		...
		while ($row = sql_fetch_assoc($res)) {
			...
			if (!is_array($hours) || $hours[$row["id"]] > $lfact_time) {
				$pr[$i]["id"]           = $row["id"];
				$pr[$i]["master"]       = 0;
				$pr[$i]["nonmaster"]    = 1;
