		...
		if ($_REQUEST["export_xml"]) {
			...
			if (!is_array($data["data"])) {
				...
				foreach ($data["data"] as $dat) {
					$string .= "<item>\n";
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", gettext("salesitem"), htmlentities($dat["subject"]));
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", gettext("contact"), htmlentities($dat["companyname"]));
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", gettext("project"), htmlentities($dat["h_project"]));
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", gettext("prospect"), $dat["h_timestamp_prospect"]);
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", gettext("quote"), $dat["h_timestamp_proposal"]);
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", gettext("order/commission"), $dat["h_timestamp_order"]);
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", gettext("invoice"), $dat["h_timestamp_invoice"]);
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", gettext("user"), htmlentities($dat["username"]));
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", gettext("score"), $dat["expected_score"]);
					$string .= sprintf("<%1\$s>%2\$s</%1\$s>\n", gettext("price"), $dat["orig_sum"]);
					$string .= "</item>\n";
				}
			}
			$string .= "</export>\n";
