...
class Layout_table Extends Layout_output {
	 ...
	 */
	public function __construct($settings="", $border=0, $auto_layout=0) {
		$this->_auto_layout = (int)$auto_layout;
	...
	public function addTableBody($settings="", $no_mobile=0) {
		if ($GLOBALS["covide"]->mobile && $no_mobile) {
			$this->block_output = 1;
		}
		$this->addTag("tbody", $settings);
	}
	public function endTableBody($settings="", $no_mobile=0) {
		if ($GLOBALS["covide"]->mobile && $no_mobile) {
			$this->block_output = 1;
		}
		$this->endTag("tbody", $settings);
	}
	/* addTableHeader {{{ */
	/**
	...
	public function addTableHeader($settings="") {
		if ($this->_auto_layout) {
			$settings["class"].= " list_header";
