<?php
	...
	Class Calendar_data {
		 ...
		 */
		public function delete($id, $ignore_groupid=0) {
			/* FIXME: change function name, it is a keyword in PHP */
			/* check if item is group calendar */
			/*
			if (!$ignore_groupid) {
				$sql = sprintf("SELECT group_id FROM calendar WHERE id = %d", $id);
				$res = sql_query($sql);
				$group_id = sql_result($res, 0);
			}
			*/
			...
			if ((int)$group_id) {
				if ($GLOBALS["covide"]->license["has_funambol"]) {
					...
					/* fetch calendar item ids */
					$q = sprintf("SELECT id FROM calendar WHERE group_id = %d", $group_id);
					$r = sql_query($q);
					while ($row = sql_fetch_assoc($r)) {
						$funambol_data->removeRecord("calendar", $row["id"]);
					}
				}
				$sql = sprintf("DELETE FROM calendar WHERE group_id = %d", $group_id);
			} else {
				if ($GLOBALS["covide"]->license["has_funambol"]) {
					$funambol_data = new Funambol_data();
		...
		public function getActivityNames() {
			...
			$sql = "SELECT id, activity, department_id FROM hours_activities ORDER BY UPPER(activity)";
			$res = sql_query($sql);
			while ($row = sql_fetch_assoc($res)) {
		...
		public function xml_check() {
			...
			WHERE
				...
				AND calendar_user.user_id IN (%s)
				AND
				(
					(
						(
							calendar.timestamp_start BETWEEN $tstamp_from AND $tstamp_to1
						) OR (
							calendar.timestamp_end BETWEEN $tstamp_from1 AND $tstamp_to
						)
					) OR (
						calendar.timestamp_start <= $tstamp_from AND timestamp_end >= $tstamp_to
					)
				)", $users_to_check);
			if ($_REQUEST["id"]) {
				$sql .= sprintf("AND calendar.id != %d", $_REQUEST["id"]);
				if ($_REQUEST["group_id"]) {
					$sql .= sprintf(" AND group_id != %d", $_REQUEST["group_id"]);
				}
			}
			$res = sql_query($sql);
		...
		public function getKmItems($options) {
			...
			foreach ($options["users"] as $user) {
				...
				/* construct sql */
				$sql  = "SELECT calendar.timestamp_start, calendar.subject, calendar.body as description, calendar.location, calendar.kilometers, calendar.deckm, calendar_user.user_id, calendar_user.status FROM calendar,calendar_user ";
		...
		public function query_items($user_id, $repeating, $timestamp_start, $timestamp_end) {
			 ...
			 * $license is used to check funambol and project availability
			 * $address_data is used to fetch the company names of the linked relations
			 * $project_data is used to fetch the project names of the linked projects if project is in license
			 * $user_data is used to fetch the user names of the linked users and groups
			 * $counter is used to give each item a unique position in the array
			 * $result will be the array we are going ot return
			 */
			$license      = $GLOBALS["covide"]->license;
			...
			if ($repeating) {
				...
				$timestamp_start);
				$repeatcolumns = ",calendar_repeats.repeat_type, calendar_repeats.timestamp_end as repeat_end, calendar_repeats.repeat_frequency, calendar_repeats.repeat_days";
			...
			//construct query
			$q = sprintf("SELECT
				calendar.id, calendar.timestamp_start, calendar.timestamp_end,
				calendar.alldayevent, calendar.is_private,
