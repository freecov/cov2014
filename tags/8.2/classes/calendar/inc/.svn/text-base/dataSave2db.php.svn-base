...
*/
	...
	if ($appointment["id"]) {
		...
		foreach ($user_arr as $user) {
			...
			//if not, add it
			if (!$count_user) {
				$sql = sprintf("INSERT INTO calendar_user (calendar_id, user_id, status) VALUES (%d, %d, 1)", $appointment["id"], $user);
				$res = sql_query($sql);
			}
		}
		...
		if ($appointment["is_repeat"]) {
			...
			if ($count) {
				...
				$q = sprintf("UPDATE calendar_repeats SET repeat_type = %d, timestamp_end = %d, repeat_frequency = %d, repeat_days = '%s' WHERE calendar_id = %d",
					$appointment["id"], $appointment_repeat["repeat_type"], $appointment_repeat["repeat_end"], 1, $appointment_repeat["repeat_days"], $appointment["id"]);
			} else {
				//insert
		...
		foreach ($user_arr as $user) {
			$q = sprintf("INSERT INTO calendar_user (calendar_id, user_id, status) VALUES (%d, %d, 1)", $appointmentid, $user);
			$res = sql_query($q);
		}
	...
	if ($appointment["campaign_id"]) {
		...
		$output = new Layout_output();
			$output->start_javascript();
				$output->addCode("
					opener.document.getElementById('options3').checked = true;
...
if (is_array($data["relmail"])) {
	...
	if ($user_body) {
		...
		$mail_body .= "---\n";
		if (count($appointment["from_day"]) > 1) {
