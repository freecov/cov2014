		...
		public function getNewsletterData($settings) {
			...
			if (is_array($settings["classifications"])) {
				...
				if ($addresstype == "users") {
					$query_count = "SELECT count(address_private.id) FROM address_private ". ($addresstype == "users" ? ",users" : ""). " WHERE (address_private.is_public=1 OR addressprivate.user_id=".(int)$_SESSION["user_id"].") ";
