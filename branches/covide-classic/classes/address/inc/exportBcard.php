<?php
	...
	if ($row["givenname"] || $row["surname"] || 1 == 1) {
		...
		$cla_ids = explode("|", $row["classification"]);
		$cla_ids = array_unique($cla_ids);
		foreach ($cla_ids as $k=>$v) {
			if (!$v) {
				unset($cla_ids[$k]);
			}
		}
		$cla_names = array();
		if (count($cla_ids)) {
			$classifications = $classification_data->getClassifications($cla_ids);
			foreach ($classifications as $cla) {
				$cla_names[] = $cla["description"];
			}
		}
		...
		if ($row["pobox"]) {
			$csv[]= gettext("PO box")." ".$row["pobox"];
			$csv[]= "";
		} else {
			$csv[]= $row["address"];
			$csv[]= $row["address2"];
		}
		$csv[]= ($row["pobox_zipcode"]) ? $row["pobox_zipcode"]:$row["zipcode"];
		$csv[]= ($row["pobox_city"])    ? $row["pobox_city"]:$row["city"];
