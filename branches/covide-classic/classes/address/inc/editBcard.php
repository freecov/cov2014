...
$output->layout_page("", 1);
	...
	$venster->addVensterData();
		...
		if ($bcardinfo["rcbc"] != 1) {
			$table->addTableRow();
				...
				$table->addTableData(array("colspan" => 3), "data");
					$table->addCheckBox("bcard[completercbc]", 1);
				$table->endTableData();
			$table->endTableRow();
		}
		$table->endTable();
		...
		$table->addTableRow(array("class" => "expand_business"));
			...
			$table->addTableData("", "data");
			if (!$view_only) {
				$table->addSelectField("bcard[business_country]", $countryArray, $bcardinfo["business_country"]);
			} else {
		...
		$table->addTableRow(array("class" => "expand_private"));
			...
			$table->addTableData("", "data");
			if (!$view_only) {
				$table->addSelectField("bcard[personal_country]", $countryArray, $bcardinfo["personal_country"]);
			} else {
		...
		$table->addTableRow(array("class" => "expand_other"));
			...
			$table->addTableData("", "data");
			if (!$view_only) {
				$table->addSelectField("bcard[other_country]", $countryArray, $bcardinfo["other_country"]);
			} else {
		...
		$venster->addTag("ul", array("class" => "bcard_tab_ul", "style" => "list-style-type: none;"));
			$venster->addTag("li", array("id" => "bcard_li_relation", "class" => "bcard_tab_li selected"));
				$venster->insertLink(gettext("contact"), array("href" => "javascript:toggleDiv('relation');"));
			$venster->endTag("li");
		...
		$venster->addTag('div', array("style" => "float: left;"));
			if ($src_id) {
				$venster->insertAction("back", gettext("back"), "javascript: history.go(-1);");
			} else {
