<?php
	...
	if(!$_REQUEST["print"]) {
		...
		$output->insertAction("print", gettext("Print this page"), "javascript: popup('?mod=calendar&action=km&print=km&start_day=$_REQUEST[start_day]&start_month=$_REQUEST[start_month]&start_year=$_REQUEST[start_year]&end_day=$_REQUEST[end_day]&end_month=$_REQUEST[end_month]&end_year=$_REQUEST[end_year]&users=$_REQUEST[users]&user_autocomplete_users=$_REQUEST[user_autocomplete_users]', 'kmprint', 820, 500, 1)");
		$output->start_javascript();
			$output->addCode(
				"
				function km_search() {
