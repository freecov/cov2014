...
Class Filesys_output {
	...
	/* show_files {{{ */
	public function show_files($id=0, $noactions=0, $max_size=0, $global_search=0) {
	...
	/* show_fileupload {{{ */
	public function show_fileupload($folderid=0, $prefix="") {
		/* if var $prefix if set, some options will be enforced                  */
		/* the filename, NOT the extension, will be forced to the specified name */
		/* no replacement or description options will be available               */
		...
		$table->addTableRow();
			$table->addTableData();
				...
				if ($quota === false || $quota["left"] > 0) {
					...
					$table->insertAction("image_multiple", gettext("image upload"), "javascript: popup('jupload.php?user=".$_SESSION["user_id"]."&map_id=".$_REQUEST["id"]."&mod=".$_REQUEST["mod"]."&image=1', 'multiimage', 0, 0, 1);");
					$table->addTag("br");
					$table->addTag("br");
				}
				...
				$table->addTag("form", array(
					...
					$table->addHiddenField("id", ($_REQUEST["id"]) ? $_REQUEST["id"]:$folderid);
					$table->addHiddenField("action", "fupload");
					$table->addHiddenField("filedata[prefix]", $prefix);
	...
	public function filesCopied($parentaction = "") {
		...
		$output->addCode("var tcx = setTimeout('closepopup();', 1000);");
	...
	public function checkSSL($folder_id, $file_id = 0) {
		...
		if ($GLOBALS["covide"]->sslmode == 1) {
			...
			if (!$_SERVER["HTTPS"] && !$_SERVER["HTTP_X_FORWARDED_PROTOCOL"] == "https") {
				...
				if (!in_array($hp["name"], array("cms", "mijn mappen")) && $folder_id) {
					/* redirect to ssl */
