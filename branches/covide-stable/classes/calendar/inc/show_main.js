function calendaritem_edit(id, user, datemask) {
	...
	popup(url, 'calendaredit', 800, 600, 1);
}

function calendaritem_remove(id, user, is_repeat, timestamp, ask_delete_notification) {
	if (is_repeat) {
		...
		if (confirm(gettext("remove calenderitem")+' ?')) {
			if (ask_delete_notification == 1) {
				if (confirm(gettext("send deletion notification to users ?"))) {
					document.getElementById('notifyuser').value = 'send';
				}
			}
...
function todos_print() {
	...
	popup(url, 'printtodo', 700, 600, 1, 0);
}
