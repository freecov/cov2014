...
$table->addTableRow();
	...
	$table->addTableData("", "data");
		if (!$calendar_item["is_registered"]) {
			...
			$table->insertAction("delete", gettext("delete"), sprintf("javascript: calendaritem_remove(%d, %d, %d, %d, %d);", $calendar_item["id"], $_REQUEST["user_id"], $calendar_item["is_repeat"], $datestamp, ($_REQUEST["user_id"] == $_SESSION["user_id"])?0:1));
