...
function calendaritem_remove(id, user, is_repeat, ask_delete_notification ) {
	if (is_repeat) {
	...
	if (confirm(gettext("remove calendar item?"))) {
		parent.document.getElementById('calendarform').action.value = 'delete';
		parent.document.getElementById('calendarform').id.value = id;
		parent.document.getElementById('calendarform').user_id.value = user;
		if (ask_delete_notification == 1) {
			if (confirm(gettext("send deletion notification to users ?"))) {
				parent.document.getElementById('notifyuser').value = 'send';
			}
		}
		if (is_repeat) {
			parent.document.getElementById('calendarform').appointmentremove_all = remove_all;
		}
...
function makeDimdimMeeting() {
	var el_address = document.getElementById('appointmentaddress_id').value;
	var from_date = document.getElementById('appointmentfrom_year').value;
	from_date += '-' + document.getElementById('appointmentfrom_month').value;
	from_date += '-' + document.getElementById('appointmentfrom_day').value;
	
	var from_hour = document.getElementById('appointmentfrom_hour').value;
	from_hour += '-' + document.getElementById('appointmentfrom_min').value;
	
	var to_hour = document.getElementById('appointmentto_hour').value;
	to_hour += '-' + document.getElementById('appointmentto_minute').value;

	var url = '?mod=dimdim&action=edit_meeting&address_id='+el_address+'&date='+from_date+'&from_hour='+from_hour+'&to_hour='+to_hour;
	popup(url, 'makedimdimmeeting', 650, 500, 1);
}

function deleteDimdim(id) {
	if (confirm(gettext("delete this meeting?"))) {
		if (!id) {
			id = document.getElementById('appointmentdimdim_meeting_id').value;
		}
		var url = '?mod=dimdim&action=delete_meeting&id='+id;
		loadXML(url);
		document.getElementById('appointmentdimdim_meeting_id').value = '';
		document.getElementById('meeting_name').innerHTML = '';
		document.getElementById('dimdim_icon_delete').style.visibility = 'hidden';
		document.getElementById('dimdim_icon').style.visibility = 'visible';
	}
}
