...
if ($id==0) {
	...
	$sql = sprintf("SELECT calendar.*, calendar_user.user_id, calendar_user.status FROM calendar inner join calendar_user on calendar.id = calendar_user.calendar_id WHERE id = %d AND user_id = %d;", $id, $user_id);
	$res = sql_query($sql);
	...
	if (!$access && !$funambol) {
		...
		// attach project name
		$appointment["project_name"]      = $project_data->getProjectNameById($row["project_id"]);
		...
		if ($row["isrecurring"] == 1) {
			$sql = sprintf("SELECT * FROM calendar_repeats WHERE calendar_id = %d LIMIT 1", $id);
			$res = sql_query($sql);
			if (sql_num_rows($res)) {
				$row = sql_fetch_assoc($res);
