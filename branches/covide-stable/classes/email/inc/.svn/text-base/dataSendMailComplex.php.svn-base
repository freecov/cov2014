 ...
 */
	...
	if (!$mdata["is_text"]) {
		...
		//replace %%EMAILADDR%% with rcpt of email
		if (!$_newsletter) {
			$mdata["body_html"] = str_replace("%%EMAILADDRLINK%%", "&email=".$mdata["to"], $mdata["body_html"]);
		}

		//create absolute links including hostname for images that start with "cmsfile/" or "/cmsfile/"
		$mdata["body_html"] = str_replace("src=\"cmsfile/", "src=\"".$GLOBALS["covide"]->webroot."cmsfile/", $mdata["body_html"]);
		$mdata["body_html"] = str_replace("src=\"/cmsfile/", "src=\"".$GLOBALS["covide"]->webroot."cmsfile/", $mdata["body_html"]);
		...
		//replace %%EMAILADDR%% with rcpt of email
		if (!$_newsletter) {
			$mdata["body"] = str_replace("%%EMAILADDRLINK%%", "&email=".$mdata["to"], $mdata["body"]);
		}
	...
	if (!$_newsletter) {
		$part_body["Body"]      = str_replace("%%EMAILADDRLINK%%", "&email=".$mdata["to"], $mdata["body"]);
		$part_body["Body_html"] = str_replace("%%EMAILADDRLINK%%", "&email=".$mdata["to"], $mdata["body_html"]);
	} else {
	...
	if ($_newsletter) {
		...
		$gen = $generate->sendMail2Smtp(&$gen, &$part_body, $hash, $skip_gmail);
	}
...
/*
	// If we are using imap, put the mail in the sent items there.
	// For google, we use [Gmail].Sent\ Items. For all others, you have to configure it. If no configuration found we will try Sent. If not present, we will try to create this folder.
	if ($user_info["mail_imap"] == 1 || $user_info["mail_imap"] == 2) {
		// using imap
		if ($user_info["mail_server"] == "imap.gmail.com") {
			$sent_items = "[Gmail]/Sent Items";
			//use the mailconnect stuff from mail_retrieve etc ;)
			$mail_retrieve = new Email_retrieve($_SESSION["user_id"]);
			$user = array();
			$user["mail_deltime"]  = $user_info["mail_deltime"];
			$user["mail_server"]   = $user_info["mail_server"];
			$user["mail_imap"]     = $user_info["mail_imap"];
			$user["mail_username"] = $user_info["mail_user_id"];
			$user["mail_password"] = $user_info["mail_password"];
			$user["outgoing"] = 1;
			$mail_retrieve->mailConnect($user);
			imap_append($mail_retrieve->mbox, str_replace("INBOX", "[Gmail]/Sent Mail", $mail_retrieve->connstr), $complete_mail, "\\Seen");
			imap_close($mail_retrieve->mbox);
			unset($mail_retrieve);
		}
	}
*/
		...
		$output->layout_page(gettext("Email"), 1);
			$output->start_javascript();
				$output->addCode("
					...
					el.document.getElementById('options4').checked = true;
