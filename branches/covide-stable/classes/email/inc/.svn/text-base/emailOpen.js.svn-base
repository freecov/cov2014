...
function deleteProject(id) {
	...
	loadXML('index.php?mod=email&action=delete_project_xml&id=' + mail_id + '&project='+id);
}
...
function nav_displayMail(mailId, folderId, addressId, projectId, redirect) {
	if (mailId == -1) {
		alert(gettext('No more e-mails to display'));
		if (redirect) {
			window.location = "?mod=email&action=list&address_id="+addressId+"&folder_id="+folderId+"&project_id="+projectId;
		}
	} else {
		window.location = "?mod=email&action=open&address_id="+addressId+"&id="+mailId+"&folder_id="+folderId+"&project_id="+projectId;
	}
}

function nav_archiveDispMail(mailId, archiveFolder) {
	var mail_id = document.getElementById('id').value;
	var descr = escape(document.getElementById('maildescription').value);

	loadXML('index.php?mod=email&action=change_folder_xml&id=' + mail_id + '&folder=' + archiveFolder + '&description=' + descr + 'no_redirect=1');
}