...
class CMS_data_createinlinethumb {

static private $_filesys_data;
static public function createInlineThumb($id, $width, $height) {

	if (!self::$_filesys_data) {
		self::$_filesys_data = new Filesys_data();
	}
	// object is always referencing in php5
	$filesys_data = self::$_filesys_data;
	...
	if ($img === FALSE) {
		$q = sprintf("insert into cms_image_cache (img_id, datetime, width, height, use_original) values (
			%d, %d, %d, %d, 1)", $id, time(), $width, $height);
		sql_query($q);
