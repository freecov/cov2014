<?php
	...
	$this->addCode(sprintf("var user_ip = '%s';\n", $GLOBALS["covide"]->user_ip));
	$this->end_javascript();
	$this->addCode("\n");
	$this->css(-1);
	...
	$this->endTag("head");
	$this->addCode("\n");
	$bodyId = "covidedashboard";
	$bodyOnUnload = "clearOverlays()";
	if (isset($_REQUEST['overlay'])) {
		$bodyId .= $_REQUEST['overlay'];
		$bodyOnUnload = "";
	}
	...
	$this->addTag("div", array("id"=>"infowait",  "class"=>"infolayer infowait"));
		$this->insertImage("misc/loading.gif", "loading", "", 1);
		$this->addTag("br");
		$this->addCode(gettext("Loading... please wait..."));
	$this->endTag("div");
	$this->addCode("\n");
	...
	if ($this->_hide_navigation) {
		...
		$this->addTag("div", array("id" => "headerwrap"));
			...
			$this->addTag("div", array("id" => "header"));
				...
				if ($_SESSION["user_id"]) {
					if (trim($GLOBALS["covide"]->license["alt_logo"])) {
						$this->addTag("a", array("href" => "?mod=desktop"));
						$this->addTag("img", array("src" => "?mod=desktop&action=showlogo", "alt" => "Covide", "title" => "Covide", "border" => 0, "class" => "imagebutton"));
						$this->endTag("a");
					} else {
					...
					$this->addCode("
						$(document).ready(function() {
							...
							$('body').bind('mouseenter',function(){
								titl = $(document).attr('title');
								postitl = titl.indexOf('*');
								if (postitl == '0' ) {
									$(document).attr('title', titl.replace('*', '') );
								}
							});
						});
						function loadAlerts() {
				...
				$this->endTag("div");
				$this->load_javascript(self::include_dir."search.js");
		if ($_SESSION["user_id"]) {
