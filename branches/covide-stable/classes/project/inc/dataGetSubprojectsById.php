...
$users_join = " LEFT JOIN users AS u1 ON project.manager = u1.id LEFT JOIN users AS u2 ON project.executor = u2.id ";
$sql = sprintf("SELECT project.*, u1.username as manager_name, u2.username as executor_name FROM project %s %s %s WHERE group_id=%d %s %s ORDER BY %s", $leftjoin, $users_join, $extjoin, $projectid, $sq, $extcondition, $ssq);
...
while ($row = sql_fetch_assoc($res)) {
	...
	$row["executor_name"]= $row["executor_name"];
	...
	$sql_reg = sprintf("SELECT * FROM hours_registration WHERE project_id=%d AND timestamp_start IS NULL AND hours IS NOT NULL", $row["id"]);
	$res_reg = sql_query($sql_reg);
	while ($row_reg = sql_fetch_assoc($res_reg)) {
		if ($row_reg["is_billable"] == 1) {
			$row["billable_hours"] += ($row_reg["hours"]*3600);
			if ($row["tarif"]) {
				$row["total_costs"] += $row_reg["tarif"];
			} else {
				$row["total_costs"] += ($row_reg["hours"]*$actcosts[$row_reg["activity_id"]]);
			}
		} else {
			$row["service_hours"] += $row_reg["hours"];
		}
	}
	$sql_reg = sprintf("SELECT * FROM hours_registration WHERE project_id=%d AND price IS NOT NULL AND hours IS NULL AND timestamp_start IS NULL", $row["id"]);
	$res_reg = sql_query($sql_reg);
	while ($row_reg = sql_fetch_assoc($res_reg)) {
		if ($row_reg["is_billable"] == 1) {
			$row["total_costs"] += $row_reg["price"];
		}
	}
	...
	$row["has_hours"]    = $this->hasHours($row["id"], $row["lfact"]);
	if ($row["allow_xs"]) {
		$projectinfo[] = $row;
	}
	unset($row);
