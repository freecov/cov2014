<?php
	...
	$resmain = sql_query($q) or die($q);
	$rowmain = sql_fetch_array($resmain);
	...
	$output->addTag("base", array(
		"href" => "http://".$_SERVER["HTTP_HOST"].$this->base_dir
	));
	$output->addCode("\n");
	...
	$output->addCode("\n");
	if ($pageid != $this->default_page) {
		$uri["rss"] = "http://".$_SERVER["HTTP_HOST"]."/rss/live/".$this->pageid;
	} else {
		$uri["rss"] = "http://".$_SERVER["HTTP_HOST"]."/rss";
	}
	$uri["txt"] = "http://".$_SERVER["HTTP_HOST"]."/text/".$this->checkAlias($this->pageid);
	$uri["def"] = "http://".$_SERVER["HTTP_HOST"]."/page/".$this->checkAlias($this->pageid);
