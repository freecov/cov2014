...
$venster->addVensterData();
	...
	$table->addTableRow();
		if ($this->has_declaration) {
			...
			$table->insertTableData(gettext("number")."/".gettext("name"), "", "header");
		}
	...
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			...
			if($projectid && !$GLOBALS["covide"]->license["has_project_ext"] && ($userinfo["xs_usermanage"] || $userinfo["xs_limitusermanage"])) {
				$table->addSpace(2);
