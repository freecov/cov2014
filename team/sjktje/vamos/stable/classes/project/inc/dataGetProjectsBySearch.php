<?php
	...
	if ($options["address_id"]) {
		...
		if ($options["searchkey"]) {
			...
			/* master projects */
			$query_main = sprintf("select * from projects_master WHERE (
				(name $like_syntax '%%%1\$s%%' OR description $like_syntax '%%%1\$s%%'))
				OR (id IN (select id from project WHERE group_id > 0 ".$sq.")
			...
			$query_sub_master = sprintf("select * from project WHERE group_id != 0 AND (name $like_syntax '%%%1\$s%%' OR description $like_syntax '%%%1\$s%%') GROUP BY group_id", $options["searchkey"]);
			#echo $query_sub;
