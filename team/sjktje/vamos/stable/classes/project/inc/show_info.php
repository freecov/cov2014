...
if ($projectinfo[0]["allow_edit"]) {
	$venster->addMenuItem(gettext("change"), "javascript: popup('index.php?mod=project&action=edit&master=$master&id=$projectid')");
}
if (!$master) {
	$venster->addMenuItem(gettext("overview"), "index.php?mod=project&action=showhours&id=$projectid");
}
...
$venster->addVensterData();
	...
	$view->addMapping(gettext("manager"), "%manager_name");
...
if ($master) {
	...
	$venster->addVensterData();
		...
		$view->addMapping(gettext("executor"), "%manager_name");
		$view->addMapping(gettext("active"), "%%complex_active");
