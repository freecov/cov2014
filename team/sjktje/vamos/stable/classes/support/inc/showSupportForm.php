<?php
	...
	$tbl->addTableRow();
		$tbl->addTableData(array("style" => "vertical-align: top"));
			$tbl->addCode(gettext("type of supportcall"));
		$tbl->endTableData();
		$tbl->addTableData();
			$tbl->addRadioField("support[type]", gettext("contact me")." (".gettext("phone").")", 1, 1);
			$tbl->addTag("br");
			$tbl->addRadioField("support[type]", gettext("question")."/".gettext("information request"), 2, 1);
			$tbl->addTag("br");
			$tbl->addRadioField("support[type]", gettext("complaint/incident"), 3, 1);
			$tbl->addTag("br");
			$tbl->addTag("br");
			$tbl->endTableData();
	$tbl->endTableRow();
