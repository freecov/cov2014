...
if ($multiuser)
	...
	$venster->addMenuItem(gettext("monthly overview"), "./?mod=calendar&action=monthview");
...
$venster->addVensterData();
						...
						if ($v[$i] && $v[$i] != $_temp_id) {
							$tableitems->addTableData(array("rowspan" => $calendar_items[$k][$v[$i]]["rowspan"], "style" => "font-weight: normal;"), "header");
								if ($calendar_items[$k][$v[$i]]["permissions"]) {
									$tableitems->insertAction("edit", gettext("change:"), "javascript: calendaritem_edit(".$calendar_items[$k][$v[$i]]["id"].", ".$k.", $datemask);");
								}
	...
	$table->addTableRow();
		$table->addTableData(array("colspan"=>2));
			$table->insertAction("print", gettext("print"), "javascript: popup('index.php?mod=calendar&action=print_main&day=$day&month=$month&year=$year', 'printmain', 300, 300, 1);");
		$table->endTableData();
	$table->endTableRow();
	$table->endTable();
