<?php
	...
	Class Calendar_data {
		...
		public function delete($id) {
			...
			$q = sprintf("select user_id, sync_guid, group_id from calendar where id = %d", $id);
			$result = sql_query($q);
			if ($result) {
			...
			if ((int)$rowx["group_id"]) {
				$sql = sprintf("DELETE FROM calendar WHERE group_id = %d", $rowx["group_id"]);
			} else {
