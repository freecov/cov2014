...
$venster_access->addVensterData();
	...
	if ($GLOBALS["covide"]->license["has_funambol"]) {
		$table->addTableRow();
			$table->addTableData("", "data");
				$table->insertImage("icoon_gprs.gif", gettext("funambol sync"));
			$table->endTableData();
			$table->addTableData("", "data");
				$table->addCode(gettext("funambol sync"));
			$table->endTableData();
			$table->addTableData("", "data");
	...
	if ($GLOBALS["covide"]->license["has_cms"]) {
		$table->addTableRow();
			$table->addTableData("", "data");
				$table->insertImage("icoon_uitloggen.gif", gettext("cms access level"));
			$table->endTableData();
			$table->addTableData("", "data");
...
$venster_email->addVensterData();
	...
	/*
	$table->addTableRow();
		$table->insertTableData(gettext("forward mail to"), "", "header");
		$table->addTableData("", "data");
	...
	*/
	...
	/*
	$table->addTableRow();
		$table->insertTableData(gettext("show cc and bcc fields by default"), "", "header");
		$table->addTableData("", "data");
	...
	*/
	...
	$table->endTableRow();
	$table->addTableRow();
		$table->insertTableData(gettext("signature"), "", "header");
		$table->addTableData("", "data");
	...
	$table->addTableRow();
		$table->insertTableData("", "", "header");
		$table->addTableData("", "data");
			$table->insertLink(gettext("more email addresses/signatures"), array("href"=>"index.php?mod=email&action=signatures&user_id=".$_user_id));
		$table->endTableData();
	$table->endTableRow();
	$table->addTableRow();
...
$venster_misc->addVensterData();
	...
	if ($adminmode && $GLOBALS["covide"]->license["has_sync4j"]) {
		$table->addTableRow();
			$table->insertTableData(gettext("Sync4j Calendar SyncSource"), "", "header");
			$table->addTableData("", "data");
				$table->addTextField("user[sync4j_source]", $userinfo["sync4j_source"]);
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("Sync4j Addressbook SyncSource"), "", "header");
			$table->addTableData("", "data");
				$table->addTextField("user[sync4j_source_adres]", $userinfo["sync4j_source_adres"]);
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("Sync4j to-do SyncSource"), "", "header");
			$table->addTableData("", "data");
				$table->addTextField("user[sync4j_source_todo]", $userinfo["sync4j_source_todo"]);
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("Sync4j userpath"), "", "header");
			$table->addTableData("", "data");
