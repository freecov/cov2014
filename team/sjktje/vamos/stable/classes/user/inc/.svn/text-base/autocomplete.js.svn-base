...
/* caller function */
function user_select_popup(id, multiple, showInActive, showEmpty, showArchiveUser, no_empty, showGroups, confirmation) {
	user_complete_initial = 0;
	var str = 'index.php?mod=user&action=pick_user&sub_action=init&field_id=';
	popup(str.concat(id, '&multiple=', multiple, '&inactive=', showInActive, '&empty=', showEmpty, '&archive=', showArchiveUser, '&no_empty=', no_empty, '&showGroups=', showGroups, '&confirm=', confirmation), 'user_select', 700, 410, 1);
}
...
function autouser_complete_field(field, span, targt, archive, multiple, noempty, showgroups, archive_uid, confirmation) {
