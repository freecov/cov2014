	...
	if ($this->_hide_navigation) {
		...
		if ($_SESSION["user_id"]) {
			...
			if (date("w")==5 && $GLOBALS["covide"]->license["has_snack"])
				$navigation->addNavigationItem(gettext("Snacks"), "module_snack", "./non-oop/snack.php");
			$navigation->addNavigationItem(gettext("Help"), "module_help", "javascript: var help = window.open('http://sourceforge.net/forum/forum.php?forum_id=590728');");
			*/
