...
function mail_action(a) {
	...
	switch (a) {
		...
		case 'reply_all':
			...
			location.href='?mod=email&action=compose&ref_id='+id+'&ref_type=reply_all&viewmode='+viewmode+'&from='+from;
			break;
		case 'reply':
			/* reply to the sender */
			location.href='?mod=email&action=compose&ref_id='+id+'&ref_type=reply&viewmode='+viewmode+'&from='+from;;
			break;
		case 'forward':
			/* forward this email */
			location.href='?mod=email&action=compose&ref_id='+id+'&ref_type=forward&viewmode='+viewmode+'&from='+from;;
			break;
		case 'back':
			/* go back in history */
			history.go(-1);
...
function mail_resize_frame() {
	...
	if (iframe.contentDocument) {
		iframe.style.height = iframe.contentDocument.body.scrollHeight+5;
	} else {
...
function replace_inline_image(html, image) {
	while (html.indexOf(image)>-1) {
...
function enable_inline_images() {
	var notfound_image = document.getElementById('notfound_image').innerHTML;
	var iframe = document.getElementById('mailContent');
	if (iframe.contentDocument) {
		var html = iframe.contentDocument.body.innerHTML;
		html = replace_inline_image(html, notfound_image);
		iframe.contentDocument.body.innerHTML = unescape(html);
	} else {
		var html = document.frames['mailContent'].document.body.innerHTML;
		html = replace_inline_image(html, notfound_image);
		document.frames['mailContent'].document.body.innerHTML = unescape(html);
	}
	document.getElementById('js_show_inline').style.visibility = 'hidden';
	/* try resize the layer in three timers */
	setTimeout('mail_resize_frame()', 500);
	setTimeout('mail_resize_frame()', 2000);
	setTimeout('mail_resize_frame()', 5000);
}
