	...
	if (!$id) {
		...
		if ($_REQUEST["ref_type"]) {
			...
			if (is_numeric($_REQUEST["from"])) {
				...
				if ($data["is_text"]) {
					...
					$prefix.= "  </td>\n";
					$prefix.= " </tr><tr>\n";
					$prefix.= "  <td valign='top'>".gettext("from").": </td><td>".$mdata[0]["sender_emailaddress_h"]."</td>\n";
					$prefix.= " </tr><tr>\n";
					...
					$prefix.= "  <td class='cell1'>".gettext("from").": </td><td class='cell2'>".$mdata[0]["sender_emailaddress_h"]."</td>\n";
					$prefix.= " </tr><tr class='head2'>\n";
		...
		if ($_REQUEST["convert_on_save"]) {
			if ($_REQUEST["is_text"]) {
				...
				$contents = sprintf("<font face='monospace' size='3'>%s</font>",
					nl2br($contents)
				);
