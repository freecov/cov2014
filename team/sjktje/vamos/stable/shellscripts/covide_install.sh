...
# Covide installer (c) 2006 Covide BV
#
...
_interactive() {
	if [ "$INTERACTIVE" == "YES" ]; then
		...
		echo "Covide Installer $VERSION Copyright (c) 2006 Covide"
		echo ""
...
- Linux Debian Sarge (current stable release) with DotDebian packages is recommended
...
Script environment:
* PHP 5.0.2 minimum (5.1.2 as Apache Module or higher strongly recommended) with the following extensions:
 - pgsql (PostgreSQL)
 - mysql (MySQL)
 - imap  (IMAP/POP3)
...
_svn_checkout() {
	...
	case "$COVIDE_VERSION" in
		stable)
			svn -q co https://svn.sourceforge.net/svnroot/covide/branches/covide-stable $COVIDE_ROOT
			;;
		trunk)
			svn -q co https://svn.sourceforge.net/svnroot/covide/trunk $COVIDE_ROOT
			;;
		vamos)
			svn -q co https://svn.sourceforge.net/svnroot/covide/branches/vamos/stable $COVIDE_ROOT
			EXTRA_DB_PATCH="vamos.sql"
			;;
		kovoks)
			svn -q co https://svn.sourceforge.net/svnroot/covide/branches/cvd_kovoks $COVIDE_ROOT
