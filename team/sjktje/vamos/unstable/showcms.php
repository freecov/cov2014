	...
	if ($_REQUEST["galleryid"]) {
		...
		if ($_REQUEST["html"]) {
			...
			$output->addTag("style", array("type" => "text/css"));
				...
				$output->addCode(sprintf(" body, td, div, A { text-decoration: none; color: black; font-face: %s; font-size: %dpt; }",
				...
				$tbl->addTableRow();
					...
					$tbl->addTableData(array("valign" => "top"));
						...
						$tbl->insertTag("a", gettext("download"), array(
							"href" => sprintf("showcms.php?dl=1&galleryid=%d&size=%s", $id, "full")
						));
						$tbl->addSpace(2);
						...
						$tbl->addSpace(2);
							$tbl->addCode(sprintf("[%d/%d]", $_cur, $i));
						$tbl->addSpace(2);
						$tbl->endTag("nobr");
						$tbl->endTag("div");
