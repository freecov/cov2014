...
if (file_exists($faxfile)) {
	...
	if (filesize($faxfile)) {
		//find relation
		if ((int)$agi["callerid"]) {
			$sql = "SELECT a.id,a.companyname FROM address as a LEFT JOIN address_businesscards as b ON a.id=b.address_id WHERE (replace(replace(a.phone_nr,'-',''), ' ','') ILIKE '%".$agi["callerid"]."' OR replace(replace(a.fax_nr,'-',''), ' ','') ILIKE '%".$agi["callerid"]."' OR replace(replace(a.mobile_nr,'-',''), ' ','') ILIKE '%".$agi["callerid"]."' OR replace(replace(b.business_fax_nr,'-',''), ' ','') ILIKE '%".$agi["callerid"]."' OR replace(replace(b.business_phone_nr,'-',''), ' ','') ILIKE '%".$agi["callerid"]."' OR replace(replace(b.business_mobile_nr,'-',''), ' ','') ILIKE '%".$agi["callerid"]."' OR replace(replace(b.personal_fax_nr,'-',''), ' ','') ILIKE '%".$agi["callerid"]."' OR replace(replace(b.personal_phone_nr,'-',''), ' ','') ILIKE '%".$agi["callerid"]."' OR replace(replace(b.personal_mobile_nr,'-',''), ' ','') ILIKE '%".$agi["callerid"]."')";
			$res = pg_query($sql);
			if (pg_num_rows($res)) {
