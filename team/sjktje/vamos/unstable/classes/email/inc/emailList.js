...
function createNewFolder() {
	var str = prompt( gettext("Enter the folder name") );
...
function editCurrentFolder() {
	var str = prompt(gettext("Supply new name for the folder:"));
	if (str && str != null && str != 'null' && str != '') {
...
function deleteAllTrash() {
	var cf = confirm( gettext("Are you sure you want to delete the entire trashbin?") );
	if (cf == true) {
		loadXML('?mod=email&action=deleteAllTrash');
		//document.location.href = "?mod=email&action=deleteAllTrash";
	}
}
