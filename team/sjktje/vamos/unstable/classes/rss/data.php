...
Class Rss_data {
	...
	public function saveFeed($feeddata) {
		...
		if ($rssdata["id"]) {
			/* update feed */
			$sql = sprintf("UPDATE rssfeeds SET name='%s', homepage='%s', url='%s', count=%d, user_id=%d  WHERE id=%d",
				$rssdata["name"], $rssdata["homepage"], $rssdata["url"], $rssdata["count"], $rssdata["user_id"], $rssdata["id"]);
		} else {
			/* insert feed into database */
			$sql = sprintf("INSERT INTO rssfeeds (name, homepage, url, count, user_id) VALUES ('%s', '%s', '%s', %d, %d)",
				$rssdata["name"], $rssdata["homepage"], $rssdata["url"], $rssdata["count"], $rssdata["user_id"]);
		}
		$res = sql_query($sql);
		if (PEAR::isError($res)) {
			return false;
