...
function update_conflict(active) {
	...
	if (active == 1) {
		...
		el.style.border='2px dotted red';
		el.innerHTML=gettext("there's already an appointment that conflicts with the times");
		el2.style.visibility='hidden';
		el3.style.visibility='hidden';
	} else if (active == 4) {
...
var calendar_check = Array('appointmentid','appointmentfrom_day','appointmentfrom_month','appointmentfrom_year','appointmentfrom_hour', 'appointmentfrom_min','appointmentto_hour', 'appointmentto_minute', 'appointmentuser', 'appointmentis_event', 'appointmentuser_id');
