...
Class Cms_data {
	...
	public function __construct() {
		...
		$this->modules = array(
			...
			"cms_permissions"    => gettext("protected items")." (n/a)",
	...
	public function savePageData($id, $skip_close=0, $req="") {
		...
		$q = sprintf("update cms_data set date_changed = %d, autosave_data = '', autosave_info = '',
	...
	public function saveAccount($req) {
		if ($req["id"]) {
			$q = sprintf("update cms_users set email = '%s', username = '%s', password = '%s', is_enabled = %d where id = %d",
				$req["cms"]["email"], $req["cms"]["username"], $req["cms"]["password"], $req["cms"]["is_enabled"], $req["id"]);
			sql_query($q);
		} else {
			$q = sprintf("insert into cms_users (email, username, password, is_enabled) values ('%s', '%s', '%s', %d)",
				$req["cms"]["email"], $req["cms"]["username"], $req["cms"]["password"], $req["cms"]["is_enabled"]);
			sql_query($q);
		}
	}
	...
	public function saveTemplate($req) {
		...
		$this->saveTemplateCache($id, $req["cms"]["category"], $req["cms"]["data"]);
		return $id;
	}
	...
	public function getCalendarItems($pageid=0, $itemid=0) {
		...
		if ($pageid) {
			...
			$res = sql_query($q);
			while ($row = sql_fetch_assoc($res)) {
				$row["date_begin_h"] = date("d-m-Y", $row["date_begin"]);
				$row["date_end_h"]   = date("d-m-Y", $row["date_end"]);
	...
	public function getFormResults($pageid) {
		...
		while ($row = sql_fetch_assoc($res)) {
			...
			$res2 = sql_query($q);
			while ($row2 = sql_fetch_assoc($res2)) {
				$row["field_".$row["id"]] = $row2["user_value"];
				if (!in_array($row2["field_name"], $fields))
					$fields["field_".$row["id"]] = $row2["field_name"];
			}
			/* modify some values */
			unset($row["pageid"]);
	...
	public function saveSiteInfo($req) {
		...
		$q = sprintf("update %s set %s, cms_favicon = '%s', cms_logo = '%s', cms_defaultpage = %d,
			google_verify = '%s', cms_hostnames = '%s'", $table, $baseq,
	...
	public function checkLinkcheckerResults() {
		...
		foreach ($data as $k=>$v) {
			if ($v["result"] == "200 OK" || $v["valid"] == "True") {
				...
				$pid = (int)preg_replace("/[^0-9]/s", "", urldecode(basename($v["parentname"])));
	...
	public function createInlineThumb($id, $width, $height, $pageid) {
		...
		if (sql_num_rows($res) == 1) {
			/* thumb is available */
			$ret = sprintf("/cmscache/%d", sql_result($res,0));
		} else {
			/* create thumb */
	...
	public function getBanners($id=0) {
		if ($id)
			$q = sprintf("select * from cms_banners where id = %d", $id);
		else
			$q = "select * from cms_banners order by name";
	...
	public function saveBanner($req) {
		require(self::include_dir."saveBanner.php");
	}
