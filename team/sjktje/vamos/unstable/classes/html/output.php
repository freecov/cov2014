...
class Layout_output {
	...
	public function insertImage($image, $alt, $link="", $theme=0, $skip_encoding=0, $id="", $is_action="", $fader=0) {
		...
		if (preg_match("/MSIE (5|6)/s", $_SERVER["HTTP_USER_AGENT"]) && preg_match("/(icons|modules)\/.*\.png$/si", $image) && $_COOKIE["disable_dx_transform"] != 1) {
			$xdir = $this->external_file_cache_handler($dir.$image);

			/* if loaded from cms, use an absolute path */
			if ($GLOBALS["site_loaded"])
				$xdir = "/".$xdir;
		...
		/* a little playing =)
		if ($fader) {
			$settings_image["onmouseover"] = "initFadeIn(this);";
			$settings_image["onmouseout"]  = "initFadeOut(this);";
			$settings_image["width"] = 32;
			$settings_image["height"] = 32;
		}
		*/
		if ($link) {
	...
	/* {{{ addLabel */
	/**
	 * Inserts a label tag with text. 
	 *
	 * The label is used to connect some text to for example a text field
	 * or a select box.
	 * 
	 * @param string $text The text that's supposed to be labelled
	 * @param string $for The id of the element the label's for.
	 * @param array $settings Array of other optional settings.
	 */
	public function addLabel($text, $for, $settings=array()) {
		$settings["for"] = preg_replace("/(\[)|(\])|( )/s", "", $for);
		$this->addTag("label", $settings);
			$this->addCode($text);
		$this->endTag("label");
	} /* }}} */		
