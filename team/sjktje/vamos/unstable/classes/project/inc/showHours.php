...
/* main venster */
$venster_main = new Layout_venster(array("title"=>$projectinfo[0]["name"], "subtitle"=>($this->has_declaration) ? gettext("file activities") : gettext("project information")));
$venster_main->addMenuItem(($this->has_declaration) ? gettext("back") : gettext("back to projects"), "javascript: history_goback();");
if ($this->has_declaration) {
...
$venster_main->addVensterData();
	...
	$table->endTableRow();
		...
		$table->addTableData("", "data");
			$table->insertAction("go_sales", gettext("view"), array("href" => "index.php?mod=sales&productId=".$projectinfo[0]["id"]));
		$table->endTableData();
	$table->endTableRow();
	$table->addTableRow();
