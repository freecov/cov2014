<?php
	...
	$reference_nr = sprintf('%d.%d', time(), rand(111,999));
	...
	if ($tpl->cms_license['recaptcha_public']) {
		...
		if ($tpl->cms_license['recaptcha_theme']) {
			$theme = $tpl->cms_license['recaptcha_theme'];
		} else {
			$theme = 'red';
		}
		$tbl->addTableRow();
			...
			$tbl->addTableData();
				...
				", $theme, $lang));
				$tbl->end_javascript();
				$tbl->addCode(recaptcha_get_html($tpl->cms_license['recaptcha_public']));
			$tbl->endTableData();
		$tbl->endTableRow();
	}
	...
	$tbl->addTableRow();
		$tbl->addTableData(array("colspan" => 2, "style" => "text-align: right"));
			if ($tpl->cms_license['recaptcha_public']) {
				$tbl->insertTag("a", gettext("renew captcha"), array(
