...
/* and another check, this time for metafields */
if ($GLOBALS["covide"]->license["address_migrated"] <= 2) {
	$metafields_data = new Metafields_data();
	$metafields_data->migrateRelationsToBCs();
}
	...
	if (1==0 && $options["privuseredit"]) {
		...
		$query_zoek = "
			...
			LEFT JOIN
				google_address_sync
			ON
				address_businesscards.id = google_address_sync.address_id AND
				google_address_sync.user_id = ".$options["funambol_user"]."
			LEFT JOIN
				users
			ON
				users.address_id = address_businesscards.id
		...
		foreach ($zoekwoorden as $zw) {
			...
			if ($options["specified"] == 0) {
				...
				$query .= "AND (";
					...
					if ($options["specified"] == 6) { $query .= "(address_businesscards.id IN (SELECT relation_id FROM meta_global WHERE value $like_syntax '%$zw%'))"; }
...
if ($options["addresstype"] == "bcards") {
	if (!$options["sort"]) {
		...
		$query_zoek .= "ORDER BY address_businesscards.".sql_filter_col($options["sort"]);
	}
	...
	if (!$options["sort"]) {
		...
		$query_zoek .= "ORDER BY address.".sql_filter_col($options["sort"]);
	}
}
