...
$date_classification  = "import-".date("YmdHi");
$cols                 = $data["col"];
unset($data);
...
if ($_REQUEST["target"] == "bcard") {
	/* bcard query */
	$prequery = "INSERT INTO address_businesscards (".implode(", ", $cols_unique).", address_id, multirel, rcbc, duplicate_with, modified) VALUES(";
	$postquery = time().")";
} else {
	/* address query */
	$prequery = "INSERT INTO address (".implode(", ", $cols_unique).", tav, contact_person, duplicate_with, is_public, is_active, is_company, modified, user_id) VALUES(";
	$postquery = "1, 1, 1, ".time().", ".$_SESSION["user_id"].")";
}
...
foreach ($data as $fld) {
	...
	foreach ($vals as $col=>$x) {
		...
		switch ($col) {
			case "classification":
				...
				//add the 'date' classification to identify this import
				if (!in_array($date_classification, $cla_list)) {
					$cdata = array();
					$cdata["description"] = $date_classification;
					$cdata["is_active"] = 1;
					$z = $classification_data->store2db($cdata);
				} else {
					$z = array_search($date_classification, $cla_list);
				}
				$zz[] = $z;
		...
		/* get the zipcode and phone to check duplicate value */
		if ($_REQUEST["target"] == "bcard") {
			if ($col == 'business_zipcode' && !$zipcode_val) {
				$zipcode_val = $x;
			}
			if ($col == 'business_phone_nr' && !$phone_val) {
				$phone_val = $x;
			}
		} else {
			if ($col == 'zipcode' && !$zipcode_val) {
				$zipcode_val = $x;
			}
			if ($col == 'phone_nr' && !$phone_val) {
				$phone_val = $x;
			}
		}
	...
	if ($zipcode_val && $phone_val && $_REQUEST["target"] != "bcard") {
		$sql_dupli = sprintf("SELECT id FROM address WHERE zipcode = '%s' AND phone_nr = '%s'", $zipcode_val, $phone_val);
		$sql_dupli = sql_query($sql_dupli);
		$dpli = sql_fetch_assoc($sql_dupli);

		if ($dpli['id']) {
			$duplicate_with = $dpli['id'];
		} else {
			$duplicate_with = 0;
		}
	...
	if ($_REQUEST["target"] == "bcard" && is_array($get_companyname)) {
		...
		foreach ($get_companyname as $g=>$v) {
			$nameNoFormat = addslashes($fld[$g]);
			$name = strtolower($fld[$g]);
			$cp = sprintf("select id from address where lower(companyname) like '%s' and is_active = 1", addslashes($name));
			$res = sql_query($cp);
		...
		if (count($company_ids) > 0) {
			...
			$bcfields = array(
				...
				"surname"   			=> $vals["surname"][0],
				"companyname"       	=> $nameNoFormat,
				"address"           	=> $vals["business_address"][0],
				"zipcode"           	=> $vals["business_zipcode"][0],
				"city"              	=> $vals["business_city"][0],
				"country"           	=> $vals["business_country"][0],
				"phone_nr"          	=> $vals["business_phone_nr"][0],
				"fax_nr"            	=> $vals["business_fax_nr"][0],
				"mobile_nr"         	=> $vals["business_mobile_nr"][0],
				"email"             	=> $vals["business_email"][0],
				"website"           	=> $vals["website"][0],
				"contact_initials"  	=> $vals["initials"][0],
				"contact_givenname" 	=> $vals["givenname"][0],
				"contact_infix"     	=> $vals["infix"][0],
				"contact_surname"   	=> $vals["surname"][0],
				"contact_letterhead" 	=> $vals["letterhead"][0],
				"contact_commencement" 	=> $vals["commencement"][0],
				"title"              	=> $vals["title"][0],
				"suffix"             	=> $vals["suffix"][0],
				"bsn"                	=> $vals["ssn"][0],
				"contact_birthday"   	=> $vals["timestamp_birthday"][0],
				"bankaccount"        	=> $vals["bankaccount"][0],
				"giro"               	=> $vals["giro"][0],
				"pobox"             	=> $vals["pobox"][0],
				"pobox_zipcode"     	=> $vals["pobox_zipcode"][0],
				"pobox_city"        	=> $vals["pobox_city"][0],
				"pobox_country"     	=> $vals["pobox_country"][0],
				"is_active"     		=> 1,
				"is_public"        		=> 1,
				"is_company"     		=> 1
			);
			$keys = array_keys($bcfields);
			$values = array_values($bcfields);
			$sql = "INSERT INTO address (".implode(",", $keys).") values ('".implode("','",$values)."')";
			$res = sql_query($sql);
			$first = sql_insert_id();
			
			//and set rcbc boolean to true
			$rcbc = true;
		}
	...
	/*contact duplicate value */
	$q .= $duplicate_with . ',' . $postquery;
