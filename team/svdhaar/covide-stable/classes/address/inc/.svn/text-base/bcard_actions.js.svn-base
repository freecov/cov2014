function bcard_save() {
	valid = "";
	if (!validate_email(document.getElementById('bcardbusiness_email'))) {
		valid = valid + "business email";
	}
	if (!validate_email(document.getElementById('bcardpersonal_email'))) {
		valid = valid + "personal email";
	}
	if (!validate_email(document.getElementById('bcardother_email'))) {
		valid = valid + "other email";
	}

	if (valid.length > 1) {
		alert(gettext("Can not be saved. please check again."));
	} else if (document.getElementById('editbcard')) {
