...
$output->layout_page("", 1);
	...
	if ($inactive)
		$output->addHiddenField("action", "deleteSelectionExec");
	else
		$output->addHiddenField("action", "deactivateSelection");
	...
	$venster->addVensterData();
		...
		$table->addTableRow();
			if ($inactive)
				$table->insertTableData(gettext("Are you sure you want to delete these contacts from the database? WARNING: This can not be undone!"), "", "header");
			else
				$table->insertTableData(gettext("Are you sure you want to set these contacts on non-active?"), "", "header");
		$table->endTableRow();
		...
		$table->addTableRow();
			$table->addTableData("", "data");
				...
				$table->insertAction("ok", gettext("ok"), "javascript: document.getElementById('removeSelection').submit();");
			$table->endTableData();
		$table->endTableRow();
