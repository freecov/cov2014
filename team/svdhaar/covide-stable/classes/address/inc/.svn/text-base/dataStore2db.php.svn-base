...
if ($address["type"] == "relations" || $address["type"] == "nonactive") {
	...
	$fields[] = "branche";              $values[] = sprintf("'%s'",   $address["branche"]);
...
if (in_array($address["type"], array("private", "users"))) {
	...
	$fields[] = "contact_initials";     $values[] = sprintf("'%s'", $address["contact_initials"]);
	$fields[] = "title";                $values[] = sprintf("%d",   $address["title"]);
	$fields[] = "skype";                $values[] = sprintf("'%s'",   $address["skype"]);
	...
	$fields[] = "business_skype";       $values[] = sprintf("'%s'", $address["business_skype"]);
...
if ($id) {
	...
	if ($oldactive == -1 && $address["is_active"] == 1 && $GLOBALS["covide"]->license["has_membersarea"]) {
		...
		$sql = sprintf("INSERT INTO cms_users (username, password, is_enabled, email, registration_date, is_active, address_id) VALUES ('%s', '%s', 1, '%s', %d, 1, %d)",
			$cms_username, $cms_password, $address["email"], time(), $id);
		$r = sql_query($sql);
	...
	if ($address["type"] != "relations" && $address["type"] != "nonactive") {
		...
		/* save meta fields */
		if (count($metafields)) {
			/* address_id requried to save meta fields */
			$_REQUEST["address"]["id"] = $address_id;
			$meta_data = new Metafields_data();
			$meta_data->meta_save_field($metafields);
		}
	}
