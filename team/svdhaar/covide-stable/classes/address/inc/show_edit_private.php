	...
	$venster->addHiddenField("address[user_id]", $address_info["user_id"]);
		...
		$table->addTableRow(array("class" => "expand_private"));
			...
			$table->addTableData("", "data");
				if ($view_only) {
					...
					$table->addTextField("address[email]", $address_info["email"], array("onblur" => "validate_email(this);"));
				}
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow(array("class" => "expand_private"));
		$table->insertTableData(gettext("skype"), "", "header");
			$table->addTableData(array("colspan" => 3), "data");
				if ($view_only && $bcardinfo["skype"]) {
					$table->addCode(sprintf("<a href=\"callto:%s\"> %s </a>", $bcardinfo["skype"], $bcardinfo["skype"]));
				} else {
					$table->addTextField("address[skype]", $address_info["skype"]);
				}
			$table->endTableData();
		$table->endTableRow();
		/* empty */
		...
		$table->addTableRow(array("class" => "expand_business"));
			...
			$table->addTableData("", "data");
				if ($view_only) {
					...
					$table->addTextField("address[business_email]", $address_info["business_email"], array("onblur" => "validate_email(this);"));
				}
			$table->endTableData();
		$table->insertTableData(gettext("skype"), "", "header");
			$table->addTableData("", "data");
				if ($view_only && $bcardinfo["business_skype"]) {
					...
					$table->addTextField("address[business_skype]", $address_info["business_skype"]);
				}
			$table->endTableData();
		$table->endTableRow();
		...
		$table->addTableRow(array("class" => "expand_other"));
			...
			$table->addTableData("", "data");
				if ($view_only) {
					...
					$table->addTextField("address[other_email]", $address_info["other_email"], array("onblur" => "validate_email(this);"));
				}
			$table->endTableData();
		$table->endTableRow();
		...
		if ($type != "overig" && !$view_only) {
			...
			$metafields = $metadata->meta_list_fields("adres_private", $id);
