...
if (is_array($_REQUEST["classifications"])) {
	foreach ($_REQUEST["classifications"] as $k => $v) {
		$output->addHiddenField("classifications[$k]", $v);
	}
}
...
if ($_REQUEST["campaign"]) {
	...
	if (!$GLOBALS["covide"]->license["multivers"] && !$GLOBALS["covide"]->license["exact"]) {
		if ($user->checkPermission("xs_addressmanage") || $user->checkPermission("xs_relationmanage")) {
			$venster->addMenuItem(gettext("add relation"), "javascript: popup('index.php?mod=address&action=edit_bcard&from=relation&addresstype=relations', 'addressedit', 800, 600, 1);", "", 0);
		}
	}
}
...
if ($user->checkPermission("xs_addressmanage") || $user->checkPermission("xs_usermanage")) {
	...
	$venster->addMenuItem("&nbsp;&nbsp;".gettext("manage selection classification"), "javascript: handle_selectionaction(); popup('index.php?mod=address&action=addcla_multi&type=" . $addresstype . "&info='+exportinfo, 'addcla_multi', 350, 150, 1);");
...
$venster->addMenuItem("&nbsp;&nbsp;".gettext("new person"), "javascript: popup('index.php?mod=address&action=edit_bcard&addresstype=private', 'addressedit', 800, 550, 1);");
...
if (!preg_match("/^searchRel/", $action)) {
	if ($addresstype == "bcards" || $addresstype == "private" || $addresstype == "users") {
...
switch ($addresstype) {
	case "users"   :
		...
		$field_sort = unserialize($user_info["default_address_fields_users"]);
	...
	case "private" :
		...
		$field_sort = unserialize($user_info["default_address_fields_private"]);
		if (is_array($field_sort)) {
			foreach ($field_sort as $field=>$sort) {
				switch ($field) {
					case "last name": $view->addMapping(gettext("last name"), "%surname"); break;
					case "given name": $view->addMapping(gettext("given name"), "%givenname"); break;
					case "mobile phone nr": $view->addMapping(gettext("mobile phone nr"), "%personal_mobile_nr", array("allow_html" => 1)); break;
					case "relation": $view->addMapping(gettext("relation"), "%%complex_companyname"); break;
					case "classifications": $view->addMapping(gettext("classifications"), "%classification_html", "", "", "", 1); break;
					case "address": $view->addMapping(gettext("address"), "%business_address"); break;
					case "telephone nr": $view->addMapping(gettext("telephone nr"), "%%phone_nr"); break;
			...
			$view->addMapping(gettext("given name"), "%givenname");
			$view->addMapping(gettext("name")." / ".gettext("free field"), "%%complex_bcardsname");
			$view->addMapping(gettext("telephone nr"), "%personal_phone_nr_link", array("allow_html" => 1));
			$view->addMapping(gettext("mobile phone nr"), "%%mobile_nr");
			$view->addMapping(gettext("relation"), "%%complex_companyname");
		}
		$view->defineSort(gettext("last name"), "surname");
...
$view->defineComplexMapping("mobile_nr", array(
	...
	if ($GLOBALS["covide"]->license["has_funambol"]
		...
		$view->defineComplexMapping("complex_sync", array(
			array(
				...
				"src"     => "f_oud.gif",
				"alt"     => gettext("toggle sync"),
				"link"    => array("javascript: toggleGoogleSync('", "%bc_id", "', 'activate');"),
				"id"      => array("toggle_sync_", "%bc_id"),
				"check"   => "%sync_no",
				"no_mobile" => 1
			),
			array(
				...
				"src"     => "f_nieuw.gif",
				"text"    => gettext("toggle sync"),
				"link"    => array("javascript: toggleGoogleSync('", "%bc_id", "', 'deactivate');"),
				"id"      => array("toggle_sync_", "%bc_id"),
				"check"   => "%sync_yes",
				"no_mobile" => 1
			),
	...
	$view->defineComplexMapping("complex_actions", array(
		...
		array(
			...
			"link"    => array("javascript: popup('index.php?mod=address&action=edit_bcard&from=relation&id=", "%bcard_id", "&address_id=", "%id", "&addresstype=" . $addresstype . "&sub=" . $sub, "', 'addressedit', 700, 600, 1);"),
			"check"   => "%addressmanage"
		),
		array(
			"type"    => "link",
			...
			"link"    => array("javascript: popup('index.php?mod=address&action=addcla_inline&type=relations&id=", "%id", "', 'addcla_multi', 350, 150, 1);"),
			"check"   => "%addressmanage"
		),
		array(
			"type"    => "link",
			"src"	  => "relations",
			"text"	  => array(gettext("add business card"), "<br />"),
			"link"	  => array("javascript: popup('index.php?mod=address&action=edit_bcard&id=0&address_id=", "%id", "', 'bcardedit', 600, 600, 1);")
		),
		array(
			"type"    => "link",
			"src"     => "vcard",
		...
		array(
			"type"    => "action",
			"src"     => "duplicate",
			"alt"     => gettext("Duplicate"),
			"link"    => array("javascript: popup('index.php?mod=address&action=duplicate_show&id=", "%id", "&duplicate_with=", "%relationduplicate", "&addresstype=" . $addresstype . "&sub=" .$sub, "', 'addressedit', 700, 600, 1);"),
			"check"   => "%relationduplicate"
		),
	), "nowrap");
...
$view->defineComplexMapping("complex_actions_bcards", array(
	...
	array(
		...
		"src"     => "view_all",
		"text"    => array(gettext("add classification"), "<br />"),
		"link"    => array("javascript: popup('index.php?mod=address&action=addcla_inline&type=bcards&id=", "%id", "', 'addcla_multi', 350, 150, 1);"),
		"check"   => "%addressmanage"
	),
	array(
		"type"    => "link",
	...
	*/
	...
	array(
			...
			"src"     => "duplicate",
			"alt"     => gettext("Duplicate"),
			"link"    => array("javascript: popup('index.php?mod=address&action=duplicate_show&id=", "%address_id", "&duplicate_with=", "%bcardduplicate", "&addresstype=" . $addresstype . "&sub=" . $sub . "", "', 'addressedit', 700, 600, 1);"),
			"check"   => "%bcardduplicate"
	),
), "nowrap");
...
if (is_array($_REQUEST["classifications"])) {
	...
	$table->addTableRow();
		...
		if ($addresstype=="bcards") {
			...
			$table->insertTableData(gettext("relations"));
		}
	$table->endTableRow();
	$table->addTableRow();
