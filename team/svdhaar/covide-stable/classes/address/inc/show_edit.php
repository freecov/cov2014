...
if ($id) {
	...
	$address_info["contact_commencement"] = 3;
	$address_info["contact_letterhead"] = 2;
}
...
$venster->addVensterData();
		...
		*/
		...
		if (!$view_only) {
			...
			$table->addTableRow(array("class" => "expand_account"));
				...
				$table->addTableData(array("colspan" => 3), "data");
					$table->addTextField("address[branche]", $address_info["branche"]);
				$table->endTableData();
			$table->endTableRow();
			$table->addTableRow(array("class" => "expand_account"));
	...
	$table->addTableRow(array("class" => "expand_address"));
		...
		$table->addTableData("", "data");
			...
			$table->addSelectField("address[country]", $countryArray, $address_info["country"], 0, array(
	...
	$table->addTableRow(array("class" => "expand_address"));
		...
		$table->addTableData("", "data");
			if ($view_only) {
				$address_info["phone_nr"] = $address_data->show_phonenr($address_info["phone_nr"], $address_info["id"], $_SESSION["user_id"]);
			}
		...
		$table->addTableData("", "data");
			if ($view_only) {
				$address_info["mobile_nr"] = $address_data->show_phonenr($address_info["mobile_nr"], $address_info["id"], $_SESSION["user_id"]);
			}
	...
	$table->addTableRow(array("class" => "expand_address"));
		...
		$table->addTableData("", "data");
			...
			$table->start_javascript();
				$table->addCode("updatePhoneNumbers();");
	...
	$table->addTableRow(array("class" => "expand_address"));
		...
		$table->addTableData(array("colspan" => 3), "data");
			$table->addTextField("address[email]", $address_info["email"], array("style" => "width: 500px;", "onblur" => "validate_email(this);"));
		$table->endTableData();
	$table->endTableRow();
	...
	if ($type != "overig") {
		...
		$table->addTableRow(array("class" => "expand_pobox"));
			...
			$table->addTableData("", "data");
				if (!$address_info["pobox_country"]) {
					$address_info["pobox_country"] = "NL";
				}
				if ($view_only) {
					$table->addCode($countryArray[$address_info["country"]]);
				} else {
