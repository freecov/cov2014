...
if (is_array($relation_info) && $bcardinfo['type'] == "relations") {
	...
	// do we have to sync this to some financial application?
	if ($GLOBALS["covide"]->license["has_exactonline"] == 1) {
		$exact_rel_data = array_merge($relation_info, $bcardinfo);
		if ($exact_rel_data["debtor_nr"] && ($exact_rel_data["is_customer"] || $exact_rel_data["is_supplier"])) {
			$exact_data = new Exactonline_data();
			$exact_data->sendRel($exact_rel_data);
		}
	}
}
