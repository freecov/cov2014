	...
	Class Address_data {
		...
		public function gendebtornr($prefix="") {
			if (!$prefix) {
				...
				$sql = "SELECT MAX(CAST(debtor_nr AS UNSIGNED)) as currentnr FROM address";
				$res = sql_query($sql);
		...
		*/
		...
		public function getAddressIdByClassification($id) {
			$regex_syntax = sql_syntax("regex");
			$q = sprintf("SELECT b.address_id, a.companyname FROM address_businesscards b LEFT JOIN address a ON a.id = b.address_id WHERE (b.user_id = 0 OR b.user_id IS NULL) AND (b.rcbc=1) AND (a.is_active=1)");
		...
		public function getNewsletterData($settings) {
			...
			if (is_array($settings["classifications"])) {
				...
				if ($user_classification != 0) {
					$sq.= " AND (".$regex_field." ".$regex_syntax." '(^|\\\\|)". $user_classification ."(\\\\||$)' ".$t.") ";
				}
				...
				if ($addresstype == "users") {
					...
					$query_count = sprintf("
						SELECT count(address_businesscards.address_id)
						FROM address_businesscards
						INNER JOIN address_info ON
							address_info.address_id = address_businesscards.address_id
						INNER JOIN address ON
							address.id = address_businesscards.address_id
						WHERE
							(address.is_public=1 OR address_businesscards.user_id= %d) AND rcbc = 1
							AND (%s)",
						$_SESSION["user_id"], $sq);

					$query_zoek = sprintf("
						SELECT address_businesscards.*, address_info.classification, address_info.provision_perc
						FROM address_businesscards
						INNER JOIN address_info ON
							address_info.address_id=address_businesscards.address_id
						INNER JOIN address ON
				...
				if ($addresstype != "bcards" && $addresstype != "users") {
					$query .= " AND (address$t.is_public=1 OR ((address$t.is_public=0) AND (address_businesscards$t.user_id=".(int)$_SESSION["user_id"]."))) ";
				}
				...
				while ($row = sql_fetch_assoc($result)) {
					...
					if ($addresstype == "users") {
						$addressinfo[$i]["email"] = ($row["email"]=="" ? $row["business_email"] : $row["email"]);
					} elseif($addresstype == "relations"){
						$addressinfo[$i]["email"] = $row["business_email"];
					} else {
