...
Class Funambol_data {
	...
	*/
	...
	public function syncUser($reset_db=0) {
		...
		if (file_exists($pidfile) && filemtime($pidfile) >= time()-60)
	...
	public function truncateCovideStore() {
		...
		$q = sprintf("delete from calendar_user where user_id = %d", $this->user_id);
		sql_query($q);
	...
	*/
	...
	private function checkRecordsCalendar($type, &$stats) {
		...
		$ts = mktime(0,0,0, date("m")-6, date("d"), date("Y")); //max 1 month back

		/* check for synable items, that do not yet exist in the sync server */
		$q = sprintf("select calendar.id from calendar, calendar_user where calendar.id = calendar_user.calendar_id AND (calendar_user.status = 1 OR calendar_user.status = 4) AND calendar.timestamp_start > %2\$d AND calendar_user.user_id = %1\$d and calendar.id NOT IN (
		...
		/* check for aged sync items in the sync server */
		$q = sprintf("select guid from funambol_calendar_sync where calendar_id IN (
			select calendar.id from calendar, calendar_user where calendar.id = calendar_user.calendar_id AND calendar_user.user_id = %1\$d and (calendar.timestamp_start < %2\$d OR
				calendar_user.status = 2))",
				$this->user_id, $ts);
		$res = sql_query($q);
		while ($row = sql_fetch_assoc($res)) {
		...
		foreach ($items as $k=>$v) {
			switch ($v["action"]) {
				case "D":
					...
					#sql_query($q);
					/* delete from db */
					$calendar_data->delete($v["calendar_id"], $this->user_id);

					$stats["D"]++;
					break;
				case "U":

					$db_data = $calendar_data->getCalendarItemById($v["calendar_id"], $this->user_id);
					...
					if (!$calendar_data->checkRegistrationState($v["calendar_id"], $this->user_id)) {
						$data["id"] = $v["calendar_id"];
				...
				case "I":
					...
					if ($data["timestamp_start"] > $ts) {
						...
						$q = sprintf("insert into funambol_calendar_sync (user_id, guid, calendar_id, datetime) values (%d, '%s', %d, %d)",
							$this->user_id, $v["guid"], $ids[0], time());
						sql_query($q);
						$stats["I"]++;
					}
					break;
	...
	public function syncRecord($type, $guid, $id=0) {
		...
		switch ($type) {
			...
			case "calendar":
				...
				$calendar_data = new Calendar_data();
				$data = $calendar_data->getCalendarItemById($id, $this->user_id, 1);
				/* debugging only
				print_r($data);
				die();
				*/
				...
				if ($data["repeat_type"] == "1" && $data["is_repeat"] == 1) {
					/* daily repeating */
				...
				} elseif ($data["repeat_type"] == "2" && $data["is_repeat"] == 1) {
					/* weekly */
					...
					$rpt = str_replace("{repeat_start_date}", $data["datetime_start"], $rpt);
					$xml = str_replace("<repeatmarker />", $rpt, $xml);
				} elseif ($data["repeat_type"] == "3" && $data["is_repeat"] == 1) {
					/* monthly */
					...
					$rpt = str_replace("{repeat_start_date}", $data["datetime_start"], $rpt);
					$xml = str_replace("<repeatmarker />", $rpt, $xml);
				} elseif ($data["repeat_type"] == "6" && $data["is_repeat"] == 1) {
					/* yearly */
	...
	public function getNextFreeGuid($type="", $user=0, $current_id=0) {
		if ($current_id)
			...
			return "R0"; //"R".rand(0,99999); //die("no id specified:". $type);
	...
	public function deleteRecord($type, $guid, $user_id = 0) {
		...
		switch ($type) {
			case "address":
				...
				$q = sprintf("update funambol_address_sync set address_id = 0, datetime = %d where guid = '%s'", time(), $guid);
				sql_query($q);
				break;
			case "calendar":
				...
				$q = sprintf("update funambol_calendar_sync set calendar_id = 0, datetime = %d where guid = '%s'", time(), $guid);
				sql_query($q);
				break;
			case "todo":
				...
				$q = sprintf("update funambol_todo_sync set todo_id = 0, datetime = %d where guid = '%s'", time(), $guid);
				sql_query($q);
				break;
			case "files":
				...
				$q = sprintf("update funambol_file_sync set file_id = 0, datetime = %d where guid = '%s'", time(), $guid);
				sql_query($q);
				break;
		}
	}
