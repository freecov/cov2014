...
Class Voip_data {
	...
	private function getActiveCallsDB() {
		...
		$res = sql_query($q);
		while ($row = sql_fetch_assoc($res)) {
			if (!$row["user_id"] && $row["timestamp"] < time()-$this->cleanup_voip) {
				...
				sql_query($q);
			} elseif ($row["user_id"] && $row["timestamp"] < time()-$this->cleanup_invites) {
	...
	public function sendSMS($userid, $body) {
		if ($userid) {
			...
			if ($userinfo["address_id"]) {
				/* get mobile number */
				$sql = sprintf("SELECT business_mobile_nr, personal_mobile_nr FROM address_businesscards WHERE id = %d", $userinfo["address_id"]);
				$res = sql_query($sql);
				$row = sql_fetch_assoc($res);
				if (trim($row["business_mobile_nr"])) {
					/* normalize the number to send */
					$msisdn = $this->normalizeMsisdn(trim($row["business_mobile_nr"]), $settings);
				} elseif (trim($row["personal_mobile_nr"])) {
					$msisdn = $this->normalizeMsisdn(trim($row["business_mobile_nr"]), $settings);
				} else {
					return false;
				}
		...
		*/
		...
		if ($req["login"] == $login && $req["password"] == $pass) {
			...
			$q = sprintf("insert into active_calls (name, address_id, timestamp, ident) values ('%s', %d, %d, '%s')",
				$address["companyname"], $address["id"], $now, md5(time().$address["id"].$address["companyname"]));
			sql_query($q);
	...
	public function mkIdent($p = array()) {
		$str = sprintf("%d|%s", time(), implode("|", $p));
