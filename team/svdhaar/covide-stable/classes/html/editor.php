...
class Layout_editor {
	...
	private $tinymce_folder = "tinymce3392";

	/* methods */
	/* __construct {{{ */
	public function __construct($editor = "tinymce", $width=910, $height=500) {
		$this->settings = array(
			"width"   => $width,
			"height"  => $height,
			"toolbar" => "full"
	...
	/* load_tinymce {{{ */
	/**
	 * Generate html code to load tinymce
	 *
	 * @param int $editor_type 0 for fullblown, 1 for limited with fontselection, 2 for limitid without fontselection
	 * @param bool $cleanup run html cleanup function of tinymce if set to true
	 * @param string $txtareaname The textarea to convert
	 * @param int $version tinymce version to use, obsolete arg
	 * @param int $allow_font if set to 1 font tags are allowed, if not set (default) font tags will be converted into spans
	 *
	 * @return string the html code to include in your page
	 */
	private function load_tinymce($editor_type=0, $cleanup="true", $txtareaname="contents", $version=3, $allow_font = 0, $relative = true) {
		if ($this->xhtml)
		...
		*/
		...
		if ($mini) {
			$toolbars = sprintf(
				...
				($font)?"theme_advanced_buttons1_add : \"fontselect,fontsizeselect,print,removeformat\",":"theme_advanced_buttons1_add : \"print,removeformat\",",
			...
			$toolbars = sprintf(
				...
				($font)?"theme_advanced_buttons1_add : \"fontselect,fontsizeselect\",":"",
		...
		$output->addCode(sprintf("
			tinyMCE.init({
				...
				paste_auto_cleanup_on_paste : $cleanup,
				cleanup : $cleanup,
				...
				entities : '38,amp,34,quot,162,cent,8364,euro,163,pound,165,yen,169,copy,174,reg,8482,trade,8240,permil,8804,le,8805,ge,176,deg,8722,minus',
		...
		",
			...
			($allow_font)?"convert_fonts_to_spans: false":"convert_fonts_to_spans: true",
	...
	/* load_wyzz {{{ */
	/**
	 * Generate html code to load wyzz
	 *
	 * @param string $txtareaname The textarea to convert
	 *
	 * @return string the html code to include in your page
	 */
	public function load_wyzz($txtareaname = "contents") {
		if ($this->settings["width"] != 910 || $this->settings["height"] != 500) {
			$editor_call = sprintf("make_wyzz('%s', %d, %d);", $txtareaname, $this->settings["width"], $this->settings["height"]);
		} else {
			$editor_call = sprintf("make_wyzz('%s');", $txtareaname);
		}
		$output = new Layout_output();
		$output->load_javascript(self::include_dir."editor_wyzz.js");
		$output->start_javascript();
		$output->addCode($editor_call);
		$output->end_javascript();
		return $output->generate_output();
	}
	/* }}} */
	 ...
	 */
	public function generate_editor($settings=0, $data="", $cleanup="true", $txtareaname="contents", $allow_fonts = 0, $relative = true) {
		...
		if ($loadeditor) {
			switch ($this->editor) {
			case "tinymce" :
				$return = $this->load_tinymce($settings, $cleanup, $txtareaname, "", $allow_fonts, $relative);
				break;
			case "wyzz" :
				$return = $this->load_wyzz($txtareaname);
				break;
			default:
				$return = false;
				break;
