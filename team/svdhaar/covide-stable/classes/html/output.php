...
class Layout_output {
	...
	public function external_file_cache_handler($file) {
		$aip =& $GLOBALS["autoloader_include_path"];

		if ($aip)
			$file2 = sprintf("%s%s", $GLOBALS["autoloader_include_path"], $file);
		...
		if (!file_exists($file) && !file_exists($file2)) {
			...
			$file = sprintf("%s%s", $webroot, $file);
			if ($file2)
				$file = sprintf("%s", $file);

			/* include to check a setting */
			require("conf/offices.php");
			if (!$aip && preg_match("/(MSIE 6)|(MSIE 7)|(Gecko\/\d{8})|(Opera)/s", $_SERVER["HTTP_USER_AGENT"])
				&& !$html["no_static_gzip_compression"]) {
				$p = pathinfo($file);
				if (preg_match("/^(themes)|(classes)|(tmp_cms)|(menudata)/s", $p["dirname"])) {
					if (in_array(strtolower($p["extension"]), array("css", "js"))) {
						$hash = md5($file.filemtime($file));
						$t = sprintf("%1\$scache/%3\$s_%2\$s.%3\$s.gz", ($GLOBALS["covide"]->temppath?$GLOBALS["covide"]->temppath:"tmp/cache"), $hash, $p["extension"]);
						if (!file_exists($t)) {
							if (!file_exists(dirname($t))) {
								/* 
								 * mkdir will (sometimes) fail to mode the given directory unless one run 
								 * umask(0) first, so let's do that.
								 *
								 * XXX: And why are we moding the directory 0777 anyway? The httpd created
								 * it and will be the owner of it, so 0777 is unnecessary (and feels unsafe.)
								 * -- sjk
								 */
								$old = umask(0);
								mkdir(dirname($t), 0755);
								umask($old);
							}

							// we do not use gzencode/compress, which does not work for this purpose
							$_fp = fopen($file, 'r');
							$_gz = gzopen($t, 'w9');
							
							// stream the contents to a compressed file using 8k blocks
							while (!feof($_fp)) 
								gzwrite($_gz, fread($_fp, 8192));
							
							fclose($_fp);
							gzclose($_gz);
							

							#exec(sprintf("gzip -n -f -c %s > %s",
							#	escapeshellarg($file),
							#	escapeshellarg($t)));
							chmod($t, 0644); /* Make sure the cache files can be read my the httpd -- sjk */
						}
						$gz = 1;
						$file = sprintf("tmp/cache/%s", basename($t));
					}
				}
			}
			$file = sprintf("%s%s", $webroot, $file);
			if (!$gz)
				$file = sprintf('fileloader/%d/%s', filemtime(($file2) ? $file2 : $file), $file);
		}
		return $file;
	}
	/* }}} */
	...
	public function load_javascript($file, $no_rewrite=0) {
		...
		if (!in_array($file, $loaded_scripts)) {
			...
			$this->output .= "<script type=\"text/javascript\" src=\"".$file."\"></script><!-- ".$file2." -->\n";
		} else {
			$this->output .= "<!-- script [$file2] already loaded -->\n";
		}
	}
	/* }}} */
	 ...
	 */
	public function start_javascript($cdata = true) {
	 ...
	 */
	public function end_javascript($cdata = true) {
		if ($cdata) {
			$this->output .= " /*]]>*/";
		}
		$this->output .= "</script>";
	...
	public function addTag($tag, $settings="", $skip_newline=0) {
		...
		if ($settings["id"]) {
			$ids =& $GLOBALS["covide"]->loaded_htmlids;
			if (!is_array($ids)) {
				$ids = array();
			}
			if (in_array($settings["id"], $ids)) {
				$this->addCode(sprintf("<!-- id [%s] is already used -->", $settings["id"]));
				$settings["id"] .= "__".md5( rand().$settings["id"].time() );
			}
			$ids[] = $settings["id"];
		}
		...
		switch ($tag) {
			...
			case "form":
				...
				if (!$settings["action"])
					$settings["action"] = "#";
				$settings["method"] = strtolower($settings["method"]);
				break;
			case "iframe":
				unset($settings["border"]);
				unset($settings["visiblity"]);
				break;
		}
		/* id cannot be zero */
		if (!$settings["id"])
	...
	public function layout_page($title="", $hide_navigation=0, $prefetch=0, $extra_header_code = "") {
		global $covide, $db;

		if (isset($_REQUEST["hide"]) && $hide_navigation == 0) {
			$this->_hide_navigation = $_REQUEST["hide"];
		} else {
	...
	public function layout_page_end($no_footer = 0) {
	...
	public function css($theme_overwrite=0) {
		...
		$this->addCode("<!-- themes/default/cms.css -->");
		$this->addCode("\n");
		...
		if (file_exists("themes/covide_print.css")) {
			$file = $this->external_file_cache_handler("themes/covide_print.css");
			$this->addTag("link", array(
				"rel"   => "stylesheet",
				"type"  => "text/css",
				"href"  => $file,
			...
			$this->addCode("<!-- themes/covide_print.css -->");
			$this->addCode("\n");
		}
		...
		if ($user_info["font"] || $user_info["fontsize"]) {
			$this->addCode("\n");
			$this->addTag("style", array("type" => "text/css"));
			$this->addCode("body, td, th, span, div, select, input, textarea, option, A { ");
			if ($user_info["font"]) {
				$this->addCode(sprintf("font-family: %s !important; ", $user_info["font"]));
			}
			if ($user_info["fontsize"]) {
				$conversion = new Layout_conversion();
				$fonts = $conversion->getFonts(2);
				$this->addCode(sprintf("font-size: %s !important; ", $fonts["sizes"][$user_info["fontsize"]]));
			}
			$this->addCode("}");
			$this->endTag("style");
		}
	...
	public function insertAction($action, $alt, $link, $id="", $no_mobile=0, $fader=0, $settings = "") {
		if (!$GLOBALS["covide"]->mobile || !$no_mobile) {
			if ($action) {
				/* 
					a very ugly hack to reduce memory usage up to 70%	
					php includes and the way covide works will eat up all
					the memory without freeing it. So we use static mode 
					as escape route here
				*/	
				require_once(self::include_dir.'actions.php');
				$img = Layout_html_actions::lookupAction($action, $alt, $link, $id, $no_mobile, $fader);
	...
	public function insertImage($image, $alt, $link="", $theme=0, $skip_encoding=0, $id="", $is_action="", $fader=0) {
		...
		if ($fader) {
			$settings_image["class"] = "imageconstanttransparancy";
		}
		if (!is_array($link)) {
	...
	public function insertCheckbox($name, $value, $checked=0, $view_compatible=0) {
		return $this->addCheckbox($name, $value, $checked, $view_compatible);
	}
	...
	public function addCheckbox($name, $value, $checked=0, $view_compatible=0) {
		...
		if ($view_compatible) {
			...
			$buf = array();
			$buf[] = "<input type='checkbox' class='inputcheckbox' name='";
			foreach ($name as $b) {
				$buf[] = $b;
			}
			$buf[] = "' id='";
			...
			$this->addTag("input", array("type"=>"checkbox", "id"=>$id, "name"=>$name, "checked"=>$checked, "value"=>$value, "class" => "inputcheckbox") );
		}
	}
	...
	public function addTextArea($name, $value, $settings="", $id="") {
		...
		$settings["onfocus"] = "handleClassFocus(this, 1);";
		$settings["onblur"]  = "handleClassFocus(this, 0);";
		if (!$settings["rows"]) $settings["rows"] = 3;
		if (!$settings["cols"]) $settings["cols"] = 80;
