...
Class Metafields {
	...
	public function __construct() {
		...
		switch ($_REQUEST["action"]) {
			...
			case "add_metagroup" :
				$meta_output = new Metafields_output();
				$meta_output->metagroup_add_field();
				break;

			case "save_add_metagroup" :
				$meta_data = new Metafields_data();
				$meta_data->metagroup_save_new($_REQUEST["metagroup"]);
				/* refresh parent */
				$output = new Layout_output();
				$output->start_javascript();
					$output->addCode("parent.location.href=parent.location.href;");
			...
			case "save_edit_metagroup" :
				$meta_data = new Metafields_data();
				$meta_data->metagroup_save_edit($_REQUEST["metagroup"]);
				/* refresh parent */
				$output = new Layout_output();
				$output->start_javascript();
					$output->addCode("parent.location.href=parent.location.href;");
				...
				$output->exit_buffer();
				break;
			case "remove_metagroup" :
				$meta_data = new Metafields_data();
				$meta_data->metagroup_remove($_REQUEST["id"]);
				break;

			case "edit_metagroup" :
				$meta_output = new Metafields_output();
				$meta_output->metagroup_edit($_REQUEST["id"]);
				break;
