<?php
	...
	Class Calendar {
		...
		public function __construct() {
			...
			} else if ($_REQUEST["action"] != "convert_to_new" &&
                             $_REQUEST["action"] != "externalcalendarsync" ){
			...
			switch ($_REQUEST["action"]) {
				...
				case "delete" :
					$calendar_data = new Calendar_data();
					$calendar_data->delete($_REQUEST["id"], $_REQUEST["user_id"]);
					$calendar_output = new Calendar_output();
				...
				case "print_month_mu" :
					...
					$user_data = new User_data();
					$userinfo = $user_data->getUserdetailsById($_SESSION["user_id"]);
					$calendar_output->print_month_mu( $userinfo["calendarselection"] );
					break;
				...
				default :
					...
					/* mode == 1 || mode == 4: daily view. Can be forced by url param or session var. if 3 then include weekoverview at bottom */
					if ($userinfo["calendarmode"] == 4 || $userinfo["calendarmode"] == 1 || $_REQUEST["forceday"] == 1 || $_SESSION["calendar_forceday"]) {
