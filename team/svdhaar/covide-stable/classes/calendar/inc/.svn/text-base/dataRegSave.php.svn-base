...
if ($batch_input) {
	...
	$hours = $regitem["hours"]+$regitem['minutes'];
	...
	if ($regitem["id"]) {
		$sql = sprintf("UPDATE hours_registration SET user_id = %d, project_id = %d, activity_id = %d, description = '%s', is_billable = %d, hours = %01.2f, purchase=%01.2f, marge = %01.2f, tarif = %01.2f, date = %d WHERE id = %d",
			$regitem["user_id"], $regitem["project_id"], $regitem["activity_id"], $regitem["description"], $regitem["is_billable"], $hours,
		...
		$sql .= sprintf("(%d, %d, %d, '%s', %d, %01.2f, %01.2f, %01.2f, %01.2f, %d)", $regitem["user_id"], $regitem["project_id"], $regitem["activity_id"], $regitem["description"], $regitem["is_billable"], $hours,
	...
	if ($regitem["id"]) {
		$sql = sprintf("UPDATE hours_registration SET activity_id = %d, user_id = %d, project_id = %d, price = %01.2f, purchase = %01.2f, marge = %01.2f, description = '%s', is_billable = %d, date = %d WHERE id = %d",
			$regitem["costid"], $regitem["user_id"], $regitem["project_id"], $regitem["price"], $regitem["purchase"], $regitem["marge"], $regitem["description"], $regitem["is_billable"], $regitem["date"], $regitem["id"]);
		$res = sql_query($sql);
		$insertid = $regitem["id"];
	} else {
		$sql  = "INSERT INTO hours_registration (activity_id, user_id, project_id, price, purchase, marge, description, is_billable, date) VALUES ";
		$sql .= sprintf("(%d, %d, %d, %01.2f, %01.2f, %01.2f, '%s', %d, %d)", $regitem["costid"], $regitem["user_id"], $regitem["project_id"], $regitem["price"], $regitem["purchase"], $regitem["marge"], $regitem["description"], $regitem["is_billable"], $regitem["date"]);
		$res = sql_query($sql);
		$insertid = sql_insert_id("hours_registration");
	}
} else {
...
if ($regitem["calendar_id"] && $regitem["update_calendar"] == 1) {
	...
	if ($calendaritem["is_repeat"] || $calendaritem["repeat_type"]) {
		$this->deleteonerep($calendaritem["id"], $_SESSION["user_id"], mktime(0, 0, 0, $regitem["month"], $regitem["day"], $regitem["year"]));
		...
		/* just update the start/end time, description and project */
		$sql = sprintf("UPDATE calendar SET timestamp_start = %d, timestamp_end = %d, body = '%s', project_id = %d WHERE id = %d",
			mktime($regitem["fromhour"], $regitem["fromminute"], 0, $regitem["month"], $regitem["day"], $regitem["year"]),
			mktime($regitem["tohour"], $regitem["tominute"], 0, $regitem["month"], $regitem["day"], $regitem["year"]),
			sql_escape_string($regitem["description"]),
			$regitem["project_id"],
			$regitem["calendar_id"]
		);
		sql_query($sql);
	...
	// small output to refresh form
	$output = new Layout_output();
	$output->start_javascript();
	$output->addCode(sprintf("document.location.href='index.php?mod=calendar&action=%s&id=0&timestamp=%d&project_id=%d&address_id=%d';", $reg_name ,time(), $regitem["project_id"], $regitem["user_id"]));
	$output->end_javascript();
	$output->exit_buffer();
}
?>
