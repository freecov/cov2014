					...
					$tableitems->addTableRow();
						...
						if ($v[$i] && !in_array($_temp_id, $p)) {
							...
							$tableitems->addTableData(array("rowspan" => $rowspan, "style" => "font-weight: normal;", "class" => "calendar_contentcolumn"), "header");
								...
								if (strpos($v[$ni], ",") !== false) {
									$j = explode(",", $v[$ni]);
									foreach ($j as $_item) {
										$tableitems->insertLink($calendar_items[$k][$_item]["human_span_short"] ."&nbsp;-&nbsp;". $calendar_items[$k][$_item]["subject"],
											array(
												"href" => "javascript: toonInfo(".$calendar_items[$k][$_item]["id"].", ".$calendar_items[$k][$_item]["user_id"].");"
											)
										);
										$tableitems->addTag("br");
									}
								} else {
									$tableitems->insertLink($calendar_items[$k][$v[$ni]]["human_span_short"] ."&nbsp;-&nbsp;". $calendar_items[$k][$v[$ni]]["subject"],
										array(
											"href" => "javascript: toonInfo(".$calendar_items[$k][$v[$ni]]["id"].", ".$calendar_items[$k][$v[$ni]]["user_id"].");"
										)
									);
								}
							$tableitems->endTableData();
