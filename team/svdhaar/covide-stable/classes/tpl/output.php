...
Class Tpl_output {
	...
	const SMARTY_FOLDER = 'smarty-3rc4';
	...
	public function exec_inline($id, $return=0) {
		...
		if (!in_array($data["category"], array("php", "main", "smarty"))) {
			...
			echo "script execution denied due wrong file type or no template specified: ".$data["category"].$this->crlf;
		}
		...
		if ($return) {
			...
			if ($data['category'] == 'smarty') {
				...
				$key = hash('md4', sprintf('%s_%s_%s', $_SERVER['HTTP_HOST'], $_SERVER['REQUEST_URI'], $_SESSION['locale']));
				...
				if ($smarty->register) {
					$smarty->register->outputFilter(array('tpl_smartyfilter', 'smarty_postfilter'));
					$smarty->security_class = 'Tpl_security';
					$smarty->enableSecurity();
					
				} else {
					$security = new Tpl_Security;
					$smarty->register_outputfilter(array('tpl_smartyfilter', 'smarty_postfilter'));
					$smarty->security_settings = array(
						'PHP_HANDLING'     => false,
						'IF_FUNCS'         => $security->php_functions,
						'INCLUDE_ANY'      => false,
						'PHP_TAGS'         => $security->allow_php_tag,
						'MODIFIER_FUNCS'   => $security->modifiers,
						'ALLOW_CONSTANTS'  => $security->allow_constants
					);
				...
				// empty the data
				$data = '';
				// if caching, try cached lookup
				if ($smarty->caching) {
					$data = $this->getApcCache($key);
					header(sprintf('ETag: %s', $key), true);
				}
				// if still no data, parse files
				if (!$data) {
	 ...
	 * @param string $separator Separator to separate the parts of the path
	 * @return void
	 */
	public function displayPath($pageid=0, $separator=" - " ) {
		if (!$pageid)
			$pageid = $this->pageid;
	...
	public function getPageThumb($id, $width, $height) {
		...
		preg_match_all("/<img[^>]*?>/sxi", $data, $imglist);
		...
		// if no thumb was found, check if this page inherits another one
		if ($page['isInherit'] && $page['inheritpage']) {
			return $this->getPageThumb($page['inheritpage'], $width, $height);
		}
		// as last resort, try the gallery
		if ($page['isGallery']) {
			$gallery = current($this->cms->getGallerydata($page['id']));
			if ($gallery) {
				$img = sprintf('<img src="/cmsgallery/page%d/%d&amp;size=list">', $gallery['pageid'], $gallery['id']);
				return $img;
			}
		}
		return false;
	...
	public function getPagesById($ids, $order="datePublication desc") {
		...
		$data = array();
		$q = sprintf("select id, pageHeader, pageTitle, datePublication, pageData, isActive, isPublic, isInherit, inheritpage from cms_data where id IN (%s)", "0".implode(",", $ids));
		if ($order)
	...
	private function checkMobile() {
		if ($_REQUEST['wurfltest']) {
			$_SESSION['text_redir'] = 0;
		}
		if ($_REQUEST['mode'] != 'text'	&& !$_SESSION['text_redir']) {
			$need_redir = false;

			// Check for mobile devices, use WURFL if possible

			include_once('classes/wurfl/TeraWurflConfig.php');
			if (class_exists('TeraWurflConfig', false)) {

				// Include the Tera-WURFL file
				require_once('classes/wurfl/TeraWurfl.php');

				// instantiate the Tera-WURFL object
				$wurflObj = new TeraWurfl();
				$match = $wurflObj->getDeviceCapabilitiesFromAgent($_SERVER['HTTP_USER_AGENT']);
			...
			if ($need_redir) {
				...
				if ($this->pageid == $this->default_page || !$this->pageid) {
			...
			// set redir check flag to prevent checking every time
			$_SESSION['text_redir'] = true;
		}
	}
	...
	private function handleAbbr(&$data, $no_inline_edit=0) {
		if (!$no_inline_edit) {
			...
			$repl = array();
			foreach ($cms as $k=>$c) {
				if (!$c['itemtype'] && in_array($this->language, $c["lang"])) {
		...
		if (count($this->has_abbr)) {
			...
			foreach ($cms as $k=>$v) {
				if (in_array($lang, $v["lang"]) && in_array($k, $this->has_abbr)) {
					$output->addCode(sprintf('<div class="tt_abbr" id="tt_abbr_%d"><b>%s</b>: %s</div>', $k, addslashes($v["abbreviation"]), addslashes($v["description"])));
				}
			}
			$output->addCode("\n<!-- end abbreviations -->\n");
			echo $output->generate_output();
	...
	private function getVeldValue($fieldname, $pageid) {
		if (preg_match("/^meta\d{1,}/si", $fieldname)) {
			// check for inherited meta data
			$q = sprintf('select useMetaData, isInherit, inheritpage from cms_data where id = %d', $pageid);
			$res = sql_query($q);
			$row = sql_fetch_assoc($res);
			if (!$row['useMetaData'] && $row['isInherit'] && $row['inheritpage']) {
				$pageid = $row['inheritpage'];
			}

			$name = (int)preg_replace("/^meta/si", "", $fieldname);
			$q = sprintf("select field_type from cms_metadef where id = %d", $name);
			$res = sql_query($q);
			$item = sql_result($res, 0, '', 2);
			if ($item == 'shop' && $this->cms_license["has_shop"]) {
				...
				$v = sql_result($res,0);
				if ($item == 'financial') {
					$v = sprintf('%s %s', $this->valuta, number_format($v, 2));
				}
				return $v;
			}
	...
	/* getPageMetadata {{{ */
	/**
	 * Gets and displays the metadata associated with the given pageid
	 *
	 * Usage: $template->getPageMetadata($pageid);
	 *
	 * @param int $id The pageid of the page from which we want to display the metadata information
	 *
	 * @return void
	 */
	public function getPageMetadata($id=0) {
		if ($this->disableMeta == 0) {
			return false;
		}
		if (!$id) {
			$id = $this->pageid;
	...
	public function googleMaps() {
		...
		if ($fetch) {
			...
			$res = sql_query($q);
			$row = sql_fetch_assoc($res);
			$last_modification = $row['lastchange'];
	...
	public function getPagesByMetadata($meta_criteria, $order="datePublication desc", $limit=0, $meta_operator="and") {
		...
		$suffix = 'select id, isInherit, inheritpage';
		$q = " from cms_data where (id IN (select pageid from cms_metadata where 1=1 ";
		...
		$q .= ')';
		$q2 = sprintf('select id %s) and %s', $q, $this->base_condition);;
		$q.= sprintf(" OR (isInherit = 1 and inheritpage IN (%s))) and apEnabled IN (%s) and %s",
			$q2, implode(",", $this->public_roots), $this->base_condition);
	...
	private function _log_visitor_login($data) {
		$sql = sprintf(
			"INSERT INTO cms_logins_log VALUES (%d, '%s', '%s', %d, '%s')",
			...
			case true:
				...
				switch ($this->doctype['version']) {
					...
					case 1.1:
						...
						break;
					default:
						$doctype = sprintf('<?xml version="1.0" encoding="utf-8"?>
							<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 %1$s//EN"
	...
	/* }}} */
        ...
        /* getSiteTitle {{{ */
        /**
         * Gets a translation from the translation table
         *
         * @author Stephan van de Haar <svdhaar@users.sourceforge.net>
         * @since 1.0, 01-11-2009
         * @return string the requested site title
		...
		return $this->cms->getTranslation($str);
        }
        /* }}} */
        /* printViaCss {{{ */
        /**
         * Prints directly via the javascript: print() option, styled by css
         *
         * @author Stephan van de Haar <svdhaar@users.sourceforge.net>
