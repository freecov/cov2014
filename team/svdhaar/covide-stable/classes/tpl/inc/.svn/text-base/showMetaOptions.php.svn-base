	...
	foreach ($data as $groupname=>$group) {
		...
		$tbl->addTableRow();
			$tbl->addTableData(array(
				...
				$tbl->addCode(sprintf("<b id='meta_categories'>category: %s<br><hr></b>", $this->getTranslation($groupname)));
			$tbl->endTableData();
		$tbl->endTableRow();
		...
		foreach ($group as $formitem) {
			...
			$tbl->addTableRow();
				$tbl->addTableData(array("style" => "vertical-align: top;"));
					...
					$tbl->start_javascript();
						$tbl->addCode(sprintf("
							document.getElementById('avail%1\$d').onclick = function() {
					...
					$tbl->addCode($this->getTranslation($formitem["field_name"]));
					$tbl->addCode(':');
					$tbl->addSpace(2);
				$tbl->endTableData();
				$tbl->addTableData(array(
