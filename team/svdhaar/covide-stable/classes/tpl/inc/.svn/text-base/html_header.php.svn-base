 ...
 */
	...
	switch ($rowmain["search_use_pagetitle"]) {
		case 1:
			$search_title .= sprintf(" - %s", $row["pageTitle"]);
			break;
		case 2:
	...
	<!-- Covide CMS : Cooperative Virtual Desktop (c) 2000-%d -->
	...
	$output->addTag("meta", array(
		"name"       => "description",
		"content"    => htmlentities(preg_replace("/(\n)|(\r)|()/si", "", $search_descr))
	));
	$output->addCode("\n\t");
	$output->addTag("meta", array(
		"name"       => "keywords",
	...
	if (!$this->textmode) {
		...
		$output->addCode("\t");
		$output->load_javascript("classes/html/inc/date.js");
		$output->addCode("\t");
		$output->load_javascript("classes/html/inc/jqueryplugins/jquery.datePicker.js");
		$output->addCode("\t");
		...
		$output->addCode("\n\t<![endif]-->\n\t");
			...
			"rel"  => "stylesheet",
			"type" => "text/css",
			"href" => $output->external_file_cache_handler("classes/tpl/inc/datePicker.css")
		));
	}
	$output->addCode("\n");
	if ($rowmain["google_verify"]) {
		$output->addCode("\n\t<!-- begin google analytics code -->\n\t");
		$output->addCode($this->googleAnalytics($rowmain));
		$output->addCode("\n\t<!-- end google analytics code -->\n");
	}

	$output->addCode("\n\t<!-- [user template specific js and css include] -->\n");
?>
