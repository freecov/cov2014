 ...
 */
	...
	$output->addCode("
		function formsubmit() {
			var stop = 0;
	");
	// if user is logged in, grab their email address
	if ($_SESSION["visitor_id"]) {
		$visitor_data = $this->cms->getAccountList($_SESSION["visitor_id"]);
		if (is_array($visitor_data[0]["address_data"]) && $visitor_data[0]["address_data"]["email"]) {
			$visitor_email = $visitor_data[0]["address_data"]["email"];
		} else {
			$visitor_email = $visitor_data[0]["email"];
		}
	} else {
		$visitor_email = "";
	}
		...
		if ($formitem["field_type"] != "hidden" && $formitem["is_required"]) {
			if ($formitem["field_type"] == "address" || $formitem["field_type"] == "crm") {
				...
				foreach ($fields as $fld=>$name) {
					if (in_array($fld, array("fax_nr", "infix", "birthday", "cla", "memo"))) continue;
					if ($fld == "email") {
						$echeck[1] = "echeck(";
						$echeck[2] = ")";
					} else {
	...
	if ($this->cms_license['recaptcha_public']) {
		$output->addCode(sprintf("
			if (stop == 0 && document.getElementById('recaptcha_response_field') && !document.getElementById('recaptcha_response_field').value) { stop = 1; alert('%s'); }
		", addslashes(gettext('Confirmation message is empty'))));
	}
	$output->addCode("
			if (stop == 0) {
		...
		function undoSubmit() {
			document.getElementById('form_submit_link').style.visibility = 'visible';
			Recaptcha.reload();
		}
	");
	$output->end_javascript();

	$tbl = new Layout_table(array('form_generator'));
	foreach ($forms as $formitem) {
		if ($formitem["field_type"] != "hidden") {
			...
			$tbl->addTableRow();
				$tbl->addTableData(array("style" => "vertical-align: top;", "class" => "frmLabel"));
				...
				$tbl->addTableData();
					switch ($formitem["field_type"]) {
						case "text":
							if ($formitem["is_mailfrom"]) {
								$fieldvalue = $visitor_email;
							} else {
								$fieldvalue = $formitem["field_value"];
							}
							$tbl->addTextField(sprintf("data[%s]", $formitem["field_name"]), $fieldvalue);
							break;
						case "upload":
							...
							break;
						case "textarea":
							$tbl->addTextArea(sprintf("data[%s]", $formitem["field_name"]), $formitem["field_value"]);
							break;
						case "select":
							$seltemp = explode("\n", $formitem["field_value"]);
							$sel     = array();
							...
							$tbl->addSelectField(sprintf("data[%s]", $formitem["field_name"]), $sel);
							break;
						case "multiselect":
							$seltemp = explode("\n", $formitem["field_value"]);
							$sel     = array();
							...
							$tbl->addSelectField(sprintf("data[%s][]", $formitem["field_name"]), $sel, array(), 1);
							break;
						case "checkbox":
							$sel = explode("\n", $formitem["field_value"]);
							foreach ($sel as $k=>$v) {
						...
						case "date":
							...
							$tbl->addTextField(
								sprintf("data[%s][d]", $formitem["field_name"]), 
								date('d-m-Y'), 
								array('class' => 'datepicker '),
								sprintf("data%s", $formitem["field_name"])
							);
							break;
						case "datetime":
							if (!$calendar)
						...
						case "address":
							...
							foreach ($fields as $fld=>$name) {
								...
								$tbl->endTableRow();
								$tbl->addTableRow();
									$tbl->insertTableData("- ".ucfirst(gettext($name)).":");
									$tbl->addTableData();
						...
						case "crm" :
							...
							if ($formsetting["user_insertmode"] == 1) {
								$fields = $this->crm_fields_minimal;
							}
							$tbl->addHiddenField(sprintf("data[%s]", "address_type"), $formsetting["address_type"]);
							$tbl->addHiddenField(sprintf("data[%s]", $formitem["field_name"]), 1);
							if (is_array($formsetting["default_classifications"])) {
								...
								$tbl->endTableRow();
										if ($fld == "companyname" && $formsetting["address_type"] == "bcard") {
											...
											$tbl->addTableRow();
												$tbl->insertTableData("- ".gettext($name).":", array("style" => "vertical-align: top;"));
												$tbl->addTableData();
												$tbl->addSelectField(sprintf("data[%s][%s]", $formitem["field_name"], $fld), $newCommencements);
										} elseif ($fld == "birthday") {
											$tbl->addTableRow();
												$tbl->insertTableData("- ".gettext($name).":", array("style" => "vertical-align: top;"));
												$tbl->addTableData();
	...
	if ($this->cms_license['recaptcha_public']) {
		$q = sprintf('select search_language from cms_data where id = %d', $this->pageid);
		$res = sql_query($q);
		$lang = sql_result($res,0);

		if (!$clang) {
			$settings = $this->cms->getCmsSettings($this->siteroot);
			$clang = $settings['search_language'];
		}
		switch ($clang) {
			case 'en':
			case 'nl':
		...
		if ($this->cms_license['recaptcha_theme']) {
			$theme = $this->cms_license['recaptcha_theme'];
		} else {
			$theme = 'red';
		}
		$tbl->addTableRow();
			$tbl->addTableData(array("style" => "vertical-align: top;"));
				$tbl->addCode(gettext('Confirmation').':');
			$tbl->endTableData();
			$tbl->addTableData();
				...
				", $theme, $lang));
				$tbl->end_javascript();
				$tbl->addCode(recaptcha_get_html($this->cms_license['recaptcha_public']));
			$tbl->endTableData();
		$tbl->endTableRow();
	}
	$tbl->addTableRow();
