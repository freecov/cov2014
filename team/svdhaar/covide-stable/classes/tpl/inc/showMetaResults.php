 ...
 */
	...
	foreach ($data as $k=>$v) {
		...
		$tbl->addTableRow();
			$tbl->addTableData();
				$tbl->addCode($this->getTranslation($field["field_name"]));
				$tbl->addCode(': ');
			$tbl->endTableData();
			$tbl->addTableData();
				$tbl->addCode($v);
	...
	if ($_REQUEST["order"]) {
		$orderby = sql_escape_string($_REQUEST["order"]);
	} else {
		$orderby = "datePublication desc";
	}

	$_data = array();
	$q = sprintf("select * from cms_data where id IN (%s) order by %s",
		$pages, $orderby);
	...
	$output->addHiddenField("order", $orderby);
