 ...
 */
	...
	if (preg_match("/[^\*]/si", $keywords)) {
		if ($id == "__search") {
			...
			echo sprintf('<li><a href="/search/?prev=%s">%s</a></li>',
				stripslashes($_REQUEST["keywords"]),
				gettext('search again')
			);
			$cms_license = $this->cms_license;
			if ($cms_license["cms_address"])
				echo sprintf("<li id='search_address'>".gettext("browse by <a href='/addressdata/'>address</a>")."</li>");

			if (!$this->disableFileSearch)
				echo sprintf("<li id='search_files'>".gettext("search inside files")." <a href='/filesearch/?prev=".str_replace("%", "%%", urlencode(stripslashes($_REQUEST["keywords"])))."'>".gettext("public files")."</a></li>");

			if ($cms_license["cms_meta"])
				$meta = $this->cms->getMetadataDefinitions(1);
				if (count($meta)) {
					echo sprintf("<li id='search_metadata'>".gettext("search by <a href='/metadata/'>category or metadata</a>"));
				}
		...
		$res = sql_query($q);
		echo "<a id=\"results\" class=\"anchor\"></a>";
		...
		$view->addMapping(gettext("page name"), "%%complex", "left", "cms_search_pagename");
		$view->addMapping(gettext("date"), "%datePublication_h", "left", "nowrap cms_search_date");
		if ($this->enable_ft_search)
		...
		if ($cms_license["cms_address"]) {
			...
			echo sprintf(" <a class='search_address' href='/addressdata/'><i>%s</i> %s</a><br /><br />", gettext("or"), gettext("search by address"));
		}
		if ($cms_license["cms_meta"]) {
			echo $this->insertAction("view_all", "", "");
			echo sprintf(" <a class='search_metadata' href='/metadata/'><i>%s</i> %s</a><br /><br />", gettext("or"), gettext("search by category or metadata"));
		}
		if (!$this->disableFileSearch) {
			echo $this->insertAction("ftype_text", "", "");
			echo sprintf(" <a class='search_address' href='/filesearch/'><i>%s</i> %s</a><br /><br />", gettext("or"), gettext("search inside files"));
		}
		echo "<br />";
	}
