...
function openGalleryItem(id, descr, lang) {
	var galleryitem = oldpopup('/showcms.php?html=1&size=medium&galleryid='+id+'&lang='+lang+'&description='+descr, 'galleryitem', 320, 300, 1);
}
...
function shopAdd(id) {
	var num = 0;
	var uri = '';
	var metaid = arguments[1];
	if (metaid) {
		uri = uri.concat('/mode/shopadd&id=', id, '&meta=', metaid, '&num=1');
		loadXML(uri);
	} else if (document.getElementById('shopcount')) {
...
function shopEdit(id, num, txt, meta) {
	...
	var uri = '';
	if (p) {
		uri = uri.concat('/mode/shopadd&id=', id, '&num=', p, '&reset=1&meta=', meta);
		loadXML(uri);
	}
}
function shopDel(id, txt, meta) {
	...
	if (p == true) {
		var uri = '';
		uri = uri.concat('/mode/shopadd&id=', id, '&num=0', '&reset=1&meta=', meta);
		loadXML(uri);
	}
}
...
function handleClassFocus(obj, state) { 
	obj.className = obj.className.replace(/cms_focus_field/g, '');
	if (state == 1) {
		obj.className = obj.className.concat(' cms_focus_field ');
	}
}

function page_loaded(s) {
	if (navigator.userAgent.indexOf('Gecko') != -1) 
		document.write('<link rel=\"stylesheet\" type=\"text/css\" href=\"/classes/tpl/inc/site_gecko.css\" />');
	else if (navigator.userAgent.indexOf('Opera') != -1) 
		document.write('<link rel=\"stylesheet\" type=\"text/css\" href=\"/classes/tpl/inc/site_opera.css\" />');
...
function load_abbr(n, obj) {
	var txt = document.getElementById('tt_abbr_' + n).innerHTML;
