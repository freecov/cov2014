 ...
 */
	...
	if ($req["system"]["is_shop"]) {
		$art = array();
		$total = 0;
		foreach ($_SESSION["shop"] as $id=>$item) {
	...
	// recaptcha
	if ($this->cms_license['recaptcha_private']) {
		require('classes/recaptcha/recaptchalib.php');
		$response = recaptcha_check_answer (
			$this->cms_license['recaptcha_private'],
			$_SERVER["REMOTE_ADDR"],
	...
	if ($num == 1 && $response->is_valid == true) {
		$forms = $this->cms->getFormData($req["system"]["pageid"]);
		if ($req["system"]["activepageid"]) {
			$req["system"]["pageid"] = $req["system"]["activepageid"];
		}
		...
		$uri  = sprintf("%s%s%s%s", $this->protocol, $_SERVER["HTTP_HOST"], 
			($this->page_less_rewrites) ? "/":"/page/",
			$this->checkAlias($req["system"]["pageid"]));
		$uri2 = sprintf("<a target=\"_blank\" href=\"http://%s/\">%s</a>",
			$this->http_host, $this->http_host);
		$uri3 = sprintf("%s%s%s/", $this->protocol, ($this->page_less_rewrites) ? "/":"/page/", $_SERVER["HTTP_HOST"]);
		...
		foreach ($forms as $k=>$v) {
			...
			if ($v["field_type"] == "address" || $v["field_type"] == "crm") {
			...
			if ($isspecial == 0) {
				...
				if ($v["field_type"] == "checkbox") {
				...
				} elseif ($v["field_type"] == "multiselect") {
					$req["data"][$v["field_name"]] = implode(", ", $req["data"][$v["field_name"]]);
					...
					foreach ($fields as $fld=>$name) {
						$t2->addTableRow();
							...
							if ($fld == "email")
								$t2->insertTableData(sprintf("<a href='mailto:%1\$s'>%1\$s</a>", $req["data"][$v["field_name"]][$fld]));
							else
								$t2->insertTableData($req["data"][$v["field_name"]][$fld]);
					...
					$address_type = $req["data"]["address_type"];
					foreach ($fields as $key=>$val) {
						if ($key == "companyname") {
							$address_record["companyname"] = $req["data"][$v["field_name"]][$key];
						} elseif ($key == "tav") {
							$address_record["surname"] = $req["data"][$v["field_name"]][$key];
						} elseif ($key == "givenname") {
							if ($address_type == "relations") {
								$address_record["contact_givenname"] = $req["data"][$v["field_name"]][$key];
							} else {
								$address_record["givenname"] = $req["data"][$v["field_name"]][$key];
							}
						} elseif ($key == "infix") {
							if ($address_type == "relations") {
								$address_record["contact_infix"] = $req["data"][$v["field_name"]][$key];
							} else {
								$address_record["infix"] = $req["data"][$v["field_name"]][$key];
							}
						} elseif ($key == "surname") {
							if ($address_type == "relations") {
								$address_record["contact_surname"] = $req["data"][$v["field_name"]][$key];
							} else {
								$address_record["surname"] = $req["data"][$v["field_name"]][$key];
							}
						} elseif ($key == "address") {
							if ($address_type == "relations") {
								$address_record["address"] = $req["data"][$v["field_name"]][$key];
							} else {
								$address_record["private_address"] = $req["data"][$v["field_name"]][$key];
							}
						} elseif ($key == "zipcode") {
							if ($address_type == "relations") {
								$address_record["zipcode"] = $req["data"][$v["field_name"]][$key];
							} else {
								$address_record["personal_zipcode"] = $req["data"][$v["field_name"]][$key];
							}
						} elseif ($key == "city") {
							if ($address_type == "relations") {
								$address_record["city"] = $req["data"][$v["field_name"]][$key];
							} else {
								$address_record["personal_city"] = $req["data"][$v["field_name"]][$key];
							}
						} elseif ($key == "country") {
							if ($address_type == "relations") {
								$address_record["country"] = $req["data"][$v["field_name"]][$key];
							} else {
								$address_record["personal_country"] = $req["data"][$v["field_name"]][$key];
							}
						} elseif ($key == "phone_nr") {
							if ($address_type == "relations") {
								$address_record["phone_nr"] = $req["data"][$v["field_name"]][$key];
							} else {
								$address_record["personal_phone_nr"] = $req["data"][$v["field_name"]][$key];
							}
						} elseif ($key == "fax_nr") {
							if ($address_type == "relations") {
								$address_record["fax_nr"] = $req["data"][$v["field_name"]][$key];
							} else {
								$address_record["personal_fax_nr"] = $req["data"][$v["field_name"]][$key];
							}
						} elseif ($key == "email") {
							$email_address = $req["data"][$v["field_name"]][$key];
							if ($address_type == "relations") {
								$address_record["email"] = $req["data"][$v["field_name"]][$key];
							} else {
								$address_record["personal_email"] = $req["data"][$v["field_name"]][$key];
							}
						} elseif ($key == "birthday") {
							$address_record["bday_year"] = $req["data"][$v["field_name"]]["bday_year"];
							$address_record["bday_month"] = $req["data"][$v["field_name"]]["bday_month"];
							$address_record["bday_day"] = $req["data"][$v["field_name"]]["bday_day"];
						} elseif ($key == "commencement") {
							$address_record["contact_commencement"] = $req["data"][$v["field_name"]]["commencement"];
						} elseif ($key == "ssn") {
							$address_record["ssn"] = $req["data"][$v["field_name"]][$key];
						} elseif ($key == "function") {
							$address_record["jobtitle"] = $req["data"][$v["field_name"]][$key];
						}
					}
					/* check if company name was set, else construct it */
					if (!$address_record["companyname"]) {
						$address_record["companyname"] = preg_replace("/ {1,}/s", " ", sprintf("%s, %s %s",
							$address_record["contact_surname"],
							$address_record["contact_infix"],
							$address_record["contact_givenname"]
						));
					}
					$address_record["is_active"] = -1;
					$cla = array();
					//add classifications to the address record.
					foreach ($req["data"][$v["field_name"]] as $fldkey => $fldvalue) {
						if (strstr($fldkey, "user_classifications")) {
							$cla = array_merge($cla, $req["data"][$v["field_name"]][$fldkey]);
						}
					}
					if (is_array($req["data"][$v["field_name"]]["default_classifications"])) {
						$cla = array_merge($cla, $req["data"][$v["field_name"]]["default_classifications"]);
					}
					/*
					if (is_array($req["data"][$v["field_name"]]["user_classification"])) {
						$cla = array_merge($cla, $req["data"][$v["field_name"]]["user_classification"]);
					}
					 */
					$address_record["classification"] = "|".implode("|", $cla)."|";
					// store in database
					$address_data = new Address_data();
					//check what kind of address record this should be.
					// Currently supported: relation and bcard
					$address_record["is_public"] = 1;
					if ($address_type == "relations") {
						$address_record["is_company"] = 1;
						$address_record["is_customer"] = 1;
						$address_record["type"] = "relations";
						$res = $address_data->store2db($address_record, array(), 1);
						/*
						//prepare cms user data
						$cms_user_data = array(
							"cms" => array(
								"email" => $email_address,
								"username" => $address_record["companyname"],
								"password" => "1234",
								"is_enabled" => 1,
								"is_active" => 1,
								"address_id" => $res
						...
						$this->cms->saveAccount($cms_user_data);
						 */
					} else {
						$address_record["type"] = "bcards";
						$address_record["address_id"] = $req["data"][$v["field_name"]]["address_id"];
						print_r($address_record);
						$res = $address_data->save_bcard($address_record, array(), 1);
					}
					$crm_address_id = $res;

					$req["data"][$v["field_name"]] = $t2->generate_output();
					//$req["data"][$v["field_name"]] = "";
				} elseif ($v["field_type"] == "hidden") {
					$req["data"][$v["field_name"]] = htmlentities($v["field_value"], ENT_NOQUOTES, "UTF-8");
				}

				if (!$v["is_mailfrom"] && !$v["is_mailto"] && !$v["field_type"] == "address" && !$v["field_type"] == "crm") {
					if (is_array($req["data"][$v["field_name"]])) {
						$req["data"][$v["field_name"]] = implode(",", $req["data"][$v["field_name"]]);
					}
					$req["data"][$v["field_name"]] = str_replace(htmlentities($this->valuta), $this->valuta, nl2br(htmlentities($req["data"][$v["field_name"]], ENT_NOQUOTES, "UTF-8")));
				}
		...
		// shop form. articles are in an array
		if (is_array($art)) {
			$html.= "<br>";
			$html.= "<table class='table1' style='background-color: white;'>";
			$html.= sprintf("
		...
		} elseif ($crm_address_id) {
			$address_id = $crm_address_id;
		}
		...
		if ($crmform != 1) {
			...
			$req["mail"] = array(
				...
				"to"         => $smtp["from"],
				"rcpt"       => $smtp["from"],
				"subject"    => sprintf("[%s] %s (%s)", $this->http_host,
					$smtp["subject"], gettext("copy of message"))
			);
			$req["contents"] = $html;
		...
		if ($req["system"]["is_shop"] && in_array($this->cms_license["ideal_type"], array("rabolite", "ogone"))) {
			$next_order = -1;
			$expected_order = 0;
			...
			if ($this->cms_license["ideal_type"] == "rabolite") {
				/* ideal integration */
				...
				$ts = date("Y-m-d\TG:i:s\Z", time()+(5*60)); //5 minutes
			...
			if ($this->cms_license["ideal_type"] == "ogone") {
				...
				/* ogone integration */
				$ideal->addTag("form", array(
					"action" => sprintf("https://secure.ogone.com/ncol/%s/orderstandard.asp", ($this->cms_license["ideal_test_mode"]) ? "test":"prod"),
					"method" => "post",
					"id"     => "idealfrm",
					"target" => "_top" //sprintf("ideal_%s", $win)
				));
				...
				$ideal->addHiddenField("PSPID", $this->cms_license["ideal_merchant_id"]);

				$ideal->addHiddenField("amount",      $this->ideal_filter((int)($total*100), 12));
				$ideal->addHiddenField("orderID",     $this->ideal_filter($ordernr, 16));
				$ideal->addHiddenField("language",    ($this->language) ? $this->language:"");
				$ideal->addHiddenField("currency",    "EUR");
				...
				/* locate the first address block */
				$address = "";
				foreach ($forms as $fv) {
					if ($fv["field_type"] == "address" && !$address) {
						$address = $_REQUEST["data"][$fv["field_name"]];
					}
				}
				$ideal->addHiddenField("CN",           $address["tav"]);
				$ideal->addHiddenField("EMAIL",        $address["email"]);
				$ideal->addHiddenField("ownerZIP",     $this->ideal_filter($address["zipcode"], 10));
				$ideal->addHiddenField("owneraddress", $address["address"]);
				$ideal->addHiddenField("ownercty",     $address["country"]);
				$ideal->addHiddenField("ownertown",    $this->ideal_filter($address["city"], 25));
				$ideal->addHiddenField("ownertelno",   $address["phone_nr"]);

				$hash = "";
				$hash = sprintf("%s%s%s%s%s",
					$this->ideal_filter($ordernr, 16),
					$this->ideal_filter((int)($total*100), 12),
					"EUR",
					$this->cms_license["ideal_merchant_id"],
					$this->cms_license["ideal_secret_key"]
				);

				$hash = str_replace(array("\t", "\n", "\r", " "), "", $hash);
				$hash = strtoupper(sha1($hash, false));

				$ideal->addHiddenField("SHASign", $this->ideal_filter($hash, 50));
				$ideal->start_javascript();
					$ideal->addCode(sprintf("
						setTimeout('document.getElementById(\"idealfrm\").submit();', 1000);
					", $win));
		...
		if ($ideal) {
			...
			if (preg_match("/^index\.php\?id=\d{1,}/si", $t))
				$smtp["result"] = preg_replace("/^(.*)index\.php\?id=(\d{1,})/si", $p."$2", $t);
		...
		$output->start_javascript();
			...
			$output->addCode('parent.undoSubmit();');
		$output->end_javascript();
		echo $output->generate_output();
