 ...
 */
	...
	if ($this->checkInternalRequest($id)==1) {
		switch ($id) {
			...
			case "__err404":
			case "__err410":
				$this->triggerError(preg_replace('/[^0-9]/s', '', $id));
		...
		} elseif (!$this->disableMeta) {
			// check if this page inherits something
			if ($page['isInherit'] && $page['inheritpage']) {
				$data .= $this->handleMetaData($data, $page['inheritpage']);
			}
		...
		} elseif ($page['isInherit'] && $page['inheritpage']) {
			$this->handleGallery($data, $page['inheritpage']);
		}
