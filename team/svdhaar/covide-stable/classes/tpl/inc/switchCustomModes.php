	...
	switch ($_REQUEST["mode"]) {
		case "shopadd":
			if (!$_REQUEST["id"]) {
				...
				if ($_REQUEST['meta']) {
					$q = sprintf('select value from cms_metadata where pageid = %d and fieldid = %d', 
						$_REQUEST['id'], $_REQUEST['meta']);
				} else {
				...
				if ($price <= 0) {
					...
					if (!is_array($_SESSION['shop'][(int)$_REQUEST['id']])) {
						$_SESSION['shop'][(int)$_REQUEST['id']] = array();
					}
	
					if ($_REQUEST["reset"]) {
						if ($_REQUEST["num"] == 0) {
							unset($_SESSION["shop"][(int)$_REQUEST["id"]][(int)$_REQUEST['meta']]);
						} else {
							$_SESSION["shop"][(int)$_REQUEST["id"]][(int)$_REQUEST['meta']] = 0;
						}
					}
					if ($_REQUEST["num"] > 0) {
						$_SESSION["shop"][(int)$_REQUEST["id"]][(int)$_REQUEST['meta']] += (int)$_REQUEST["num"];
					}
					if ($_REQUEST["reset"])
