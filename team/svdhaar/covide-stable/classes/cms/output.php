...
Class Cms_output {
	...
	private $editarea_url = "editarea0820/edit_area";
	...
	public function addMenuItems($venster) {
		...
		if ($cms_license["cms_changelist"] || $cms_license["cms_versioncontrol"]) {
			$venster->addMenuItem(gettext("page history"), "?mod=cms&action=pagehistory&id=".$_REQUEST["id"]);
		}
		...
		if ($cms_license['cms_searchengine']) {
			$venster->addMenuItem(gettext('keyword density'), sprintf('?mod=cms&action=keyworddensity&id=%d', $_REQUEST['id']));
			$venster->addMenuItem(gettext('text-only preview'), sprintf('?mod=cms&action=textpreview&id=%d', $_REQUEST['id']));
		}	
	...
	private function switchFieldType(&$tbl, &$v) {
		...
		switch ($v["field_type"]) {
			case "shop":
				$tbl->addCode(gettext("leave empty for article default or specify custom value"));
				$tbl->addTag('br');
			case "financial":
			...
			case "numeric":
				if ($output_only) {
					...
					if ($v['field_type'] != 'text') {
						$tbl->addTextField(sprintf($meta."[%s]", $v["id"]), $v["value"], array(
							"onchange" => "this.value = this.value.replace(/,/g, '.').replace(/[^0-9\.]/g, '');"
						));
					} else {
	...
	public function editAbbreviations() {
		...
		$view->addMapping( gettext("text"), "%abbreviation" );
		$view->addMapping( gettext("replacement"), "%description" );
	...
	/* cmsPageHistory {{{ */
	/**
	 * Show list of changes to the page
	 *
	 * @param int $id The page to list the changes for
	 */
	public function cmsPageHistory($id) {
		$output = new Layout_output();
		$output->layout_page("cms", 1);

		$venster = new Layout_venster(array(
			"title" => gettext("CMS"),
			"subtitle" => gettext("history for this page")
		));

		$cms_data = new Cms_data();
		$cms = $cms_data->getPageHistory($id);
		...
		$view->defineComplexMapping("complex_actions", array(
			array(
				...
				"src"  => "delete",
				"alt"  => gettext("delete"),
				"link" => array("?mod=cms&action=deletepagehistory&itemid=", "%id", "&id=", $id)
			),
			array(
				"type" => "action",
				"src"  => "ok",
				"alt"  => gettext("restore this version of the page"),
				"link" => array("javascript: restore_revision(", $id, ",", "%id", ");"),
			),
		));
		...
		$output->start_javascript();
		$output->addCode("
			function restore_revision(pageid, revision) {
				if (confirm(gettext('Are you sure you want to revert to this version? All changes after this version will be discarded.'))) {
					document.location.href='?mod=cms&action=restorepagehistory&itemid='+revision+'&id='+pageid;
				}
			}
		");
		$output->end_javascript();
	...
	/* }}} */
	/* keywordDensity {{{ */
	public function keywordDensity($id) {
		require_once(self::include_dir."cmsKeywordDensity.php");
		cmsKeywordDensity::run($id, $this);
	}
	/* }}} */
	/* textPreview {{{ */
	public function textPreview($id) {
		require_once(self::include_dir."cmsTextPreview.php");
		cmsTextPreview::run($id, $this);
	}
	/* }}} */
	/* loginLog {{{ */
	/**
	...
	public function loginLog() {
		$cms_data = new Cms_data();
		$logins = $cms_data->getLoginLog();
		$output = new Layout_output();
		$output->layout_page("cms", 1);
		$venster = new Layout_venster(array("title" => "CMS", "subtitle" => gettext("login log")));
		$venster->addVensterData();
		$view = new Layout_view();
		$view->addData($logins);
		$view->addMapping(gettext("username"), "%username");
		$view->addMapping(gettext("date"), "%h_timestamp");
		$view->addMapping(gettext("ip"), "%remote_addr");
		$venster->addCode($view->generate_output());
		$venster->endVensterData();
		$output->addCode($venster->generate_output());
		$output->layout_page_end();
