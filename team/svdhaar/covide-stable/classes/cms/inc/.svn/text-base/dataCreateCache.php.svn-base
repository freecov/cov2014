 ...
 */
	...
	$cache_file_list = sprintf("%s/%s/%s/%d_%s.jpg",
		$GLOBALS["covide"]->filesyspath, "gallery", $pageid, $file_id, "list");
	...
	// thumbnail size for lists
	if ($width*2 > $height) {
		if ($width > 100) {
			$scale_list = (100/$width);
		} else {
			$scale_list = "1";
		}
	} else {
		if ($height > 50) {
			$scale_list = (50/$height);
		} else {
			$scale_list = "1";
		}
	}
	//new w+h for thumbnail list
	$w = (int)($scale_list*$width);
	$h = (int)($scale_list*$height);
	...
	imagejpeg($dest, $cache_file_list);
	imagedestroy($dest);
