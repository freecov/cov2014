 ...
 */
	...
	$venster = new Layout_venster(array(
		"title" => gettext("CMS"),
		"subtitle" => sprintf('%s [%d]', gettext("Edit page"), $_REQUEST['id'])
	));
	...
	if ($cms["isSource"] && !$_REQUEST["noredir"] && $_REQUEST["action"] != "editpagetext") {
		header("Location: ?mod=cms&action=editpagetext&id=".$_REQUEST["id"]);
		exit();
	}
	if ($cms["isSticky"]) {
	...
	if ($cms["autosave_info"]) {
		// this detection behaves wrong!
		#if ($cms["autosave_data"] == $cms["pageData"])
		#	$no_change = 0;

		// calculate levensteihn difference, but levensteihn is horrible slow after more than 10k bytes ..
		$p = array();
		$t1 = preg_split('/.{10000}/s', $cms['pageData'], -1, PREG_SPLIT_DELIM_CAPTURE + PREG_SPLIT_NO_EMPTY);
		$t2 = preg_split('/.{10000}/s', $cms['autosave_data'], -1, PREG_SPLIT_DELIM_CAPTURE + PREG_SPLIT_NO_EMPTY);
		foreach ($t1 as $k=>$v) {
			similar_text($v, $t2[$k], &$lv);
			$p[] = $lv;
		}
		...
		$tbl->addTableRow(array("id" => "autosave_info", "style" => ($no_change)?"display:none;":""));
			...
			$tbl->addTag("div", array("style" => "border: 1px dashed red; padding: 2px;"));
				...
				//similar_text($cms["autosave_data"], $cms["pageData"], $p);
				//$p = number_format(100-$p, 2);
				//$tbl->addCode(" (".$p."% ".gettext("of the content has been changed").")");
				$tbl->endTag("b");
				$tbl->addTag("br");
				$tbl->addTag("br");
				$tbl->addCode(gettext('The text changed').': ');
				$tbl->insertTag('b', sprintf('%s%%. ', $p)); 
				...
				$tbl->endTag("iframe");
				if ($no_change) {
					$auto_init_truncaterestorepoint = 1;
				}
	...
	$tbl->addTableRow();
		...
		$tbl->addTableData("", "data");
			...
			if(!$cms["timestamp"]) { $cms["timestamp"] = time(); }
	...
	$tbl->addTableRow();
		...
		$tbl->addTableData(array("valign" => "top"), "header");
			...
			$tbl->addCode(' / ');
			$tbl->addCode(gettext('order'));
		$tbl->endTableData();
		$tbl->addTableData(array("valign" => "top"), "data");
			$tbl->addTextField("cms[pageLabel]", $cms["pageLabel"], array("style"=>"width: 240px;"));
		$tbl->endTableData();
	$tbl->endTableRow();
	...
	if ($_REQUEST["id"]) {
		$tbl->addTableRow();
			$tbl->addTableData(array("valign" => "top"), "header");
				$tbl->addCode(gettext("page header"));
			$tbl->endTableData();
			$tbl->addTableData(array("colspan" => 3), "data");
				$tbl->addTextArea("cms[pageHeader]", $cms["pageHeader"], array("style"=>"width: 100%; height: 60px;"));
			$tbl->endTableData();
		$tbl->endTableRow();
	}
	...
	if ($_REQUEST["id"]) {
		if ($text_only) {
			...
			// Tidy
			#$tidy = new Tidy();
			#$tidy->parseString($cms["pageData"], $config, 'utf8');
			...
			$cms["pageData"] = $email->stripBodyTags($cms["pageData"]);
			$cms["pageData"] = str_replace(array("<", ">"), array("\n<", ">\n"), $cms["pageData"]);
			$cms["pageData"] = preg_replace("/\n{1,}/s", "\n", $cms["pageData"]);
		}
		...
		if (!$text_only) {
			...
			$type = ($_REQUEST['font_toolbar']) ? 0 : 2;
			$venster->addCode( $editor->generate_editor($type, $cms["pageData"]) );
			$venster->addHiddenField("cms[isSource]", $cms["isSource"]);
		} else {
			// edit this page in source editor default from now on
			$venster->addCheckBox("cms[isSource]", 1, ($cms["isSource"]) ? 1 : 0);
			$venster->addSpace();
			$venster->addCode(gettext("Edit this page by default in source mode from now on"));
			$venster->addTag("br");
			$venster->addTag("br");

			$venster->addCode($this->highlight_show("php", "contents"));
			$venster->start_javascript();
				$venster->addCode("function saveSettings() { savePage(); }");
		...
		if (!$text_only) {
			...
			// enable the font toolbar
			if (!$_REQUEST['font_toolbar']) {
				$venster->insertAction('view_all', gettext('show font toolbar'), '?'.$_SERVER['QUERY_STRING'].'&font_toolbar=1');
			}
		}	
	...
	if ($auto_init_truncaterestorepoint) {
		$output->start_javascript();
			//$output->addCode("addLoadEvent(truncateRestorePoint());");
		$output->end_javascript();
	}
