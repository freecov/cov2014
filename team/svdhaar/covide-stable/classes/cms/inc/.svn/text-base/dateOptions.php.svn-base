	...
	$venster->addVensterData();
		...
		$tbl->addTableRow();
			$tbl->addTableData(array("colspan"=>2), "header");
				...
				$tbl->addSpace();
				$tbl->addCode(gettext("publicationdate ranges"));
			$tbl->endTableData();
		$tbl->endTableRow();
		$tbl->addTableRow();
		...
		$tbl->addTableRow();
			$tbl->addTableData(array("colspan"=>2), "header");
				...
				$tbl->addSpace();
				$tbl->addCode(gettext("events in calendar"));
			$tbl->endTableData();
		$tbl->endTableRow();
		...
		$tbl->addTableRow();
			$tbl->addTableData(array("colspan"=>2), "data");
				...
				$view->addMapping( gettext("from"), "%date_begin_h" );
				$view->addMapping( gettext("till"), "%date_end_h" );
				$view->addMapping( gettext("repeating"), "%repeating_h" );
				...
				$view->defineComplexMapping("complex_actions", array(
					array(
						...
						"link" => array("javascript: popup('?mod=cms&action=dateOptionsItemEdit&pageid=".$id."&id=", "%id", "', 'settings', 640, 480, 1);")
					),
					array(
						"type" => "action",
						"src"  => "delete",
						"alt"  => gettext("delete"),
						"link" => array("javascript: if (confirm(gettext('Weet u zeker dat u deze entry wilt verwijderen?'))) document.location.href='index.php?mod=cms&action=dateOptionsItemDelete&id=", "%id", "&page_id=", $id, "';")
					)
				));
				$cms_license = $cms_data->getCmsSettings();
				if ($cms_license["cms_date"]) {
					$tbl->addCode($view->generate_output());
				} else {
					$tbl->addCode(gettext("This module is disabled and is not available"));
				}
		...
		if ($cms_license["cms_date"])
			$venster->insertAction("new", gettext("new item"), "javascript: popup('?mod=cms&action=dateOptionsItemEdit&pageid=".$id."', 'settings', 640, 480, 1);");
