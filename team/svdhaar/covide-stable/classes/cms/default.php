...
Class Cms {
	...
	public function __construct() {
		$exceptions = array("recoverAccountPassword", "registerAccount", "registerAccountConfirm", "initdb");
		if (!$_SESSION["user_id"] && !in_array($_REQUEST["action"], $exceptions)) {
			$GLOBALS["covide"]->trigger_login();
		} else if (!in_array($_REQUEST["action"], $exceptions)) {
		...
		switch ($_REQUEST["action"]) {
			...
			case "keyworddensity" :
				$cms_output = new Cms_output();
				$cms_output->keywordDensity($_REQUEST['id']);
				break;
			case "textpreview" :
				$cms_output = new Cms_output();
				$cms_output->textPreview($_REQUEST['id']);
				break;
			case "loginlog" :
				$cms_output = new Cms_output();
				$cms_output->loginLog();
