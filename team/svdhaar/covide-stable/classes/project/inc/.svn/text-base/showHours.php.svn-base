...
if (!$this->has_declaration) {
	...
	$view_misclist->addMapping(gettext("date"), "%human_date", array("class" => "project_date"));
...
$venster_main->addVensterData();
	...
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			$table->insertAction("folder_open", gettext("view"), array("href" => "javascript: popup('index.php?mod=filesys&action=opendir&id=".$filesysfolder."&hide=1', 'showfiles', 0, 0, 1);"));
	...
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			$table->insertAction("mail_forward", gettext("view"), array("href" => "javascript: popup('index.php?mod=email&folder_id=".$archive_mail["id"]."&project_id=".$projectinfo[0]["id"]."&hide=1','showmail',0,0,1);"));
	...
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			$table->insertAction("support", gettext("view"), array("href" => "javascript: popup('index.php?mod=support&search[project_id]=".$projectinfo[0]["id"]."&hide=1','showsupport',0,0,1);"));
	...
	if ($_REQUEST["user"]) {
		...
		$table->addTableRow();
			...
			$table->addTableData();
				...
				$table->addCode( $useroutput->user_selection("user", $user, 1, 0, 0, 1, 1, 0, 1) );
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
