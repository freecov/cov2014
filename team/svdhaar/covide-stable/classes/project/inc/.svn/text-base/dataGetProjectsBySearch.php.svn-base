	...
	if ($options["address_id"]) {
		...
		if ($options["searchkey"]) {
			...
			if ($GLOBALS["covide"]->license["has_project_ext"]) {
				...
				$rjoin        = $projectext->prepareMetaSearchQuery($options["searchkey"]);
				$extjoin      =& $rjoin["join"];
				$extcondition =& $rjoin["cond"];
			}
			$like_syntax = sql_syntax("like");
			/* normal standalone projects */
			$query_sub  = sprintf("select project.* from project %2\$s WHERE is_active ".$is_active." AND group_id = 0 ".$sq." %3\$s %4\$s
				(name $like_syntax '%%%1\$s%%' OR description $like_syntax '%%%1\$s%%') ORDER BY lower(name)", $options["searchkey"], $extjoin, $extcondition, ($_REQUEST["projectext"]["metatype"])?"OR":"AND");

			/* normal projects with master project */
			$query_sub_master  = sprintf("select project.* from project %2\$s WHERE is_active ".$is_active." AND group_id > 0 ".$sq." %3\$s %4\$s
				(name $like_syntax '%%%1\$s%%' OR description $like_syntax '%%%1\$s%%') GROUP BY group_id", $options["searchkey"], $extjoin, $extcondition, ($_REQUEST["projectext"]["metatype"])?"OR":"AND");
			...
			$query_main = sprintf("select * from projects_master WHERE (is_active ".$is_active." ".$sq."
				AND (name $like_syntax '%%%1\$s%%' OR description $like_syntax '%%%1\$s%%')
					...
					id IN (
						select group_id from project WHERE is_active ".$is_active." AND group_id > 0 ".$sq."
					)
				AND (name $like_syntax '%%%1\$s%%' OR description $like_syntax '%%%1\$s%%'))
				order by lower(name)", $options["searchkey"]);

			$query_sub_count = str_replace("select project.*", "select count(*)", $query_sub);
