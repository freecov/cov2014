...
if (!$_REQUEST["from_day"] && !$_REQUEST["t_from"]) {
	...
	if ($user_info["xs_projectmanage"]) {
		$venster->addMenuItem(gettext("payroll export list"), "index.php?mod=project&action=list_payrollexports");
	}
		...
		foreach ($projectinfo["users"] as $userid=>$userinfo) {
			$table->addTableRow();
				$table->addTableData("", "", "data");
					$table->insertLink($user_data->getUsernameById($userid), array("href"=>"index.php?mod=project&action=hour_stats&user_id=".$userid."&t_from=".$timestamp_from."&t_to=".$timestamp_to));
				$table->endTableData();
