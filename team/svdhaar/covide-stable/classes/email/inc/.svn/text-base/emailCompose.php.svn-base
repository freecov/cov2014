 ...
 */
	...
	$output->addHiddenField("campaign_id", $_REQUEST["campaign"]);
	...
	if ($newsletter["count"] > 0) {
		...
		$table->addTableRow();
			...
			$table->addTableData("", "data");
				...
				$table->insertAction("state_special", gettext("add a classification"), "javascript: popup('?mod=email&action=selectCla&field=mailrcpt', 'search_address');");
			$table->endTableData();
		$table->endTableRow();
		/* cc */
		$table->addTableRow(array(
			...
			$table->addTableData("", "data");
				...
				$table->insertAction("state_special", gettext("add a classification"), "javascript: popup('?mod=email&action=selectCla&field=mailcc', 'search_address');");
			$table->endTableData();
		$table->endTableRow();
		/* bcc */
		$table->addTableRow(array(
			...
			$table->addTableData("", "data");
				...
				$table->insertAction("state_special", gettext("add a classification"), "javascript: popup('?mod=email&action=selectCla&field=mailbcc', 'search_address');");
			$table->endTableData();
		$table->endTableRow();
		/* toggle */
		$table->addTableRow(array(
	...
	if (!$mdata["is_text"]) {
		...
		$table->addTableRow("", 1);
			...
			$table->addTableData(array("colspan"=>2), "data");
				...
				$table_template->addTableRow();
					$table_template->addTableData();
						$table_template->addCode( gettext("include unsubscription link") );
					$table_template->endTableData();
					$table_template->addTableData();
					$template_unsubscribe = ($mdata["template_unsubscribe"] ? $mdata["template_unsubscribe"] : 1);
					if ($newsletter["count"] > 0) {
						$table_template->addSelectField("mail[template_unsubscribe]", array(
							"1" => gettext("yes"),
							"0" => gettext("no")
		...
		$table->addTableData("", "data");
			...
			if(sql_num_rows($res)>0) {
				while($row= sql_fetch_assoc($res)) {
					...
					$project_names .= "<a href='javascript:popup(\"?mod=project&action=showhours&id=$project_id\");'>".$project_data->getProjectNameById($project_id)."</a>"."\t\t"."<a href='javascript:deleteProject(\"$project_id\");' title='remove'>[X]</a>"."\t,\t";
				}
				$table->addCode($project_names);
			} else {
	...
	$table->addTableRow();
		...
		$table->endTableData();
		$table->addTableData("", "data");
			$table->insertAction("close", gettext("back"), "javascript: closepopup();");
			$table->addSpace();
	...
	$table->addTableRow();
		...
		if (!$mdata["is_text"]) {
			...
			if ($user_info["htmleditor"] == 3) {
				$editor = new Layout_editor("wyzz", 700, 500);
			} else {
				$editor = new Layout_editor();
			}
			$table->addCode( $editor->generate_editor("", $mdata["body_html"], "false", "contents", 1, "false") );
		} else {
			/* if mode = text */
