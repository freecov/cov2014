 ...
 */
	...
	while ($row = sql_fetch_assoc($res)) {
		...
		*/
		...
		if (!$row["is_text"]) {
			/* include pre tags for emails containing script tags, for email overview */
			$body = substr(trim($this->html2text($row["body"])), 0, 256);
			if (preg_match("/&lt;script(.*?)/", $body) || preg_match("/<script(.*?)/", $body)) {
				$row["h_body"] = "<PRE wrap='1'>".htmlspecialchars($body."</PRE>");
			} else {
				$row["h_body"] = $body;
			}
		} else {
