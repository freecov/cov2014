 ...
 */
	...
	for ($x=0;$x<=$xmail->p;$x++){
		...
		// microsoft documents are sometimes misinterpreted as subtype ZIP, while we need X-ZIP
                // else IE will fail at downloading the attachments.
                $extentions = array( ".docx", ".xlsx", ".pptx");
                if (in_array(substr($xmail->name[$x],-5), $extentions)){
                        $xmail->type[$x] = "APPLICATION/X-ZIP";
                }

		//write into db
		if (!$xmail->name[$x]){
			$xmail->name[$x] = sprintf("[%s].dat", gettext("no name"));
		}
		...
		if ($xmail->size[$x] > $this->size_limit) {
			...
			$buf = "<html><body>";
			$buf.= sprintf("%s<br>", gettext("This file was bigger then the limits, and removed"));
			$buf.= sprintf("%s: %s<br>", gettext("filename"), $xmail->name[$x]);
			$buf.= sprintf("%s: %s %s<br>", gettext("filesize"), $xmail->size[$x], gettext("bytes"));
			$buf.= sprintf("%s: %s %s<br>", gettext("limit"), $this->size_limit, gettext("bytes"));
			$buf.= "</body></html>";
			$xmail->data[$x]      =& $buf;
			$xmail->name[$x]      =  sprintf("%s: [%s].htm", gettext("removed"), $xmail->name[$x]);
			$xmail->fetchData[$x] =  0;
		...
		if ($xmail->fetchData[$x]==1) {
			...
			// If the part number is an empty string we are looking at an email
			// with the binary content as only content.
			// Examples are mail sent directly from ms-word/ms-excel
			// In those emails the attachment is actually part 1
			if ($xmail->part[$x] == "") {
				$xmail->part[$x] = 1;
			}
