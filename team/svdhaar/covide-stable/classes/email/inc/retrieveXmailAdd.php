 ...
 */
	...
	//We can find the name on several places. Make sure to check all before assuming no name
	if ($obj->dparameters[0]->value) {
		foreach ($obj->dparameters as $par) {
			if ( strtolower($par->attribute) == "filename")
				$fname = $par->value;
		}
	} elseif ($obj->parameters) {
		if ( is_array($obj->parameters) ) {
			foreach ($obj->parameters as $par) {
				if (strtolower($par->attribute) == "name")
					$fname = $par->value;
			}
		}
	}
	...
	if (strtolower($fname) == "winmail.dat") {
