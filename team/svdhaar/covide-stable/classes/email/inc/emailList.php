 ...
 */
	...
	$table->addTableRow();
		$table->addTableData();
			if ($list_of_address) {
				...
				$addressinfo_0 = $address_data->getAddressById($list_of_address);
				...
				if ($GLOBALS["covide"]->license["address_strict_permissions"] && !$userperms["xs_addressmanage"]) {
					...
					$cla_address = explode("|", $addressinfo_0["classifi"]);
				...
				$bcards = $addressData->getBcardsByRelationID($list_of_address, "", $cla_xs);
	...
	if ($_short_view != 1) {
		...
		/* get parent folder */
		$parent_folder = $mailData->getParentFolder( $folder_id );
		if ($sent_items["id"] == $parent_folder || $sent_items["id"] == $folder_id || (is_array($sent_items_user) && $sent_items_user["id"] == $folder_id))
