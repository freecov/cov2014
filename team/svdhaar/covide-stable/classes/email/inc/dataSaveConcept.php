 ...
 */
 ...
 /*                            */
 /* It is correct you see      */
 /* $REQUEST and $POST instead */
 /* of $_REQUEST and $_POST    */
 /* See ../data.php            */
 /******************************/
	...
	if ($id) {
		$q = sprintf("select folder_id from mail_messages where id = %d", $id);
		$res = sql_query($q);
		$folder = sql_result($res,0);

		$concepts = $this->getSpecialFolder("Concepten", $session_user_id);
		if ($concepts["id"] != $folder) {
			$output = new Layout_output();
			$output->start_javascript();
			$output->addCode(sprintf("alert('%s');",
				addslashes(gettext("This email is already send in another window. Please close this window."))
			));
	...
	*/
	...
	if (!$id) {
		...
		$folderdata = $this->getSpecialFolder("Concepten", $session_user_id);
		$concept_folder = $folderdata["id"];

		if ($REQUEST["ref_type"]) {
			$mdata = $this->getEmailById($REQUEST["ref_id"]);
			...
			if (!$data["is_text"]) {
				...
				$config = array(
					...
					'word-2000'      => false, //disabled, because it crashes the tidy processor and hangs up the process
					'clean'          => true
				);
				// Tidy
				$tidy = new Tidy();
				...
				/* if the header matches outlook express 6 */
				/* then we need to filter some empty div paragraphs */
				$head = preg_replace("/<\/head[^>]*?>.*$/si", "", $data["body"]);
				if (preg_match("/<meta content=\"MSHTML 6.00[^>]*?>/si", $head)) {
					preg_match_all("/<div[^>]*?>.*?<\/div>/sxi", $data["body"], $matches);
					$matches = array_unique($matches[0]);
					foreach ($matches as $m) {
						$test = strtolower(preg_replace("/(\t|\r|\n| )/s", "", $m));
						if ($test == "<div>&nbsp;</div>") {
							$data["body"] = str_replace($m, "<div><br></div>", $data["body"]);
						}
					}
				}
			}
			...
			switch ($REQUEST["ref_type"]) {
				case "reply_all":
					$ownemail = $this->getEmailAliasesPlain($session_user_id);
					...
					foreach ($tmp_cc as $ckey=>$cval) {
						...
						if (in_array(trim($cval), $ownemail) || !preg_match("/^.*@.*\..*$/s", $cval) ) {
							unset($tmp_cc[$ckey]);
						} else {
			...
			if (is_numeric($REQUEST["from"])) {
				if ($data["is_text"]) {
					/* text */
					$sig = $this->getEmailSignature($REQUEST["from"]);
					...
					$prefix.= "  </td>\n";
					$prefix.= " </tr><tr>\n";
					$prefix.= "  <td valign='top'>".gettext("from").": </td><td>".str_replace(array("<", ">", "&lt;", "&gt;"), array("[", "]", "[", "]"), $mdata[0]["sender_emailaddress_h"])."</td>\n";
					$prefix.= " </tr><tr>\n";
					...
					$sig = $this->getEmailSignatureHtml($REQUEST["from"]);
					...
					$prefix.= "  <td><span class='cell1'>".gettext("from").": </span></td><td><span class='cell2'>".str_replace(array("<", ">", "&lt;", "&gt;"), array("[", "]", "[", "]"), $mdata[0]["sender_emailaddress_h"])."</span></td>\n";
					$prefix.= " </tr><tr class='head2'>\n";
					...
					$data["body"] = "<p>".$prefix.$data["body"].$suffix."</p>";
				}
			}
			...
			$user_data = new User_data();
			$userinfo = $user_data->getUserdetailsById($session_user_id);
			if ($REQUEST["view_mode"]=="html" || $userinfo["mail_html"]) {
				$sig = $this->getEmailSignatureHtml($REQUEST["mail"]["from"]);
				$data["is_text"] = 0;
				$data["body"] = "<br>".$sig;
			} else {
				$sig = $this->getEmailSignature($REQUEST["mail"]["from"]);
				$data["is_text"] = 1;
				$data["body"] = "\n\n".$sig;
			}
			$data["from"] = $REQUEST["mail"]["from"];
		...
		if ($REQUEST["relation"])
			$data["address_id"] = $REQUEST["relation"];

		if ($REQUEST["new_subject"])
			$data["subject"] = $REQUEST["new_subject"];
		...
		$fields["user_id"]             = array("d",$session_user_id);
		$fields["date"]                = array("d",time());
		...
		if ($REQUEST["ref_id"]) {
			$fields["options"]             = array("s",$this->encodeMailOptions( array("related_id"=>$REQUEST["ref_id"]) ));
		}

		if ($REQUEST["campaign_id"]) {
			$fields["options"]             = array("d",$this->encodeMailOptions( array("campaign_id"=>$REQUEST["campaign_id"]) ));
		}
		...
		/* copy projects attached to original email */
		if (!$id && $REQUEST["ref_id"]) {
			$q = sprintf("select * from mail_projects where message_id = %d", $REQUEST["ref_id"]);
			$res = sql_query($q);
			if (sql_num_rows($res) > 0) {
				while ($row = sql_fetch_assoc($res)) {
					$q = sprintf("INSERT INTO mail_projects (message_id, project_id) VALUES (%d, %d);", $new_id, $row["project_id"]);
					$r = sql_query($q);
				}
			}
		}
		...
		if ($data["copy_attachments"]) {
			...
			$attachments = $this->attachments_list($REQUEST["ref_id"]);
		...
		$data["campaign_id"] = $_REQUEST["campaign_id"];
		...
		//save body
		$contents = stripslashes($REQUEST["contents"]);
		if ($REQUEST["convert_on_save"]) {
			if ($REQUEST["is_text"]) {
				/* replace signature */
				$sig = $this->getEmailSignature($REQUEST["mail"]["from"]);
				$sig_html = $this->getEmailSignatureHtml($REQUEST["mail"]["from"]);

				$contents = str_replace("  ", " &nbsp;", $contents);
				$contents = str_replace($sig, "##signature##", $contents);

				/* make html */
				$contents = sprintf("<font face='monospace'>%s</font>", nl2br($contents));
