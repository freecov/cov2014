 ...
 */
	...
	if ($row["is_public"] == 2 && $session_user_id != $row["user_id"])
		$xs = 0;
	...
	if (!$xs && $row["address_id"] && $row["folder_id"] == $archive_mail && $row["is_public"]!=2) {
		...
		if ($row["bcard_id"]) {
			$bcardinfo = $address_data->getAddressById($row["bcard_id"], "bcards");
		}
		...
		if ($userperms["xs_addressmanage"]) {
			...
			if ($bcardinfo["classification"]) {
				$cla_address = explode("|", $bcardinfo["classification"]);
			} else {
	...
	if ($row["date_received"]) {
		...
		$row["is_sentmail"] = false;
	} else {
		$row["is_sentmail"] = true;
	}
	...
	//remove possible slashes from description added by addslashes
