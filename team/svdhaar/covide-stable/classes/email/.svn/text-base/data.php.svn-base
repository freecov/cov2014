...
Class Email_data {
	...
	public function multipleMove() {
		...
		/* Delete email in Google Apps if necessary */
		$user_folder = $this->getSpecialFolder("Verwijderde-Items", $_SESSION["user_id"]);
		if ($folder == $user_folder["id"]) {
			$this->moveGoogleMail($ids);
		} else {
		/* Archive email in Google Apps if necessary */
			$this->moveGoogleMail($ids, 1);
		}
	...
	public function encodeMailOptions($data) {
		...
		$opts["template_unsubscribe"]     = $data["template_unsubscribe"];
	...
	public function decodeMailOptions($str) {
		...
		foreach ($data as $setting) {
			...
			switch ($d[0]) {
				...
				case "template_unsubscribe"    : $opts["template_unsubscribe"]     = $d[1]; break;
	...
	/* moveGoogleMail{{{ */
	/**
	 * Move multiple emails to 'Trash/Archive' in Gmail
	 */
	public function moveGoogleMail($ids, $archive_flag = 0) {
		/*Archive  mail (from a shared folder), in Google. */
		if (is_array($ids)) {
			$email_arr = $this->getEmailById($ids[0]);
		} else {
			$email_arr = $this->getEmailById($ids);
		}
		$email_user = $email_arr[0]["user_id"];
		$email_user = ($email_user != $_SESSION["user_id"] ) ? $email_user : $_SESSION["user_id"];

		$userdata = new User_data();
		$userinfo = $userdata->getUserdetailsById($email_user);
		$check_userinfo = $userdata->getUserdetailsById($ids);

		if ($userinfo["mail_imap"] == 2 && $userinfo['mail_server'] == "imap.gmail.com") {
			$trash_boxes = array("Trash", "Bin", "Prullenbak");
			...
			$mail = new Email_retrieve();
			$mail->mailConnect($user);
			$mbox =& $mail->mbox;
			$server = '{imap.gmail.com:993/ssl}';
			$mailboxes = imap_list($mbox, $server, '*');
			foreach ($trash_boxes as $trash) {
				foreach ($mailboxes as $v) {
					if (substr_count($v, $trash) > 0) {
						$trash_name = str_replace($server, '', $v);
						break;
					}
				}
			}
			...
			foreach ($email_id as $id) {
				$sql = sprintf("select * from status_list where mail_id = %d and user_id = %d", $id, $email_user);
				$rs = sql_query($sql);
				if (sql_num_rows($rs) > 0) {
					$email_arr = $this->getEmailById($id);
					$result = imap_search($mbox, 'SUBJECT "'.$email_arr[0]["subject"].'" ON "'.date("D, d M Y", $email_arr[0]["date"]).'"');
					if (is_array($result)) {
						foreach ($result as $v) {
							$google_mail_headers = imap_headerinfo($mbox, $v);
							/* chck timestamps of mails, in case any 2 mails have similar subjects */
							if ($email_arr[0]["date"] == $google_mail_headers->udate) {
								imap_delete($mbox, $v);
								if ($archive_flag == 0) {
									imap_mail_move($mbox, $v, $trash_name);
								}
							}
						}
					}
	...
	public function mail_delete($id=0) {
		...
		//check if this mail is in the deleted items or sent items of the user
		$deleted_items = $this->getSpecialFolder("Verwijderde-Items", $row["user_id"]);
		$sent_items = $this->getSpecialFolder("Verzonden-Items", $row["user_id"]);
		$allowed_folders = array($deleted_items["id"], $sent_items["id"]);
		if (!in_array($row["folder_id"], $allowed_folders)) {
			//its not, but it can be in the folder of a user that had a folder shared with this user
			$own_deleted_items = $this->getSpecialFolder("Verwijderde-Items", $_SESSION["user_id"]);
			$own_send_items = $this->getSpecialFolder("Verzonden-Items", $_SESSION["user_id"]);
			$own_allowed_folders = array($own_deleted_items["id"], $own_send_items["id"]);
			if(!in_array($row["folder_id"], $own_allowed_folders)) {
				return false;
			}
		}
	...
	public function getSpecialFolder($name, $user_id=0) {
		...
		$q = sprintf("select * from mail_folders where (parent_id is null OR parent_id = 0) and user_id %s and LOWER(name) = '%s'", $user_id, $name);
		$res = sql_query($q);
		if (sql_num_rows($res) == 0) {
	...
	public function change_folder_xml() {
		...
		if ($folder == $user_folder["id"]) {
			/* Delete email in Google Apps if necessary */
			$this->moveGoogleMail($id);
		} else {
			/* Archive email in Google Apps if necessary */
			$this->moveGoogleMail($id, 1);
		}
		...
		foreach ($list["list"] as $tracker) {
			...
			if ($tracker["is_sent"] < 1) {
				...
				if (strlen(trim($email)) && $this->validateEmail($email)) {
					...
					$hashcode = md5($mailcode.$email);
					$ins_sql = sprintf("insert into mail_unsubscription (email, hashcode) values ('%s', '%s')", $email, $hashcode);
					mysql_query($ins_sql);
					$this->insertUnsubscription($tmp_data, $hashcode, $email);
	...
	public function insertUnsubscription(&$data, $hashcode, $email) {
		$mail_data = preg_split("/(\#(=(\r){0,1}\n){0,1}\#)/s", $data, -1, PREG_SPLIT_DELIM_CAPTURE|PREG_SPLIT_NO_EMPTY);
		...
		foreach ($mail_data as $k=>$v) {
			...
			if (trim($v) == "##") {
				$i++;
				if ($i % 2 == 0) {
					$m = sprintf("%s%s%s", $mail_data[$k-2], $mail_data[$k-1], $mail_data[$k]);
	...
	public function change_private_xml() {
		...
		if ($private) {
			...
			$name_temp = $address_info["surname"].", ".$address_info["givenname"]." ".$address_info["infix"];
			$name = preg_replace("/ {2,}/", " ", $name_temp);
		} else {
			$name = gettext("none");
		}
