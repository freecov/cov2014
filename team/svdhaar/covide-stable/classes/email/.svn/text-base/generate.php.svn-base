...
Class Email_generate Extends Email_data {
	...
	/* methods */
	//}}}----------------------------------------------------------------------
	//{{{ moveTo2Cc: put multiple recipients in cc field and leave first as to
	//-------------------------------------------------------------------------
	...
	public function generateMail($body, $attIds=array()) {
		...
		//timestamp = msg_id
		$body["timestamp"] = ( strtoupper(dechex(time())).".".$this->generateRandInt(7)."@".preg_replace("/^([^\@]*?\@)/i","",$body["From_mail"]) );
		//generate date field
		...
		if ($body["IsHtml"]==0) {
			...
			if ($template) {
				$cidstat[1] += count($body["template-cid"]);
			} else {
				$body["template_type"] = "";
			}

			if (!$attIds && $body["template_type"] != "inline") {
		...
		case "html/att/inl":
			...
			//XXX Why are we adding the attachments twice ?
			/*
	...
	private function insertAtt($attIds, $boundary, $inline=0) {
		...
		foreach ($attIds as $att) {
			...
			$contents = $fsdata->FS_readFile($myFile);
	...
	function insertTemplateAtt($template_id, $boundary, $cid) {
		if ($template_id && count($cid)>0) {

			$header = "";
			foreach ($cid as $k=>$v) {
				$fs_data = new Filesys_data();
				$fileinfo = $fs_data->getFileById($v["fileid"]);

				// now for the attachement
				$header .= "--" . $boundary . "\n";
				$header .= "Content-Type: ".strtolower($fileinfo["type"]).";\n";
				$header .= "\tname=\"".$fileinfo["name"]."\"\n";
				$header .= "Content-Transfer-Encoding: base64\n";
				$header .= "Content-ID: <".$v["cid"].">\n\n";

				// add binary data
				if (!$fileinfo["fsize"]) {
					$contents = "\n\n";
				} else {
					$contents = $fileinfo["binary"];
				}
				$encoded_attach = chunk_split(base64_encode($contents));
				$header .= $encoded_attach."\n\n";
	...
	private function add_text_warning($str) {
		$tmp = gettext("This message contains HTML, but your client doesn't support this format.")."\n";
		$tmp.= gettext("A plain text copy of the message is included below this line.")."\n\n";
		$tmp.= "*** ".gettext("begin message")." ***\n\n";
		//disable this for now. Need to get default office lang and init gettext here
		//return ($tmp.$str."\n");
		return $str."\n";
	}
	...
	public function sendMail2BackgroundSmtp(&$data, $opts, $list, $mail_id) {
		...
		/* TODO: after this, the email is still transfered to "Sent-Items" in a later stage
		/* if id can be found in campaign tracker ids, put it in special folder

		$email_data = new Email_data;
		$campaign_folder  = $email_data->getSpecialFolder("Campaign-Archive", $_SESSION["user_id"]);
		$campaign_data = new Campaign_data;
		$in_campaign = $campaign_data->campaignHasTracker($mail_id);

		if ($in_campaign) {
			$q = sprintf("update mail_messages set is_new = 0, folder_id = %d where id = %d",  $campaign_folder["id"], $mail_id);
			sql_query($q);
		}
		*/
		...
		$file_plain = "newsletter_".md5(rand()*time()).".mail";
		...
		$venster = new Layout_venster( array(
			...
			"subtitle" => gettext("send")
		));
		$venster->addVensterData();

			$venster->addTag("span", array("id"=>"newsletter_progressbar"));
				$venster->addCode("(".gettext("initializing")."...)" );
			$venster->endTag("span");
	...
	/* sendMail2Smtp {{{ */
	/**
	 * send mail directly using smtp class (only 1 or very few addresses)
	 *
	 * @param string &$data The complete email as a text string
	 * @param array $opts Options to format/send the mail
	 * @param string $hash Unique hash to use in tracker items to identify this email
	 * @param int $force_local If set to 1, use localhost to send the mail
	 *
	 * @return string the result of the smtp commands
	 */
	public function sendMail2Smtp(&$data, $opts, $hash, $force_local = 0) {
		...
		$smtp = new Email_smtp();
		if ($force_local) {
			$smtp->set_server("localhost", "25");
			$smtp->set_server_options(array("tls" => 0, "smtpauth" => array()));
		}
	...
	public function parseTemplate($part_body, $data) {
		...
		$server_url = "http://".preg_replace("/\/{2,}/", "/", $_SERVER["HTTP_HOST"].dirname($_SERVER["SCRIPT_NAME"])."/");
		...
		switch ($data["template_type"]) {
			...
			case "inline":
				foreach ($tf as $k=>$v) {
					$tf[$k] = "\"cid:".str_replace("#posdata#",$k,$cid)."\"";
				}
				break;
			case "external":
			case "tracking":
		...
		/* parse template options and put them in $part_body["Body_html"] */
		$mail_data = new Email_data();
		$decodedMail = $mail_data->decodeMailOptions($data["options"]);
		$tpldata = $mail_data->getTemplateFields($decodedMail["template"]);

		foreach ($decodedMail AS $key=>$value) {
			preg_match_all("/tpl_(.*)/", $key, $tpl_value, PREG_SET_ORDER);
			if (is_array($tpl_value[0])) { $tpl_values[] = $tpl_value; }
		}

		// Replace template images and variables
		$tmpbody = $mail_data->stripBodyTags($part_body["Body_html"]);

		$part_body["Body_html"] = $tpldata["html_data"];
		if (is_array($tf)) {
			foreach($tf as $k=>$v)
				$part_body["Body_html"] = str_replace("{{tpl_img_$k}}", "<img src=".$v." />", $part_body["Body_html"]);
		}
		// Replace {{ }} variables with saved string
		if (is_array($tpl_values)) {
			foreach ($tpl_values as $var)
				$part_body["Body_html"] = str_replace("{{".trim($var[0][1])."}}", nl2br($decodedMail[$var[0][0]]), $part_body["Body_html"]);
		}

		// Replace template body with emailbody.
		if ($part_body["template_cmnt"]) {
			$part_body["Body_html"] = str_replace(
				"{{tpl_cmnt}}",
				"##rcptcmnt##",
				$part_body["Body_html"]);
		} else {
		...
		if ($data["template_unsubscribe"]=="1") {
			$unsubs_url = "http://".preg_replace("/\/{2,}/", "/", $_SERVER["HTTP_HOST"].dirname($_SERVER["SCRIPT_NAME"])."/")."unsubscribe.php?code=##mailid##&hashcode=##hashcode##";
			$unsubs_anchor = "<a target='_blank' href='".$unsubs_url."' title='"."Unsubscribe from newsletter"."'>Unsubscribe from this newsletter</a>";
			$part_body["Body_html"] = str_replace(
				"{{tpl_unsubscribe}}",
				$unsubs_anchor,
				$part_body["Body_html"]);
		...
		$part_body["Body_html"] = preg_replace("/ href='([^']*?)'/si"," href=\"$1\"", $part_body["Body_html"]);
		preg_match_all("/[^\/](cmsfile\/(\d{1,}))/si", $part_body["Body_html"], $matches);
		if (is_array($matches[1])) {
			$matches[1] = array_unique($matches[1]);
			$matches[2] = array_unique($matches[2]);
			$track_url_set = 0;
			$cidfiles = array();
			foreach ($matches[2] as $matchid=>$match) {
				switch ($data["template_type"]) {
				case "inline":
					$part_body["Body_html"] = str_replace($matches[1][$matchid], "cid:".str_replace("#posdata#", $matchid, $cid), $part_body["Body_html"]);
					$cidfiles[$matchid] = array("cid" => str_replace("#posdata#", $matchid, $cid), "file" => $matches[1][$matchid], "fileid" => $matches[2][$matchid]);
					break;
				default:
					if ($track_url_set)
						$part_body["Body_html"] = str_replace($matches[1][$matchid], $server_url_alt.$match, $part_body["Body_html"]);
					else {
						$part_body["Body_html"] = str_replace($matches[1][$matchid], $server_url.$match, $part_body["Body_html"]);
						$track_url_set = 1;
					}
					break;
				}
		...
		$part_body["Body_html"] = $mail_data->stylehtml($part_body["Body_html"]);
