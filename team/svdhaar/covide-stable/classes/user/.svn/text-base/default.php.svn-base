...
Class User {
	public function __construct() {
		...
		*/
		...
		$exceptions = array("validate", "translate", "cron", "cron_email", "cron_calendar", "cron_other", "convert_db", "getUserinfoXML", "logout");
		if (!$_SESSION["user_id"] && !in_array($action,	$exceptions)) {
			$GLOBALS["covide"]->trigger_login();
		} else if (!in_array($action,	$exceptions)) {
		...
		switch ($action) {
			...
			case "cron_calendar":
				...
				$user_data = new User_data();
				$user_data->preCron();
				$user_data->cron_calendar(false, $id);
				$user_data->postCron();
				break;
			case "cron_other":
				if (!$_REQUEST["param"]) {
			...
			case "radiusconf":
				$user_output = new User_output();
				$user_output->showRadiusSettings();
				break;
			case "save_radius":
				$user_data = new User_data();
				$user_data->saveRadiusSettings($_REQUEST["radius"]);
				$user_output = new User_output();
				$user_output->showRadiusSettings();
			default :
				$user = new User_output();
