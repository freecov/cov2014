...
function login_challenge() {
	...
	if (typeof(cvd_crypt_challenge) == 'undefined') 
		return false;
	...
	if (document.getElementById('loginuse_cookie_password').value == 0) {
		...
		var str = new String().concat( $.md5( pw.value ), cvd_crypt_challenge );
		realpw.value = $.md5(str);
	} else {
		/* if set by cookie */
		var str = new String().concat(realpw.value, cvd_crypt_challenge );
		realpw.value = $.md5(str);
	}
...
function login() {
	if (check_radius == 1) {
		...
		var username = document.getElementById('loginusername').value;
		radius_auth = loadXMLContent('index.php?mod=user&action=getUserinfoXML&username='+username);
	} else {
		radius_auth = 0;
	}
	if (radius_auth == 0) {
