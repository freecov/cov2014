...
$output->layout_page(gettext("User settings"));
	...
	$venster->addMenuItem(gettext("new user"), "?mod=user&action=useredit&id=0", "", 0);
	if ($_SESSION["locale"] == "nl_NL") {
		$venster->addMenuItem(gettext("help (wiki)"), "http://wiki.covide.nl/Gebruikersbeheer", array("target" => "_blank"), 0);
	}
	$venster->addMenuItem(gettext("group policy"), "?mod=user&action=groupindex");
		...
		if (preg_match("/^(pdo_){0,1}mysql(i){0,1}:\/\//si", $GLOBALS["covide"]->dsn)) {
			...
			if ($currname == "administrator" || $current_user["xs_usermanage"]) {
				...
				if ($currname == "administrator") {
					$venster->addMenuItem(gettext("Radius"), "?mod=user&action=radiusconf");
					$venster->addMenuItem(gettext('Wurfl'), 'classes/wurfl/admin/', array('target' => '_blank'));
				}
			}
		}
