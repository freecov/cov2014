...
if ($GLOBALS["covide"]->license["has_hrm"]) {
	...
	/* XXX: This is not working anymore right ?
...
/* basic user info like password and address settings */
	...
	$table->endTableRow();
	$table->addTableRow();
		$table->insertTableData(gettext("generate new password"), "", "header");
		$table->addTableData("", "data");
			$table->insertAction("new", gettext("generate new password"), "javascript: generatePassword();");
			$table->addSpace();
			$table->insertTag('span', '', array('id' => 'newpassword'));
		$table->endTableData();
	$table->endTableRow();
...
$table->endTableRow();
$table->addTableRow();
$table->insertTableData(gettext("google apps username"), "", "header");
$table->addTableData("", "data");
$table->addTextField("user[google_username]", $userinfo["google_username"]);
$table->endTableData();
$table->endTableRow();
$table->addTableRow();
$table->insertTableData(gettext("google apps password"), "", "header");
$table->addTableData("", "data");
