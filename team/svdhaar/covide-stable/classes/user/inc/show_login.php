<?php
	...
	@session_destroy();
	@session_unset();
	if (is_object($GLOBALS["covide"]->session)) {
		$GLOBALS["covide"]->session = new Session();
	} else {
		session_start();
	}
	...
	$_SESSION["cvd_crypt_challenge"] = crc32(session_id().rand().time());

	$output = new Layout_output();
	$output->layout_page(gettext("login"));
	$output->addTag("div",  array("id" => "login_main"));
	...
	*/
			...
			//$venster->addVensterData();
				...
				$table1->addTableRow();
					$table1->addTableData(array("colspan"=>2, "align"=>"center"));
						$table1->addCode("<div id=\"logocontainer\">");
				...
				if ($error == 97) {
					$table1->addTableRow();
						$table1->insertTableData(gettext("Maximum logged in users limit reached"), array("colspan" => 2), "header");
					$table1->endTableRow();
				}
				if ($error == 98) {
					$table1->addTableRow();
				...
				if ($GLOBALS["covide"]->mobile)
					$ok = 1;
				/* mozilla */
				if ($browser["browser_name"] == "moz" && $browser["math_version"] >= 1.9)
					$ok = 1;
				/* msie */
				if ($browser["browser_name"] == "ie" && $browser["math_version"] >= 7.0)
					$ok = 1;
				/* konqueror */
				#if ($browser["browser_name"] == "konq" && $browser["math_version"] >= 3.5)
				#	$ok = 1;
				/* opera */
				if ($browser["browser_name"] == "op" && $browser["math_version"] >= 9.8)
					$ok = 1;
				/* safari */
				...
				$table1->addTableRow();
					...
					$table1->endTableData();
				$table1->endTableRow();
				/* table row for popup blocker message */
				$table1->addTableRow(array("id" => "popup_tester_msg", "style" => "display: none;"));
					$msg = gettext("This computer blocks popups, and therefor some functionality in Covide wont work correctly.");
	...
	$output->start_javascript();
		$output->addCode("
			var cvd_crypt_challenge = '".$_SESSION["cvd_crypt_challenge"]."';

			if (!cvd_crypt_challenge) {
	...
	//$output->endTag("div");
	if ($GLOBALS["covide"]->contrib["USE_CONTRIB_LOGIN"]) {
		$file = "contrib/".$GLOBALS["covide"]->contrib["USE_CONTRIB_LOGIN"]."/post_login.php";
		if (file_exists($file))
			require_once($file);
	}
