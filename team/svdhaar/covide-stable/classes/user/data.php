	...
	Class User_data {
		...
		public function _init_roles() {
			$manager_roles = array(
				...
				"xs_limited_projectmanage" => array(
					"name"           => gettext("limited project manager"),
					"action"         => "module_project",
					"license"        => array("has_project"),
					"disable_basics" => true
				),
		...
		public function getUserPermissionsById($id) {
			...
			if (!$this->_cache["userdetails"][$id]) {
				...
				$res = sql_query($sql);
				while ($externalrow = sql_fetch_row($res)) {
		...
		public function getEmployeedetailsById($id) {
			...
			if ($userinfo["address_id"]) {
				...
				$sql = sprintf("SELECT givenname,surname,infix FROM address_businesscards WHERE id=%d", $userinfo["address_id"]);
				$r = sql_query($sql);
		...
		public function validate_login($userinfo) {
			// check if a 'max concurrent users' setting is in the license table.
			// If it's bigger then 0, check how many users we already have logged in
			// If this new login would exceed the max, return error
			$sql = "SELECT concurrent_users FROM license";
			$res = sql_query($sql);
			$concurrent_users = sql_result($res, 0);
			if ($concurrent_users) {
				// grab current amount of users
				$sql = "SELECT user_id FROM login_current GROUP BY user_id";
				$res = sql_query($sql);
				$current_users = sql_num_rows($res);
				if ($current_users >= $concurrent_users) {
					$user_output = new User_output();
					$user_output->show_login(97);
					exit();
				}
			}
			...
			if (sql_num_rows($res)>0) {
				...
				if ($GLOBALS["covide"]->license["has_radius"] && strtolower($row["authmethod"]) == "radius") {
					...
					case RADIUS_ACCESS_ACCEPT:
						...
						$logininfo["ip"]      = $GLOBALS["covide"]->user_ip;
						...
						if ($_REQUEST["uri"]) {
							...
							$output->start_javascript();
								$output->addCode(sprintf("
									if (opener) {
										opener.location.href = '%1\$s'; setTimeout('window.close();', 500);
									} else {
										location.href = '%1\$s';
									}
					...
					if ($serverpw == $clientpw) {
						...
						if ($_REQUEST["uri"]) {
							...
							$output->start_javascript();
								$output->addCode(sprintf("
									if (opener) {
										opener.document.location.href = opener.document.location.href.replace('/#.*$/', '');
										window.close();
									} else {
										location.href = '%1\$s';
									}
		...
		public function logout() {
			...
			if ($_REQUEST["redir"] && $_REQUEST["redir"] != "close") {
		...
		public function useredit_check($checkdata) {
			...
			$mailserver = sprintf("%s", $checkdata["mailserver"]);
			$mailproto  = sprintf("%d", $checkdata["mailproto"]);
			$mailuser = sprintf("%s", $checkdata["mailuser"]);
			$mailpass = sprintf("%s", $checkdata["mailpass"]);
			/* check mailsettings */
			if ($mailserver && $mailproto && $mailuser && $mailpass) {
				switch ($mailproto) {
				case 1:
				...
				$connstr = sprintf("{%s:%s}INBOX",  $mailserver, $protocol);
				$ok = (
					$mbox = imap_open(
						$connstr,
						$mailuser,
		...
		public function saveLicense($req) {
			if ($_FILES["alt_logo"]["error"] == UPLOAD_ERR_OK) {
				//move the file
				$name = basename($_FILES["alt_logo"]["name"]);
				$tmp_name = $_FILES["alt_logo"]["tmp_name"];
				if ($name && $tmp_name) {
					$base = $GLOBALS["covide"]->filesyspath;
					move_uploaded_file($tmp_name, sprintf("%s/bestanden/%s", $base, $name));
					$alt_logo = $name;
				}
			} else {
				$alt_logo = "";
		 ...
		 * Pre-cron operations
		 */
		public function preCron($silent = false) {
			ob_clean();
			ob_start();
			ob_implicit_flush(1);
		...
		/* cron_email() {{{ */
		/**
		 * Fetch e-mail per user
		 */
		public function cron_email($silent = false, $user_id = -1) {
			if ($user_id != -1) {
				$q = sprintf("select * from users where id = %d order by id", $user_id);
			} else {
			...
			while ($row = sql_fetch_assoc($res)) {
				if ($row["mail_user_id"] && $row["mail_server"] && $row["mail_password"] && $_REQUEST["param"]["funambol"] != "single") {
					...
					if (!$silent)
						echo sprintf("%s: [mailfetch] id: %d, username: %s\n", date("H:i:s"), $row["id"], $row["username"]);
					$email = new Email_retrieve($row["id"], 0);
					$r = $email->retrieve();
					if (!$silent)
						echo sprintf(date("H:i:s").": [mailfetch] %s\n", $r);
					unset($email);
				}
			}
		}
		/* cron_calendar() {{{ */
		/**
		 * Sync Google Calendars, if necessary
		 */
		public function cron_calendar($silent = false, $user_id = -1) {
			if ($user_id != -1) {
				$q = sprintf("select * from users where id = %d order by id", $user_id);
			} else {
				$q = sprintf("select * from users where is_active = 1 order by id");
			...
			while ($row = sql_fetch_assoc($res)) {
				//var_dump($row);
				if ($row["google_username"] && $row["google_password"] && $_REQUEST["param"]["funambol"] != "single") {
					/* only output if something needs to be done */
					if (!$silent) echo sprintf("%s: [calendar sync] id: %d, username: %s\n", date("H:i:s"), $row["id"], $row["username"]);
					$google_data = new Google_data();
					$gClient = $google_data->getGoogleClientLogin($row["google_username"], $row["google_password"], "calendar");
					$google_data->syncGoogleCalendarClient($row["id"], $gClient, false);
					if (!$silent) echo date("H:i:s").": [calendar sync] done\n";
				}
		...
		/* cron_other() {{{ */
		/**
		 * Run a Funambol sync if necessary, clean up and move files
		 */
		public function cron_other($silent = false, $user_id = -1) {
			if ($user_id != -1) {
				$q = sprintf("select * from users where id = %d order by id", $user_id);
			} else {
				$q = sprintf("select * from users where is_active = 1 order by id");
			...
			while ($row = sql_fetch_assoc($res)) {
				if ($row["xs_funambol"] && $row["xs_funambol_version"] == 6 && $_REQUEST["param"]["funambol"] != "disabled") {
					/* only output if something needs to be done */
					if (!$silent)
						echo sprintf("%s: [funambol] id: %d, username: %s\n", date("H:i:s"), $row["id"], $row["username"]);
					...
					unset($fnbl);
					if (!$silent) echo date("H:i:s").": [funambol] done\n";
				}
			}
			if ($_REQUEST["param"]["funambol"] != "single") {
				if (!$silent) echo sprintf("%s archiving email", date("H:i:s"));
				$email = new Email_data();
				$email->archiveOldEmails();
				unset($email );
				if (!$silent) echo " [done]\n";

				/* XXX: mvanbaak - 20100418: This is obsolete right ? It messes up the custom logo
				if (!$silent) echo sprintf("%s moving files on filesystem", date("H:i:s"));
				$filesys_data = new Filesys_data();
				...
				if (!$silent) echo sprintf("%s moving gallery files", date("H:i:s"));
				$cms_data = new Cms_data();
				$cms_data->migrateGalleryItems();
				unset($cms_data);
				if (!$silent) echo " [done]\n";

				if (!$silent) echo sprintf("%s fetching rss feed items", date("H:i:s"));
				$rss = new Rss();
				unset($rss);
				if (!$silent) echo " [done]\n";
				if ($GLOBALS["covide"]->license["has_multivers"]) {
					if (!$silent) echo sprintf("%s syncing multivers addresses\n", date("H:i:s"));
					$address_data = new Address_data();
		 ...
		 * run post-cron
		 */
		public function postCron($silent = false) {
			if (!$silent) {
		...
		public function cron($user_id) {
			...
			$this->preCron($silent, $user_id);
			$this->cron_email($silent, $user_id);
			$this->cron_calendar($silent, $user_id);
			$this->cron_other($silent, $user_id);
			$this->postCron($silent, $user_id);
		}
		/* }}} */
		/* getOnlineUsers() {{{ */
		/**
		...
		public function getOnlineUsers($timelimit = 600) {
			...
			$sql = sprintf("SELECT user_id, U.username from login_current LC JOIN users U ON LC.user_id = U.id WHERE time > %d ORDER BY username", $last_minutes);
			$res = sql_query($sql);
		...
		/* removelogo {{{ */
		/**
		 * Remove the logo from fs and database.
		 */
		public function removelogo() {
			$imagepath = $GLOBALS["covide"]->filesyspath."/bestanden/";
			$imagepath .= $GLOBALS["covide"]->license["alt_logo"];
			$sql = "UPDATE license SET alt_logo = ''";
		...
		/* saveRadiusSettings {{{ */
		/**
		 * Store Radius information in the database
		 *
		 * @param array $data Should at least contain the keys radius_server, radius_port, shared_secret, nas_ip and auth_type
		 *
		 * @return bool true on success, false on failure
		 */
		public function saveRadiusSettings($data) {
			//if we have no record, create one, else update it
			$sql = "SELECT COUNT(radius_server) FROM radius_settings";
			$res = sql_query($sql);
			if (sql_result($res, 0)) {
				//we have a record, update
				$sql = sprintf("UPDATE radius_settings SET radius_server = '%s', radius_port = %d, shared_secret = '%s', nas_ip = '%s', auth_type = '%s'",
					sql_escape_string($data["radius_server"]), sql_escape_string($data["radius_port"]),
					sql_escape_string($data["shared_secret"]), sql_escape_string($data["nas_ip"]), sql_escape_string($data["auth_type"]));
			} else {
				//we have no record, insert
				$sql = sprintf("INSERT INTO radius_settings VALUES ('%s', %d, '%s', '%s', '%s')",
					sql_escape_string($data["radius_server"]), sql_escape_string($data["radius_port"]),
					sql_escape_string($data["shared_secret"]), sql_escape_string($data["nas_ip"]), sql_escape_string($data["auth_type"]));
			}
			$res = sql_query($sql);
