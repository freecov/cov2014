...
Class Filesys_data {
	...
	public function detectMimetype($file, $nocheck=0) {
		...
		if (file_exists($file.".gz") && !$nocheck) {
			$gzip_found = 1;
			$cmd = sprintf("file -riz %s", escapeshellarg($file.".gz"));
		} else {
			$cmd = sprintf("file -ri %s", escapeshellarg($file));
		}

		exec ($cmd, $ret, $status);
		$ret = explode(": ", $ret[0]);
		$ret = trim(strtoupper(preg_replace("/\;.*$/s","",$ret[1])));
		$ret = trim(preg_replace("/^-/s", "", $ret));
 		if ($gzip_found == 1) {
  			$ret = str_replace(" (APPLICATION/OCTET-STREAM)", "", $ret);
 			$ret = str_replace(strtoupper("compressed-encoding=application/x-gzip"), "", $ret);
 			$ret = trim($ret);
 		}
	...
	public function file_edit_save($formdata) {
		$sql = sprintf("UPDATE filesys_files SET description='%s', timestamp=".time().", user_id=%d WHERE id=%d AND folder_id=%d", $formdata["fedit"]["description"], $_SESSION["user_id"], $formdata["fileid"], $formdata["folderid"]);
		$res = sql_query($sql);
		$output = new Layout_output();
	...
	public function file_upload_bindata($data) {
		...
		$q = sprintf("update filesys_files set name = '%s', size = %d, timestamp = %d where
			...
			addslashes($name), $size, time(), $data["id"], $folder);
		sql_query($q);
	...
	public function file_upload_alt($data) {
		...
		$q.= sprintf("(%d, '%s', '%s', '%s', %d, %d, '%s')", $folderid, addslashes($name), $size, $type, time(), $_SESSION["user_id"], '');
		sql_query($q);
	...
	public function create_dir($dirdata, $return=0) {
		...
		if($dirdata["isPopup"]) {
			...
			$output->addCode("var tcx = setTimeout('closepopup();', 50);");
			$output->end_javascript();
			$output->exit_buffer();
	...
	public function folderSave($folder) {
		...
		$output->start_javascript();
			$output->addCode("
				parent.location.href='index.php?mod=filesys&action=opendir&id=".$folder["parent_id"]."';
				closepopup();
			");
		$output->end_javascript();
		$output->exit_buffer();
	...
	public function save_attachment() {
		...
		foreach ($ids as $id) {
			...
			/* insert file into dbase */
			$q = "insert into filesys_files (folder_id, name, size, type, timestamp, user_id, description) values ";
			$q.= sprintf("(%d, '%s', '%s', '%s', %d, %d, '%s')", $folder, $name, $size, $type, time(), $_SESSION["user_id"], $description);
			sql_query($q);
			$new_id = sql_insert_id("filesys_files");
	...
	public function file_preview_header() {
		...
		$output->layout_page("preview", 1);
			...
			$output->addSpace(2);
			$output->insertAction("close", gettext("close window"), "javascript: parent.closepopup();");
			$output->endTag("div");
		$output->layout_page_end();
		$output->exit_buffer();
	}
