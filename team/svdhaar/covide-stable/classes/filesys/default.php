...
Class Filesys {
	...
	public function __construct() {
		/* do login check */
		if (!$_SESSION["user_id"] && !$_REQUEST["filedata"]["prefix"] && $_REQUEST["action"] != "fupload") {
			$GLOBALS["covide"]->trigger_login();
		} else if (!$_REQUEST["filedata"]["prefix"] && $_REQUEST["action"] != "fupload") {
		...
		switch ($_REQUEST["action"]) {
			...
			case "create_subdir":
				$filesys_output = new Filesys_output();
				$filesys_output->create_subdir();
				break;
			case "migrate_files":
				$filesys_data = new Filesys_data();
				$filesys_data->FS_migrateFiles(1);
				break;
			default :
				$filesys_data = new Filesys_data();
