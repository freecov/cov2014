		...
		switch ($hp["name"]) {
			...
			case "declaration templates":
				...
				if (!$xs) {
					...
					$row["description"] = gettext("no access");
				}
				break;
			case "relaties":
				/* upgrade permissions */
		...
		if ($row["parent_id"]==0) {
			switch ($row["name"]) {
				case "mijn documenten":
					...
					$row["h_name"] = gettext("My documents");
					break;
				case "mijn sync files":
					$row["foldericon"] = "folder_sync";
					$row["h_name"] = gettext("My sync files");
					break;
				case "mijn mappen":
					$row["foldericon"] = "folder_myfolders";
					$row["h_name"] = gettext("My files");
					break;
				case "google mappen":
					$row["foldericon"] = "google";
					$row["h_name"] = gettext("Google docs");
					break;
				case "projecten":
					/*
					...
					$row["h_name"] = gettext("Project files");
					break;
				case "relaties":
					$row["foldericon"] = "folder_relation";
					$row["h_name"] = gettext("Contact files");
					break;
				case "hrm":
					if (!$user_info["xs_hrmmanage"]) {
						$xs = "H"; // H = hide
