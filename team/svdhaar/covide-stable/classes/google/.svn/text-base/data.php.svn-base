...
Class Google_data {
	...
	/* getGoogleClientLogin {{{ */
	/** 
	 * Get a ClientLogin object to use for Google authentication
	 * 
	 * @param string $username Google username
	 * @param string $password Google password
	 * @param string $service Google service
	 * 
	 * @return HttpClient Google ClientLogin HTTP client
	 */
	public function getGoogleClientLogin($username, $password, $service) {
		switch ($service) {
			case "calendar":
		...
		try {
			...
			if (!$silent) {
				echo "\n!!!   Captcha challenge issued by Google. Visit http://google.com/a/<office>/UnlockCaptcha\n";
			}
			return false;
		} catch (Zend_Gdata_App_AuthException $e) {
			if (!$silent) {
				echo "\n!!!   AuthException thrown, check username/password!.\n";
			}
			return false;
		}
		return $client;
	...
	public function getGoogleDocList($current_folder=0, $file_search="", $subaction="") {
		...
		foreach ($feed->entries as $entry) {
			...
			$a = $entry->getAuthor();
			$c = $entry->getCategory();
			if (is_object($entry->getEditLink())) {
	...
	public function getGoogleCalendar($username, $password, $options = array()) {
		$client = $this->getGoogleClientLogin($username, $password, "calendar");
		return $this->getGoogleCalendarClient($client, $options);
	}
	/* }}} */	
	 ...
	 * @return array Google calendar items
	 */
	public function getGoogleCalendarClient($client, $options = array()) {
	...
	public function createGoogleCalendarItem($username, $password, $data) {
		$client = $this->getGoogleClientLogin($username, $password, "calendar");
		return $this->createGoogleCalendarItemClient($client, $data);
	}
	/* }}} */
	/* createGoogleCalendarItemClient {{{ */
	/**
	 * Create an item on the Google calendar service
	 *
	 * @param string $username Google account username
	 ...
	 * @return string The google id of the new item
	 */	
	public function createGoogleCalendarItemClient($client, $data) {
		$recurrence = "";
		if ($data["is_repeat"] == 1) {
			if ($data["is_event"] == 1) {
		...
		$event->content = $service->newContent($data["description"]);
	...
	public function updateGoogleCalendarItem($username, $password, $id, $data) {
		$client = $this->getGoogleClientLogin($username, $password, "calendar");
		return $this->updateGoogleCalendarItemClient($client, $id, $data);
	}
	
	public function updateGoogleCalendarItemClient($client, $id, $data) {
		$service = new Zend_Gdata_Calendar($client);
	...
	public function deleteGoogleCalendarItem($username, $password, $id) {
		$client = $this->getGoogleClientLogin($username, $password, "calendar");
		return $this->deleteGoogleCalendarItemClient($client, $id);
	}
	/* }}} */
	/* deleteGoogleCalendarItemClient {{{ */
	/**
	 * Remove a specific calendar item from the Google calendar service
	 *
	 * @param HttpClient a Google Clientlogin client
	 * @param string $id Google calendar item id
	 *
	 * @return bool true on success, false on failure
	 */	
	public function deleteGoogleCalendarItemClient($client, $id) {
		$service = new Zend_Gdata_Calendar($client);
	 ...
	 * @param string $username Google account username
	 * @param string $password Google account password
	 * @param boolean $silent Whether output is required
	 *
	 * @return void
	 */
	public function syncGoogleCalendar($user_id, $username, $password, $silent = true) {
		$client = $this->getGoogleClientLogin($username, $password, "calendar");
		return $this->syncGoogleCalendarClient($user_id, $client, $silent);
	}
	
	/* }}} */
	/* syncGoogleCalendarClient {{{ */
	/**
	 * Grab items from google and insert/update covide database
	 *
	 * @param int $user_id The covide user_id
	 ...
	 * @return void
	 */
	public function syncGoogleCalendarClient($user_id, $client, $silent = true) {
		$service = Zend_Gdata_Calendar::AUTH_SERVICE_NAME;
		$service = new Zend_Gdata_Calendar($client);
		// grab all google_id's from users calendar
		$sql = sprintf("SELECT calendar_id, google_id, calendar.subject FROM calendar_user right join calendar on calendar.id = calendar_user.calendar_id WHERE timestamp_start >= %d AND user_id = %d AND google_id != '0'", (time()-86400), $user_id);
		$res = sql_query($sql);
		while ($row = sql_fetch_assoc($res)) {
			try {
				...
				if ($response != null) {
					...
					echo "\n!!! No response from server at event: '".$row['subject']."'\n";
				}
			} catch (Exception $e) {
				if (!$silent) {
					echo "\n!!!   An appointment was not synched: '".$row['subject']."'\n";
				}
				continue;
			}
		}
		$cal = $this->getGoogleCalendarClient($client);
		foreach ($cal as $v) {
			$v["subject"] = 	mysql_real_escape_string($v["subject"]);
			$v["description"] = mysql_real_escape_string($v["description"]);
			$v["location"] = 	mysql_real_escape_string($v["location"]);
			...
			if (sql_num_rows($res)) {
				...
				$sql = sprintf("UPDATE calendar SET subject = '%s', body = '%s', location = '%s', timestamp_start = %d, timestamp_end = %d WHERE id = %d",
					$v["subject"], nl2br($v["description"]), $v["location"], $v["timestamp_start"], $v["timestamp_end"], sql_result($res, 0));
				$res = sql_query($sql);
			} else {
				if (date("d", $v["timestamp_start"]) != date("d", $v["timestamp_end"]) && date("H", $v["timestamp_start"]) == "00") {
					// this is an event
					$v["is_event"] = 1;
				} else {
					$v["is_event"] = 0;
				...
				$sql = sprintf("INSERT INTO calendar (subject, body, location, timestamp_start, timestamp_end, alldayevent) VALUES ('%s', '%s', '%s', %d, %d, %d)",
					$v["subject"], nl2br($v["description"]), $v["location"], $v["timestamp_start"], $v["timestamp_end"], $v["is_event"]);
				$res = sql_query($sql);
		...
		$res = sql_query($sql);
		while ($row = sql_fetch_assoc($res)) {
			$event_id = $this->createGoogleCalendarItemClient($client, $row);
	...
	public function deleteGoogleContact($username, $password, $id) {
		...
		try {
			...
			// If the returned code is a 404, the address has been deleted from the
			// google contacts services using the google webinterface.
			// This means we have to remove it from covide sync table as
			// well, so return true as if it succeeded normally.
			if ($http_code == 404) {
				return true;
			}
			return false;
	...
	/* }}} */
	/* getSpreadsheetUserToken {{{ */
	/**
	 * Returns a Google Spreadsheets login token
	 *
	 * @param string $username Google account username
	 * @param string $password Google account password
	 * @return string Google Spreadsheets login token
	 */
	public function getSpreadsheetUserToken($username = "", $password = "") {
		$service = Zend_Gdata_Spreadsheets::AUTH_SERVICE_NAME;
		$client = Zend_Gdata_ClientLogin::getHttpClient($username, $password, $service);
	...
	/* getFromSpreadsheet {{{ */
	/**
	 * Lists entries in the spreadsheet
	 *
	 * @param string $token A Google Spreadsheets login token, as returned by $this->getSpreadSheetToken
	 * @param array $options Optional array with options (spreadsheetId, worksheet -index or name-, queryString)
	 *
	 * @return array Array of elements
	 */
	public function getFromSpreadsheet($token, $options = array()) {
		if (count($options) == 0 || !isset($options["spreadsheetId"])) {
    ...
    if (is_string($options["worksheet"])) {
      $index = 0;
      foreach ($feed->entries as $entry) {
        if ($entry->content->text == $options["worksheet"]) {
        ...
        $index ++;
		  ...
		  $worksheetIndex = $options["worksheet"];
		}
    $worksheet = $feed->entries[$worksheetIndex];
		...
		if (isset($options["queryString"])) {
			$query->setSpreadsheetQuery($options["queryString"]);
		}
		$res = $service->getSpreadsheetListFeedContents($query);
	...
	/* updateSpreadsheet {{{ */
	/**
	 * Updates entries in a Google spreadsheet
	 *
	 * Warning: All entries found in the spreadsheet with given query will be updated!
	 *
	 * @param string $token A Google Spreadsheets login token, as returned by $this->getSpreadSheetToken
	 * @param array $rowDate array with data to be inserted ($column => $value)
	 * @param array $options Optional array with options (spreadsheetId, worksheetIndex, queryString)
	 *
	 * @return array Array of elements
	 */
	public function updateSpreadsheet($token, $rowData, $options = array()) {
		if (count($options) == 0 || !isset($options["spreadsheetId"])) {
			die("no valid options");
		}
		if (!isset($options["worksheetIndex"])) {
    ...
    $entries = $listFeed->entries;
    $curData = $service->getSpreadsheetListFeedContents($query);
    $updatedEntries = array();
    
    //Update all records, retain any original data that doesn't need to be overwritten.
    for ($i = 0; $i < sizeof($curData); $i++) {
      if ($entries[$i]) {
        ...
        $entry = $service->updateRow($entries[$i], $newData);
        $updatedEntries[] = $entry->save();
      }
    }
