<?php
     ...
     */
		...
		public function store2db($data) {
			if (isset($data["bcard_id"])) {
				$sql = sprintf("select * from address_businesscards where id = %d", $data["bcard_id"]);
				$res = sql_query($sql) or die(sql_error());
				$row = sql_fetch_assoc($res);

				$classifications = $row["classification"];
				$class_arr = explode("|", trim($classifications));

				if (!in_array($data["id"], $class_arr)) {
					$class_arr[] = $data["id"];
				}
				$new_class = "|";
				...
				$sql = sprintf("UPDATE address_businesscards SET classification = '%s' WHERE id = %d", $new_class, $data["bcard_id"]);				
				$res = sql_query($sql);
			}
