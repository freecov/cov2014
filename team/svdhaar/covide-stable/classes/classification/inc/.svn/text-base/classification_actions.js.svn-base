...
function cla_group_edit(id) {
	url = 'index.php?mod=classification&action=cla_group_edit&id='+id
	popup(url, 'cla_group_edit', 0, 0, 1);
}

function cla_group_remove(id) {
	if (confirm(gettext('Delete classification group?'))) {
		document.getElementById('action').value = 'cla_group_remove';
		document.getElementById('id').value = id;
		document.getElementById('claform').submit();
	}
}

function cla_group_save() {
	if (document.getElementById('clagroupedit')) {
		document.getElementById('clagroupedit').submit();
	}
}
