...
Class Covide {
	/* variables {{{ */
	/**
	 * @var int size of pages in paginated views, can be overwritten bij usersettings
	 */
	public $pagesize = 20;
	/**
	 * @var int default pagesize if none is set by the user
	 */
	public $pagesize_default = 20;
	/**
	 * @var int Alternative pagesize used in some places
	 */
	...
	public $db;
	/**
	 * @var string The dsn used to connect to the database
	 */
	public $dsn;
	/**
	 * @var string The type of database used, can by mysql, mysqli or pdo_mysql for now
	 */
	public $dbtype;
	/**
	 * @var string The database server in use
	 */
	public $dbserver;
	/**
	...
	public $filesyspath = "";
	/**
	 * @var string Path where temp files will be stored
	 */
	public $temppath = "";
	/**
	 * @var string Path where logs will be kept
	 */
	public $logpath = "";
	/**
	 * @var string Path where covide is running in
	 */
	public $webroot = "";
	/**
	 * @var int Mode that ssl is enforced XXX: list possibilities here please.
	 */
	public $sslmode;
	/**
	 * @var string Relative path to the ssl certificate for IE users to download
	 */
	public $certificate;
	/**
	 * @var int If set, the visitor is a mobile device
	 */
	public $mobile = 0;
	/**
	 * @var object Conversion object
	 */
	public $conversion;
	/**
	 * @var object Frame object to use for menu
	 */
	public $venster;
	/**
	 * @var array Currently running module name
	 */
	public $current_module = array();
	/**
	 * @var array Contribs loaded into this instance
	 */
	public $contrib        = array();
	/**
	 * @var array Statistics about synced records for Funambol
	 */
	public $sync_stats     = array();
	/**
	 * @var array Backlog of sql queries to put into the logfile
	 */
	public $query_log      = array();
	/**
	 * @var array Files currently opened to write sql logs to
	 */
	public $query_file     = array();

	/**
	 * @var int If set to 1, Covide will output valid xhtml 1.0
	 */
	public $output_xhtml   = 0;
	/**
	 * @var object If we use database session storage this variable will hold a reference to the sessionhandlig object
	 */
	public $session;
	/**
	...
	public $devmode = 0;
	/**
	 * @var string the ip of the visitor
	 */
	public $user_ip;
	/* }}} */
	/* methods */
	 ...
	 *
	 * @param int $skip_session_init If set, dont setup session.
	 */
	public function __construct($skip_session_init = 0) {
		// set the ip of the visitor
		if ($_SERVER["REMOTE_ADDR"] == "127.0.0.1" && array_key_exists("HTTP_X_FORWARDED_FOR", $_SERVER)) {
			// the x-forwarded-for can contain multiple ip addresses seperated by a ,
			if (strpos($_SERVER["HTTP_X_FORWARDED_FOR"], ",") !== false) {
				$forwarders = explode(",", $_SERVER["HTTP_X_FORWARDED_FOR"]);
				// the first ip listed is the most distant from us. use that as origin
				$ip = $forwarders[0];
			} else {
		...
		if (!preg_match('/^pdo_/s', $dsn)) {
			...
			$d = preg_split('/[\/:@\(\)]/s', $dsn, -1, PREG_SPLIT_NO_EMPTY);
			$this->dbserver = $d[4];
			unset($d);
			$this->dbtype =  get_class($this->db);
		} else {
			// compatibility
			...
			try {
				if ($d[4] == "mysql") {
					...
					$opts = array(PDO::ATTR_PERSISTENT => false);
				}
				$this->db = new PDO(sprintf('mysql:host=%s;port=%s;dbname=%s',
				$d[4], $d[5], $d[6]), $d[1], $d[2], $opts);
			} catch(exception $e) {
				header($_SERVER['SERVER_PROTOCOL'].' 500 Internal Server Error', true, 500);
			...
			unset($d);
			$this->dbtype =  get_class($this->db);
			$this->dbtype .= "_".$this->db->getAttribute(constant("PDO::ATTR_DRIVER_NAME"));
		}
		...
		if ($this->db instanceof MDB2_Driver_Mysql || $this->db instanceof MDB2_Driver_Mysqli) {
			require_once("common/functions_pear.php");
			$this->db->setFetchMode(DB_FETCHMODE_ASSOC);
			$this->db->setOption("autofree", 1);
		} else {
			require_once('common/functions_pdo.php');
		}
		/* set temp path */
		...
		if (!$skip_session_init) {
			// if we have a setting for database sessions
			if (is_array($conf) && $conf["sessions"] == "database") {
	...
	public function run_module($module="") {
		...
		switch ($module) {
			...
			case "twitter":            $mod = new Twitter();            break;
			default:
	...
	/* detect_mobile {{{ */
	/**
	 * Check current visitor against our mobile clients definition
	 *
	 * @param stning $str mobile clients
	 *
	 * @return void
	 */
	...
	/* force_ssl {{{ */
	/**
	 * Force usage of HTTPS or HTTP
	 * @todo Document states
	 *
	 * @param int $state State to force
	 *
	 * @return void
	 */
	...
	/* trigger_login {{{ */
	/**
	 * Redirect user to login screen when needed
	 *
	 * @return void
	 */
	...
	private function _check_officereq() {
		...
		if (!$_SESSION["allok"]) {
			...
			//make an exception for the finance module
			if (strstr($_SERVER['PHP_SELF'], "/classes/finance/non-oop")) {
				$ary[1] = "../../../../".$ary[1];
				$ary[2] = "../../../../".$ary[2];
				$ary[3] = "../../../../".$ary[3];
			}
	...
	/* cleanUpTempDir {{{ */
	/**
	 * Cleanup temp files and temp dirs
	 *
	 * @return void
	 */
	public function cleanUpTempDir() {
		...
		//temp/code smarty cache dir
		$files = array();
		if (file_exists($this->temppath."smarty/")) {
			$files = scandir($this->temppath."smarty/");
			foreach ($files as $file) {
				if (is_file($file)) {
					$ctime = @filectime($file);
					$cdiff = 120; //two minutes lifetime of template cache
	...
	/* run_once_a_day {{{ */
	/**
	 * Function that sets qotd and cleansup chatlogs.
	 *
	 * This function is part of the cronjob handling and
	 * will only do it's work once a day
	 *
	 * @return void
	 */
	private function run_once_a_day() {
		...
		if ($this->license["dayquote"] != $today) {
			...
			$q = sprintf("update license set dayquote = %d, dayquote_nr = '%s'",
				$today, addslashes($this->license["dayquote_nr"]));
			#$this->db->query($q);
			sql_query($q, $this->db);
			...
			if (is_object($this->session)) {
				$this->session->gc($this->session->lifetime);
			}
		}
	}
	/* }}} */
	/* checkPatchLevel {{{ */
	/**
	 * Check if we have to run patches
	 *
	 * @return void
	 */
	...
	/* patchAll {{{ */
	/**
	 * Check and run sql patches when needed
	 *
	 * @return void
	 */
			...
			if (end($files) > $lastpatch) {
				...
				foreach ($files as $f) {
					...
					if ($f > $lastpatch) {
						...
						$sql_patch_full = trim(file_get_contents($fn));
						$sql_patches = preg_split("/(;$|; )/six", $sql_patch_full);
						foreach ($sql_patches as $sql_patch) {
							if (trim($sql_patch)) {
								$res = sql_query(trim($sql_patch), 0, 0, 0, 0, 1);
							}
						}
					}
				}
				/* update autopatcher lastpatch */
				$sql_patch = sprintf("UPDATE license SET autopatcher_lastpatch='%s'", end($files));
				$res = sql_query($sql_patch);
			}
		}
	}
	/* }}} */
