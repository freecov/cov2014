...
Class ProjectDeclaration_data {
	...
	public function generateDocument($req) {
		...
		if ($bcard_id > 0) {
			...
			if (trim($btav["tav"]))
				...
				$address["tav"] = preg_replace("/ {1,}/", " ", sprintf("%s %s %s", $bcard["givenname"], $bcard["infix"], $bcard["surname"]));
		}
		...
		$html = str_replace("##dob##", date("d-m-Y", $address["timestamp_birthday"]).$br, $html);
		...
		$fulladdress = $address_data->getAddressById($fulluser["address_id"], "bcards");
	...
	public function deleteRegistration($id, $redirect = true) {
		$q = sprintf("select * from projects_declaration_registration where id = %d", $id);
		$res = sql_query($q);
		$row = sql_fetch_assoc($res);
	...
	public function deleteRegistrationMulti($ids) {
		$array = explode("|", $ids);
		foreach ($array as $id) {
			$this->deleteRegistration($id, false);
		}
