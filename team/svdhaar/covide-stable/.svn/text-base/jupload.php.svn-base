<?php
	if ($_REQUEST["action"] == "close") {
		...
		echo stripslashes(urldecode($_REQUEST['uri']));
		echo "</script>";
		exit();
	}
	$skip_run_module = 1;
	...
	$param = "?action=store&image=".$_REQUEST["image"]."&md5=".$md5."&user_id=".$_REQUEST["user"]."&map_id=".$_REQUEST["map_id"]."&mod=".$_REQUEST["mod"];
		...
		foreach ($_FILES as $file) {
			...
			if ($file["error"] == UPLOAD_ERR_OK && $file["tmp_name"]) {
				...
				if ($_REQUEST["mod"] == "filesys") {
					...
					$q.= sprintf("(%d, '%s', '%s', '%s', %d, %d, '%s')", $_REQUEST["map_id"], $name, $size, $type, time(), $_REQUEST["user_id"], ($_REQUEST["image"]) ? "- image upload" : "- multiple upload");
					sql_query($q);
					...
					if (in_array(strtolower($ext), array("jpg", "png", "gif", "jpeg"))) {
						...
						if ($_REQUEST['map_id'] == -1) {
							$mapid = 'banner';
						} else {
							$mapid = $_REQUEST['map_id'];
						}
						$dest = sprintf("%s/%s/%s/%d_full.jpg",
							$GLOBALS["covide"]->filesyspath, "gallery", $mapid, $dbid);
...
<?php
  	if ($_REQUEST["mod"] == "filesys") {
		...
		$uri = "parent.document.getElementById('velden').submit();";
  	} else {
		$policy = "PictureUploadPolicy";
		$uri    = "parent.location.href='index.php?mod=cms&action=cmsgallery&id=".$_REQUEST["map_id"]."';";
  	}
...
<?php if ($_REQUEST["mod"] != "filesys" || $_REQUEST["image"]) { ?>
...
<?php } ?>
<applet code="wjhk.jupload2.JUploadApplet" archive="jupload/wjhk.jupload.jar?m=<?php echo filemtime("jupload/wjhk.jupload.jar") ?>" width="800" height="500">
	...
	<param name="afterUploadURL" value="jupload.php?action=close&uri=<?php echo urlencode($uri) ?>" />
</applet>
</body>
</html>
<?php } ?>
