...
/* start the output */
$subtitle = gettext("zoekresultaat");
$output = new Layout_output();
$output->layout_page("", 1);
	$output->addCode(gettext("Gezocht op")." '".$_REQUEST["searchkey"]."'");
	$output->addTag("br");
	$output->addCode(gettext("relatie")." ".$address_data->getAddressNameById($_REQUEST["address_id"]));

	/* note results */
	$venster = new Layout_venster(array("title" => gettext("notities"), "subtitle" => $subtitle));
	$venster->addVensterData();
		...
		$view->addMapping(gettext("gelezen"), "%%is_read");
		$view->addMapping(gettext("afgehandeld"), "%%is_done");
	...
	$venster->addVensterData();
		...
		$view->addMapping(gettext("gebruiker"), "%h_user", "right");
