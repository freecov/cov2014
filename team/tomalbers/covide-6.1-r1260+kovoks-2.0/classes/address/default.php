...
Class Address {
	...
	public function __construct() {
		...
		switch ($_REQUEST["action"]) {
			...
			case "edit_bcard" :
				$address_output = new Address_output();
				$address_output->edit_bcard($_REQUEST["id"], $_REQUEST["address_id"]);
				break;
			case "save_bcard" :
				$address_data = new Address_data();
				$address_data->save_bcard();
				break;
			case "cardrem" :
				$address_data = new Address_data();
			...
			case "cardshow" :
				$address_output = new Address_output();
				$address_output->show_bcard($_REQUEST["cardid"]);
				break;
			case "financialshow" :
				$address_output = new Address_output();
				$address_output->show_FinancialCard($_REQUEST["cardid"]);
				break;
			case "relationsshow" :
				$address_output = new Address_output();
				$address_output->show_RelationsCard($_REQUEST["cardid"]);
				break;
			case "financialedit" :
				$address_output = new Address_output();
				$address_output->edit_FinancialCard($_REQUEST["cardid"]);
				break;
			case "relationsedit" :
				$address_output = new Address_output();
				$address_output->edit_RelationsCard($_REQUEST["cardid"]);
				break;
			case "save_financialcard" :
				$address_data = new Address_data();
				$address_data->save_financialcard();
				break;
			case "save_relationscard" :
				$address_data = new Address_data();
				$address_data->save_relationscard();
				break;
			case "financialbankedit" :
				$address_output = new Address_output();
				$address_output->edit_FinancialBank($_REQUEST["bankid"]);
				break;
 			case "pick_bank" :
                                $address_output = new Address_output();
                                $address_output->pick_bank();
                                break;
			case "save_financialbank" :
				$address_data = new Address_data();
				$address_data->save_financialbank();
				break;
			case "edit" :
				$address_output = new Address_output();
				$address_output->show_edit($_REQUEST["id"], $_REQUEST["addresstype"], $_REQUEST["sub"]);
				break;
			case "save" :
				$address_data = new Address_data();
			...
			case "togglesync" :
				$address_data = new Address_data();
				$address_data->toggleSync($_REQUEST["address_id"], $_REQUEST["identifier"], $_REQUEST["toggleaction"], $_SESSION["user_id"]);
				break;
			case "export" :
				$address_output = new Address_output();
