		...
		switch ($table) {
			...
			case "address_private" :
				...
				if (sql_result($res2,0)>0) {
					$data["contactperson"] = str_replace("  "," ",$row["givenname"]." ".$row["surname"])." (".gettext("medewerker").")";
				} else {
					$data["contactperson"] = str_replace("  "," ",$row["givenname"]." ".$row["surname"])." (".gettext("persoon").")";
				}
			...
			case "address_other":
				...
				$data["contactperson"]  = $row["companyname"]." (".gettext("overig").")";
				//internal vars
				if ($row["is_public"]!=1) {
					$data["prive"]          = $row["user_id"];
					$data["priveperson"]    = 2;
