	...
	public function getNotes($options = "") {
		...
		switch ($options["note_type"]) {
			...
			case "sent"    :
				...
				break;
			case "show"    :
				$query = "SELECT $fields FROM notes $join where (is_read=0 OR is_read IS NULL) AND sender=".(int)$options["user_id"]." AND delstatus!=1 $q ORDER BY $order";
				$query_count = "SELECT COUNT(*) FROM notes where (is_read=0 OR is_read IS NULL) AND sender=".(int)$options["user_id"]." AND delstatus!=1 $q";
				$showasold = 1;
				break;
			case "all" :
