<?php
	...
	if ($options["nolimit"] && $options["folder"] == $archive["id"] && !$options["relation"] && !$options["relation_inbox"]) {
		...
		if ($options["search"]) {
			$sq2.= " INNER JOIN mail_messages_data ON mail_messages_data.mail_id = mail_messages.id ";
		}
		$sq2.= " WHERE 1=1 ";
	}
	...
	if ($options["relation_inbox"] || $options["global_index"]) {
		...
		if ($options["search"]) {
			$sq2 = $this->getEmailBySearchAddSearchQuery($sq2, $options["search"], $use_subquery);
		}
	} else {
		...
		$sq2.= sprintf("(folder_id = %d AND (is_public IN (0,1) OR is_public IS NULL))) %s ", $options["folder"], $sq);
	}
	...
	while ($row = sql_fetch_assoc($res)) {
		...
		if (!strlen(trim($row["subject"]))) { $row["subject"] = "[".gettext("geen onderwerp")."]"; }
		...
		$row["short_date"]          = date("d-m-Y", $row["date"]);
		$row["short_time"]          = date("H:i", 	$row["date"]);
		...
		if ($row["h_description"]) {
			$row["h_description"] = sprintf("%s: %s", gettext("omschrijving"), $row["h_description"]);
		}
		...
		if (!$options["skip_extends"]) {
			/* get tracker items */
			$tracker = $this->get_tracker_items($row["id"]);
			if ($tracker["count"]>0) {
				$row["tracking"] = 1;
				$current = $this->status_queue($row["id"]);
				if ($current != count($tracker["list"]) && $current > 0) {
					$row["tracking_resume"] = 1;
					$row["tracking_status"] = $current." ".gettext("van")." ".count($tracker["list"]);
				}
			}
			/* parse the header */
