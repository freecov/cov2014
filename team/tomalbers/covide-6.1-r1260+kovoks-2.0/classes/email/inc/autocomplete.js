...
function init_layer(ary) {
	...
	var html = '';
	html = html.concat('<tr><td style="border-bottom: 1px solid #999;"><span class="d"><b>', titles[0], '</b></span></td><td style="border-bottom: 1px solid #999;"><span class="d"><b>', titles[1], '</b></span></td><td style="border-bottom: 1px solid #999;"><span class="d"><b>', titles[2], '</b></span></td></tr>');
	var tmp = '';
	for (i=2;i<ret.length-1;i++) {
		flag++;
		tmp = ret[i].split('|');
		tmp[0] = '<a href="javascript: void(0);" onclick="javascript: set_complete(\''+field+'\', \''+tmp[0]+'\');">&lt;'+tmp[0]+'&gt;</a>';
		html = html.concat('<tr><td><span class="d">', tmp[0], '</span></td><td><span class="d">', tmp[1], '</span></td><td><span class="d">', tmp[2], '</span></td></tr>');
	}
	if (flag==0) {
	...
	if (ret.length >= 20) {
		if (email_complete_expand_results == 1) {
			html = html.concat('&lt;', gettext("minder resultaten"), '&gt;');
		} else {
			html = html.concat('&lt;', gettext("meer resultaten"), '&gt;');
		}
	}
