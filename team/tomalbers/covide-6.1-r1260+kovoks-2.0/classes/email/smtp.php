	...
	public function send() {
		...
		if (!$fp) {
			...
			$output->layout_page("error", 1);
				...
				$venster->insertTag("b", gettext("Er is een fout opgetreden tijdens het verzenden van de email.")." (".gettext("Poging").": ".($_REQUEST["try"]+1).")" );
				$venster->addTag("br");
				$venster->addTag("br");
				...
				$venster->insertAction("mail_send", gettext("opnieuw proberen"), "?mod=email&action=mail_send&dl=1&id=".$_REQUEST["id"]."&try=".($_REQUEST["try"]+1));
				$venster->addCode(" ".gettext("om het opnieuw te proberen").".");
				$venster->addTag("br");
				$venster->addTag("br");
				$venster->addCode(gettext("Klik")." ");
				$venster->insertTag("a", gettext("hier"), array(
				...
				$venster->insertAction("back", gettext("naar covide"), "?mod=email&action=compose&id=".$_REQUEST["id"]);
				$venster->addCode(" ".gettext("om terug te gaan naar de email").".");
				$venster->addTag("br");
				$venster->addTag("br");
				$venster->addTag("br");
				$venster->addCode(gettext("De exacte foutmelding is:"));
				$venster->addTag("hr");
				$venster->addTag("br");
