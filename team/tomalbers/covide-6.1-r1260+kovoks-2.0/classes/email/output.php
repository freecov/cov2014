...
Class Email_output {
	...
	public function viewHtml() {
		...
		$body = preg_replace("/<a ([^>]*?)>/si", "<a target=\"_blank\" $1>", $body);
	...
	public function getSelectList($name, $folders, $current, $settings="", $folders_shared="") {
		...
		if ($folders_shared) {
			...
			$list = array(
				gettext("mijn mappen") => $list,
				gettext("gedeelde mappen") => $list2
			);
		}
	...
	public function emailShowTracking() {
		...
		if ($mdata[0]["classifications_type"]=="and") {
			$mdata[0]["h_classifications_type"] = gettext("unieke classificaties (en)");
		} elseif ($mdata[0]["classifications_type"]=="or") {
			$mdata[0]["h_classifications_type"] = gettext("opgetelde classificaties (of)");
		}

		if ($mdata[0]["classifications_target"]=="relations") {
			$mdata[0]["h_classifications_target"] = gettext("adressen");
		...
		$tbl->addTableRow();
			$tbl->addTableData();
				...
				$view->addMapping(gettext("classificatie adres type"), "%h_classifications_target");
				$view->addMapping(gettext("classificatie selectie"), "%h_classifications_type");
				$view->addMapping(gettext("classificaties positief"), "%h_classifications_positive", "", "list_hidden");
				$view->addMapping(gettext("classificaties negatief"), "%h_classifications_negative", "", "list_hidden");
		...
		$tbl->addTableRow();
			$tbl->addTableData(array("colspan"=>2));
				...
				$view->addMapping(gettext("ontvanger"), "%email");
				$view->addMapping(gettext("eerst/laatst gelezen"), array(
					"%read_first_h",
					"\n",
					"%read_last_h"
				));
				$view->addMapping(gettext("aantal"), array("%count", "x gelezen"));
	...
	public function insertFromToHeader($html=0, $maildata, $hide_orig_msg=0) {
		...
		if ($html) {
			...
			if (!$hide_orig_msg) {
				$output->addCode( sprintf("----- %s -----", gettext("Oorspronkelijk Bericht")) );

			}
			$output->addTag("div", array("style"=>"background: #e4e4e4; font-color: black"));
				$output->insertTag("b", gettext("naar"));
				$output->addSpace();
				$output->addCode($maildata["to"]);
			$output->endTag("div");
			$output->addTag("div");
				$output->insertTag("b", gettext("van"));
				$output->addSpace();
			...
			$output->addTag("div");
				$output->insertTag("b", gettext("datum"));
				$output->addSpace();
				$output->addCode($maildata["h_date"]);
			$output->endTag("div");
			$output->addTag("div");
				$output->insertTag("b", gettext("onderwerp"));
				$output->addSpace();
	...
	public function showInfo() {
		...
		$fields[gettext("van")]              = $mdata["sender_emailaddress_h"];
		$fields[gettext("naar")]             = $mdata["to"];
		$fields[gettext("onderwerp")]        = $mdata["subject"];
		$fields[gettext("cc")]               = $mdata["cc"];
		$fields[gettext("bcc")]              = $mdata["bcc"];
		$fields[gettext("reply naar")]       = $mdata["replyto"];
		$fields[gettext("datum verzonden")]  = $mdata["h_date"];
		$fields[gettext("datum ontvangen")]  = $mdata["h_date_received"];
		$fields[gettext("leesbevestiging")]  = $mdata["readconfirm"];
		$fields[gettext("prioriteit")]       = $mdata["prioriteit"];
		$fields[gettext("omschrijving")]     = $mdata["description"];
		$fields[gettext("relatie")]          = $addressData->getAddressNameById($mdata["address_id"]);
		...
		if ($mdata["check_askwichrel"]) {
			$table->addTableRow();
				$table->insertTableData(gettext("kies relatie"), "", "header");
				$table->addTableData();
	...
	public function templateList() {
		...
		$view->addMapping( gettext("template omschrijving"), "%description" );
		...
		$venster->addMenuItem(gettext("nieuw"), "?mod=email&action=templateEdit");
	...
	public function signatureList($user_id=0) {
		...
		$view->addMapping( gettext("omschrijving"), "%subject" );
		$view->addMapping( gettext("handtekening"), "%signature" );
		$view->addMapping( gettext("volledige naam"), "%realname" );
		...
		$venster->addMenuItem(gettext("nieuw"), "?mod=email&action=signatureEdit");
		$venster->addMenuItem(gettext("terug"), "javascript: history_goback();");
		$venster->generateMenuItems();
	...
	public function filterList() {
		...
		$view->addMapping( gettext("van email"), "%sender" );
		$view->addMapping( gettext("ontvanger email"), "%receipient" );
		$view->addMapping( gettext("onderwerp"), "%subject" );
		$view->addMapping( gettext("naar map"), "%folder_name" );
		...
		$venster->addMenuItem(gettext("nieuw"), "?mod=email&action=filterEdit");
		$venster->addMenuItem(gettext("terug"), "javascript: history_goback();");
		$venster->generateMenuItems();
	...
	public function permissionsList($user_id=0) {
		...
		$venster = new Layout_venster( array(
			...
			"subtitle" => gettext("mappen delen en rechten instellen")
		));
		...
		$view->addMapping( gettext("mapnaam"), "%%complex_folder" );
		$view->addMapping( gettext("gebruikers"), "%h_users" );
		...
		$venster->addMenuItem(gettext("nieuw"), "?mod=email&action=permissionsEdit&user_id=".$_REQUEST["user_id"]);
