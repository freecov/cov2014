...
$table->addTableRow();
       $table->addTableData(array("colspan"=>27));
               ...
               $table->insertLink(gettext("all"), array(
                       "href" => "index.php?mod=product&sub=all"
               ) );
               $table->addSpace(2);
               $table->insertLink(gettext("actief"), array(
                       "href" => "index.php?mod=product&sub=active"
               ) );
               $table->addSpace(2);
               $table->insertLink(gettext("inactive"), array(
                       "href" => "index.php?mod=product&sub=inactive"
               ) );
       $table->endTableData();
$table->endTableRow();
	...
	$table->addTableRow();
		$table->addTableData();
			...
			if ($productinfo_arr["total_count"]) {
				$table->addCode(gettext("products")." ".($productinfo_arr["top"]+1)." ".gettext("to")." ".$productinfo_arr["bottom"]." ".gettext("of")." ".$productinfo_arr["total_count"]);
			}
		$table->endTableData();
	$table->endTableRow();
$table->endTable();
...
$table->addTableRow();
        $table->addTableData(array("style"=>"text-align: right", "colspan"=>1));
                $url = "index.php?mod=product&action=$action&addresstype=$addresstype&top=%%&l=$l&sub=".$sub."&search=$search";
