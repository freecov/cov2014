...
$table->addTableRow();
	$table->addTableData();
		...
		$product_name = $address_data->getProductNameById($deb);
		$table->addCode( gettext("product").": ".$product_name);
		$table->addSpace(2);
		$table->insertAction("last", gettext("show all products"), "javascript: drop_filter();");
	$table->endTableData();
$table->endTableRow();
$table->addTableRow();
	$table->addTableData();
