...
Class Desktop_output {
	...
	public function show_desktop() {
		...
		if ($userinfo["rssnews"]) {
			...
			$venster = new Layout_venster();
			$venster->addVensterData( array("width"=>"100%") );
			$venster->addCode("<b>".gettext("nieuws")."</b>");
			if ($userinfo["xs_usermanage"])
				$venster->insertAction("edit", gettext("wijzigen"), "index.php?mod=rss&action=listFeeds");
			$venster->addTag("br");
				...
				$table->addTableData(array("colspan" => 3));
					...
					$venster->addVensterData();
						...
						foreach ($bd as $k=>$v) {
							...
							$venster->addCode( " ".$v["name"] );
							if ($v["company_name"]) {
								$venster->addCode(" (".gettext("van")." <a href='?mod=address&action=relcard&id=".$v["company_id"]."'>".$v["company_name"]."</a>)");
							}
