...
if ($_SESSION["user_id"]) {
	...
	if ($_SESSION["user_id"]!=$archief_user_id) {
		...
		if ($GLOBALS["covide"]->license["disable_basics"]!=1) {
			...
			$ondAlt  = array(gettext("Adressen"), gettext("Agenda"), gettext("Notities"), gettext("E-mail"), gettext("Bestandsbeheer"));
			$ond[count($ond)]         = "nieuwsbrief";
			$ondLink[count($ondLink)] = $parent."?mod=newsletter";
			$ondAlt[count($ondAlt)]   = gettext("Nieuwsbrief");
		} else {
			/*
			$ond     = array("adressen", "bestandsbeheer");
			$ondLink = array($parent."adres/", $parent."bestandsbeheer/");
			$ondAlt  = array(gettext("Adressen"), gettext("Bestandsbeheer"));
			*/
			if ($GLOBALS["covide"]->license["has_voip"]) {
				...
				$ondAlt  = array(gettext("Adressen"), gettext("Notities"));
			} else {
				$ond     = array("adressen");
				$ondLink = array($parent."?mod=address");
				$ondAlt  = array(gettext("Adressen"));
			}
		}
		...
		if ($GLOBALS["covide"]->license["has_project"]==1) {
			...
			$ondAlt[count($ondAlt)]   = gettext("Projecten");
		}
		...
		if ($GLOBALS["covide"]->license["has_faq"]==1) {
			...
			$ondAlt[count($ondAlt)]   = gettext("Vraag en Antwoord");
		}
		...
		if ($GLOBALS["covide"]->license["has_announcements"]==1) {
			...
			if ($rp) {
				...
				$ondAlt[count($ondAlt)]   = gettext("Prikbord");
			}
		}
	...
	if ($_SESSION["user_id"]!=$archief_user_id) {
		...
		$ondOAlt  = array(gettext("Instellingen"), gettext("Handleiding"));
		...
		if ((date("w")==5) && $licensie["has_snack"]==1) {
			...
			$ondOAlt[count($ondOAlt)]   = gettext("Snackbestelling");
		}
	...
	$ondOAlt[count($ondOAlt)]   = gettext("Uitloggen");
}
?>
