...
$venster->addVensterData();
	...
	$venster->insertAction("back", gettext("terug"), sprintf(
	...
	if (!$addressinfo[0]["is_active"]) 
		$view->addMapping("&nbsp", "<span style=\"background-color: #d34545; color: white;\">&nbsp;".gettext("INACTIEF")."&nbsp;</span>");
	$view->addMapping(gettext("bedrijfsnaam"), "%%companyname");
	$view->addMapping(gettext("snelkies nr"), "%speeddial");
	...
	$view->addMapping(gettext("relationtype"), "%%relation_type");
	$view->addMapping(gettext("classificatie(s)"), "%classification_names");
	$view->addMapping(gettext("financial"), "%%showfinance");
	if ($addressinfo[0]["is_supplier"])
		$view->addMapping(gettext("products"), "%%showproducts");
	if ($addressinfo[0]["is_supplier"] || $addressinfo[0]["is_customer"])
		$view->addMapping(gettext("relations"), "%%showrelations");
        if ($addressinfo[0]["is_customer"])
        {
                $view->addMapping(gettext("offers"), "%%showoffers");
                $view->addMapping(gettext("prices"), "%%showprices");
        }
	...
	$view->defineComplexMapping("showfinance", array(
		array(
			"type"   => "link",
			"text"   => gettext("show financial data"),
			"link"   => array("javascript: popup('index.php?mod=address&action=financialshow&cardid=". $_REQUEST["id"]. "', 'view', 0, 0, 1);")
		)
	));
	$view->defineComplexMapping("showproducts", array(
		array(
			"type"   => "link",
			"text"   => gettext("show products"),
			"link"   => array("index.php?mod=product&action=showonesupplier&supplierid=". $_REQUEST["id"])
		)
	));
	$view->defineComplexMapping("showoffers", array(
		array(
			"type"   => "link",
			"text"   => gettext("show offers"),
			"link"   => array("index.php?mod=product&action=showoffers&customerid=". $_REQUEST["id"])
		)
	));
	$view->defineComplexMapping("showprices", array(
		array(
			"type"   => "link",
			"text"   => gettext("show prices"),
			"link"   => array("index.php?mod=product&action=showprices&customerid=". $_REQUEST["id"])
		)
	));
	if ($addressinfo[0]["is_supplier"])
		$t .= gettext("Supplier").", ";
	if ($addressinfo[0]["is_customer"])
		$t .= gettext("Customer").", ";
	if ($addressinfo[0]["is_transporter"])
		$t .= gettext("Transporter").", ";
	if ($addressinfo[0]["is_contact"])
		$t .= gettext("Contact").", ";
	if (strlen($t)>2)
		$t = substr($t,0,-2);
		
	$view->defineComplexMapping("relation_type", array(
		array(
			"type"	 => "text",
			"text"   => $t,
			"check"	 => $t
	...
	unset($t);
	if ($addressinfo[0]["is_supplier"] && $addressinfo[0]["is_customer"])
		$t = gettext("show customers and suppliers");
	else if ($addressinfo[0]["is_supplier"])
		$t = gettext("show customers");
	else if ($addressinfo[0]["is_customer"])
		$t = gettext("show suppliers");
	$view->defineComplexMapping("showrelations", array(
		array(
			"type"   => "link",
			"text"   => $t,
			"link"   => array("javascript: popup('index.php?mod=address&action=relationsshow&cardid=". $_REQUEST["id"]. "', 'view', 0, 0, 1);")
		)
	));
	unset($t);
...
$venster->addVensterData();
	...
	$view->defineComplexMapping("complex_actions", array(
		...
		array(
			...
			"link"    => array("javascript:if(confirm(gettext('Dit verwijdert de businesscard en alle koppelingen. Weet u dit zeker?'))){document.location.href='index.php?mod=address&action=relcard&id=$id&history=".$_REQUEST["history"]."&relcardaction=cardrem&cardid=", "%id", "';}")
		)
	));
...
$venster->addVensterData();
	...
	$view->defineComplexMapping("subject", array(
		array(
			"type" => "link",
			"link" => array("javascript: popup('index.php?mod=note&action=message&hidenav=$hidenav&msg_id=", "%id", "', 'shownote', 0, 0, 1);"),
			"text" => "%subject"
		)
	));
	$venster->addCode($view->generate_output());
...
$output->insertAction("back", gettext("terug"), sprintf(
	"javascript: history_goback('%d');" , $_REQUEST["restore_point_steps"]));
