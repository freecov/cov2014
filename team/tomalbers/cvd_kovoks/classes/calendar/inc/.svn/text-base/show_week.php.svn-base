<?php
	...
	$venster->addVensterData();
		...
		$table->addTableRow();
			$table->addTableData();
				...
				for ($i=0;$i<7;$i++) {
					...
					/* weekday */
					$day = mktime(0,0,0,date("m",$weekstart), date("d",$weekstart)+$i, date("Y", $weekstart));	
					$start_day   = date("d", $day);
					$start_month = date("m", $day);
					$start_year  = date("Y", $day);
		...
		$venster->insertAction("back", gettext("vorige week"), sprintf("?mod=calendar&extrauser=%s&day=%d&month=%d&year=%d", $_REQUEST["extrauser"], $day-7, $month, $year));
		$venster->insertAction("forward", gettext("volgende week"), sprintf("?mod=calendar&extrauser=%s&day=%d&month=%d&year=%d", $_REQUEST["extrauser"], $day+7, $month, $year));
