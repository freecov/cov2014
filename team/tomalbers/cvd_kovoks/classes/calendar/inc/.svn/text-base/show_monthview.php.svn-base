<?php
	...
	$venster = new Layout_venster(array(
		"title" => gettext("maandoverzicht")
	));
	$venster->addMenuItem(gettext("dagoverzicht"),  sprintf("./?mod=calendar&forceday=1&day=%d&month=%d&year=%d",
	...
	$table->addTableRow();
		$table->addTableData();
			...
			$table_cal->addTableRow();
				$table_cal->insertTableData(gettext("zo"), array("width" => "14%", "style" => "border: 1px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("ma"), array("width" => "14%", "style" => "border: 1px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("di"), array("width" => "14%", "style" => "border: 1px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("wo"), array("width" => "14%", "style" => "border: 1px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("do"), array("width" => "14%", "style" => "border: 1px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("vr"), array("width" => "14%", "style" => "border: 1px solid #CCC"), "header");
				$table_cal->insertTableData(gettext("za"), array("width" => "14%", "style" => "border: 1px solid #CCC"), "header");
					...
					if (($i+1) == date("d") && $month == date("m")) {
						...
						$table_day->addTableRow();
							$table_day->addTableData(array("width" => "100%", "align" => "right"));
								...
								$table_day->insertAction("new", gettext("nieuw"), "javascript: calendaritem_edit(0,".$user_id.",".$datemask.");");
							...
							$table_day->addTableData();
								if (count($calendar_data->calendar_items)) {
									foreach ($calendar_data->calendar_items as $v) {
										$table_day->insertAction("info", gettext("tonen"), "javascript: toonInfo(".$v["id"].");");
										if (!$v["is_registered"]) {
											$table_day->insertAction("edit", gettext("wijzig"), "javascript: calendaritem_edit(".$v["id"].", ".$user_id.", ".$datemask.");");
										}
										if ($v["is_event"] != 1) {
	...
	$table->addTableRow();
		$table->addTableData();
			...
			$table->insertAction("forward", gettext("selecteer"), "javascript: document.getElementById('search').submit();");
			$table->addHiddenField("user_id", $user_id);
			$useroutput = new User_output();
			$table->addCode( $useroutput->user_selection("user_id", $user_id, 0, 0, 0, 1) );
			$table->insertAction("forward", gettext("search"), "javascript: document.getElementById('search').submit();");
			$table->endTag("form");
		$table->endTableData();
	$table->endTableRow();
	$table->endTable();
