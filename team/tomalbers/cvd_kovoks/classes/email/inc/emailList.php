<?php
	...
	$table->addTableRow();
		...
		$table->addTableData();
			...
			$table->insertAction("view_all", gettext("korte / lange weergave"), "javascript: toggle_shortview();");
		$table->endTableData();
		$table->addTableData( array("align"=>"right") );
			...
			$table->insertAction("permissions", gettext("mappen delen"), "javascript: popup('index.php?mod=email&action=show_permissions&user_id=".$_SESSION["user_id"]."');");
	...
	if (!$_REQUEST["short_view"]) {
		...
		$view->addMapping(gettext("onderwerp"), "%%data_subject");
		$view->addMapping("", "%%complex_flags");
		$view->addMapping(gettext("afzender"), "%sender_emailaddress");
		$view->addMapping(gettext("datum"), "%short_date");
	...
	$view->defineComplexMapping("complex_flags", array(
		array(
			...
			"src"   => "mail_new",
			"check" => "%is_new",
			"alt"   => gettext("deze email is ongelezen"),
		...
		array(
			...
			"src"   => "mail_headers",
			"check" => "%h_description",
			"alt"   => gettext("deze email heeft een opmerking"),
		...
		array(
			...
			"alt"   => gettext("deze email heeft attachments"),
			"link" => array("?mod=email&action=$_action&sort=".$_REQUEST["sort"]."&id=", "%id")
		),
	));
	...
	$view->defineComplexMapping("data_actions", array(
		...
		array(
			...
			"src"   => "important",
			"alt"   => array(
				gettext("deze nieuwsbrief is afgebroken bij "),
				"%tracking_status",
				" ".gettext("adressen").", ",
				gettext("zie ook icoon rechts")."."
			),
		...
		array(
			"type"  => "action",
			"src"   => "folder_denied",
			"alt"   => gettext("deze nieuwsbrief is afgebroken, klik hier om verder te gaan met versturen."),
	...
	if ($shared_folders) {
		$venster_mappen_shared = new Layout_venster(array("title" => gettext("Gedeelde mappen")));
