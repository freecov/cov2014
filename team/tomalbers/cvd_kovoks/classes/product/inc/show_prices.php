...
if ($_REQUEST["prodcardaction"] == "rem" && $_REQUEST["priceid"]>0) {
    $productdata->delPrice($_REQUEST["priceid"]);
}
...
$output->addHiddenField("action", "showprices");
...
$venster = new Layout_venster(Array(
	"title"    => gettext("Price agreements"),
	"subtitle" => $subtitle
));
$venster->addMenuItem(gettext("new price"), "index.php?mod=product&action=editprice&customerid=".$customerid);
...
$venster->addCode($table->generate_output());
unset($table);
$productinfo_arr = $productdata->getPriceList($options);
$table = new Layout_table();
	$table->addTableRow();
		$table->addTableData();
			$table->addSpace(1);
	...
	$table->addTableRow();
		$table->addTableData();
			...
			if ($productinfo_arr["total_count"]) {
				$table->addCode(gettext("prices")." ".($productinfo_arr["top"]+1)." ".gettext("to")." ".$productinfo_arr["bottom"]." ".gettext("van de")." ".$productinfo_arr["total_count"]);
			}
		$table->endTableData();
	$table->endTableRow();
$table->endTable();
...
if ($customerid == '')
    $view->addMapping(gettext("customer"), "%human_customer");
$view->addMapping(gettext("producer"), "%producer");
$view->addMapping(gettext("product"), "%productinfo_name");
$view->addMapping(gettext("label"), "%productinfo_label");
$view->addMapping(gettext("year"), "%productinfo_year","right");
$view->addMapping(gettext("box"), "%productinfo_box","right");
$view->addMapping(gettext("agreement price"), "%price", "right");
$view->addMapping(gettext("pricelist price"), "%productinfo_pricelist","right");
$view->addMapping(gettext("Valid from"), "%human_start_date");
$view->addMapping(gettext("Valid till"), "%human_end_date");
$view->addMapping(gettext("productnr cust"), "%productnr_cust");
...
$view->defineComplexMapping("complex_actions", array(
	array(
		...
		"src"     => "edit",
		"alt"     => gettext("edit"),
		"link"    => array("index.php?mod=product&action=editprice&id=", "%id"),
	),
        array(
                "type"    => "action",
                "src"     => "delete",
                "alt"     => gettext("verwijderen"),
                "link"    => array("javascript:if(confirm('This will delete price agreement ","%id",". Are you sure you want to continue?'))".
                                   "{document.location.href='index.php?mod=product&customerid=$customerid&action=showprices&priceid=","%id","&history=".
...
if (!$productinfo_arr["total_count"]) {
	$venster->addCode(gettext("no prices found"));
