...
Class Product {
	...
	public function __construct() {
		...
		switch ($_REQUEST["action"]) {
			...
			case "summary" :
			        $product_output = new Product_output();
				$product_output->showSummary($_REQUEST["productid"]);
				break;
			...
			case "showprices" :
			        $product_output = new Product_output();
				$product_output->showPrices();
			case "editprice" :
				$product_output = new Product_output();
				$product_output->editPrice($_REQUEST["id"]);
				break;
                        case "saveprice" :
                                $product_data = new Product_data();
                                $product_data->storePrice2db($_REQUEST["data"], $_REQUEST["metafield"]);
                                break;
			...
			case "offersummary" :
				$product_output = new Product_output();
    			        $product_output->showOfferSummary($_REQUEST["customerid"],$_REQUEST["supplierid"],$_REQUEST["productid"]);
				break;
