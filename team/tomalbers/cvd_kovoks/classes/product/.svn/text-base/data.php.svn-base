<?php
	...
	/* -------------------------------------------- Products ----*/
		...
		/* }}} */
             ...
             public function getProducerFromProduct($productid) {
	                $sql = sprintf("SELECT supplierid FROM products WHERE id=%d", $productid);
	                $res = sql_query($sql);
     ...
     * @param metafields Metafield data (optional)
     */
    public function storePrice2db($data, $metafields = array()) 
    {
        require(self::include_dir."dataStorePrice2db.php");
    }


    public function getPriceList($options)
    {
        require(self::include_dir."dataGetPriceList.php");
        return $priceinfo;
    }
    
    public function delPrice($priceid)
    {
        $sql =  sprintf("DELETE FROM products_prices where id=%d limit 1",  $priceid);
        $res = sql_query($sql);
    }
    
    public function getPriceByID($id)
    {
        $sql = sprintf("SELECT * FROM products_prices WHERE id=%d", $id);
        $res = sql_query($sql);
        $priceinfo = sql_fetch_assoc($res);
        return $priceinfo;
    }
