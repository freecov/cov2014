<?php
	...
	if ($userpermissions["xs_usermanage"] == 1 || $userpermisions["xs_limitusermanage"] == 1) {
		...
		if ($userpermissions["xs_usermanage"] == 1) {
			...
			$velden[] = "xs_offermanage";         $waarden[] = sprintf("%d", $userinfo["xs_offermanage"]);
		...
		#$velden[] = "mail_forward";                  $waarden[] = sprintf("%d",   $userinfo["mail_forward"]);
		...
		$velden[] = "mail_signature";                $waarden[] = sprintf("'%s'", $userinfo["mail_signature"]);
		...
		if ($userinfo["id"]==$_SESSION["user_id"]) {
			...
			//allowed
			$query = "UPDATE users SET ";
			$query.= sprintf("mail_signature='%s'", $userinfo["mail_signature"]);
			...
			//not allowed
			die(gettext("u heeft geen rechten om deze gebruiker aan te passen."));
		}
	}
	if (!$error) {
