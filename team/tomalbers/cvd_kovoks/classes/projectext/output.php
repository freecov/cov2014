...
Class ProjectExt_output {
	...
	public function genOverviewSearch($formname="projectsearch") {
		...
		foreach ($data as $k=>$v) {
			if ($v["field_type"] == 2) {
				$sel[$v["id"]] = gettext("datum").": ".$v["field_name"];
			} elseif ($_REQUEST["action"] == "showinfo" && in_array($v["field_type"], array(0,1,3,5)) ) {
				$sel[$v["id"]] = gettext("tekst").": ".$v["field_name"];
			}
		}
		if (count($sel)>0) {
			$output->addSpace(5);
			...
			$output->addCode(" ".gettext("gebruik het zoekveld links om een zoekterm op te geven voor dit veld"));
			$output->endTag("span");
			$output->addTag("span", array(
				"id" => "datesearch",
				"style" => "display: none;"
