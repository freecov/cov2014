...
Class Filesys_output {
	...
	public function file_edit($id) {
		...
		$output->layout_page("", 1);
			...
			$venster = new Layout_venster(array(
				...
				"subtitle" => gettext("bestand bewerken")
			));
			$venster->addVensterData();
				$table = new Layout_table();
				$table->addTableRow();
					$table->insertTableData(gettext("bestandsnaam"), array("style"=>"vertical-align: top;"), "header");
					$table->addTableData("", "data");
						$table->addCode($file["name"]);
					$table->endTableData();
				$table->endTableRow();
				$table->addTableRow();
					$table->insertTableData(gettext("omschrijving"), array("style"=>"vertical-align: top;"), "header");
					$table->addTableData();
				...
				$table->addTableRow();
					...
					$table->addTableData("", "header");
						$table->insertAction("save", gettext("opslaan"), "javascript: fsave();");
					$table->endTableData();
				$table->endTableRow();
				$table->endTable();
	...
	public function folderEdit($id) {
		...
		$output->layout_page("", 1);
			...
			$venster = new Layout_venster(array(
				"title"    => gettext("bestandsbeheer"),
				"subtitle" => gettext("map bewerken")
			));
			$venster->addVensterData();
				$table = new Layout_table();
				$table->addTableRow();
					$table->insertTableData(gettext("naam"), array("style"=>"vertical-align: top;"), "header");
					$table->addTableData("", "data");
	...
	public function show_fileupload() {
		...
		$table->addTableRow();
			$table->addTableData();
				$table->addCode(gettext("bestand uploaden"));
				$table->addTag("br");
				...
				$table->endTag("form");
				$table->addTag("br");
				$table->insertAction("save", gettext("bestand uploaden"), "javascript: filesys_upload_files();");
				$table->addSpace(10);
				$table->insertAction("file_multiple", gettext("multiple file upload"), "javascript: popup('jupload.php?user=".$_SESSION["user_id"]."&map_id=".$_REQUEST["id"]."', 'multiupload', 0, 0, 1);");
			$table->endTableData();
		$table->endTableRow();
		$table->endTable();
	...
	public function show_newfolder() {
		...
		$table->addTableRow();
			$table->addTableData();
				$table->addCode(gettext("map aanmaken"));
				$table->addTag("br");
				$table->addTag("form", array(
				    "id" => "createfolder",
					"method" => "POST",
				...
				$table->endTag("form");
				$table->addTag("br");
				$table->insertAction("save", gettext("map aanmaken"), "javascript: filesys_create_dir();");
			$table->endTableData();
		$table->endTableRow();
		$table->endTable();
		return $table;
	...
	public function filesCopied($parentaction = "") {
		...
		$venster = new Layout_venster(array(
			"title" => gettext("bestandsbeheer"),
			"subtitle" => gettext("bestanden verplaatst")
		));
		$venster->addVensterData();
			$venster->addCode( gettext("De bestanden zijn opgeslagen")." ... " );
		$venster->endVensterData();
		$output->addCode($venster->generate_output());
