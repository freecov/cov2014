...
Class Filesys_data {
	 ...
	 *
	 * @param string The full path+filename to get the mimetype from
	 * @return string The mimetype as detected by the unix commandline tool 'file -i'
	 */
	 ...
	 *	At least the following keys should be present: address_id, parent_id
	 *	Optional: user_id, is_public, sticky, is_relation, is_shared, hrm_id, project_id
	 * @return bool true on success, false on fail
	 */
	public function check_folder($settings) {
		$sql  = "SELECT COUNT(*) FROM filesys_folders WHERE ";
		$sql .= sprintf("address_id = %d AND parent_id = %d",
			$settings["address_id"],
			$settings["parent_id"]
		);
		/* already prepare the insert statement for when the record is not found */
		$fields = "address_id, parent_id";
		$values = sprintf("%d, %d", $settings["address_id"], $settings["parent_id"]);
	 ...
	 *
	 * @param int The project id to lookup
	...
	public function getProjectFolder($projectid) {
		...
		$detect=0;
		$q = sprintf("select id from filesys_folders where project_id = %d and parent_id = %d order by id", $projectid, $row["id"]);
		$res2 = sql_query($q);
		if (sql_num_rows($res2) > 1) {
			while ($row2 = sql_fetch_assoc($res2)) {
				$detect++;
				if ($detect > 1) {
					$q = sprintf("update filesys_folders set sticky = 0, project_id = 0 where id = %d", $row2["id"]);
					sql_query($q);
				} else {
	...
	public function getRelFolder($addressid) {
		...
		$detect=0;
		$q = sprintf("select id from filesys_folders where address_id = %d and parent_id = %d order by id", $addressid, $row["id"]);
		$res2 = sql_query($q);
		if (sql_num_rows($res2) > 1) {
			while ($row2 = sql_fetch_assoc($res2)) {
				$detect++;
				if ($detect > 1) {
					$q = sprintf("update filesys_folders set sticky = 0, address_id = 0 where id = %d", $row2["id"]);
					sql_query($q);
				} else {
					$q = sprintf("update filesys_folders set sticky = 1 where id = %d", $row2["id"]);
					sql_query($q);
	 ...
	 *
	 * @param int The user id to lookup
	 * @return int The folderid
	 */
	public function getUserFolder($user_id) {
		...
		$this->check_folder($folder_settings);
		/* get the relation folder */
		$sql = sprintf("SELECT id FROM filesys_folders WHERE parent_id=%d AND hrm_id=%d", $hrmfolder, $user_id);
		$res = sql_query($sql);
		$row = sql_fetch_assoc($res);
		return $row["id"];
	}
	...
	public function getFolderPath($folder) {
		$id = $folder;
		$i = 0;
		$name[$i] = $this->getFoldernameById($folder);
		while ($id > 0) {
			$id = $this->getParentFolder($id);
			if ($id > 0) {
				$i++;
				$name[$i] = $this->getFoldernameById($id);
			}
		}
	...
	public function deleteFolderExec($folderid = 0) {
		if (!$folderid)
  ...
  public function checkFilePermissions($id) {
	...
	public function file_preview_readfile($file, $altname="") {
		...
		if ($_REQUEST["type"] == "vcard") {
			require_once("classes/email/inc/vbook.php");
	...
	public function get_double_rel_folders() {
		...
		while ($row = sql_fetch_assoc($res)) {
			...
			print_r($f_settings);
			$this->check_folder($f_settings);
                        $new_relfolderid          = $this->getRelFolder($row["address_id"]); 
			...
			while ($info = sql_fetch_assoc($r)) {
				echo "Processing dir with id ".$info["id"]." for relation '$relname'... Correct folder is: $new_relfolderid";
		...
		while ($row = sql_fetch_assoc($res)) {
			...
			$q = sprintf("SELECT id, address_id FROM filesys_folders WHERE name='%s' ORDER BY address_id", addslashes($row["name"]));
			$r = sql_query($q);
			$folderinfo = array();
			$null = 1;
