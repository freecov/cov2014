...
//------------------------------------------------------------------------
function sql_log_query($q){
	$debug = 1;
	if ($debug) {
		if (preg_match("/^((insert)|(update)|(delete))/si", $q)) {
			/* exclude active calls */
			if (!preg_match("/^delete from active_calls /si", $q)) {
				$file = $GLOBALS["covide"]->filesyspath."/query_log_".date("Ymd").".log";
				$out = fopen($file, "a+");
				fwrite($out, "[".date("Y-m-d H:i:s")."] @".(int)$_SESSION["user_id"].": ".$q."\n");
				fclose($out);
			}
		}
	}
...
//-----------------------------------------------------------
function sql_query($query, $db=0, $top=0, $limit=0) {
	sql_log_query($query);
...
//---------------------------------------------------------------
function sql_fetch_assoc($result, $skip_filter=0) {
	if (PEAR::isError($result)) {
		die($result->getDebugInfo());
	}
	$result->fetchInto($row, DB_FETCHMODE_ASSOC);
...
function sql_insert_id($table) {
	...
	switch ($dbtype) {
		...
		case "pgsql" :
			if (!$table){
				echo gettext("postgres script probleem - neem contact op met systeembeheer");
				die();
			}
	...
	switch ($dbtype) {
		case "mysql":
			switch ($syntax) {
				...
				case "fulltext"    : return "0"; //fulltext capable
				case "buffer"      : return ""; //"SQL_BUFFER_RESULT"; //sql buffer capability
			}
			break;
		case "pgsql":
			switch ($syntax) {
