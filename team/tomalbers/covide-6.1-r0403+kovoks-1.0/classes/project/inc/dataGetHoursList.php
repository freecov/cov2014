...
while ($row = sql_fetch_assoc($res)) {
	...
	if ($row["is_billable"]) {
		...
		$total_costs += round($costs*100);
		//($activitycosts[$row["activity_id"]]*$row["hours_bill"]);
	} else {
		/* service hours */
		$row["hours_bill"] = 0;
...
$hoursinfo["total_hours_billable"] = round($total_secs_billable/3600).":".$mins;
$hoursinfo["total_hours_raw"] = $total_secs_billable/3600;
$mins = date("i", mktime(0, 0, $total_secs_service, 1, 1, 1970));
if ($mins < 10) $mins = "0".$mins;
$hoursinfo["total_hours_service"] = round($total_secs_service/3600).":".$mins;
