	...
	// Kijk of gebruiker toegang heeft
		...
		while ($row = sql_fetch_array($result)) {
		  ...
		  if (sql_num_rows($result)==0) {
		  	$line.="\"".gettext("Geen uren geboekt")."\"\n";
		  }

			//kopjes
			$line.= "\"".gettext("datum")."\",\"".gettext("uren")."\",\"".gettext("gebruiker")."\",";
			$line.= "\"".gettext("activiteit")."\",\"".gettext("omschrijving")."\",\"".gettext("bedrag")."\"\n";
				...
				$line.= "\n\"".gettext("totaal uur:")."\",\"".number_format($totaal,1)."\"\n";
				$line.= "\n\"".gettext("totaal prijs:")."\",\"EUR ".number_format($totaalprijs,2,",",".")."\"\n";
...
<html>
<head>
<title><?=gettext("Project overzicht")?></title>
</head>
<body>
<style type="text/css">
...
<div name="loading" id="loading" style="position:absolute;top:50px;left:50px;" align="center">
	<font size="2"><b><?=gettext("Bezig met inlezen gegevens")?>.... <?=gettext("een moment geduld a.u.b.")?>...</b></font>
</div>
	...
	// Kijk of gebruiker toegang heeft
		...
		while ($row = sql_fetch_array($result)) {
			...
			<table width="100%">
				<tr>
					<td>
						...
						<? } ?>
						<br>
						<?=gettext("datum afdruk")?>: <?=strftime("%d-%m-%Y %H:%M")?><BR><BR>
					</td>
					<td align="right">
						f = <?=gettext("facturabele uren")?><br>
						s = <?=gettext("service uren")?>
					</td>
				</tr>
			</table>
		  ...
		  if (sql_num_rows($result)==0) {
			...
			//kopjes
			print "<tr valign=\"top\">";
			print "<td><b>".gettext("datum")." /</b><BR>";
			print "<b>".gettext("tijdsduur")."</b></td>";
			print "<td align=\"right\">";

			print " &nbsp;</td>";
			print "<td><b>".gettext("gebruiker")." /</b><br>";
			print "<b>".gettext("activiteit")."</b></nobr></td>";
			print "<td><b>&nbsp;<br>".gettext("omschrijving")."</b></td>";
			print "<td align=\"right\"><b>&nbsp;<br>".gettext("bedrag")."</b></td>";
			print "</tr>\n";
...
<tr>
			...
			<td colspan="6">
				<? echo gettext("totaal uur:"); ?> <b><?=number_format($totaal,1)?></b>
			</td>
			<td align="right" colspan="2"><? echo gettext("totaal prijs:"); ?> <b>&euro;&nbsp;<?=number_format($totaalprijs,2,",",".")?></b></td>
		  </tr></></td>
		</tr>
	</table>
	</div>
