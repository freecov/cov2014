<?php
	...
	if (!$GLOBALS["covide"]->mobile) {
		...
		$this->addCode("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Transitional//EN\">\n");
	}
	$this->addCode("<html>\n");

	if (!$GLOBALS["covide"]->mobile) {
		$this->addComment("Covide	 : Cooperative Virtual Desktop copyright (c) 2004-2005 Terrazur Barveld");
	...
	if ($this->_hide_navigation) {
		...
		$this->addTag("tr");
		$this->addTag("td", array("width"=>"100%") );
		...
		if ($_SESSION["user_id"]) {
			...
			$navigation->addNavigationItem(gettext("Help"), "icoon_help.gif", "http://sourceforge.net/forum/forum.php?forum_id=590728");
			...
			$table->addTableRow();
				...
				$table->addTableData();
					...
					$table->addTag("div", array("id"=>"menuDate") );
						...
						if (!$GLOBALS["covide"]->mobile) {
							$table->addTag("span", array("id"=>"clock_container"));
