<?php
	...
	if ($userpermissions["xs_usermanage"] == 1 || $userpermisions["xs_limitusermanage"] == 1) {
		...
		if ($agendasel[0]) {
			$agendasel_ar = implode(",", $agendasel);
		}
		...
		if ($userinfo["id"]==0) {
			// new user
			if ($userinfo["pers_nr"]) {
				$result = sql_query("SELECT id FROM users WHERE pers_nr=".$userinfo["pers_nr"]);
		...
		if ($userinfo["id"]==$_SESSION["user_id"]) {
			...
			#$query.= sprintf(", mail_forward=%d", $userinfo["mail_forward"]);
			...
			//not allowed
			html_header();
			dialog("fout", "u heeft geen rechten om deze gebruiker aan te passen.", "index.php|terug");
			html_footer();
		}
	}
	if (!$error) {
