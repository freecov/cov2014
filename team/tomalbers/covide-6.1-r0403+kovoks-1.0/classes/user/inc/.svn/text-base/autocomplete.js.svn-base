...
function user_set_complete(str, userid, archive) {
	...
	if (user_complete_multi==1) {
		...
		/* single user mode */
		if (!archive) {
			var list = ("<li class='special'>");
		} else {
			var list = ("<li class='enabled'>");
		}
		list = list.concat("<a href=\"javascript: remove_user('"+userid+"', '"+user_complete_target+"', '"+user_complete_span+"', '"+user_complete_noempty+"');\">", str, "</a>");
		el_span.innerHTML = list;
...
function user_init_layer(ary) {
	...
	for (i=0;i<ret.length-1;i++) {
		flag++;
		tmp = ret[i].split('|');
		tmp[0] = '<a class="autocomplete" href="javascript: void(0);" onclick="javascript: user_set_complete(\''+tmp[0]+'\', \''+tmp[1]+'\', \''+tmp[2]+'\');">'+tmp[0]+'</a>';
...
function UserloadXMLList() {
	var ret = loadXMLContent('?mod=user&action=autocomplete&str='+user_complete_str+'&archive='+user_complete_archive);
	user_init_layer(ret);
}

/* function to init autocomplete on the field */
function autouser_complete_field(field, span, targt, archive, multiple, noempty) {
