...
Class Email_generate Extends Email_data {
	...
	public function generateMail($body, $attIds=array()) {
		...
		//max word wrap (998 chars RFC 2882) (not RFC 822 (=78) !!)
		$max_wrap = 850;
		...
		case "html":
			...
			$header.= "Content-Type: text/plain; charset=ISO-8859-15; format=flowed\n";
			$header.= "Content-Transfer-Encoding: 7bit\n\n";
			$header.= $this->text_enc( $this->add_text_warning( $body["Body"], $max_wrap) );
			$header.= "\n\n";
			...
			$header.= "Content-Transfer-Encoding: 7bit\n\n";
			$header.= $this->text_enc($body["Body_html"], $max_wrap);
			$header.= "\n\n";
			$header.= "--".$body["boundary0"]."--\n\n";
			break;
		case "html/inl":
	...
	private function insertAlternative($body, $body_html, $boundary0, $boundary1, $max_wrap) {
		...
		$header.= "Content-Transfer-Encoding: 7bit\n\n";
		$header.= $this->text_enc( $this->add_text_warning($body, $max_wrap) );
		$header.= "\n\n";
		...
		$header.= "Content-Transfer-Encoding: 7bit\n\n";
		$header.= $this->text_enc($body_html, $max_wrap);
		$header.= "\n\n";
	...
	//-----------------------------------------------------------------------------------------
	private function text_enc($str, $wrap_at=850) {
		#$wrap_at = 78; //RFC822 - 999=RFC2822
		$str = trim( stripslashes( preg_replace('%(\s*)([^>]{'.$wrap_at.',})(<|$)%e', "'\\1'.wordwrap('\\2', '".$wrap_at."', '\n', 1).'\\3'", $str)) );
		return $str;
	}
	...
	public function sendMail2BackgroundSmtp(&$data, $opts, $list, $mail_id) {
		...
		$venster->addVensterData();
			...
			$venster->addTag("span", array("id"=>"newsletter_progressbar"));
				$venster->addCode("(".gettext("bezig met versturen").")" );
			$venster->endTag("span");
	...
	public function sendMail2Smtp(&$data, $opts, $hash) {
		...
		if ($hash) {
			$data = str_replace("##trackerid##", $opts["To"], $data);
			$data = str_replace("##mailcode##", $hash, $data);
		}
	...
	public function parseTemplate($part_body, $data) {
		...
		$header = sprintf("<font face='%s' size='%s'><i>%s</i></font>",
			$part_body["template_font"], $part_body["template_size"], $template["header"]);

		/* footer */
		$footer = sprintf("<font face='%s' size='%s'><i>%s</i></font>",
			$part_body["template_font"], $part_body["template_size"], $template["footer"]);
		...
		if ($templateaanhef=="1") {
			$_personal_cmnt = "<font face='$templatefont' size='$templatesize'>##rcptcmnt##</font>";
		}
		...
		$_prefix.= "</td>\n";
		$_prefix.= "<td style=\"padding: 2px 2px 2px 2px;\">".$template[0]["header"]."<br><br>$_personal_cmnt</td>\n";
		if ($tf["r"]) {
			//right image
		...
		$_suffix.= "</tr>\n";
		$_suffix.= "<tr>\n";
		$_suffix.= "<td style=\"padding: 2px 2px 2px 2px;\"><br><br>".$template[0]["footer"]."</td>\n";
		$_suffix.= "</tr>\n";
		$_suffix.= "<tr>\n";
		...
		$part_body["Body_html"] = $_prefix . $part_body["Body_html"] . $_suffix;
