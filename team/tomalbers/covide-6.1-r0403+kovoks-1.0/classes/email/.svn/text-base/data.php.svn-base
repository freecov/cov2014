...
Class Email_data {
	...
	public function encodeMailOptions($data) {
		...
		$opts["template_size"]            = $data["template_size"];
	...
	public function sendMail($to, $subject, $message, $additional_options="", $user_id="") {
		/* syntax usage is like the php mail command:                           */
		/*  to                 - rcpt address (or internal user id if numeric   */
		/*  subject            - the subject                                    */
		/*  message            - the message part (text-only)                   */
		/*  additional options - array with the following options:              */
		/*    cc  : string or array with cc addresses (of user ids if numeric)  */
		/*    bcc : same as cc                                                  */
		/*    project_id : id of the project                                    */
		/*    relation_id : id of the relation                                  */
		/*  user id            - id of the sender/user                          */
		if ($user_id) {
	...
	public function upload_list() {
		...
		foreach ($att as $k=>$v) {
			$output->insertAction("delete", gettext("verwijder attachment"), "javascript: attachment_delete('".$v["id"]."');");
			$output->insertAction("file_download", gettext("download attachment"), "?mod=email&action=download_attachment&id=".$v["id"]);
	...
	public function change_description() {
		...
		echo sprintf("document.getElementById('description_notify').innerHTML = '%s';", gettext("- opgeslagen -"));
	}
	...
	public function toggle_private_state_xml() {
		...
		if ($state == 2) {
			...
			$new_state = 0;
			$output->addCode( gettext("deze email is publiek toegankelijk") );
			$output->insertAction("state_public", gettext("deze email is publiek toegankelijk"), "");
		} else {
			/* state is public, set to private */
			$new_state = 2;
			$output->addCode( gettext("deze email is alleen prive toegankelijk") );
			$output->insertAction("state_private", gettext("deze email is alleen prive toegankelijk"), "");
		}
	...
	public function getEmailAliases() {
		...
		$email[ gettext("standaard") ][-1] = "&lt;".$userinfo["mail_email"] ."&gt; ".gettext("standaard email adres");
		if (trim($userinfo["mail_email1"])) {
			$email[ gettext("standaard") ][-2] = "&lt;".$userinfo["mail_email1"] ."&gt; ".gettext("alternatief email adres");
		}

		$q = sprintf("select * from mail_signatures where user_id = %d", $_SESSION["user_id"]);
		$res = sql_query($q);
		while ($row = sql_fetch_assoc($res)) {
			$email[ gettext("alternatieve handtekeningen") ][$row["id"]] = "&lt;".$row["email"]."&gt; ".$row["subject"];
		}
		return $email;
	}
		...
		while ($row = sql_fetch_assoc($res)) {
			if ($record > 0) {
				...
				if ($row["is_new"]==1) {
					...
					$buf .= "<span class='d'><center>".gettext("info").": ".gettext("mappen/vrije schijfruimte opgeruimd")."</center></span><br>";
					//get fspath
					$fspath = $GLOBALS["covide"]->filesyspath;
	...
	public function send_queue() {
		...
		for ($i=0;$i<$blocksize;$i++) {
			$tracker = $this->get_current_tracker_item($id);
			if (is_array($tracker)) {
				$record   = $tracker[0];
				$email    = $tracker[1];
				$mailcode = $tracker[2];
		...
		if ($mail_id) {
			...
			while ($row = sql_fetch_assoc($res)) {
				if ($row["timestamp_first"]) {
					...
					$row["read_first_h"] = gettext("nog niet gelezen");
				}
		...
		if ($data["email"] && $data["mailcode"]) {
			...
			if (sql_num_rows($res)>0) {
				...
				if ($codeid) {
					...
					$q = sprintf("update mail_tracking set timestamp_first = %d, timestamp_last = %d, count = %d, clients = '%s', agents = '%s' where id = %d",
						$timestamp_first, $timestamp_last, $count, $clients, $agents, $codeid);
					sql_query($q);
				}
			}
		}
	...
	/* signatures */
	public function get_signature_list($id="") {
		$data = array();
		if ($id) {
			$q = sprintf("select * from mail_signatures where user_id = %d and id = %d", $_SESSION["user_id"], $id);
		} else {
			$q = sprintf("select * from mail_signatures where user_id = %d order by email", $_SESSION["user_id"]);
		}
		$res = sql_query($q);
		while ($row = sql_fetch_assoc($res)) {
			$data[] = $row;
	...
	public function signatureDelete($id) {
		...
		/* filters */
	public function get_filter_list($id="") {
		$data = array();

		if ($id) {
			$q = sprintf("select * from mail_filters where user_id = %d and id = %d", $_SESSION["user_id"], $id);
		} else {
			$q = sprintf("select * from mail_filters where user_id = %d order by priority", $_SESSION["user_id"]);
		}
		$res = sql_query($q);
		while ($row = sql_fetch_assoc($res)) {
	...
	public function getPrefix($no_html_tags=0) {
		if (!$no_html_tags) {
			$prefix = "<html>\n";
			$prefix.= "<head>\n";
			$prefix.= " <base target=\"_new\">\n";
			$prefix.= " <meta http-equiv=\"Content-Type\" content=\"text/html; charset=iso-8859-15\">\n";
		}
		$prefix.= " <style type='text/css'>\n";
		$prefix.= "  body, td, span, div {\n";
	...
	public function checkFilePermissions($id, $mail_id=0) {
		$xs = 0;
	...
	public function mime_encode($str) {
		if (imap_utf7_encode($str) != $str) {
			return sprintf("=?ISO-8859-1?Q?%s?=", $this->quoted_printable_encode($str));
		} else {
			return $str;
		}
	}

	private function quoted_printable_encode($sString) {
	...
	/* getDelegationByUser {{{ */
	/**
	 * get info about what folders beside my own I have access to
	 *
	 * @param int The userid to lookup permissions for
	 * @param int The (optional) folderid to lookup access for
	 * @param text from == lookup access I provided, to == lookup access on non-self folders
	 * @return array The accesslevel, folderid and owner userid as recordset
	 */
	public function getDelegationByUser($userid, $folderid = 0, $direction = "from") {
		$allowed = array();
		/* we can have both lookups for what folders a user has access to and what folders a user is sharing */
		if ($direction == "to") {
			$sql = sprintf("SELECT * FROM mailfolder_permissions WHERE user_id_visitor = %d", $userid);
		} elseif ($direction == "from") {
			$sql = sprintf("SELECT * FROM mailfolder_permissions WHERE user_id = %d", $userid);
		} else {
			return $allowed;
		}
		if ((int)$folderid > 0) {
			$sql .= sprintf(" AND folder_id = %d", $folderid);
		}
		$res = sql_query($sql);
		$i = 0;
		while ($row = sql_fetch_assoc($res)) {
			$allowed[$i] = array(
				"owner"    => $row["user_id"],
				"visitor"  => $row["user_id_visitor"],
				"folderid" => $row["folder_id"],
				"access"   => $row["permissions"]
			);
			$i++;
		}
		return $allowed;
	...
	public function dropMailBodyToFilesys($id) {
		...
		if (!file_exists($file)) {
			...
			$q = sprintf("select sender_emailaddress, cc, %sto%s, subject, mail_messages_data.body from mail_messages left join mail_messages_data on mail_messages_data.mail_id = mail_messages.id where mail_messages.id = %d", $syntax, $syntax, $id);
			$res = sql_query($q);
			$row = sql_fetch_assoc($res);
