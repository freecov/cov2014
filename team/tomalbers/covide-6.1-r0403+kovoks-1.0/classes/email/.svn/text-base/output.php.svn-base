...
Class Email_output {
			...
			$table->addTableRow();
				$table->addTableData("", ($current==$v["id"]) ? "header nowrap":"data nowrap");
					...
					if ($move_folder && ($v["id"]==$current)) {
						$table->addCode(gettext($v["name"]));
					} else {
						$table->insertLink(gettext($v["name"]), array(
	...
	public function getSelectList($name, $folders, $current, $settings="") {
		$list = array();
		$output = new Layout_output();
		foreach ($folders as $k=>$v) {
	...
	public function signatureList() {
		...
		$mailData = new Email_data();
		$list = $mailData->get_signature_list();
		...
		$view->defineComplexMapping("complex_actions", array(
			array(
				...
				"src"  => "edit",
				"alt"  => gettext("bewerken"),
				"link" => array("?mod=email&action=signatureEdit&id=", "%id")
			),
			array(
				"type" => "action",
				"src"  => "delete",
				"alt"  => gettext("verwijderen"),
				"link" => array("?mod=email&action=signatureDelete&id=", "%id")
			)
		));
	...
	public function signatureEdit($id) {
