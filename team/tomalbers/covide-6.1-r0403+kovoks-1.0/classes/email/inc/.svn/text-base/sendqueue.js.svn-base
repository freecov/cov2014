function trigger_newsletter() {
	...
	if (cur_val <= total_val) {
		setTimeout("loadXML('?mod=email&action=send_mail_queue&id="+mail_id+"&datafile="+datafile+"&from="+from+"');", 10);
		//document.write("?mod=email&action=send_mail_queue&id="+mail_id+"&datafile="+datafile+"&from="+from);
	} else {
