<?php
	...
	if ($num_messages > 0) {
		...
		$filters = $mailData->get_filter_list();
	}
		...
		if (array_key_exists($msg_id, $_statuslist_msg)) {
			...
			foreach ($filters as $f) {
				if (!$data["new_folder_id"]) {
					...
					if ($f["sender"]) {
						$matches_needed++;
						if (strpos(strtolower($from), strtolower($f["sender"])) !== false) {
							$matches_found++;
							$d["sender"] = 1;
						}
					}
					if ($f["receipient"]) {
						$matches_needed++;
						if (strpos(strtolower($rcpt), strtolower($f["receipient"])) !== false) {
							$matches_found++;
							$d["receipient"] = 1;
						}
					}
			...
			$q = sprintf("insert into mail_messages_data (mail_id, body, header) values (%d, '%s', '%s')",
				$id, addslashes($data["body"]), addslashes($data["header"]));
			sql_query($q);
