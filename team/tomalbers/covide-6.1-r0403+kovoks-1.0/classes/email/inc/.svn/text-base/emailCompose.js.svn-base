...
/* function to retrieve the editor content */
function mail_get_contents() {
	document.getElementById('contents').value = editor_get_contents();
}
...
function mail_send() {
	...
	if (ok == false) {
		alert(gettext("Geen geldige ontvanger opgegeven"));
	} else {
		...
		if (subject_ok == true) {
			...
			document.getElementById('js_command').value = "location.href='index.php?mod=email&action=mail_send&id="+document.getElementById('id').value+"';";
			mail_save_db();
		}
	}
}
...
function add_from_filesys() {
	...
	popup('?mod=filesys&subaction=add_attachment&address='+relation, 'attachment', 750, 550, 1);
}
