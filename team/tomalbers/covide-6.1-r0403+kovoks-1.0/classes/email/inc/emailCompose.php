<?php
	...
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			...
			$table->endTag("span");
			$table->insertAction("edit", "wijzigen", "javascript: popup('?mod=project&action=search_project', 'search_project');");
		$table->endTableData();
	$table->endTableRow();
	...
	$table->addTableRow();
		...
		if (!$mdata["is_text"]) {
			...
			$table->addCode( $editor->generate_editor() );
		} else {
			/* if mode = text */
	...
	$table->addTableRow();
		...
		$table->addTableData("", "data");
			...
			$table->insertAction("save", gettext("opslaan"), "javascript: mail_save_db();");
			if ($mdata["is_text"]) {
				$table->insertAction("ftype_html", gettext("converteer naar text"), "javascript: mail_convert();", "", 1);
			} else {
				$table->insertAction("ftype_text", gettext("converteer naar text"), "javascript: mail_convert();");
			}
			$table->addSpace(3);
