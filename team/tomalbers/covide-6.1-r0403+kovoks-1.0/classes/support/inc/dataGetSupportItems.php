...
if ($options["address_id"]) {
	...
	$sql = sprintf("SELECT issues.*, address.companyname, project.name as projectname, users.username FROM issues %s WHERE $is_solved AND issues.user_id=%d ORDER BY ", $join, $_SESSION["user_id"]);
	$sql_count = sprintf("SELECT COUNT(*) FROM issues WHERE $is_solved AND user_id=%d", $_SESSION["user_id"]);
}
if (!$options["sort"]) {
