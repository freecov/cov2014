	...
	Class Note_output {
	  ...
	  public function __construct() {
		...
		public function generate_list($options = "") {
			...
			$venster->addMenuItem(gettext("afgehandeld"), "./?mod=note&action=old");
			$venster->addMenuItem(gettext("verstuurd"), "./?mod=note&action=sent");
			$venster->addMenuItem(gettext("verstuurd, ongelezen"), "./?mod=note&action=show");
			$venster->addMenuItem(gettext("zoeken"), "./?mod=index&search[private]=1&search[notes]=1");
			if ($options["note_type"] != "current") {
				$venster->addMenuItem(gettext("berichtenlijst"), "./?mod=note");
			}
			...
			$venster->addVensterData();
				...
				$url = "index.php?mod=note&action=$action&top=%%";
				$paging->setOptions($top, $notes_arr["total_count"], $url, $limit);
