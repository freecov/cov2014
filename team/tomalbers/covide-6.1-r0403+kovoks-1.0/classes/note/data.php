...
Class Note_data {
	...
	public function getNotes($options = "") {
		...
		if ($options["zoekstring"]["ondinh"]) {
			$q = "AND (subject $like_syntax '%".sprintf("%s", $options["zoekstring"]["ondinh"])."%' OR body $like_syntax '%".sprintf("%s", $options["zoekstring"]["ondinh"])."%')";
		}
		if ($options["user_id"] == "all") {
			$q_userid = "1 = 1";
		} else {
			$q_userid = sprintf("notes.user_id = %d", $options["user_id"]);
		}
		if ($options["address_id"]) {
			$q_address = sprintf(" AND notes.address_id=%d", $options["address_id"]);
		}
		if ($options["project_id"]) {
			$q_project = sprintf(" AND project_id = %d", $options["project_id"]);
		}
		if (!$options["sort"]) {
		...
		switch ($options["note_type"]) {
			case "old"     :
				$query = "SELECT $fields FROM notes $join WHERE $q_userid AND is_done=1 $q $q_address $q_project ORDER BY $order";
				$query_count = "SELECT COUNT(*) FROM notes WHERE $q_userid AND is_done=1 $q $q_address $q_project";
				$showasold = 1;
				break;
			case "sent"    :
				$query = "SELECT $fields FROM notes $join where sender=".(int)$options["user_id"]." AND delstatus!=1 $q $q_address $q_project ORDER BY $order";
				$query_count = "SELECT COUNT(*) FROM notes where sender=".(int)$options["user_id"]." AND delstatus!=1 $q $q_address $q_project";
				$showasold = 1;
				break;
			case "show"    :
				$query = "SELECT $fields FROM notes $join where sender=".(int)$options["user_id"]." AND is_read=0 AND delstatus!=1 $q $q_address $q_project ORDER BY $order";
				$query_count = "SELECT COUNT(*) FROM notes where sender=".(int)$options["user_id"]." AND is_read=0 AND delstatus!=1 $q $q_address $q_project";
				$showasold = 1;
				break;
			case "all" :
				$query = "SELECT $fields FROM notes $join where $q_userid $q $q_address $q_project ORDER BY $order";
				$query_count = "SELECT COUNT(*) FROM notes where $q_userid $q $q_address $q_project";
				break;
			case "current" :
			default        :
				$query = "SELECT $fields FROM notes $join WHERE $q_userid AND (is_done IS NULL OR is_done = 0) AND delstatus!=2 $q $q_address $q_project ORDER BY $order";
				$query_count = "SELECT COUNT(*) FROM notes WHERE $q_userid AND (is_done IS NULL OR is_done = 0) AND delstatus!=2 $q $q_address $q_project";
				break;
		}
		...
		while ($row = sql_fetch_assoc($result)) {
			if (!trim($row["subject"])) { $row["subject"] = gettext("geen onderwerp"); }
			$row["relation_name"] = $row["address_name"];
			$row["from_name"]     = $users[$row["sender"]];
			$row["to_name"]       = $users[$row["user_id"]];
			$extra_ont = explode(",", $row["extra_receipients"]);
			$extra_names = array();
	...
	public function getNotecountByUserId($user) {
		...
		$sql_new    = "SELECT COUNT(*) FROM notes WHERE $q_userid AND is_read=0";
		$sql_active = "SELECT COUNT(*) FROM notes WHERE $q_userid AND (is_done is null OR is_done=0)";
	...
	public function store2db($note) {
		...
		if (!$note["id"]) {
			foreach ($users as $rcpt) {
				...
				$sql  = "INSERT INTO notes (is_support, timestamp, extra_receipients, subject, body, sender, is_read, address_id, project_id, user_id) VALUES (";
				$sql .= sprintf("%d, %d, '%s', '%s', '%s'", $note["is_support"], mktime(), implode(",", $extra), $note["subject"], $note["body"]);
				$sql .= sprintf(",%d, %d, %d, %d, %d", $note["from"], 0, $note["address_id"], $note["project_id"], $rcpt);
				$sql .= ")";
				$res = sql_query($sql);
			...
			if (!$newnote && ($note["is_todo"] || $note["is_sales"])) {
				...
				$sql .= sprintf(",%d, %d, %d, %d, %d", $note["from"], 1, $note["address_id"], $note["project_id"], $_SESSION["user_id"]);
				$sql .= ")";
				$res = sql_query($sql);
				$newnote = sql_insert_id("notes");
			}
			...
			$sql .= sprintf("is_support=%d, timestamp=%d, extra_receipients='%s'", $note["is_support"], mktime(), $users);
			$sql .= sprintf(", subject='%s', body='%s', sender=%d", $note["subject"], $note["body"], $note["from"]);
			$sql .= sprintf(", is_read=%d, address_id=%d, project_id=%d", $note["is_read"], $note["address_id"], $note["prqect_id"]);
			$sql .= sprintf(", user_id=%d WHERE id=%d", $note["to"], $note["id"]);
			$res = sql_query($sql);
			$newnote = $note["id"];
			unset($sql);
		}
	...
	public function searchAll($search, $private=0) {
