...
Class Templates_output {
	...
	public function templatePrint($pdf=0) {
		...
		*/
			...
			$output->addTag("div", array(
				...
				$tbl->addTableRow();
					...
					$tbl->addTableData(array(
						"align" => ($settings["address_position"]) ? "right":"left"
					));
						...
						$table = new Layout_table();
							...
							$table->addTableData(array("align"=>"left"));
								...
								if ($data["address_businesscard_id"]) {
									$address = $address_data->getAddressByID($v, "bcards");
								} else {
									$address = $address_data->getAddressByID($v, "address");
								}
		...
		$out = fopen($file, "w");
		fwrite( $out, $output->generate_output() );
		fclose($out);
	...
	public function selectAddress(){
		...
		foreach ($list["address"] as $k=>$v) {
			if ($_REQUEST["addresstype"] == "bcards") {
				...
				if ($v["companyname"]) {
					$data[$v["id"]].= sprintf(" (%s)", $v["companyname"]);
				}
			} else {
	...
	public function selectCla() {
		...
		$output_alt->insertAction("back", gettext("terug"), "javascript: window.close();");
		$output_alt->insertAction("forward", gettext("verder"), "javascript: document.getElementById('velden').submit();");
	...
	public function templateEdit($id=0) {
		...
		$tbl->addTableRow();
			$tbl->addTableData("", "header");
				$tbl->addCode( gettext("pagina instellingen") );
			$tbl->endTableData();
		...
		$tbl->addTableRow();
			$tbl->addTableData("", "header");
				$tbl->addCode( gettext("faxnummer") );
			$tbl->endTableData();
			$tbl->addTableData("", "data");
				$tbl->addCheckbox("tpl[fax_nr]", "1", $tpl["fax_nr"]);
				$tbl->addSpace();
				$tbl->addCode( gettext("faxnummer tonnen bij de adresgegevens") );
			$tbl->endTableData();
		$tbl->endTableRow();
		/* subject */
		$tbl->addTableRow();
			$tbl->addTableData("", "header");
				$tbl->addCode( gettext("betreft") );
			$tbl->endTableData();
			$tbl->addTableData("", "data");
		...
		$tbl->addTableRow();
			$tbl->addTableData("", "header");
				$tbl->addCode( gettext("plaats")." / ".gettext("datum") );
			$tbl->endTableData();
			$tbl->addTableData("", "data");
		...
		$tbl->addTableRow();
			$tbl->addTableData("", "header");
				$tbl->addCode( gettext("geaddresseerden") );
			$tbl->endTableData();
			$tbl->addTableData("", "data");
				/* address selection */
				...
				if ($tpl["and_or"]) {
					$opts["cla"]["selectiontype"] = $tpl["and_or"];
					...
					$opts["addresstype"] = ($tpl["address_businesscard_id"]) ? "bcards":"address";
					$opts["nolimit"] = 1;
				...
				foreach ($ids as $k=>$v) {
					if ($v) {
						if ($tpl["address_businesscard_id"]) {
							$tmp = $address_data->getAddressByID($v, "bcards");
							if ($tmp["companyname"]) {
							...
							$tmp = $address_data->getAddressByID($v, "address");
				...
				$names = implode(", ", $names);
				if ($tpl["and_or"]) {
					$names = gettext("op classificaties").": ".$names;
				} else {
					$names = gettext("op addressen").": ".$names;
				}
				...
				$uri = "&addresstype=".($tpl["address_businesscard_id"]) ? "bcards":"address";
				...
				$tbl->insertAction("addressbook", gettext("selecteer adressen"), "javascript: popup('?mod=address&action=searchRel', 'search_address');");
				$tbl->insertAction("state_special", gettext("selecteer classificaties"), "javascript: popup('?mod=templates&action=selectCla&$uri', 'search_address', 450, 500);");
			$tbl->endTableData();
		$tbl->endTableRow();
		/* default fonts */
		$tbl->addTableRow();
			$tbl->addTableData("", "header");
				$tbl->addCode( gettext("standaard lettertype") );
			$tbl->endTableData();
			$tbl->addTableData("", "data");
		...
		$tbl->addTableRow();
			$tbl->addTableData("", "header");
				$tbl->addCode( gettext("inhoud brief") );
			$tbl->endTableData();
			$tbl->addTableData("", "data");
		...
		*/
		...
		$tbl->addTableRow();
			$tbl->addTableData("", "header");
				$tbl->addCode( gettext("handtekening") );
			$tbl->endTableData();
			$tbl->addTableData("", "data");
				$tbl->addCheckbox("tpl[signature]", "1", $tpl["signature"]);
				$tbl->addSpace();
				$tbl->addCode( gettext("ruimte voor handtekening vrijlaten") );
			$tbl->endTableData();
		$tbl->endTableRow();
		*/
		/* sender data */
		...
		$tbl->addTableRow();
			$tbl->addTableData("", "header");
				$tbl->addCode( gettext("afzendergegevens") );
			$tbl->endTableData();
			$tbl->addTableData("", "data");
		...
		*/
		...
		$tbl->addTableRow();
			$tbl->addTableData(array("colspan"=>2), "header");
				$tbl->insertAction("close", gettext("venster sluiten"), "javascript: window.close();");
				$tbl->insertAction("save", gettext("opslaan"), "javascript: save();");
				$tbl->addSpace();
				$tbl->insertAction("print", gettext("afdrukken"), "javascript: printTemplate(0);");
				$tbl->insertAction("ftype_pdf", gettext("exporteer als PDF"), "javascript: printTemplate(1);");
	...
	public function show_welcome() {
		...
		/* menu items */
		$venster->addMenuItem(gettext("nieuwe template"), "javascript: popup('index.php?mod=templates&action=edit', 'salesedit', 0, 0, 1);");
		$venster->addMenuItem(gettext("instellingen"), "?mod=templates&action=settings");
		$venster->addMenuItem(gettext("adresboek"), "?mod=address");
		$venster->generateMenuItems();
		$venster->addVensterData();

		$template_data = new Templates_data();
		$data = $template_data->getTemplateBySearch("", $_REQUEST["start"]);
	...
	public function settingsList() {
		$output = new Layout_output();
		$output->layout_page(gettext("templates settings"));
		...
		/* menu items */
		$venster->addMenuItem(gettext("nieuwe instelling"), "javascript: popup('index.php?mod=templates&action=edit_settings', 'salesedit', 850, 600, 1);");
		$venster->addMenuItem(gettext("templates"), "?mod=templates");
		$venster->addMenuItem(gettext("adresboek"), "?mod=address");
		$venster->addMenuItem(gettext("printer calibratie"), "javascript: popup('?mod=templates&action=calibrate');");
		$venster->generateMenuItems();
		$venster->addVensterData();
	...
	public function settingsEdit() {
		...
		$output = new Layout_output();
		$output->layout_page(gettext("templates settings"), 1);
		...
		/* generate nice window */
		$venster = new Layout_venster(array(
			"title"    => gettext("template instellingen"),
			"subtitle" => gettext("wijzigen")
		));
		$venster->addVensterData();
			...
			$tbl->addTableData("", "top");
				...
				$pos->addTableRow();
					$pos->addTableData( array("colspan"=>2), "header");
						$pos->addCode( gettext("positionering") );
					$pos->endTableData();
				$pos->endTableRow();
				/* outline - left/right */
				$pos->addTableRow();
					$pos->addTableData("", "data");
						$pos->insertTag("b", gettext("uitlijning header").":" );
					$pos->endTableData();
					$pos->addTableData("", "data");
						$pos->addRadioField("data[address_position]", gettext("links"), 0, $data["address_position"]);
						$pos->addRadioField("data[address_position]", gettext("rechts"), 1, $data["address_position"]);
					$pos->endTableData();
				$pos->endTableRow();
				/* outline - horizontal */
				/*
				...
				*/
				...
				$pos->addTableRow();
					$pos->addTableData( array("colspan"=>2), "header");
						$pos->addCode( gettext("pagina marges") );
					$pos->endTableData();
				$pos->endTableRow();
				/* outline - left */
				$pos->addTableRow();
					$pos->addTableData("", "data");
						$pos->insertTag("b", gettext("links").":" );
					$pos->endTableData();
					$pos->addTableData("", "data");
				...
				$pos->addTableRow();
					$pos->addTableData("", "data");
						$pos->insertTag("b", gettext("rechts").":" );
					$pos->endTableData();
					$pos->addTableData("", "data");
				...
				$pos->addTableRow();
					$pos->addTableData( array("colspan"=>2), "header");
						$pos->addCode( gettext("omschrijving") );
					$pos->endTableData();
				$pos->endTableRow();
				$pos->addTableRow();
				...
				$pos->addTableData( array("colspan"=>2), "header");
					$pos->addCode( gettext("logo")." / ".gettext("afbeelding") );
				$pos->endTableData();
				$pos->addTableRow();
					$pos->addTableData( array("colspan"=>2), "data");
						$file = $tpldata->getTemplateFile($_REQUEST["id"]);
						if (!$_REQUEST["id"]) {
							$pos->addCode( gettext("sla eerst de template op") );
						} else {
							if ($file["id"]) {
								$pos->insertAction("delete", gettext("verwijderen"), sprintf("javascript: delfile();", $file["id"], $_REQUEST["id"]));
								$pos->addSpace();
				...
				$pos->addTableRow();
					$pos->addTableData("", "data");
						$pos->insertTag("b", gettext("uitlijning footer").":" );
					$pos->endTableData();
					$pos->addTableData("", "data");
					$pos->addSelectField("data[footer_position]", array(
						"left"   => gettext("links"),
						"center" => gettext("midden"),
						"right"  => gettext("rechts")
				...
				$pos->addTableRow();
					$pos->addTableData( array("colspan"=>2, "align"=>"right"), "header");
						$pos->insertAction("close", gettext("sluiten"), "javascript: window.close();");
						$pos->insertAction("save", gettext("opslaan"), "javascript: document.getElementById('velden').submit();");
					$pos->endTableData();
				$pos->endTableRow();
				$pos->endTable();
