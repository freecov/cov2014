...
Class ProjectExt_output {
		...
		switch ($v["field_type"]) {
			...
			case 5:
				...
				foreach ($data as $key=>$record) {
					...
					if (strlen($record[$v["default_value"]-1]) > 30) {
						$sel[$key] = substr($record[$v["default_value"]-1], 0, 30)."...";
					} else {
						$sel[$key] = $record[$v["default_value"]-1];
					}
					if ($key == 0) {
						$sel[$key] = "-- ".$sel[$key]." --";
					}
	...
	public function extShowMetaTable() {
		$project_data = new ProjectExt_data();
		$data = $project_data->getMetaTableData($_REQUEST["metafield"]);
	...
	public function genOverviewSearch() {
		$days[0] = "-";
		for ($i=1;$i<=31;$i++) {
			$days[$i] = $i;
		}
		...
		if (count($sel)>0) {
			...
			$output->insertAction("forward", gettext("zoeken"), "javascript: document.getElementById('projectsearch').submit();");
		}
		return $output->generate_output();
	}
