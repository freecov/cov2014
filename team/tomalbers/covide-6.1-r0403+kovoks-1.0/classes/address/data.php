<?php
	...
	/* }}} */
		...
		public function getRelationsArray($active=1) {
			...
			$res = $GLOBALS["covide"]->db->query($sql);
		...
		public function getBcardsByRelationID($address_id) {
			...
			$query = sprintf("SELECT * FROM address_businesscards WHERE address_id=%1\$d OR multirel ".$like." '%%%1\$d%%' ORDER BY surname,givenname", $address_id);
			$res = sql_query($query);
			while($res->fetchInto($row)) {
