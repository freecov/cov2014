...
if ($_REQUEST["relcardaction"] == "cardrem") {
	$sql = sprintf("DELETE FROM address_businesscards WHERE id=%d AND address_id=%d", $_REQUEST["cardid"], $_REQUEST["id"]);
	$res = sql_query($sql);
}
/* init user object */
...
$venster->addVensterData();
	...
	$view->addMapping(gettext("financieel"), "%%showfinance");
	...
	$view->defineComplexMapping("showfinance", array(
		array(
			"type"   => "link",
			"text"   => gettext("toon financi&euml;le gegevens"),
...
$venster->addVensterData();
	...
	$view->defineComplexMapping("complex_actions", array(
		...
		array(
			...
			"link"    => array("javascript:delete_bcard($id,", "%id", ",", "%address_id",")")
		)
	));
...
unset($venster_settings);
	...
	if ($count >= 6) {
		$limit_height = "height: 300px; overflow-y:auto;";
	} else {
		$limit_height = "";
	}
