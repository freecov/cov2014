...
$days   = array(0 => gettext("nvt"));
$months = array(0 => gettext("nvt"));
$years  = array(0 => gettext("nvt"));
for ($i=1; $i<=31; $i++) {
	if ($i < 10) { $i = "0".$i; }
	$days[$i] = $i;
}
...
$output->layout_page("edit hrm", 1);
	...
	$venster->addVensterData();
		...
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("geslacht"), "", "header");
			$table->addTableData("", "data");
				$table->addSelectField("hrm[gender]", $genders, $hrminfo["gender"]);
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("sofinummer"), "", "header");
			$table->addTableData("", "data");
				$table->addTextField("hrm[social_security_nr]", $hrminfo["social_security_nr"]);
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("datum in dienst"), "", "header");
			$table->addTableData("", "data");
				$table->addSelectField("hrm[start_day]",   $days,   (int)date("d", $hrminfo["timestamp_started"]));
				$table->addSelectField("hrm[start_month]", $months, (int)date("m", $hrminfo["timestamp_started"]));
				$table->addSelectField("hrm[start_year]",  $years,  (int)date("Y", $hrminfo["timestamp_started"]));
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("datum uit dienst"), "", "header");
			$table->addTableData("", "data");
				$table->addSelectField("hrm[end_day]",   $days,   (int)date("d", $hrminfo["timestamp_stop"]));
				$table->addSelectField("hrm[end_month]", $months, (int)date("m", $hrminfo["timestamp_stop"]));
				$table->addSelectField("hrm[end_year]",  $years,  (int)date("Y", $hrminfo["timestamp_stop"]));
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("dienstverband"), "", "header");
			$table->addTableData("", "data");
				$table->addTextArea("hrm[contract_type]", $hrminfo["contract_type"], array("rows" => 10, "cols" => 50));
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("aantal uren"), "", "header");
			$table->addTableData("", "data");
		...
		$table->endTableRow();
		$table->addTableRow();
			$table->insertTableData(gettext("eindevaluatie"), "", "header");
			$table->addTableData("", "data");
		...
		$table->addTableRow();
			...
			$table->addTableData("", "data");
				$table->insertAction("save", gettext("opslaan"), "javascript: document.getElementById('hrmform').submit();");
