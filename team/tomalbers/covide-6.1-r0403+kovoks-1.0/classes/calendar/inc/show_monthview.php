<?php
	...
	$venster->addMenuItem(gettext("dagoverzicht"), "?mod=calendar&forceday=1");
					...
					if (($i+1) == date("d") && $month == date("m")) {
						...
						$table_day->addTableRow();
							$table_day->addTableData(array("width" => "100%", "align" => "right"));
								...
								$table_day->insertAction("new", gettext("nieuw"), "javascript: calendaritem_edit(0,0,".$datemask.");");
							...
							$table_day->addTableData();
								if (count($calendar_data->calendar_items)) {
									foreach ($calendar_data->calendar_items as $v) {
										...
										if (!$v["is_registered"]) {
											$table_day->insertAction("edit", gettext("wijzig"), "javascript: calendaritem_edit(".$v["id"].", ".$_SESSION["user_id"].", ".$datemask.");");
										}
