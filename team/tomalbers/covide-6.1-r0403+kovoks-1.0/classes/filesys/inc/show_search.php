<?
	...
	$venster->addMenuItem(gettext("terug"), "./?mod=filesys");
	...
	if ($_REQUEST["search"]) {
		...
		$view->defineComplexMapping("complex_name", array(
			...
			array(
				"type"  => "link",
				"text"  => array(" ", "%name"),
				"link"  => array("?mod=filesys&action=opendir&search=1&id=", "%id")
			)
		));
		...
		$view->defineComplexMapping("complex_folder", array(
			...
			"link"  => array("?mod=filesys&action=opendir&search=1&id=", "%folder_id")
			)
		));
