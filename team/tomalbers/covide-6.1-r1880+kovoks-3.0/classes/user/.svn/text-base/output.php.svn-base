<?php
	...
	Class User_output {
		 ...
		 */
		public function user_selection($id, $current="", $multiple=0, $showArchiveUser=0, $showInActive=0, $no_empty=0, $showGroups=0) {
			...
			$output->insertAction("edit", gettext("kies gebruiker(s)"),
				sprintf("javascript: user_select_popup('%s', '%s', '%s', '%s', '%s', '%s', '%s');",
					$id, $multiple, $showInActive, $showEmpty, $showArchiveUser, $no_empty, $showGroups
			));
			...
			$output->addCode("
				document.getElementById('user_autocomplete_$id').onkeyup = function() {
					 autouser_complete_field('user_autocomplete_$id', 'user_name_$id', '$id', '$showArchiveUser', '$multiple', '$no_empty', '$showGroups', '$archive_uid');
				}
