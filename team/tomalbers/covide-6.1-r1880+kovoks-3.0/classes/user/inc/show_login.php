<?php
	...
	@include(self::include_dir."login_pre.php");
	#sleep(1);
		...
		$table->addTableData();
			...
			$venster->addVensterData();
				...
				$table1->addTableRow();
					...
					$table1->endTableData();
					$table1->addTableData();
						$table1->addTextField("login[username]", ($_REQUEST["login"]["username"]) ? stripslashes(str_replace("\"", "&#".ord("\"").";", $_REQUEST["login"]["username"])):$login_cookie[0]);
					$table1->endTableData();
				$table1->endTableRow();
				$table1->addTableRow();
					$table1->addTableData((!$GLOBALS["covide"]->mobile) ? array("align"=>"right"):"");
				...
				$table1->addTableRow();
					$table1->addTableData(array("colspan"=>2, "align"=>"right"));
						...
						if ($GLOBALS["covide"]->sslmode == 2) {
							...
							$table1->addCode("use ssl encryption");
							$table1->addSpace(2);
						} else {
