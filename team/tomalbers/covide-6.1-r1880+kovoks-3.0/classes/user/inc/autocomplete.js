...
/* caller function */
function user_select_popup(id, multiple, showInActive, showEmpty, showArchiveUser, no_empty, showGroups) {
	user_complete_initial = 0;
	var str = 'index.php?mod=user&action=pick_user&sub_action=init&field_id=';
	popup(str.concat(id, '&multiple=', multiple, '&inactive=', showInActive, '&empty=', showEmpty, '&archive=', showArchiveUser, '&no_empty=', no_empty, '&showGroups=', showGroups), 'user_select', 700, 410, 1);
}
...
function remove_user(id, ltarget, lspan, override_noempty) {
	...
	if (tg.length == 1 && user_complete_noempty == 1) {
		alert(gettext("U moet minimaal een gebruiker kiezen."));
	} else {
...
function user_set_complete(str, userid, archive) {
	...
	if (user_complete_multi==1) {
		...
		if (found==0) {
			...
			list = list.concat("<a href=\"javascript: remove_user('"+userid+"', '"+user_complete_target+"', '"+user_complete_span+"', '"+user_complete_noempty+"');\">", str, "</a>");
		}
...
function UserloadXMLList() {
	var ret = loadXMLContent('?mod=user&action=autocomplete&str='+user_complete_str+'&archive='+user_complete_archive+'&showgroups='+user_complete_groups);
	user_init_layer(ret);
}

/* function to init autocomplete on the field */
function autouser_complete_field(field, span, targt, archive, multiple, noempty, showgroups, archive_uid) {
