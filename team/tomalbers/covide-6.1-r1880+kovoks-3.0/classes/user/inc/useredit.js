...
function handle_error(type) {
	...
	if (type == 1) {
		...
		errordiff.innerHTML=gettext("Fout: Wachtwoorden komen niet overeen");
		errordiff.style.visibility='visible';
	} else if (type == 2) {
		errordiff.style.border='2px dotted red';
		errordiff.innerHTML=gettext("Fout: Het personeelsnummer bestaat al.");
		errordiff.style.visibility='visible';
	} else if (type == 3) {
		errordiff.style.border='2px dotted red';
		errordiff.innerHTML=gettext("Fout: Gegevens missen.");
		errordiff.style.visibility='visible';
	} else if (type == 4) {
		errordiff.style.border='2px dotted red';
		errordiff.innerHTML=gettext("Fout: Het wachtwoord moet uit minimaal 6 tekens bestaan.");
		errordiff.style.visibility='visible';
	} else {
		errordiff.style.border='2px dotted red';
		errordiff.innerHTML=gettext("Fout: Gegevens missen.");
