...
$venster_access->addVensterData();
	...
	$table->addTableRow();
		$table->addTableData("", "data");
			$table->insertImage("icoon_produkten.gif", gettext("offer manager"));
		$table->endTableData();
		$table->addTableData("", "data");
			$table->addCode(gettext("offer manager"));
		$table->endTableData();
		$table->addTableData("", "data");
			$table->addCheckBox("user[xs_offermanage]", "1", $userinfo["xs_offermanage"]);
		$table->endTableData();
	$table->endTableRow();
	if ($GLOBALS["covide"]->license["has_hrm"]) {
		$table->addTableRow();
	...
	if ($GLOBALS["covide"]->license["has_cms"]) {
		$table->addTableRow();
			$table->addTableData("", "data");
				$table->insertImage("icoon_gprs.gif", gettext("cms toegangs niveau"));
			$table->endTableData();
			$table->addTableData("", "data");
				$table->addCode(gettext("cms toegangs niveau"));
			$table->endTableData();
			$table->addTableData("", "data");
