  ...
  ?>
	...
	<input type="hidden" name="project[id]" value="<?=$project["id"]?>">
      ...
      <? userlistInit("userlist_1", "document.getElementById('userlist1')");?>
		...
		<td align="right" <?=td(0)?>><? echo gettext("deelproject van"); ?></td>
		<td <?=td(1)?>>
	  ...
	  <? if ($row["groep"]!=0) { ?>
		...
		<td <?=td(1)?>><span class="d">
		  <select name="project[status]" class="inputselect">
			<option value="1"><? echo gettext("geopend"); ?></option>
			<option value="2"><? echo gettext("lopend"); ?></option>
			<option value="3"><? echo gettext("afgesloten"); ?></option>
	  ...
	  <? } ?>
		<td align="right" <?=td(0)?>><? echo gettext("actief"); ?></td>
		<td <?=td(1)?>>
		  <input type="radio" value="1" name="project[actief]" <? if ($project["actief"]==1) { print "CHECKED"; }?>><? echo gettext("ja"); ?>
		  <input type="radio" value="0" name="project[actief]" <? if ($project["actief"]==0) { print "CHECKED"; }?>><? echo gettext("nee"); ?>
		</td>
	  </tr><tr>
		<td align="right" <?=td(0)?>><? echo gettext("relatie"); ?></td>
		<td <?=td(1)?>>
		...
		?>
			<?=@sql_result($result,0)?>&nbsp;&nbsp;&nbsp;<a href="javascript:var pop = window.open('../adres/?action=zoekRelProj');"><? echo gettext("wijzig"); ?> <img src="../img/knop_rechts.gif" alt="Wijzig" border="0" valign="absmiddle"></a>
			<?
			} else {
				?>
				<a href="javascript:var pop = window.open('../adres/?action=zoekRelProj');"><? echo gettext("zoeken"); ?> <img src="../img/knop_rechts.gif" border="0" alt="Zoeken"></a>
				<?
			}
			?>
		<? } else { ?>
			<a href="javascript:var pop = window.open('../adres/?action=zoekRelProj');"><? echo gettext("zoeken"); ?> <img src="../img/knop_rechts.gif" alt="Zoeken" border="0"></a>
		<? } ?>
		</td>
	  </tr>
		<tr>
	...
	<td <?=td(1)?>>
		<select class="inputselect" name="project[bcards]">
			<option value="0">- <?=gettext("geen")?> -</option>
			<?
		...
		<tr>
			...
			<td align="right" <?=td(0)?>><? echo gettext("uren"); ?></td>
	...
	function info() {
	  ...
	  ?>
	  <tr>
		<td align="right"><span class="d"><? echo gettext("naam"); ?></span></td>
		<td><span class="d"><?=$row["naam"]?>&nbsp;</td>
	  </tr><tr>
		<td align="right"><span class="d"><? echo gettext("omschrijving"); ?></span></td>
		<td><span class="d"><?=nl2br($row["omschrijving"])?>&nbsp;</td>
	  </tr><tr>
		<td align="right"><span class="d"><? echo gettext("projectmanager"); ?></span></td>
		<td><span class="d">
		<?
		if ($row["beheerder"]==0) {
		  echo gettext("geen");
		...
		<td align="right"><span class="d"><? echo gettext("actief"); ?></span></td>
		<td><span class="d">
		<?
		if ($row["actief"]==0) {
		  echo gettext("nee");
		...
		<td align="right"><span class="d"><? echo gettext("debiteur"); ?></span></td>
		<td>
		<?
		...
		?>
			...
			<td align="right"><span class="d"><?=gettext("map")?></span></td>
			<td><a href="../bestandsbeheer/?action=folder&id=<?=getfolderinfo($id,0)?>"><?=gettext("openen")?></a></td>
		</tr>
	  <?
	  venster_footer();
	  html_footer();
...
<td colspan="2"><span class="d"><nobr><a href="javascript:document.zoeken.zoek.value='*';document.zoeken.submit();"><? echo gettext("Alle projecten tonen"); ?></a></nobr></span></td>
</tr></form>
<?
venster_footer();
if ($f["zoek"] == "*") { $f["zoek"] = "%"; }
...
<tr>
	...
	<td <?=td(0)?>><span class="dT"><? echo gettext("naam"); ?></span></td>
	<td <?=td(0)?>><span class="dT"><? echo gettext("omschrijving"); ?></span></td>
</tr>
...
//				echo("<a href=\"../bestandsbeheer/?action=folder&id=".getfolderinfo($val["id"],0)."\"><img src=\"../img/folder_closed.gif\" border=\"0\" alt=\"".gettext("map")."\"></a>&nbsp;");
		...
		echo gettext("geen projecten gevonden");
		echo("</nobr></td></tr>");
	}
} else {
	$uren = Array();
	...
	//new permission code check
		if (!$instelling["xs_projectmanage"]) {
			...
			echo("<tr><td ".td(0)." colspan=\"3\"><nobr>");
			echo gettext("geen actieve projecten<br>met geaccordeerde uren");
			echo("</nobr></td></tr>");
		}
	} else {
