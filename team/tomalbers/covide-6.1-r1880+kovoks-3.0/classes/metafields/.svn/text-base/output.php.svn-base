...
Class Metafields_output {
	...
	public function meta_add_field($tablename, $record_id) {
		...
		$fieldtypes = array(
			...
			3 => gettext("datum"),
			4 => gettext("ja/nee"),
			5 => gettext("businesscard"),
			6 => gettext("businesscard met deze classificatie")
		);
		$fieldorder = array();
		for ($i=-5;$i<=5;$i++) {
			$fieldorder[$i] = $i;
		...
		switch($field_data["fieldtype"]) {
			...
			case 5:
				$address = new Address_data();
	                        $address_info = $address->getAddressNameByID($field_data["value"]);
				$id = $field_data["id"];

				$ret .= "<input type=\"hidden\" id=\"address_search_meta".$id."\" name=\"metafield[".$id."]\" ";
				$ret .= "value=\"".$field_data["value"]."\">\n\n";
				$ret .= "<a href=\"javascript:popup('?mod=address&action=searchRel', 'address_search_meta',"; 
                                $ret .= "0, 0, 1,'address_search_meta".$id."');\" ";
				$ret .= "id=\"humanaddress_search_meta".$id."\">";
				$ret .= $address_info;
				$ret .= "</span>\n\n";
				break;
                        case 6:
				...
				$ret .= "<input type=\"hidden\" id=\"address_search_meta".$id."\" name=\"metafield[".$id."]\" ";
				$ret .= "value=\"".$field_data["value"]."\">\n\n";
                                $ret .= "<a href=\"javascript:popup('?mod=address&action=searchRel&freeclasssearch=".$field_data["fieldname"]."', 'address_search_meta',";
                                $ret .= "0, 0, 1,'address_search_meta".$id."');\" ";
				$ret .= "id=\"humanaddress_search_meta".$id."\">";
                                $ret .= $address_info;
                                $ret .= "</span>\n\n";
	...
	public function meta_print_field($field_data) {
		switch($field_data["fieldtype"]) {
			...
			case 4: if ($field_data["value"]) 
					$ret = gettext("Ja"); 
				else 
					$ret = gettext("Nee");              break;  
			case 5:
			case 6: $address = new Address_data();
                                $ret = $address->getAddressNameByID($field_data["value"]);
									    break;
		}
		return $ret;
	}
	 ...
	 */
	// FIXME: Seems not to be used anywhere. Delete? Switching types will result in all kinds of hacks imho - TA20060628.
	public function meta_edit_field($tablename, $record_id, $meta_id) {
		...
		if (!$meta_id) {
			...
			$sql = "SELECT * FROM meta_table WHERE id=$meta_id";
			$res = sql_query($sql);
		...
		<tr>
			...
			<td <?=td(1)?>>
				<select name="meta[fieldtype]" class="inputselect">
					...
					<option value="5" <? if ($row["fieldtype"]==5) { echo "SELECTED"; }?>><?=gettext("businesscard")?></option>
					<option value="6" <? if ($row["fieldtype"]==6) { echo "SELECTED"; }?>><?=gettext("businesscard met deze classificatie")?></option>
				</select>
			</td>
		</tr><tr>
