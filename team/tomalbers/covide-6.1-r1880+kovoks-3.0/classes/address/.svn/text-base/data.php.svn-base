	...
	/* }}} */
		...
		public function getAddressByID($addressid, $type="relations", $sub="kantoor") {
			...
			if ($user->checkPermission("xs_addressmanage")) {
				...
				if ($adresinfo["accmanager"] && in_array($adresinfo["accmanager"], $accmanager_arr)) {
					$adresinfo["addressacc"] = 1;
				} else {
		...
		public function lookupRelationEmail($email) {
			...
			$q = "select companyname, address_businesscards.personal_email as email, business_email, address_id as id from address_businesscards left join address on address_businesscards.address_id = address.id where lower(address_businesscards.email) like '$filter' or lower(business_email) like '$filter' ";
			$res = sql_query($q);
			while ($row = sql_fetch_assoc($res)) {
		...
		/* }}} */
		/* 	getFinanceByRelationID {{{ */
	    /**
	     * 	put all financial data for a relation in an array
	     * @return array all relevant info
	     */
		public function getFinanceByRelationID($address_id) {
			$query = sprintf("SELECT * FROM address_finance WHERE address_id=%d", $address_id);
			$res = sql_query($query);
			$res->fetchInto($row); 
			return $row;
		}
		...
		/* 	getFinanceBankDetailsByRelationID {{{ */
	    /**
	     * 	put all financial data for a relation in an array
	     * @return array all relevant info
	     */
		public function getFinanceBankDetailsByRelationID($address_id) {
			$ret = array();

			$sq = "SELECT * FROM address_finance_bank WHERE 1=1";

			if ($address_id > 0)
				$sq .= sprintf(" AND (address_id=%d)", $address_id);

			if ($_REQUEST["searchinfo"]) {
			       $escape = sql_syntax("escape_char"); 
 	                       $like_syntax = sql_syntax("like");
        	               $sq .= sprintf(" AND (".$escape."desc".$escape." $like_syntax '%%%s%%') ", $_REQUEST["searchinfo"]);
                	}

			$res = sql_query($sq);
			while($res->fetchInto($row)) { 
				$ret[] = array_merge($row,
                                                     array("client_name"=>$this->getAddressNameByID((int)$row["address_id"])));
			}
			return $ret;
		}
		/* }}} */

		/* 	getFinanceBankNameByBankID {{{ */
	    /**
	     * 	put the description of an bankaccount
	     * @return string with the name
	     */
		public function getFinanceBankNameByBankID($bank_id) {
			$escape = sql_syntax("escape_char"); 
			$query = sprintf("SELECT ".$escape."desc".$escape." FROM address_finance_bank WHERE id=%d", $bank_id);
			$res = sql_query($query);
			$res->fetchInto($row);  
			return $row["desc"];
		}
		...
		/* 	getCustomerOrSupplierByAdressID  {{{ */
	    /**
	     * 	put the info about if a relation is a customer or a supplier in an array
	     * @return array all relevant info ["is_supplier"] is 1 for supplier and ["is_customer"] for customers.
	     */
		public function getCustomerOrSupplierByAdressID($address_id) {
			$query = sprintf("SELECT is_customer, is_supplier FROM address WHERE id=%d", $address_id);
			$res = sql_query($query);
			$res->fetchInto($row);
			return $row;
			}
		...
		/* 	getRelationsByAddressID {{{ */
	    /**
	     * 	put all customer-supplier  data for a relation in an array
	     * @return array all relevant info ["supplier"], holds all info about suppliers, and ["customer"] for customers.
	     */
		public function getRelationsByAdressID($address_id, $customerOrSupplier) {
			$ret = array();
			$meta_data = new Metafields_data();
			$metaoutput = new Metafields_output();
			if ($customerOrSupplier["is_customer"]) {
				$query2 = sprintf("SELECT * FROM address_relations where customer_id=%d", $address_id); 
				$res2 = sql_query($query2);
				while($res2->fetchInto($row)) {
					$metafields = array();
					$metafields = $meta_data->meta_list_fields("address_relations", $row["id"]);
					foreach($metafields as $temp)
						$store .= $temp["fieldname"].": ".$metaoutput->meta_print_field($temp)."\n ";
					unset($metafields);		
					$ret["supplier"][] = array_merge($row,
								array("name"=>$this->getAddressNameByID((int)$row["supplier_id"])),
								array("transporter_name"=>$this->getAddressNameByID((int)$row["transporter"])),
								array("bankname"=>$this->getFinanceBankNameByBankID((int)$row["bank_pref"])),
								array("meta"=>$store)
								);
					unset($store);
				}
			...
			if ($customerOrSupplier["is_supplier"]) {
				$query3 = sprintf("SELECT * FROM address_relations where supplier_id=%d", $address_id); 
				$res3 = sql_query($query3);
				while($res3->fetchInto($row)) {
					$metafields = array();
                                        $metafields = $meta_data->meta_list_fields("address_relations", $row["id"]);
                                        foreach($metafields as $temp)
                                                $store .= $temp["fieldname"].": ".$metaoutput->meta_print_field($temp)."\n ";
					...
					$ret["customer"][] = array_merge($row,
								array("name"=>$this->getAddressNameByID((int)$row["customer_id"])),
								array("transporter_name"=>$this->getAddressNameByID((int)$row["transporter"])),
								array("bankname"=>$this->getFinanceBankNameByBankID((int)$row["bank_pref"])),
								array("meta"=>$store)
								);
		...
		/* 	getRelationDataByRelationID {{{ */
	    /**
	     * 	put all data for this relation record in an array
	     * @return array all relevant info.
	     */
		public function getRelationDataByRelationID($id) {
			$query = sprintf("SELECT * FROM address_relations where id=%d", $id); 
			$res = sql_query($query);
			$res->fetchInto($row); 
			if (is_array($row))
		        	return array_merge($row,
					array("name"=>$this->getAddressNameByID((int)$row["customer_id"]), 
					      "bankname"=>$this->getFinanceBankNameByBankID((int)$row["bank_pref"])));
			else 
				return;
		}
		/* }}} */

                public function getPaymentRemark($customer,$supplier) 
                {
                    $query = sprintf("select pay_remark from address_relations where customer_id=%d and supplier_id=%d", $customer, $supplier);
                    $res = sql_query($query);
                    $res->fetchInto($row);
                    return $row["pay_remark"];
                }
                
		/* 	getFinanceBankDetailsByBankID {{{ */
	    /**
	     * 	put all financial data for a specific bank account in an array
	     * @return array all relevant info
	     */
		public function getFinanceBankDetailsByBankID($bankid) {
			$query = sprintf("SELECT * FROM address_finance_bank WHERE id=%d", $bankid);
			$res = sql_query($query);
			$res->fetchInto($row); 
			return $row;
		}
		...
		public function getTitles() {
			$q = "select id, title from titles order by title asc";
			$res = sql_query($q);
			$bd = array();
			while ($row = sql_fetch_assoc($res)) 
                        {
		...
		public function getLetterheads() {
			$q = "select id, letterhead from letterheads order by letterhead asc";
			$res = sql_query($q);
			$bd = array();
			while ($row = sql_fetch_assoc($res)) 
                        {
                            $id = $row["id"];
                            $bd[ $id ]= $row["letterhead"]; 
                        }
			return $bd;
		}
		/* }}} */
		...
		/* save_financialcard {{{ */
		/**
		 * store modified/created financialcard in db
		 */
		public function save_financialcard() {
			require(self::include_dir."dataSaveFinancialCard.php");
		}
		/* }}} */

		/* save_relationscard {{{ */
		/**
		 * store modified/created relationscard in db
		 */
		public function save_relationscard() {
			require(self::include_dir."dataSaveRelationsCard.php");
		}
		/* }}} */
		 ...
		 * store modified/created financialcard in db
		 */
		public function save_financialbank() {
			require(self::include_dir."dataSaveFinancialBank.php");
		}
		/* }}} */
		...
		/* checkcla_array {{{ */
                /**
                 * Search the classification in the database, returns a string of ids,like "|1|2|3|", can be none.
                 *
                 * @param array Fields to use in check
                 */
                public function checkcla_array($data) {
                        if (!strlen($data["name"])) 
				return;

    			$like = sql_syntax("like");
                        $sql = sprintf("SELECT id FROM address_classifications WHERE UPPER(description) ".$like." '%%%s%%'", strtoupper($data));
                        $res = sql_query($sql);
 			while ($row = sql_fetch_assoc($res)) 
                        	$ret .= "|".$row["id"];
