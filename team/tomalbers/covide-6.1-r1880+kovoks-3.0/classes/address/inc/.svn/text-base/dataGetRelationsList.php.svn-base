...
if ($addresstype == "users" || $addresstype == "private" || $addresstype == "overig") {
	...
	$query_zoek = "SELECT address$t.*".($addresstype == "users" ? ",users.username" : "")." FROM address$t ". ($addresstype == "users" ? ",users" : ""). " WHERE (address$t.is_public=1 OR address$t.user_id=".$_SESSION["user_id"].") ";
	$query_csv = "SELECT address$t.*".($addresstype == "users" ? ",users.username" : "")." FROM address$t ". ($addresstype == "users" ? ",users" : ""). " WHERE (address$t.is_public=1 OR address$t.user_id=".$_SESSION["user_id"].") ";
	...
	$query_zoek = "SELECT address_businesscards.id,address_businesscards.alternative_name,address_businesscards.surname,address_businesscards.infix,address_businesscards.givenname,address_businesscards.surname,address_businesscards.business_mobile_nr AS mobile_nr,address_businesscards.business_phone_nr AS phone_nr,address_businesscards.business_email AS email, address_businesscards.personal_email AS personal_email, address_businesscards.address_id as address_id, address.companyname as companyname FROM address_businesscards $bjoin WHERE ($sq) ";
...
while ($row = sql_fetch_assoc($result)) {
	...
	if ($options["addresstype"] == "relations" || $options["addresstype"] == "nonactive" || $options["addresstype"] == "overig") {
		...
		if ($user->checkPermission("xs_addressmanage")) {
			...
			if ($row["accmanager"] && in_array($row["accmanager"], $accmanager_arr)) {
				$addressinfo["address"][$i]["addressacc"] = 1;
			} else {
