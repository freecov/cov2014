...
$output->layout_page("", 1);
	...
	$venster->addVensterData();
		...
		$table->addTableRow();
			...
			$table->addTableData(array("colspan" => 3), "data");
				...
				$table->insertTag("span", $relname, array( "id" => "humanbcardaddress_id"));
				$table->addSpace(1);
				$table->insertAction("edit", gettext("wijzigen"), "javascript: popup('?mod=address&action=searchRel', 'searchrel', 0, 0, 1,'bcardaddress_id');");
			$table->endTableData();
		$table->endTableRow();
		$table->addTableRow();
	...
	foreach ($bcardinfo["multirel"] as $aid=>$aname) {
		$output->addCode("selectRel($aid, '".addslashes($aname)."', 'bcardaddress_id');\n");
	}
