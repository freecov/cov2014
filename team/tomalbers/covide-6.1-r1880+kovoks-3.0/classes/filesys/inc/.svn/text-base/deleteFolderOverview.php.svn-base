<?
	...
	$venster = new Layout_venster(array(
		...
		"subtitle" => gettext("mapstructuur verwijderen")
	));
	$venster->addVensterData();
		...
		$table->addTableRow();
			$table->addTableData("", "data");
				...
				$table->addCode( gettext("U staat op het punt de volgende mappen te verwijderen").": ");
				$table->addTag("br");
				...
				if ($deny == 1) {
					...
					$tbl->addTableRow();
						...
						$tbl->endTableData();
						$tbl->addTableData();
							$tbl->addCode( gettext("U kunt de gekozen map niet compleet verwijderen omdat te weinig rechten hebt op de met rood aangegeven mappen."));
							$tbl->addTag("br");
				...
				$view->defineComplexMapping("complex_name", array(
					...
					array(
						"text"  => array(" ","%name", " (", gettext("met"), " ",  "%filecount", " ", gettext("bestanden"), ")")
					)
				));
				...
				$table->addSpace(3);
				if (!$deny) {
					$table->insertAction("ok", gettext("verder met verwijderen"), "?mod=filesys&action=delete_folder_exec&id=".$folder);
				}
