...
Class Classification_output {
	...
	/* pick_cla {{{ */
	public function select_classification($prefix="", $suffix="", $allow_mixed=0) {
	...
	public function classification_selection($id, $current="", $type="enabled", $show_only=0) {
		...
		if (!$show_only) {
			$output->addTag("br");
			$output->insertAction("edit", gettext("kies classificaties"), "javascript: classification_select_$id();");
